...
- `journeys/<name>/run` is started by `journey.py`, which uppercases every key `discover` prints into the environment (`smoke.py` reads `ORIGIN` and `INSTANCE_TOKEN` from there). `harness/target.py` does not implement the break switch, so these journeys run without `--break`: each runs a second time with the product stopped and every discovered address repointed to a closed port, the script is not told which pass it is in, and that pass must fail, else the run reports `JOURNEY GREEN WITHOUT PRODUCT`.
