...
flowchart LR
    ...
    W["帮手 AI<br/>阶段内自驱落地<br/>(常驻 · 可续 · 撑满才换)"]:::ai
...
| 续接不重做 | 每步一提交(已做的跳过)+ 续同帮手 context 原封 | 暂停/换帮手都不白干,省 token |
...
| 源 | 贡献 |
|---|---|
| 旧 plugin 实测 | plan 级帮手自治、从已提交计数恢复、续同帮手修复、撑满才换帮手、上限数据+hook 双持有、转根因调查;也挖出它的坑(手写整块状态、轮次编进文件名、魔数阈值)——全改掉 |
