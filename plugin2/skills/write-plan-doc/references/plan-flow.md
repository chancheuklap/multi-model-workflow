...
> write-plan-doc 走到「编排」步时读本文**全文**:判单/多计划、映射、跨 plan 合同骨架、fan-out plan-writer、亲验、回填、就绪门,加角色声音 + Git 纪律,全在这。别凭记忆默写。
...
| 怎么写 | **主线程自己**照 `task-pack.md` + `plan-rigor.md` 直接写这份 plan(自己拆小 issue + Task Pack),不派 plan-writer——省一次派发往返 | 见 Step 3,逐 issue 派 `plan-writer` |
...
## Step 6:就绪门 + 跨 plan 覆盖自检(→ 读 `plan-self-check.md` 全文)

plan-writer 已各自过 Pre-delivery Self-Check(保自己那份)。主 Agent **打开 `plan-self-check.md`**,从**跨 plan 视角**再过一遍覆盖与 ownership——跨 plan 一致性归你,判据按那份文件。
