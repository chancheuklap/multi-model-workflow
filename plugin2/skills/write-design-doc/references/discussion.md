...
#### Step 4：细化选定方向（方向已由上游 propose 阶段拍定）
给 2-3 方案 + HITL 指定哪个,是上游 propose 阶段做的(`prev_outputs` 的 `direction` 文件)。这里**细化选定的那一个**,不重摆备选(被事实击穿 → handoff `needs-redirection` 回上游换向)。把系统拆成目的清晰、接口明确、可独立测试的小单元(深模块优先,用 `codebase-design` 的词汇理解现有边界),遵循既有模式。内化「架构判断本能」(无聊技术默认 / 增量非革命 / 可逆性 / 本质 vs 偶然复杂度 / blast radius / 推倒重来——读 `design-rigor.md` 全文)。
