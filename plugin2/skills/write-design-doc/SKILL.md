...
读 investigate 现状报告 → 选路（访谈 A / 综合 B）→ 与用户讨论 → 设计文档（+ 项目领域文档）→ 自检 → 拆 issue 立骨架 → handoff。
...
#### Step 4：提方案
提 2-3 个讲 trade-off，推荐放第一个，对话式呈现。让取舍真正张开：至少一个**最小可行**（最少文件 / 最快上线）+ 一个**理想架构**（长期最优）+ 可选**另辟蹊径**；三个长得差不多就是没张开。把系统拆成目的清晰、接口明确、可独立测试的小单元（深模块优先，用 `codebase-design` 的词汇理解现有边界），遵循既有模式。内化「架构判断本能」（无聊技术默认 / 增量非革命 / 可逆性 / 本质 vs 偶然复杂度 / blast radius / 推倒重来——见 `references/design-rigor.md`）。
