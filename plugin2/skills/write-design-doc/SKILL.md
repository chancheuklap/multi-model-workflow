...
A / B 产出同一份设计文档、走同样的自检 / 审 / 拆 issue，只是「怎么想清楚」不同。**选哪条读完 investigate 现状报告（Step 1）才有判据**：
...
第一轮用户对话前调 `domain-modeling` skill，全程维护领域文档（见 §双文档、§Domain Alignment）。领域文档与设计文档**地位等同**，不能只写设计不维护领域。
...
- **选路**（判据见 §两条路）：现状报告 + 已聊到的已覆盖问题/方案/验收且核心假设被对抗过 → 走 B 直接起草；否则走 A → Steps 2-4。一句话告诉用户（可推翻）。
...
方向粗选 + HITL 指定在 Step 1.5 做过了,这里**细化选定的那一个**,不重摆备选(被事实击穿才回 1.5 换向)。把系统拆成目的清晰、接口明确、可独立测试的小单元(深模块优先,用 `codebase-design` 的词汇理解现有边界),遵循既有模式。内化「架构判断本能」(无聊技术默认 / 增量非革命 / 可逆性 / 本质 vs 偶然复杂度 / blast radius / 推倒重来——见 `references/design-rigor.md`)。Step 1.5 粗选时让取舍真正张开(最小可行 / 理想架构 / 另辟蹊径,三个长得差不多 = 没张开)。
...
信息足够且用户确认方向后，**打开 `references/design-doc-template.md` 按模板一次成文**。写作规则、section 清单、每节细则、UI 对照 `design-rigor.md`、Cross-Plan Anchors 占位规则全在模板里——以模板为单一源。

## 收尾：自检 → 拆 issue → handoff
...
写文档、自检这两步必须现读对应 reference 全文，没读全凭记忆默写就回去补读。
