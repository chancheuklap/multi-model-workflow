# Plan 撰写（写一份 plan 从头到尾就读这一份）

> plan-writer 写一份 plan 的完整方法论:Task Pack 模板 + TDD Implementation 步骤 + 测试规划严谨度 + Issue 质量 + 反模式 + 保存前自检 + Pack 就绪门。**一份读完,不跳别的文件。**（编排 / fan-out / 回填是主 Agent 的活,在 `plan-flow.md`,不在这。)
