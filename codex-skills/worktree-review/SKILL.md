...
description: 你(独立审者:Codex 或 Claude 无头 CLI)被主线程派去独立审一份产物(设计 / 计划 / 分支 / 合并)时读本 skill。它是你整个审查的总纲:开工读共享审查纪律 → 按被派的 stage 读对应审查角度 → 只读验证、引 file:line → 按 Return Contract 回结构化 findings。细纪律在 references,到那步再读(渐进加载,不一次性塞满)。
---
