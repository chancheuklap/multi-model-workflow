...
> design 阶段主线程跟用户讨论出设计。方法顺序(按需走,不是游标,你随时可被用户拉回任何一步):**讨论(本文)→ 涉 UI/UX 或非平凡状态模型才走 `prototype-mockup.md` → 按 `design-doc-template.md` 成文 → 按 `design-self-check.md` 自检 → 设计预审 → 请用户 `/approve-design` 过门**。
>
...
问答途中用户喊"补充上下文 / 查一下 X"(或你自己发现缺关键事实)时,**按流程做一次定向收集再续聊,不打断节奏**。缺口大到要立计划、跑脚本、真调 API、多候选横向实测(选供应商/选模型/验第三方合同)→ 那是**取证战役**,读 `evidence-campaign.md` 照它打,打完回来续聊;小缺口走下面四步:
