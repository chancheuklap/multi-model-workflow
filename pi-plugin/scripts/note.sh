...
# 路径安全:拒绝绝对路径 / .. / 软链 / 带换行的文件名(fail-closed)。
cmd_fingerprint() {
  local -a reports=()
  ...
  for rel in "${reports[@]}"; do
    case "$rel" in /*|*'
'*) die "报告路径非法(绝对路径/换行): $rel" ;; esac
    case "/$rel/" in */../*) die "报告路径非法(..): $rel" ;; esac
    abs="$top/$rel"
    [ -e "$abs" ] || die "报告路径不存在: $rel"
    [ ! -L "$abs" ] || die "报告路径是软链,拒绝: $rel"
  done
  {
    for rel in "${reports[@]}"; do
      abs="$top/$rel"
      if [ -f "$abs" ]; then
        git hash-object "$abs" 2>/dev/null
      elif [ -d "$abs" ]; then
        [ -z "$(find "$abs" -type l -print -quit)" ] || die "目录内含软链,拒绝: $rel"
        ( cd "$abs" && find . -type f ! -path '*/.git/*' | LC_ALL=C sort | while IFS= read -r f; do git hash-object "$f" 2>/dev/null; done )
      else
        echo "MISSING:$rel"
      fi
    done
