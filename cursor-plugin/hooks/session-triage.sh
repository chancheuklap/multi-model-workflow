#!/usr/bin/env bash
# Cursor 会话分诊:只在在管任务 worktree 回报书签、最近提交、待拍板与状态新鲜度；未在管仓库静默。
set -euo pipefail
...
if [ -f "$man" ]; then
  ...
  jq -r --arg mmw "$MMW" '
    ...
    "[multi-model-workflow-cursor] 在管任务 worktree: slug=\(.slug) scenario=\(.scenario) phase=\(.phase) status=\(.status) mode=\($mode)。" +
