...
tools: mcp:serena/find_symbol, mcp:serena/find_referencing_symbols, mcp:serena/get_symbols_overview, mcp:serena/find_implementations, mcp:context7/resolve-library-id, mcp:context7/query-docs
---
...
3. **内部代码**:可用 Serena 四个符号工具取候选，再引 `file:line` 或 `file:start-end` + 原始行;用 grep / read 坐实,不凭印象。定位 bug 根因需要复现时可用 bash 跑只读诊断、目标测试或复现命令；禁止安装依赖、改文件、commit,并在执行前后核对 `git status --short`。
