#!/usr/bin/env bash
# review.sh —— 起一道审(定编制 + 出派发指南,一条命令;Cursor 原生)
#
...
#       model/工具白名单看 ~/.cursor/agents),读 ~/.cursor/skills/worktree-review；④final 按场景/风险分档。
...
- Task({subagent_type:"reviewer-design-a", run_in_background:true, ...}) 负责轴A 设计内容
- Task({subagent_type:"reviewer-design-b", run_in_background:true, ...}) 负责轴B 项目对齐
...
- Task({subagent_type:"reviewer-plan-a", run_in_background:true, ...}) 负责轴A 覆盖与质量
- Task({subagent_type:"reviewer-plan-b", run_in_background:true, ...}) 负责轴B 合规与交叉验证
...
- Task({subagent_type:"reviewer-final-a", run_in_background:true, ...}) 先基线2独立代码审计,再基线1回归+意图
...
- Task({subagent_type:"reviewer-final-a", run_in_background:true, ...}) label=基线1(回归+意图+跨plan)
- Task({subagent_type:"reviewer-final-b", run_in_background:true, ...}) label=基线2(独立代码审计,全新眼光)
...
- Task({subagent_type:"reviewer-final-a", run_in_background:true, ...}) label=基线1(回归+意图+跨plan)
- Task({subagent_type:"reviewer-final-b", run_in_background:true, ...}) label=基线1
- Task({subagent_type:"reviewer-final-a", run_in_background:true, ...}) label=基线2(独立代码审计,全新眼光)
- Task({subagent_type:"reviewer-final-b", run_in_background:true, ...}) label=基线2
...
EOF
    ...
    merge-impl)
      cat <<EOF
同轮并行多个 Task(两个跨模型审者):
- Task({subagent_type:"reviewer-final-a", run_in_background:true, ...}) label=跨 worktree 集成审路线1
- Task({subagent_type:"reviewer-final-b", run_in_background:true, ...}) label=跨 worktree 集成审路线2
...
EOF
    ...
    cat <<EOF
REVIEW_STARTED stage=plan-impl host=cursor
...
EOF
  ...
  skill="$(mmw_user_skills_dir)/worktree-review"
  dispatch="$(dispatch_for "$stage" "$source" "$skill" "$scen" "$tier")"

  cat > "$brief" <<EOF
# 审派发指南(stage=$stage · host=cursor · 机器生成,主线程读完直接派审者)
...
同轮并行多个 Task(subagent_type=reviewer-*-a/b, run_in_background:true);主线程汇总。
...
EOF
  ...
  cat <<EOF
REVIEW_STARTED stage=$stage host=cursor
1. 主线程读 $brief,按「派审者」段直接派(同轮并行 Task,读 worktree-review skill 出结构化 findings)。
