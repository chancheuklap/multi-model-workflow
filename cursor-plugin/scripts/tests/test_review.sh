#!/usr/bin/env bash
# review.sh 空跑(Cursor 原生):审闸一条命令——brief 落盘(主线程读它直接派审者)、纯路由指向已装 worktree-review skill、
...
for s in design plan final; do
  ...
  echo "$OUT" | grep -q "REVIEW_STARTED stage=$s host=cursor" && ok "$s → REVIEW_STARTED host=cursor" || no "$s REVIEW_STARTED"
  ...
  echo "$B" | grep -qiE 'Task|subagent_type|reviewer-' && ok "$s 派发=Task 并行" || no "$s Task/派发指南"
  echo "$B" | grep -q 'codex ''exec' && no "$s brief 不该派 codex(Cursor 宿主无 Codex CLI)" || ok "$s 无 codex 派发"
...
printf '# d\n## Cross-Plan Contract Anchors\n| owner | provider |\n| 001 | 002 |\n' > docs/design/d.md
OUT="$(bash "$REVIEW" start --stage plan-impl --source docs/design/d.md 2>/dev/null)"
echo "$OUT" | grep -q "REVIEW_STARTED stage=plan-impl host=cursor" && ok "③ 有合同 → 起门" || no "③ 有合同起门"
...
# 主仓库状态平面已遮蔽:起审后 git status 不冒 ?? .cursor/
grep -qxF 'multi-model-workflow/' .cursor/.gitignore && ok "review start 遮蔽主仓库状态平面" || no "review 遮蔽"
