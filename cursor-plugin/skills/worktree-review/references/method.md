...
遇到调用/引用、连接、依赖路径或影响面，先读 `../../orchestrate/references/retrieval-doctrine.md`。上游候选只扩大检查面，不代表审者调用过工具；每条 finding 仍须引用目标 checkout 的 `file:line`。工具不可用或命中已知盲区时按 doctrine 退化。
