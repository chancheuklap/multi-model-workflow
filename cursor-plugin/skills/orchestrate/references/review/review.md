...
红线:**写者≠验者**(设计/计划作者与审者不同家;Cursor 用 a/b 双模型审者钉死)。
...
2. **直接派审者**:读 brief 按「派审者」段用 Task 派 `reviewer-*`（`subagent_type`=角色名，`run_in_background:true` 并行），各自干净 context、读 `worktree-review` skill 出结构化 findings。**禁止前台阻塞式串跑双轴**——主线程要能同时处理其它事，回执齐了再亲验。**别塞你自己的问题清单。**
...
**Advisor 短提醒(Cursor)**：主线程综合审者 verdict 拍板前，若路线仍有盲点，可 Task(subagent_type="advisor") + handoff pack；不作审闸、不替 HITL、不写 reviews 留痕。
