...
1. **先拿候选。** 符号问题用 Serena；关系 / 影响面 / 调用链 / 跨模块问题用 Graphify。Graphify 是默认结构检索入口之一：先 `pi-graphify-ensure --repo <root>` 保证图对应当前内容（缺则建、脏则重建），再 `graphify query|affected|path|explain`。按问题选合适入口，不为形式重复跑。
2. **再亲验。** 每项回目标 checkout 用 read/grep 核对。进入 finding、plan、investigating、handoff 或交付物的承重结论必须给 `file:line`。
3. **无损退化。** Serena 不可用、ensure/查询失败、歧义或空结果时退到另一工具 + read/grep；空结果不证明不存在。
...
候选只表示上游检索，不代表下游调用过工具，也不授予新工具。Cursor 只有 agents 花名册明确列出的角色可直接用四个 Serena 符号工具（由本插件 `mcp.json` 提供）；synthesizer 不得获得。investigate-topic 另可使用 Context7（`resolve-library-id` / `query-docs`）做外部库文档取证。Graphify 通过 Shell 调用本机 `pi-graphify-ensure` + `graphify` CLI；工人 / 任务 wt 初始化也会预热图谱（项目 `.cursor/worktree-init.sh` 优先，否则 `pi-graphify-ensure`）。resume 沿用原会话和快照。
