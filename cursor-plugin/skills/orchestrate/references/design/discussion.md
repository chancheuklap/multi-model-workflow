...
- **关键咨询(本阶段主战场,0–2 次)**:在架构/对象边界/状态机**定型前**、请用户 `/approve-design` 过门前、或讨论中前提**可能被击穿**时,可调 Task(subagent_type="advisor") 拿短判决（须组 handoff pack：goal/evidence/tentative_decision/open_questions/constraints）(零参数，自动转发全对话，不需要写 brief)。小改/无边界风险默认跳过。本阶段**不自派** `reviewer-design-a/b`(设计预审由 selfcheck 收尾统一起)。
