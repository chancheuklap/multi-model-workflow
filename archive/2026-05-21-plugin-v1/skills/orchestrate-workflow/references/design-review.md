...
只在 release strategy / migration-deploy order / rollback / manual gate 必须提前判定时追加 `codex-release-reviewer`（via `codex:codex-rescue --model gpt-5.5`）。普通 production-risk 由 baseline 转成 risk flags。
...
Design finding 默认 route 给 `parent` 或 `docs-worker` 做 document repair；domain / UX / ownership / target-state ambiguity route 给 `orchestrate-discovery`；产品承诺、业务规则、UX、发布策略、架构 trade-off 无法从 source artifacts 判定时 route 给 `user decision` 或 `orchestrate-discovery`。
