...
5. Result: `node "$CODEX_SCRIPT" result "$(cat .claude/multi-model-workflow/review-prompts/<gate>.job-id)"` -> `review-results/<gate>.md`
   ...
   - **Disk fallback** (when `result` returns `No job found for <JOB_ID>` despite Step 4 reporting the job completed — a known openai-codex plugin limitation where `result` can't re-fetch an already-finished job): read the job's persisted output directly from its on-disk JSON `.result.rawOutput` field, e.g. `cat ~/.claude/plugins/data/codex-openai-codex/state/*/jobs/<JOB_ID>.json | jq -r '.result.rawOutput'` -> save to `review-results/<gate>.md`, then continue to Step 6 as normal. This is a one-off recovery read, not a routine path; prefer `result` first.
...
> **幻觉抑制四件套**（Confidence rubric / Pre-emit Verification Gate / 证据表 / Bias indicators）由 `dispatch-review.sh validate` 在派发时**自动追加**到 prompt 文件（单源 `_shared/review-prompt-quartet.md`），主线程不需手抄、也不需在此读取。它保证每个 review prompt 都带四件套（比手抄更可靠），同时把 ~2400 字符移出主线程的派发读取面。
