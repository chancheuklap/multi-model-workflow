...
## Dispatch：1 个 baseline `code_reviewer`

同一 reviewer 先做 Spec Compliance，通过后才做 Code Quality。
...
- 能说清改哪里 → original worker / coding_worker / complex_coding_worker。
- 根因不明 → complex_code_explorer。
- desired behavior 不清 → orchestrate-discovery。
- bad seam → improve-codebase-architecture。
- 满足 release gate → release_reviewer。
