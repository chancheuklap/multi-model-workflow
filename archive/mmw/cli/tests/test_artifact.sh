...
absent "artifact 顶层用法不含 list" "mmw artifact list" "$(cat "$LAST_ERR")"
...
check "artifact list 动作退出码" "2" "$LAST_STATUS"
contains "artifact list 动作回退到用法" "mmw artifact path <类别>" "$(cat "$LAST_ERR")"
...
contains "错误声明逐条汇总缺名字段" "docs/specs/check-errors/spec.md: artifact_refs[0]: 缺少或无效的 name" "$(cat "$LAST_ERR")"
...
planner_body_source="$(cat "$HERE/../../skills-src/mmw-planner/plan-body.md")"
...
contains "spec 用 artifact path 取路径" "mmw artifact path spec" "$to_spec_source"
contains "ticket 生产产物引用固定节" "## Artifact refs" "$to_tickets_source"
contains "ticket 产物引用写无" "Write \`none\` when there are none" "$to_tickets_source"
contains "ticket 拥有产物引用解析" "Resolve each artifact-ref line" "$to_tickets_source"
contains "to-plan 向 planner 传产物引用" "artifact refs" "$to_plan_source"
contains "planner 解析产物引用" "mmw artifact path" "$planner_source"
contains "planner 计划正文含产物引用" "artifact_refs: []" "$planner_body_source"
contains "implement 向 worker 传产物引用" "the plan's artifact refs" "$implement_source"
...
expect_path "全小写名字段" "docs/specs/beta/spec.md" \
...
check "显式名字段查询不建立目录" "$artifact_path_dirs_before" "$(find . -type d -print | sort)"
...
expect_error "四栏 task 不写文件" "宿主启动块的标准输入" \
...
absent "无参用法不含 list 说明" "mmw artifact list" "$usage_text"
