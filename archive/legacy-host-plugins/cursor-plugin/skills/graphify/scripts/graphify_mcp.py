#!/usr/bin/env python3
"""把 Graphify 结构检索包成常驻 MCP 工具（Claude / Cursor 共用）。

对齐 Pi 侧原生扩展（pi-local-extensions/extensions/graphify.ts）：
同一组 action、同样的默认 budget/depth、调用前先跑 ensure 保证图对得上当前 checkout。
查询仍直接调官方 `graphify` CLI，本文件不重写检索语义。

刻意不用官方 `graphify-mcp`（`python -m graphify.serve`）：那是另一套工具面
（query_graph / get_node / …），假定已有静态 graph.json，不跑 ensure，也与
Serena·Graphify 双入口纪律和 skill / Pi / Claude 既有合同不一致。
"""
