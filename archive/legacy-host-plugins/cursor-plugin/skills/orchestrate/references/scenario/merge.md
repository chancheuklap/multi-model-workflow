...
| **复杂·根因明** | 功能/合同冲突;多文件;但谁该 win 清楚 | Coordinator 修,或派 `pack-executor` 定向修:先写一份 mini-plan(冲突点 + 谁该 win + 验收命令)落状态平面,再跑 `mmw worker dispatch --mode merge --plan <mini-plan绝对路径> --worktree <合并worktree绝对路径>`;照它打印的 DISPATCH 派后台 Task 执行 |
...
起审:`mmw review start --stage merge-impl` 生成 brief,并行派 `reviewer-final-a` + `reviewer-final-b`。审者按 `stage=merge-impl` 走七角度,不信各 worktree 的旧终审。findings 落状态平面并由主线程亲验。修复软上限 1 轮;修完自验,不过则 BLOCKED 报用户。
