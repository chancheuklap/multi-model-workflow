#!/usr/bin/env bash
# mmw —— Cursor 原生 multi-model-workflow 统一 CLI
#
...
#   mmw task new|adopt|resume|scope|cleanup|team|escalate
...
#   任务(入口/收尾,主仓库):
#     mmw task adopt --scenario <…>   # Cursor 默认:已在悬空 git wt 内就地挂分支+manifest(不 worktree add)
#     mmw task new --scenario <small-change|develop|bug> --slug <s> --title <t> --request '<需求>' --entry-capability <能力>... --entry-evidence '<用户原话或只读证据>' [--direction-given] [--with-wayfind]
...
#   写码工人派发(准备 worktree/prompt/账本,工人由协调者在会话内 Task 工具派 subagent_type=pack-executor, background):
...
# 状态平面固定为 .cursor/multi-model-workflow/; 任务 wt 常由 Cursor UI 创建,工人 wt 默认根 .cursor/worktrees/。
set -euo pipefail
D="$(cd "$(dirname "$0")" && pwd)"
