...
#
#   new     建命名 worktree(本地 HEAD 分叉)+ scaffold docs + 写 manifest
#           主仓库内建=主仓库 HEAD;任务 worktree 内建=父 HEAD + parent/child_tasks 双向登记
#   adopt   已在悬空 git worktree 内就地挂分支+manifest(Cursor UI 建 wt 后用)
...
IGN
  ...
  plugin_version="$(jq -r '.version // ""' "$SCRIPT_DIR/../.cursor-plugin/plugin.json" 2>/dev/null || echo "")"
...
# ---------- adopt (Cursor 默认新建:已在悬空 git worktree 内就地挂分支+manifest) ----------
cmd_adopt() {
  local scenario="" slug="" title="" request="" entry_evidence="" direction_given=false with_wayfind=false
  local -a entry_capabilities=()
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --entry-capability) entry_capabilities+=("$2"); shift 2 ;;
      --entry-evidence) entry_evidence="$2"; shift 2 ;;
      --direction-given) direction_given=true; shift ;;
      --with-wayfind) with_wayfind=true; shift ;;
      *) die "未知参数: $1" ;;
    esac
  done
  [ -n "$scenario" ] || die "--scenario 必填(small-change|develop|bug)"
  ...
  [ -n "$phases_json" ] || die "routes.json 未定义预设 $scenario 的 phases"
  if [ "$with_wayfind" = true ]; then
    [ "$scenario" = "develop" ] || die "--with-wayfind 仅 develop 可用"
    phases_json="$(printf '%s' "$phases_json" | jq -c '["wayfind"] + .')"
  fi
  local phase; phase="$(printf '%s' "$phases_json" | jq -r '.[0]')"
  local entry_capabilities_json
  ...
  local wt; wt="$(git_toplevel)"
  # 必须已在 git worktree(含主仓检出或悬空 wt);不执行 git worktree add
  local man="$wt/$STATE_SUBDIR/$MANIFEST_NAME"
  [ ! -f "$man" ] || die "已有 task.json:$man;续跑用 mmw task resume / where"
  mmw_ensure_state_ignore "$(git -C "$wt" rev-parse --git-common-dir 2>/dev/null | xargs dirname 2>/dev/null || echo "$wt")"
  mmw_ensure_wt_state_ignore "$wt"

  local cur_branch base
  cur_branch="$(git -C "$wt" branch --show-current)"
  base="$(git -C "$wt" rev-parse HEAD)"
  if [ "$cur_branch" != "$slug" ]; then
    if git -C "$wt" show-ref --verify --quiet "refs/heads/$slug"; then
      git -C "$wt" checkout "$slug" >&2
    else
      git -C "$wt" checkout -b "$slug" >&2
    fi
  fi
...
IGN
  ...
  jq -n \
    ...
    '{schema_version:$sv, slug:$slug, title:$title, request:$request,
      ...
      branch:$branch, worktree_path:$wt, docs:{investigating:$inv, design:$ddoc, issues:$idir, plans:$pdir, context:$ctx},
      repair_count:0, turnaround_count:0, attendance:$attendance, unattended_policy:null,
      note:null, approval:null, prototype:null, parent:null, child_tasks:[],
      artifacts:[], phase_outputs:{}, open_items:[], subtasks:[], history:[]}' \
    > "$man"
...
base_commit=$base
design_doc=docs/design/$slug/$slug.md(主文档与文件夹同名)
NEXT=mmw where; 然后进入 $scenario 的 $phase 阶段(本 folder 即任务 wt,无需再 cd)
EOF
}
...
cmd_cleanup() {
  ...
  # 过门:worktree 内含 .cursor 临时状态(gitignore),--force 一并删。
...
cmd_team() {
  ...
  echo "TEAM"
  local found=0 man
  # 扫 git worktree list 下全部在飞 manifest。
...
case "${1:-}" in
  ...
  team)     shift; cmd_team "$@" ;;
  *) die "用法: prepare.sh new|adopt|resume|cleanup|escalate|team ..." ;;
esac
