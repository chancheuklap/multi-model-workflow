...
WT="$(bash "$PREPARE" new --scenario develop --slug 2026-06-29-e2e --title "端到端" --request "端到端走通全流程" --entry-capability explicit-request --entry-evidence "端到端测试明确要求 MMW" 2>/dev/null | sed -n 's/^worktree_path=//p')"
