#!/usr/bin/env bash
# MMW worktree 初始化合同：项目 hook 优先(.claude 先于 .pi)；普通仓库走唯一用户级图谱生命周期模块。
set -euo pipefail
...
mkdir -p "$BIN"
export MMW_GRAPHIFY_TEST_LOG="$TMP/graph-manager.args"
cat >"$BIN/graphify-ensure" <<'SH'
#!/usr/bin/env bash
set -euo pipefail
printf '%s\n' "$@" >"$MMW_GRAPHIFY_TEST_LOG"
if [ "${MMW_GRAPHIFY_TEST_FAIL:-0}" = 1 ]; then
  echo 'fixture graph failure' >&2
  exit 7
fi
echo 'REUSED fixture warnings=0'
SH
chmod +x "$BIN/graphify-ensure"
...
cat >"$REPO/.claude/worktree-init.sh" <<'SH'
#!/usr/bin/env bash
set -euo pipefail
source_wt="$1"
target_wt="$2"
printf '%s\n' "$source_wt" >"$target_wt/.claude/init-source"
printf '%s\n' "$target_wt" >"$target_wt/.claude/init-target"
SH
chmod +x "$REPO/.claude/worktree-init.sh"
printf 'seed\n' >"$REPO/seed.txt"
git -C "$REPO" add -A
git -C "$REPO" commit -qm base
git -C "$REPO" worktree add -q "$TARGET" HEAD
rm -f "$MMW_GRAPHIFY_TEST_LOG"

if mmw_prepare_worktree "$REPO" "$TARGET" >"$TMP/hook.out" 2>"$TMP/hook.err" \
  && [ ! -s "$TMP/hook.out" ] \
  && [ "$(cat "$TARGET/.claude/init-source")" = "$REPO" ] \
  && [ "$(cat "$TARGET/.claude/init-target")" = "$TARGET" ] \
  && [ ! -e "$MMW_GRAPHIFY_TEST_LOG" ]; then
  ok "project worktree hook wins over generic graph manager"
else
  no "project worktree hook precedence"
fi

# 只有 .pi hook 的既有项目仓库仍被识别(Pi 侧先落地的项目不必再加一份)
PI_ONLY="$TMP/pi only repo"
PI_ONLY_TARGET="$TMP/pi only target"
mkdir -p "$PI_ONLY/.pi"
git -C "$PI_ONLY" init -q
git -C "$PI_ONLY" config user.email test@example.com
git -C "$PI_ONLY" config user.name Test
cat >"$PI_ONLY/.pi/worktree-init.sh" <<'SH'
#!/usr/bin/env bash
set -euo pipefail
printf '%s\n' "$2" >"$2/.pi/init-target"
SH
chmod +x "$PI_ONLY/.pi/worktree-init.sh"
printf 'seed\n' >"$PI_ONLY/seed.txt"
git -C "$PI_ONLY" add -A
git -C "$PI_ONLY" commit -qm base
git -C "$PI_ONLY" worktree add -q "$PI_ONLY_TARGET" HEAD
rm -f "$MMW_GRAPHIFY_TEST_LOG"
if mmw_prepare_worktree "$PI_ONLY" "$PI_ONLY_TARGET" >"$TMP/pi.out" 2>"$TMP/pi.err" \
  && [ "$(cat "$PI_ONLY_TARGET/.pi/init-target")" = "$PI_ONLY_TARGET" ] \
  && [ ! -e "$MMW_GRAPHIFY_TEST_LOG" ]; then
  ok "legacy .pi worktree hook still recognised"
else
  no "legacy .pi hook contract"
fi

cat >"$TARGET/.claude/worktree-init.sh" <<'SH'
#!/usr/bin/env bash
printf 'fixture failure\n' >&2
exit 9
SH
chmod +x "$TARGET/.claude/worktree-init.sh"
...
if mmw_prepare_worktree "$PLAIN" "$PLAIN_TARGET" >"$TMP/plain.out" 2>"$TMP/plain.err" \
  && [ ! -s "$TMP/plain.out" ] \
  && [ "$(cat "$MMW_GRAPHIFY_TEST_LOG")" = "$EXPECTED_ARGS" ] \
...
if MMW_GRAPHIFY_TEST_FAIL=1 mmw_prepare_worktree "$PLAIN" "$PLAIN_TARGET" >"$TMP/generic-fail.out" 2>"$TMP/generic-fail.err" \
...
if PATH="/usr/bin:/bin" mmw_prepare_worktree "$PLAIN" "$PLAIN_TARGET" >"$TMP/missing.out" 2>"$TMP/missing.err" \
  && [ ! -s "$TMP/missing.out" ] \
  && grep -q '找不到 graphify-ensure' "$TMP/missing.err"; then
