...
cmd_handoff() {
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --waiting-for) waiting_for="$2"; shift 2 ;;   # needs-context 专用:等用户答的具体问题(落盘,跨会话可见)
...
# 主线程(Claude Code)读 ~/.claude/skills;Codex 工人读 ~/.agents/skills,两侧都查。
mmw_warn_ext_skills() {
  local main_dir="$HOME/.claude/skills" worker_dir="$HOME/.agents/skills"
...
cmd_where() {
  ...
  if ! m="$(find_manifest)"; then
    echo "UNMANAGED"
    echo "当前没有在管任务。仅在入口已确认需要 MMW 治理能力时，选择场景并新建任务:"
...
repair_count=$rc
turnaround_count=$tc
turnaround_ledger=$(jq -rc '.turnaround_ledger // {}' "$m")
phases=$phases
...
EOF
  ...
  loopf="$top/$MMW_STATE_SUBDIR/loop-state.json"
  if [ -f "$loopf" ]; then
    local lstate
