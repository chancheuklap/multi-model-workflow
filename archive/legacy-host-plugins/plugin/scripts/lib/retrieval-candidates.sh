...
mmw_retrieval_candidates_snapshot() { # $1=输入绝对 JSON（空=无结构问题） $2=快照落点
  ...
  jq -e '
    type == "array" and all(.[];
      type == "object" and
      ((keys | sort) == ["fallback_reason","locators","query","status","summary","tool"]) and
      (.tool == "graphify" or .tool == "serena") and
      (.query | type == "string") and
      (.status == "used" or .status == "not_available" or .status == "unsupported" or .status == "failed") and
      (.locators | type == "array") and all(.locators[]; type == "string") and
      (.summary | type == "string") and
      (.fallback_reason | type == "string")
    )' "$input" >/dev/null || {
      echo "ERROR: retrieval candidates 必须是精确六字段数组(tool/query/status/locators/summary/fallback_reason)" >&2
      return 2
    }
  jq -S '.' "$input" >"$output"
}

mmw_retrieval_candidates_prompt() { # $1=规范化快照
