...
> `prev_outputs`:small-change 无上游产物;bug 带 investigate 钉的根因报告(`docs/design/<slug>/investigating.md`)。
...
每步严格 TDD(循环用已装 `/tdd` skill)。测试的资格线按下面完整权威(与 Codex 工人、终审同一份单源)，同时遵守目标仓库项目指令链公开的测试规则:
