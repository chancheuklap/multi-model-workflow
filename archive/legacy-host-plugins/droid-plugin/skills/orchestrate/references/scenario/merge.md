...
读各 PR 的 design 文档建立方向,**回仓库级架构验**(读 `CONTEXT.md` 类领域模型,需要时用外部 `improve-codebase-architecture` skill(对模型不可见,Read `~/.factory/skills/improve-codebase-architecture/SKILL.md`)或 `Explore` 验代码、省你 context)。**并行**扫所有 PR,七维度:
