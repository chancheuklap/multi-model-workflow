...
触碰不变量 / 合同 / 数据权威 / 权限 / 计费 / 发布风险时,尤其确保上面每条都过。自检本身就是这一步的质量门;**自检完不另起一道复查**——下一步的设计预审已由独立审者跑。

## 收尾:handoff 交还引擎(`mmw where` 的 `then` 已给好钉产物的命令模板,照抄即可)

- 自检全过 → 先跑 `mmw where`，照 `then` 中已经解析好的唯一主设计文档路径执行 `mmw pin`；再以同一路径起设计预审：`mmw review start --stage design --source <主设计文档>`，脚本自动把 accepted prototype README + selected 加进同一 Source；照 brief 派审者，findings 落盘亲验标处置。禁止同时新建单文件版和目录版。
