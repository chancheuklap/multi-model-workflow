...
三个方向的取舍彼此接近、现状报告里有相互矛盾的证据、或选定方向会锁死难以回头的架构 / 数据权威时,`Task`→`decision-advisor` 调一次:prompt 给原始任务、调查与已排除方向、现状报告要点、报告路径与承重 `file:line`/`url`、准备亮给用户的 2-3 个方向及取舍;让它判 framing 是否解对问题、最大失败模式是什么、什么证据会推翻判断,不重写方案表。方向已被一手证据唯一决定、或走降级 propose(`direction-given`)时不调。

用户仍拍板;decision-advisor 只补风险 / 取舍,不替用户选。本阶段最多 1 次。
