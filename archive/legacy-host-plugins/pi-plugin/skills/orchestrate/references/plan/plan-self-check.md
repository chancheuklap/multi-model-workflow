...
plan 阶段末步:就绪门自检过了就 `mmw handoff` 交还引擎,引擎触发 `reviewer-plan-a/b` 两轴独立计划审。本阶段不自派审、不自己跳阶段。计划由 plan-writer 写成,主 Agent 本步做的是**验收 + 跨 plan 视角**:per-pack 就绪门由 writer 交付前自检,主线程抽验;多计划再从跨 plan 视角过一遍覆盖与 ownership。
