...
三个方向的取舍彼此接近、现状报告里有相互矛盾的证据、或选定方向会锁死难以回头的架构 / 数据权威时,调一次 advisor 工具(零参数，自动转发全对话；它要判的是 framing 是否解对问题、最大失败模式、什么证据会推翻判断，不重写方案表)。方向已被一手证据唯一决定、或走降级 propose(`direction-given`)时不调。

用户仍拍板;advisor 只补风险 / 取舍,不替用户选。本阶段最多 1 次。
