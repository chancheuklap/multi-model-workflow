...
调查开始时，MMW 没有 Codex 专用浏览器动作。共享技能只在 bug 诊断和外部系统取证中提到 Playwright。界面原型、视觉合同、实施验收和终审已经定义了可观察结果，但没有规定 Codex 如何打开页面、向用户展示页面、保留标签页和采集证据。

本任务分支已把目标状态落到八条流程：UI grilling、界面原型走查、外部系统取证、界面 bug 复现、界面 ticket 验收、final 审查证据、plan 验证分工和 spec 视觉合同。Codex 通过宿主动作使用内置浏览器；其他宿主保留各自的浏览器入口。
