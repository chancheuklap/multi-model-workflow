...
### A1 验收标准的编号与人工项的写法（**只剩编号还活着，其余被 I2 与 I6 取代**）
...
- 用户裁决：「都可以」。
- 结论：三条全采。
- 后续：`MANUAL:` 这个写法被 I2 取代；I6 让 `## Acceptance criteria` 只收机器判得了的事，「人工项」与「不过半」一并没有了对象。只剩「`AC<n>` 出票时编、不重排」还活着。
...
### I6 验收标准只收机器判得了的事（取代 I2 的第二条出路，取消 A1 的第三条）

- 触发：落地 #68 时，票上三条没有 `CHECK:` 的标准每一轮自跑都显示未过，第三轮被点名要求登记成「跑了没过」，而它们实际都成立。追下去发现这不是脚本的毛病。
- 现状与设计目的：整条流水线要回答的是「一张票说自己做完了，凭什么信」，答案是把「做完了」从一句判断变成一件可重放的事实——出票时每条标准带命令，机器跑，再换一个 agent 跑同一批命令。**「谁跑」和「谁判」分得开，前提是「跑」由机器做。** I2 新造的第二条出路（读者是 agent、写不出命令 → 留在票上由 worker 自己判）恰好从这道前提下面穿了过去：这类标准的判定人就是写代码的那一个，而且票面上看不出它和别的条不一样。
- 后果三层：① 跑标准那一步数「连续几轮没过」，数的对象是「跑了、失败了」，而这类标准根本没跑过，于是健康的票被要求把成立的标准登记成失败；② `agents/verifier/body.md` 写着「票上每条标准都带命令，没有要你用眼睛判的东西」，复验看到它们未过只能选「跑了但有没过的」——一张好票拿到一个说它坏了的判决，而那是关票门唯一认的东西；③ 就算前两层都补好，这类标准仍然只经过写代码的那一双眼睛，正是 #60 Problem Statement 第一句针对的东西。
- 摆出的选项：① 取消这一条出路；② 保留，但判定权交给 verifier；③ 保留，只修机器的误判，并在收尾评论里把它显式列出来。
- 讨论中确认的判据（用户裁定）：不许拿这一批落地票当证据——「不能使用改造中的 mmw v2 去改造它自己」「这次的 spec 和 issue 是服务于这次改造的，不是需要被改造的内容」；说不出背后逻辑的就是臆想。于是改从「一条命令什么时候写得出来」推：要同时满足三件事——说的状态机器看得见、机器够得着、判定规则是一次比对。坏第一条与第二条，读者都是人；**坏第三条（判定是一次评价、材料够得着）才是「读者是 agent、写不出命令」的唯一来源**。而这一类在流水线里早有一个不是 worker 的判定人：code review，由 worker 另起一个会话跑，两条轴，其中 `Spec` 轴问的正是「这次改动跟票和 spec 要的一不一样」。
- 用户裁决：「收窄」。
- 结论：`## Acceptance criteria` 只收判定规则是一次比对、材料机器够得着的事，每条都有 `CHECK:` 与 `EXPECT:`。写标准时按五问顺次判，停在第一个 yes：比对且够得着 → 标准；评价且够得着 → code review；被判的性质是一个人的反应 → `reaction` 类的 `ready-for-human` 票；机器判得了但够不着 → `reach` 类的 `ready-for-human` 票，并写明补上什么之后它就不需要；不是在验而是在取舍 → 取个默认值往下走、写进收尾评论，走不下去才是决策票，且该在出票之前问。
- 连带取消：A1 第三条「人工项不过半」——没有对象了。
