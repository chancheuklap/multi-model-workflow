...
**技能产物**：
从技能源为 Pi、Claude Code、Codex 或 Cursor 物化的宿主版本。
_Avoid_: 技能源、手工分叉
...
**Cursor 任务树与结果树**：
Cursor 为任务和结果创建的 linked worktree，物理位置在 `~/.cursor/worktrees/<仓库>/<slug>`。主 agent 用 `mmw task bind` 绑定；`mmw task new` 与 `mmw task cleanup` 在这个宿主上不可用。
_Avoid_: 仓库 `paths.worktrees`、`mmw task new`
