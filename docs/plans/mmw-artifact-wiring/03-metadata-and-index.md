...
- 03 独占现有 14 份 ADR 和两个索引副本。它只改 `AGENTS.md` 的 ADR 读取句。（spec:833-839）
...
- `ADR-FORMAT.md` 有上游对应项。先按 `upstream-skill-fidelity` 把本次变化归类为已批准的 MMW 接线。保留简短 ADR、可选章节和编号方法。
...
| `mmw/skills-src/mmw-domain-modeling/ADR-FORMAT.md` | Modify | 把 ADR 模板固定为 `date` 与 `amends`，移除旧 `Status` frontmatter 选项。 |
...
| `docs/adr/0001-*.md` 至 `docs/adr/0014-*.md` | Docs·迁移 | 为现有 14 份 ADR 补两字段元数据块。 |
...
- **Metadata:** spec 固定六个字段：`slug`、`summary`、`date`、`branch`、`spec_issue`、`artifact_refs`。plan 固定两个字段：`ticket`、`artifact_refs`。ADR 固定两个字段：`date`、`amends`。（spec:405-437）
...
2. **三类生产方写出固定元数据块**
   ...
   - Change: 在 `ADR-FORMAT.md` 中保留上游正文方法。用 spec 已批准的两字段元数据合同替换旧 `Status` frontmatter 选项。
