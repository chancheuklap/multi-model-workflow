...
- CLI 主入口只在顶层用法和顶层分发各增加 `artifact` 一行。不得调整其他子命令。来源：Cross-Plan Contract Anchors。
...
| `mmw/cli/mmw` | Modify | 只在顶层用法和顶层分发各增加 `artifact` 一行 |
...
3. **CLI 只接入 `artifact` 分区**
   ...
   - Change: 顶层 `case` 增加一行。该行把参数原样交给 `artifact.sh`。
   - Change: 详细用法、两列表和 `path` 动作都留在 `artifact.sh`。
   - Change: 不重排或改写其他子命令的顶层用法与分发行。
   - Files: `mmw/cli/mmw`、`mmw/cli/tests/test_artifact.sh`。
   - Verify: `bash mmw/cli/tests/test_artifact.sh` → `mmw artifact` 列出全部类别；`mmw artifact path` 可从顶层入口执行。
