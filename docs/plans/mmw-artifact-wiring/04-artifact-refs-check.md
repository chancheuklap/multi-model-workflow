...
**Goal:** 固定结构承载产物引用。`mmw artifact check` 能检查全部 spec 与 plan，并在生产方完成文件后运行。
...
- `/mmw-to-spec` 发布 spec issue 时只要求摘要、spec 路径和输入出处。它没有固定的 `## 产物引用` 节。（`mmw/skills-src/mmw-to-spec/SKILL.md:79-89`）
...
- `bash mmw/test.sh` 通过 `run` 汇总各测试入口。01 负责加入 `test_artifact.sh` 的入口行。（`mmw/test.sh:15-33`；Cross-Plan Contract Anchors）
...
| `mmw/skills-src/mmw-to-spec/SKILL.md` | Modify | 把实际使用的产物写成两种声明，并在 spec 写完后运行 `mmw artifact check`。 |
...
3. **把产物引用传过五个固定位置**
   - Change: `/mmw-to-spec` 从本次实际使用的 prototype 资产和 research 形成产物引用。spec 元数据块使用 03 的 YAML 形状。spec issue 固定写 `## 产物引用`，并使用 Markdown 形状。
...
5. **物化并完成整仓验证**
   ...
   - Verify: `mmw skills materialize --host all` → 三个宿主产物生成成功。
   - Verify: `mmw skills materialize --host all --check` → 没有缺、异或多出的技能产物。
