...
- 四栏 task 与角色报告不落盘。派发进度日志仍在失败时落盘。来源：spec 第 17 节、ADR `0004-no-dispatch-files.md:1-16`。
...
- 派发进度日志依赖 plan 01 的路径命令和 plan 02 的工作名字段。两份合同可先并行编码，但本 plan 的日志验收要在它们集成后执行。
...
| `mmw/cli/adapters/claude-code.sh` | Modify | 让两条模型路径消费正文。删除角色报告文件。把失败日志写到新的派发进度日志落点。 |
| `mmw/cli/adapters/pi.sh` | Modify | 用正文构造原生 `subagent` 参数，不再读取 task 文件。 |
| `mmw/cli/lib/materialize_skills.py` | Modify | 改 Claude Code 派发动作块。动作块直接传正文，不再要求先写 task 文件。 |
| `mmw/cli/tests/guardrails.sh` | Test | 覆盖两种正文入口、逐字传递、报告标准输出、失败日志和既有拒绝语义。 |
| `mmw/cli/tests/test_materialize_skills.py` | Test | 覆盖各工作目录模式与审查启动组的新派发动作。 |
| `mmw/skills-claude-code/` | Modify（物化） | 保存从技能源重新物化的 Claude Code 派发动作块。 |
| `mmw/skills-pi/`、`mmw/prompts-pi/`、`mmw/skills-codex/` | Test（物化检查） | 确认同次全宿主物化没有漂移或遗留标记。 |
...
- **Consumes — plan 01:** 使用 `mmw artifact path scratch [--name <工作名>] [--issue <编号>] --sub dispatch --absolute` 取得日志目录。不得在 adapter 内重写路径公式。
- **Consumes — plan 02:** `mmw task state` 的 `bound` 行第四字段是工作名。adapter 在 `MMW_D_CWD` 对应的 worktree 中解析路径，让 plan 01 缺省读取该字段。
...
2. **一次切换 CLI 与全部宿主 adapter**
   ...
   - Change: gpt 执行路径删除 `.dispatch/`、`-o` 和 `report:`。标准错误仍只进入派发进度日志。
   - Change: 日志目录通过 `mmw artifact path` 取得。文件名改为 `<角色>-<时间戳>.log`。成功时删除日志，失败时保留并输出路径。
...
3. **同步全部派发动作块和技能产物**
   ...
   - Change: 扩展物化测试，覆盖 `worktree`、`current`、`none` 和审查启动组。断言产物不再要求 task 文件，也不再出现旧参数。
...
| 派发进度日志使用新名称和路径形状 | 让假 `codex` 失败，检查保留日志的绝对路径和文件名 | `bash mmw/cli/tests/guardrails.sh` → 路径含工作名及可选范围段，文件名为角色加时间戳 |
