...
- 仓库根类别不形成可扫描的目录前缀。解析固定类别根时不得让空值或仓库根值变成匹配任意占位符的正则。来源：spec 第 2 节的 `context-map` 记录。
- 扫描范围是 `mmw/skills-src/**/*.md`。必须排除只保存旧背景材料的 `mmw/skills-src/mmw-setup/`。来源：ticket `#47`、spec Testing Decisions。
...
- 不校产物质量、方法选择、完成度、字面值数量、目标仓库的实际产物，或反向相关性。不得增加计数阈值和豁免清单。来源：spec Testing Decisions、ADR `0009`。
...
- **Consumes — 07、08 → 11:** 两份 plan 交付改写后的 `mmw/skills-src/`。本 plan 扫描它们的合并结果，不修改技能源，也不增加临时豁免。
...
1. **两条规则先在一次性测试目录中固定**
   ...
   - Change: 从 `artifacts.json` 解析固定类别根。对目录根统一末尾斜杠，再安全转义成正则。跳过不能形成目录前缀的仓库根记录。
   ...
   - Change: 在一次性测试目录的 `mmw-setup` 下放入同类错误。证明该目录被排除。
...
2. **在 07 与 08 的最终技能源上执行真实扫描**
   - Change: 按文件名排序扫描 `mmw/skills-src/` 下全部 Markdown。按路径段排除 `mmw-setup`。
...
| 旧背景材料目录不进入扫描 | `mmw-setup` 下的违规测试文件不产生失败 | `bash mmw/cli/tests/test_skill_paths.sh` → 排除目录用例通过 |
