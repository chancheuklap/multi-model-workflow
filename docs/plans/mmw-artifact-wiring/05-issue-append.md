...
**Goal:** 两个并发追加调用保住各自的新行，已有 issue 也能通过同一套父子关系端点设置父 issue。
...
- issue 实现和 `test_issue.sh` 归本 plan 独占。CLI 主入口只增加这两个动作的用法与分发行，不重排其他行。来源：Cross-Plan Contract Anchors。
...
- `mmw/cli/mmw:132-164` 的 issue 用法目前只列五个动作。正文修改仍指向直接使用 `gh issue edit`。
...
| `mmw/cli/mmw` | Modify | 只增加 append 与 set-parent 的用法段和对应分发行。 |
| `mmw/cli/tests/test_issue.sh` | Test | 扩展 stateful `gh` stub，并覆盖参数、时序、两项检查、重做、并发和父子端点失败。 |
...
3. **复用父子端点，并公开两个 CLI 动作**
   ...
   - Change: 在 `usage_issue` 只加入 append 与 set-parent 两段。在 `cmd_issue` 只加入对应分发行。
   - Files: `mmw/cli/lib/issue.sh`、`mmw/cli/mmw`。
   - Verify: `bash mmw/cli/tests/test_issue.sh` → set-parent 成功请求和端点失败用例通过；两个动作的无参或缺参调用返回 issue 用法和非零状态。
