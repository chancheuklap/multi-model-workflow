...
- [x] 钥匙没声明 `fix_executor` / `derive` 时，`cmd_dispatch_direct` 停成 `needs-context`
      并说清楚原因，不再 `die`——那会把「这把钥匙没配这个」说成引擎坏了。
- [x] `post_fix_gate` 缺席时记 `skipped` 直接返回。「跑了并通过」和「压根没有这一关」
      在回执里必须分得开。
- [x] `event_sink` 缺席时不落地，直接返回。
- [x] 交付目录缺省值不再是 `D:\agentflow-releases`，改成从构建机已经给的事实推
      （远端根目录旁边的 `<root>-delivered`）。
- [x] 路径硬禁止清单给一份技能自带的底线（`.env`、私钥、证书、锁文件、`.git/`、
...
- [x] **`diagnose` 给缺省值。** 四把钥匙里这一段一模一样，只有 `--adapter` 后面的路径不同，
      而那条路径引擎手里就有（`manifest_path`）。钥匙不声明时引擎用技能自己的
      `diagnose_core.py`；声明了才用钥匙的。
- [x] **标准三阶段由引擎提供。** 每把钥匙的 `assemble` 与 `build` 逐字相同，只有钥匙路径不同——
...
- [x] 技能侧渲染 setup/teardown。
- [x] 引擎把 `remote-build.json` 的 `build_env` 放进 `release-context.json`。
- [x] 两个仓库的 `build_machine_setup.py` 只留产品自己那部分。
...
- [x] `python_backend.builder`：只有一个取值的 Literal。
- [x] `python_backend.output_mode` 与 `folder_per_target`：四个产品全是 onefile。
      改成永远 `--standalone --onefile`。
- [x] `python_backend.include_data_files`：没人用，`include_data_dirs` 覆盖了真实场景。
- [x] `electron.package_manager` / `install_args` / `build_script`：五个产品全是 pnpm +
      `install --frozen-lockfile --prefer-offline` + `run build`。改回装配器里的常量。
      （`_required_tools` 跟着改成常量。）
- [x] `NativeExtDll.dll_source` 的 `"repo"` 取值与 `repo_dir`：没人用。
- [x] 没有 Electron 外壳的产品那条分支：没有这样的产品。`electron` 与
...
- [x] `key.md` 里对应的段落同一批删掉：`output_mode` 那一行、`electron` 段的前三个字段与
...
- [x] 传完就删两端的 `source.zip`（上传后删 Mac 那份，解压后删构建机那份）。`SOURCE_COMMIT.txt`
      留着，源码随时从 git 取回。
- [x] 构建**成功**且安装包已收拢到交付目录后，删掉远端整个构建目录。**失败的留着**——
      现场只存在于那里。
- [x] 失败目录只留最近两个，再老的在下一轮开工时一并删。
- [x] `init` 不能继承上一轮的 attempt 目录。

四条都在 `f6ff12a9`。假构建机跟着改成真的执行这几段语义（删 zip、按 glob 收拢安装包并打
DELIVERED），否则测的是引擎说了什么，不是构建机上发生了什么。
...
- [x] `assemble` 完成之后改技能，`_loop/release.ps1` 就过期了，而引擎只按产品仓库的 HEAD
      判断要不要重来一遍。于是 `stage run --stage build` 拿旧脚本去构建机跑，日志里每一步
      都对——只是跑的不是你刚改的那一份。现在只能手工照 `stages[assemble].run` 重装配一次。

技能开发期天天撞；正式出包时，如果技能在两个阶段之间被升级，同样会撞。要么把生成脚本的
输入也纳入「变了就重来」的判断，要么允许显式重跑一个已完成的 stage。

## 1f. 钥匙能悄悄关掉引擎的流水线

小刺猬的钥匙里抄着 `assemble` 与 `build`。旧规则是「钥匙用了标准阶段名就整条自己负责」，
于是引擎的 `verify_key` 一段都不加——它整整一轮跑在一条没有验钥匙的流水线上，日志每一步
都是绿的。**是抄，不是选**：那两段是从别的钥匙复制过来的。

- [x] `verify_key`/`assemble`/`build` 定为保留字，`StageSpec` 在 init 之前拒。
- [x] 引擎的合并规则去掉条件分支：钥匙的 stages 在前，标准三段永远追加在后。
- [x] `key.md` 与根 `AGENTS.md` 改写这一段。

## 1g. 每个产品都要的检查还留在一个产品仓库里

- [x] 钥匙里每条 argv 引用的仓库脚本存不存在 → 技能的 `verify_key.py`。
- [x] electron-builder 的 `directories.output` 与钥匙的 `electron.dist_dir` 是不是同一处
  → 同上。
- [x] agentflow 的 `release_key.py` 只留混淆串接、`asset_roots` 与资产树、派生 runtime 包
  目录重生这三件。

## 2. 删掉 v1

- [x] `release_contracts.py`：`SchemaVersion` 只剩 `"2"`，删 v1 的分支与
      `_version_matches_content` 里的 v1 段。
- [x] `release_script_assembler.py`：删 `_render_bootstrap` 的 v1 分支、`_hook_calls`、
      `_render_hook_calls`、`_render_hook_functions`（v1 那份）、`_check_v1`。
- [x] 删 `release_templates/windows_core_exe.ps1.tmpl` 与
      `release_templates/windows_electron_python.ps1.tmpl`。
- [x] 删 `build_target` 里 v2 不再用的字段：`runtime_lane`、`entry_module`、
      `nuitka_include`、`nuitka_nofollow`、`deps_extra`，以及 `RuntimeLane`。
      （`extra="forbid"`，所以删字段和改钥匙必须同一批做完。）
- [x] `tests/test_release_script_assembler.py`（v1 那份）随之删掉或改写。

## 3. 自检入口由钥匙说了算

- [x] `BuiltExeSmoke.run_module` 换成 `args: list[str]`：模板现在把 `--run-module <模块>`
      写死了，那是我们三个产品的约定，不是通用事实。换成钥匙给整串参数。
- [x] 模板 `Invoke-BuiltExeSmoke` 的 `-Module` 改成 `-Arguments`。
- [x] 四把钥匙跟着改。

## 4. 钥匙改回正名

- [x] `agentflow`：`{duck,parrot,hedgehog}.release-adapter.v2.json` → 覆盖同名的 v1 文件。
- [x] `douyin-master-resolve`：`douyin-master-resolve.release-adapter.v2.json` → 同上。
- [x] 两个仓库里指向 `.v2.json` 的地方一起改：钥匙自己的 `stages`/`diagnose`、
      两份测试、`tests/support/release_contracts_plugin.py`。
- [x] 改完 `SKILL.md` 第 2 步那条 grep 才扫得到它们（`*.release-adapter.json` 不匹配
      `.v2.json`，并存期间就是要这样）。
...
- [x] 三个仓库的测试全绿（agentflow 只剩两条要连 Postgres 的既有失败，抖音两条既有 UI 失败）。
- [x] 小刺猬 2.3.0 用新通路真出过一个包。
- [ ] 小黄鸭、鹦鹉、抖音各出一个包——这三个产品一次都没走过新通路，它们的钩子、
  钥匙与产品侧脚本还没有被真实构建验证过。
- [ ] 用户单独确认后再删。
