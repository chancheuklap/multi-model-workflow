...
**lint**:
`verify-ticket.py <n> --lint`: gate-lint, plus the ticket graph (`cycle`, dangling references, `cross-batch`), plus the worker-label check, plus the screen-contract checks. Given the spec instead of a ticket — an issue with no `## Acceptance criteria` carrying `mmw:spec`, or, with no layer label, one with no parent and sub-issues of its own — it checks every sub-issue the same way, closed ones included, then the graph once, and ends with the list of open tickets that had an `ERROR`; a closed ticket's `ERROR` is printed with a `closed-ticket` `WARN` and does not affect the exit code, because it is never started again. Every finding carries a problem tag and a level, `ERROR` or `WARN`; only `ERROR` affects the exit code. When the graph has no cycle and no dangling reference it prints the **start levels** — the order tickets may be started in, built from this spec's own blocking links. A batch converges when `ERROR` is at zero and every `WARN` has been looked at and either fixed or kept on purpose. It is run at the read-back step, and again before a night's first `advance`.
