...
ADR 0004 把工作名和任务分支名做成两个值，并把工作名写进 git worktree 配置。一项 Wayfinder effort 有多条任务分支，用各条 ticket 分支名当产物目录会把同一项 effort 的 research、prototype 和 spec 散开。这个理由仍成立。不成立的是「所以必须另存一个工作名」。
...
**用每条当前分支的 slug 当名字段，Wayfinder ticket 也不例外**（否决）。这就是 ADR 0004 否决过的散开问题：同一项 effort 的产物会落到多个目录。
