...
- 0018 的边界多一个动词 `self`（本进程所在会话的号：0 打出、3 不在这个 runner 里、1 在但读不出）。它 Consequences 的最后两条不再成立：`verify-ticket.py` 不再用 `paseo send`，Orca 与 Herdr 上的调用方不再靠反复 `wait`。Orca 的 `send` 回执按实测的形状读（`result.send.prompt.stages`、`result.warnings`）。`send` 的 4 收窄成「字已交给会话、没看到回合开始」（Orca 的 `input_accepted` 而无 `turn_started`、Herdr 的 `agent_prompt_stalled` 与 `timeout`），中继把它当送到、不在下一轮再打一遍；什么都没送出去（列表读不到、会话正忙、runner 自己报错）一律答 3，中继下一轮再送。
