...
ADR 0006 规定技能正文只写类别名和 `mmw artifact path`，不写路径字面值。把这条规定变成机械校验时，判据定成两条正则规则：类别根字面值后紧跟一个 `<…>` 占位符段就失败；工作目录根的默认取值出现在技能源正文就失败，不论后面跟不跟占位符。固定类别根不带占位符地单独出现则通过。理由是机器分不出「散文里提到一个知识库」和「拼一条落点」这两种写法，而 `AGENTS.md` 禁止用豁免清单撑着的校验。
