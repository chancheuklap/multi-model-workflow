...
Claude Code 在同一条件下答「没有」，它的二进制里 `.agents/skills` 零命中，技能根目录全部落在 `.claude` 体系内。能追加技能目录的设置键也搜不到——`skillDirs`、`extraSkill`、`additionalSkill` 三个都零命中。插件可以从别处带技能进来，但 ADR `0018` 已经否掉了插件这条路。
...
- ADR `0018` 的「五个宿主由 `install.sh` 统一散装」对 subagent 仍然成立，对技能不再成立。0018 取消插件打包、由安装器散装这个决定本身没有变。
