...
- `spec.merged` 是新 merge 推送成功的事实，也是重跑边界：有它以后只补做逐项清理，不再产生第二个 merge commit。base branch 已经包含在 project branch 时不再合并，直接逐项清理。
...
- **同时合回默认分支。** 否决。project branch 何时发布到默认分支是用户的发布决定，不属于一夜的 branch 清理。
