...
- **C2. 合同机器强制 + 执行纪律三层迁移**。plan-return 合同对 Codex 比对 Claude 子代理更可强制：`--output-schema <plan-return-v1 派生 schema>` 强制最终回复结构，或直接要求 Codex 写 `plan-return.json`（`--add-dir` 已放行）。
  现 `pack-executor.md` / `complex-pack-executor.md`（各约 300 行）承载的执行纪律按三层迁移，逐条有着落、零丢失：
  1. **派工手册层**（纪律大头）：核心纪律（scope 边界 / owned files / 禁改 docs / 缺字段退回 needs-plan-revision）、实现要求（测 public behavior / mock 政策 / Pydantic contract / Mockup specs）、Worker Loop 全套（5 步启动序列 / Pack 循环 / TDD 红绿 / 三次失败协议 / scope drift 自检 / need-fresh-worker 自监控）——全是文本协议，迁为 Codex 派工 prompt + Read 同一份 `execution-worker-dispatch.md` handbook（Codex 读文件能力同 Claude）。Worker Loop 是 build 模板锚点内容，模板新增 Codex 版 handbook 产出目标。
  2. **AGENTS.md 层**（项目感知，零成本）：「读 CLAUDE.md / ENGINEERING-RULES / AGENTS.md 理解项目约定」→ Codex **原生自动读 AGENTS.md**；本仓各目录的 `AGENTS.override.md` 体系正好直接生效。
  3. **Codex skill 层**（用户确认 2026-06-10）：frontmatter `skills: tdd` 及正文 `diagnose` / `prototype` 调用 → Codex 侧**已同步安装同名 skill**，派工 prompt 点名调用即等价。
  frontmatter 其余字段映射：`model`/`effort` → CLI 参数；`tools` 白名单 → 沙箱策略；`memory` → AGENTS.md 体系。
