...
- **C2. 合同机器强制 + 执行纪律三层迁移**。plan-return 合同对 Codex 比对 Claude 子代理更可强制：`--output-schema <plan-return-v1 派生 schema>` 强制最终回复结构，或直接要求 Codex 写 `plan-return.json`（`--add-dir` 已放行）。
  ...
  2. **换载体（约三成，目的保留、机制换 Codex）**：SendMessage 续修 → `codex exec resume`；SubagentStop → agent-return-handler 返回解析 → 写 plan-return.json + Coordinator 读产出（C3）；「guard-doc-edit hook 强制拦截」表述 → 沙箱围栏 + 合并前 docs diff 检查（C6）；**`enforce-plan-commit` 的 commit 格式 hook 校验对 Codex 同样失效** → 回收时校验 git log 格式（C3 ingest 顺带）；`Skill({...})` 调用语法 → 「使用 Codex 侧已装 tdd / diagnose / prototype skill」的 prompt 指令（用户确认已同步安装）；frontmatter `model`/`effort`/`tools` → CLI 参数 + 沙箱策略；`${CLAUDE_PLUGIN_ROOT}` 路径变量 → 派工 prompt 注入绝对路径。
  3. **直接删（约一成，Claude 专属且不造等价物）**：Memory 策略段（`.claude/agent-memory/`——其内容本就该沉淀到 Codex 原生读的 `AGENTS.override.md`）；voice-directive 风格调教段（含禁止词表）；模式 2b 定向修复（属 bug-investigation 等不换轨场景，留在 Claude agent 文档）；所有对 Claude hooks 行为的描述性引用。
  **试跑校准两项**：need-fresh-worker 阈值（现 packs_in_session ≥ 5 按 Claude 上下文压力定，协议保留——「从 execution-state 重建计数」平台无关——阈值按 Codex 实测调）；UI Pack 浏览器验证（mockup 对照）的 Codex 侧 skill 覆盖待试跑确认。
  **维护形态**：`codex-worker-handbook` 由 build 模板生成，与 Claude 版共享「保留类」锚点源（纪律只维护一份），「换载体/删除」差异段独立维护。AGENTS.md 项目感知层照旧：Codex 原生自动读，各目录 `AGENTS.override.md` 直接生效。
...
- **C7. Claude executor 退役盘点**。按「不做兼容、删干净」纪律：C 全面接管 formal/light execution 后，`pack-executor` / `complex-pack-executor` 在 execution 主路径的引用退役。已知保留引用场景（本次不换轨）：multi-pr-merge 冲突修复 worker、bug-investigation 修复路径、direct-repair 路径 B（SendMessage worker）。盘点结论预期：两个 agent 定义**保留**、frontmatter 下方文档标注「仅存上述引用场景，execution 主路径已由 Codex 接管」；待这些 route 二期换轨后再物理删除定义。execution SKILL.md / handbook 中指向 Claude executor 的派发模板从主路径移除。
