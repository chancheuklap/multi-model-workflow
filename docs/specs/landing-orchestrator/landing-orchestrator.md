...
**停车 issue。** 每卡点一张，标签 `blocked:decision`，挂任务父 issue 为父；正文固定四段：问题、选项、各选项后果、不裁决时的默认（格式蓝本是 pstack `store.ts` 的 `renderGates` 渲染器，`gates.md` 是它生成的产物；加「后果」段）。停车不阻塞循环，该票让路。
