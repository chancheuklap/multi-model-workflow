...
- **句柄的形态**：gpt 族句柄是 `codex exec` 的 `thread_id` 原文；claude 族句柄是确定性生成的名字 `<角色>-<task 文件名去 .md 扩展名>`。`--resume` 只接受句柄原文，不接受文件路径。
- gpt 族派发加 `--json`，从事件流第一行取 `thread_id`，写入句柄文件 `.dispatch/<角色>-<task 文件名去 .md 扩展名>.session`，内容是单行句柄原文；报告改由 `-o` 文件唯一承载；`mode: executed` 输出增加 `session: <句柄原文>` 一行。
...
- 新增 `[[mmw-resume:<角色>]]` 动作块，与 `[[mmw-launch:…]]` 同构，由 `mmw skills materialize` 按宿主整块展开。
...
- **dispatch 输出合同**：`mode: executed` 的行集从「report、log、exit」扩展为「report、session、log、exit」；host-tool 输出增加 `handle:` 行。归属方是 Claude Code 宿主 adapter；消费方是技能产物里的派发指令和主 agent。句柄文件与报告文件同目录、同名异后缀，生命周期跟随现行 `.dispatch/` 清理规则。
- **`[[mmw-resume:<角色>]]` 物化合同**：技能源写动作块，物化层按宿主整块替换；技能正文不出现宿主名称分支。归属方是物化层；消费方是三个宿主的技能产物（Pi、Claude Code、Codex——Cursor 只有 agent 物化面，没有技能产物）。
