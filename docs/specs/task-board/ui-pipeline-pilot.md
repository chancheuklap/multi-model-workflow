...
试点计划是 #541：把任务板当成新产品，从 `wayfinder` 走到 `finish`。设计决定取自生产版任务板，不重新讨论；产物由新流程真实生成。本文件按步骤记录每一步技能实际怎样执行（讨论部分是文字模拟），以及发现的衔接问题。最后一节回答 #541 的待回答问题。
...
### 14. 实现与验收（#556–#564，不开 night）

**做法（有意偏离）。** 不开 night：夜间调度（派发、唤醒、夜里合并、看门程序）不是本试点要检验的东西。每张实现票由 Orca 起一个 Codex worker（`gpt-5.6-sol`，high），在自己的 `.worktrees/issue-<n>` 里按 `implement` 做，只在本地提交，把汇报写进文件。主 session 验收：重跑票上每条 `CHECK:`、跑 board 测试集、核对改动不出 `## Owns`、在测试数据上启动任务板截图；全过才合进 `task-board-pilot` 并关票。流水线本身的问题先修 MMW，再让 worker 继续。次序：#556 → #557 → #558–#561 并行 → #562 → #563；#564 由用户在测试数据上看新版任务板。没有跑 `code-review`、`retro` 与 `finish`。
...
- 验收用的启动程序只要页面能打开就算启动成功，而测试用的假 GitHub 没有准备任务板要问的仓库名，每次读都失败，看板是空的（#556 第一轮验收截图发现）。MMW 的规则本来就要求"能用才算启动成功"。已修（提交 `07831a46`）：假 GitHub 的答案补齐；第一次读数据失败时 `start` 直接拒绝，并说出缺的是哪个答案；加测试。

## 待回答问题的答案（#541）

| # | 问题 | 答案 |
| --- | --- | --- |
| 1 | 非 React 的 design system 能否从生产代码建成，Claude Design 能否照它画页 | 能。design system 从 `mmw-v2/board/page/` 的样式建成，本地打包组件、design-sync 上传；设计页用它的组件画出，线上与离线渲染都通过（第 6–8 步） |
| 2 | 空 design system 能否由 agent 经 MCP 写入 | 能写入文件，但 Claude Design 不会因外部写入重新编译组件包；组件包要在本地打包后一并上传（第 6 步） |
| 3 | design system 改值后，绑定项目的 `_ds/` 是否自动更新；备用办法 | Claude Design 首次放进项目的副本只有样式与空组件包；用 `copy_files` 从 design system 刷新可行，已实跑（第 10 步，第 24 行）。改值后是否自动跟随，没有单独测 |
| 4 | pull 是否取到 `_ds/`；注入片段是否让大小检查一次通过 | 取到。注入格式变化曾让每页差 1 字节（第 25 行）；大小检查随后整体去掉（第 30 行） |
| 5 | 用户编辑后 `data-ui` 是否保留；复制控件是否产生重复 id | 未回答：设计页取自生产版，用户没有在 Claude Design 里改过页面 |
| 6 | 新的 scene data 形状能否喂给各 story adapter 与整页 | 不能单靠显示文字；加了 scene input（第 42 行）后，44 个场景全部喂得进，最终 `STORY OK 44/44` |
| 7 | element parity 的误报条数与类型；容差要不要调 | 最终 44 个场景全部一致，容差没有改。实现过程中每轮 `DIFF` 的条数没有记录，误报率给不出数字 |
| 8 | 合同行数 | 68 行，旧版 222 行（第 11 步） |
| 9 | `--break` 能否让"本机配置保存"在接口失败时判失败 | 能。#563 的 journey 在保存接口故意失败时判失败；修好端口误判（第 63 行）后通过验收 |
| 10 | UI axis 的 token 用量与发现；盲测 | 未回答：没有开 night，没有跑 `code-review` |
| 11 | 各步的衔接问题 | 见"发现"一节第 1–64 行 |
| 12 | #444–#447 哪几节要修订 | 未评估。发现都已直接修在技能与脚本里，没有逐节对照这四份 spec |

**三个决定。** `dev` 合并进 `main`：用户 2026-09-22 决定合并。UI axis 留不留：本试点没有数据，未决定。#444–#447 的修订：未决定（第 12 问未评估）。
