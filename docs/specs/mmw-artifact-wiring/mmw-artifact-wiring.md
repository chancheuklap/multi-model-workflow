...
多数文件只有一份 plan 会碰。下面五处会被多份 plan 同时碰，因此**归属划到分区**而不是整个文件——文件级归属在这几处太粗，会让别的 plan 无法完成自己的验收。碰同一个文件的 plan 只许改自己分区内的行。

| 共享文件 | 分区归属 |
| --- | --- |
| CLI 主入口 | 一行一个子命令。01 加 `artifact` 分发行与用法段；05 加 `issue` 的两个新动作的用法段；06 改 `dispatch` 的用法段与参数解析；09 删 `wiki` 分发行与用法段。谁都不重排别人的行 |
| 测试入口 | 一行一份测试。01 加产物落点测试那一行；09 删 Wiki 测试那一行；11 加落点字面值测试那一行 |
| `AGENTS.md` | 03 改读取 ADR 那一句；09 改提交检查一段里与 Wiki 有关的部分；10 改唯一事实来源一段与领域上下文一段。三份各改各段 |
...
| 其余技能源 | 06 只改派发动作块；07 改落点字面值、取名规则和入口分支判断；08 改对谈技能的取得事实一节与调查技能的索引那一节。三者的行互不重叠 |

独占归属，没有分区问题的：产物落点数据归 01；任务状态实现归 02；14 份 ADR 与索引副本归 03；issue 实现归 05；宿主适配器与派发护栏归 06；初始化实现归 09；领域文档 leaf 与 Context Map 归 10；查引用测试归 11。

### 跨 plan 接口

| 提供方 | 消费方 | 接口 | 字段 |
| --- | --- | --- | --- |
| 01 | 02、03、04、07、09、11 | 产物落点数据的记录结构与 `mmw artifact path` 的调用形态 | 字段待回填 |
| 01 | 04 | 产物引用四项到路径的解析规则，`mmw artifact check` 复用它逐条解析 | 字段待回填 |
| 02 | 01 | 任务状态输出里的工作名位。01 交付时 `--name` 必填，02 让它可缺省，缺省时读这个输出位 | 字段待回填 |
| 03 | 04 | 元数据块里承载产物引用的那个字段名与形状 | 字段待回填 |
| 05 | 08 | 追加动作的调用形态。wayfinder 用它写 map 正文的决定索引 | 字段待回填 |
| 09 | 01 | 目标仓库配置里保留的四个工作目录根键名。01 解析工作目录根时读它们 | 字段待回填 |
| 07、08 | 11 | 技能源改写完成后的落点写法。11 的两条正则规则要在改完的技能源上全绿 | 字段待回填 |

字段在第 5 步由主 agent 从各份 plan 的变更表与合同段回填。
