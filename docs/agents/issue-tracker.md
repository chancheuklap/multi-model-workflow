...
- **Re-parent**: `gh issue edit <n> --parent <m>` attaches issue `<n>` under parent `<m>`; `gh issue edit <n> --remove-parent` detaches it.
- **Transfer**: `gh issue transfer <n> <owner>/<repo>` moves the issue to another repository. The parent–child link breaks; origin remains the body's first line.
...
- **Map**: a single issue labelled `wayfinder:map`, holding the Destination / Notes / Decisions-so-far / Not yet specified body. `gh issue create --label wayfinder:map`.
...
- **Frontier query**: list the map's open children (`gh issue list --state open`, scoped to the map's sub-issues / task list), drop any with an open blocker (`issue_dependencies_summary.blocked_by > 0`, or an open issue in the `Blocked by` line) or an assignee; first in map order wins. This query serves the wayfinder skill's maps; the night's frontier is a different one, defined in `mmw-v2/skills/dispatch/scripts/status.py`.
...
- `gh issue list --state open --label needs-triage` — what fell over in the night. Run the triage skill over it before anything else.
- `gh issue list --state open --label ready-for-human` — what only the user can do.
