...
设计树和按轮批量提问可以分开。设计树负责保持决定依赖和分支完整性。frontier 与 round 负责安排问题何时出现。上游当前默认一次提出整个 frontier，但官方文档把“一次一个问题”列为正式支持的用户偏好。保留设计树并不要求 MMW 改成批量提问。

MMW 当前没有完整引入设计树。`mmw-grilling` 只保留了“沿着决定之间的依赖关系逐支推进”。它没有定义节点状态、事实调查对分支的阻塞、frontier 重算、分支重开、遍历完成和最终共同理解确认。MMW 也已经把 `frontier` 用作 issue tracker 上可认领 ticket 的持久状态。直接照搬上游术语会造成两种 frontier 混用。
...
两者共享“当前可推进的边缘”这一抽象，但运行合同不同。MMW 如果以后采用设计树，需要在技能正文中写清会话内决定集合与 tracker ticket 集合的边界。

## 对 MMW 的准确判断

MMW 已经吸收了设计树的一个局部原则：按决定依赖逐支推进。它没有吸收设计树作为完整 grilling 状态模型，也没有吸收上游的完成判据。

这个缺口与一次问一个问题无关。MMW 可以保持当前逐题交互，同时补齐分支发现、依赖状态、答案后的重算、分支重开和共同理解确认。是否改成整轮提问，应当作为单独的用户体验决定。
