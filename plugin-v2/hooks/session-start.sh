...
# 4. Skill namespace
- Plugin skills (multi-model-workflow: prefix):
  orchestrate-workflow, orchestrate-discovery, orchestrate-plan-writing,
  orchestrate-execution, orchestrate-final-review, orchestrate-multi-pr-merge
- User-level skills (short name, NO prefix):
...
- orchestrate-discovery (includes Design Review) returns DISCOVERY_READY → check issue hierarchy; missing → to-issues → orchestrate-plan-writing
- orchestrate-plan-writing (includes Plan Review) returns PLAN_CREATED → orchestrate-execution
...
# 7. Compaction-durable constraints
...
# 8. Agent roles and dispatch rules
