#!/usr/bin/env bash
# Tests hotfix route post-push review: push writes pending_post_push_reviews.
# Verifies the state.sh update workflow described in route-4-hotfix.md.
set -euo pipefail
...
RUN_ID="test-hotfix"
bash "$STATE_SH" init --run-id "$RUN_ID" --slug "hotfix-1" --route "hotfix" >/dev/null

# Verify hotfix route gets unlimited budget
run_test "hotfix route has unlimited budget" \
...
# Write pending post-push review (as documented in route-4-hotfix.md)
