...
**详细 dispatch templates + flow 见 `references/bug-investigation-route.md`**（Steps 15-18 完整模板）。

流程概要：Step 15（dispatch root-cause-analyst）→ Step 16（handle analyst return：fixed / root cause found / design gap / unable）→ Step 17（simple bug: Codex review → Closing）/ Step 18（complex bug: worker dispatch → Codex review → Closing）。

**Analyst Resolution 路由**：`fixed` → Codex review / `root cause found, not fixed` → 派 worker / `root cause in design/plan` → 更新 Scope Contract + 创建 Budget File → Formal Orchestrate（Discovery seed）/ `unable to reproduce` 或 `unable to determine` → 报告用户。
