...
读取 `references/execution-dispatch-templates.md` 的 Pack Brief Template。Dispatch prompt 必须自足——worker 不读 SKILL.md、不读 references、不读 plan 文件。Coordinator 从 plan 中提取并在 prompt 中写全所有字段（goal behavior、tasks 完整文本、owned files、contract anchors、verification commands、return contract 等）。
...
读取 `references/execution-dispatch-templates.md` 的 Pack Review Codex Dispatch Template。通过 `codex:codex-rescue --model gpt-5.4` 派发 1 个 baseline reviewer，整合 Spec Compliance / Code Quality / Contract & Risk 三个角度。
...
**Root-Cause-Analyst 截断调度**：读取 `references/execution-dispatch-templates.md` 的 RCA 截断模板。附前两轮 accepted findings + worker 修复内容 + diff scope + 原 Pack Brief subset。

**Analyst Resolution 路由**：`fixed` → Targeted Re-Review（Round 3） / `root cause found, not fixed` → 重新 dispatch worker（Round 3） / `root cause in design/plan` → 写回 design/plan → orchestrate-discovery 或 orchestrate-plan-writing / `unable to reproduce` → 报告用户 / `unable to determine` → BLOCKED。
