...
run_test "document review -> gpt-5.5" \
  grep -q "document review ->.*gpt-5.5" "$TEMPLATE"

run_test "code review -> gpt-5.4" \
  grep -q "code review ->.*gpt-5.4" "$TEMPLATE"

# Verify injected content in actual files
run_test "execution-review-dispatch has gpt-5.4 for code review" \
  grep -q "gpt-5.4" "$PLUGIN_DIR/skills/orchestrate-execution/references/execution-review-dispatch.md"
