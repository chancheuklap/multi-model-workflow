...
- Review dispatch templates must not pass per-phase model overrides for `codex_reviewer`; model, reasoning, verbosity, and sandbox belong to `agents/codex_reviewer.toml`.
...
- Signpost templates must use phase names accepted by `scripts/state.sh` transition matrix: `workflow -> discovery -> plan-writing -> execution -> final-review -> closed`.
