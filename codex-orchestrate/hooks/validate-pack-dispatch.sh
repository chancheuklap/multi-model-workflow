#!/usr/bin/env bash
# PreToolUse compatibility hook for worker dispatch (pack_executor / complex_pack_executor).
...
ENVELOPE=$(echo "$PROMPT" | bash "$PARSE_ENVELOPE" 2>/dev/null) || {
  echo "[codex-orchestrate] BLOCKED: DISPATCH_ENVELOPE missing or malformed." >&2
  exit 2
}
...
if [[ -n "$EXISTING" ]]; then
  echo "[codex-orchestrate] BLOCKED: duplicate dispatch (idempotency_key=$IDEMPOTENCY_KEY)." >&2
  exit 2
fi
...
if [[ "$BUDGET_STATUS" == "pending_plan_count" ]]; then
  echo "[codex-orchestrate] BLOCKED: budget not initialized. Run 'state.sh budget initialize --plan-count N' first." >&2
  exit 2
fi
...
if [[ "$DC" == "pending" ]]; then
  if [[ "$AGENT_ROLE" != "reviewer" ]]; then
    echo "[codex-orchestrate] BLOCKED: Direction Check pending." >&2
    exit 2
  fi
fi
...
if [[ -n "$PACK_ID" && "$PACK_ID" != "null" && -f "$ESF" ]]; then
  ...
  if [[ "$PACK_STATUS" != "pending" && "$PACK_STATUS" != "unknown" ]]; then
    echo "[codex-orchestrate] BLOCKED: Pack $PACK_ID status is '$PACK_STATUS', expected 'pending'. Cannot re-dispatch." >&2
    exit 2
  fi
fi
...
if [[ -n "$PACK_ID" && "$PACK_ID" != "null" && -f "$ESF" ]]; then
  ...
  if [[ -n "$EXISTING_AGENT_ID" && "$EXISTING_AGENT_ID" != "null" ]]; then
    echo "[codex-orchestrate] BLOCKED: Pack $PACK_ID already has agent_id=$EXISTING_AGENT_ID. Repair must use send_input({target:\"$EXISTING_AGENT_ID\",message:\"...\"}) or resume_agent({id:\"$EXISTING_AGENT_ID\"}) to resume the original worker." >&2
    exit 2
  fi
fi
...
if [[ "$PA_BLOCKED" -gt 0 ]]; then
  case "$AGENT_ROLE" in
    ...
    *) echo "[codex-orchestrate] BLOCKED: Path A exhausted, must use Path B worker." >&2; exit 2 ;;
  esac
fi
...
if [[ "$REPAIR_ROUND" -ge 1 ]] 2>/dev/null; then
  ...
  for ref in $DISP_REFS; do
    ...
    if [[ "$FOUND_DISP" != "accepted" ]]; then
      echo "[codex-orchestrate] BLOCKED: finding $ref not accepted in review_dispositions." >&2
      exit 2
    fi
    ...
    if [[ -z "$FOUND_EV" || "$FOUND_EV" == "null" ]]; then
      echo "[codex-orchestrate] BLOCKED: finding $ref has no evidence." >&2
      exit 2
    fi
  done
fi
