...
# re-dispatch via Agent() is BLOCKED — repair must use send_input/resume_agent to resume.
#
...
run_test "block message references pack status or send_input/resume_agent" \
  bash -c "cd '$WORKSPACE' && STDERR=\$(echo '$INPUT_BLOCKED' | bash '$HOOK' 2>&1 >/dev/null || true); echo \"\$STDERR\" | grep -qE 'send_input/resume_agent|Cannot re-dispatch'"
