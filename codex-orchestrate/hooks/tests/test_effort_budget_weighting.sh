#!/usr/bin/env bash
# Tests track-effort-budget.sh Codex agent-role weighted effort increments.
...
# Helper: build Codex SubagentStart hook input JSON
make_agent_input() {
  local agent_type="$1"
  jq -n --arg t "$agent_type" '{"hook_event_name":"SubagentStart","agent_type":$t,"prompt":"test"}'
}
...
# Test 1: non-subagent input should NOT increment effort
