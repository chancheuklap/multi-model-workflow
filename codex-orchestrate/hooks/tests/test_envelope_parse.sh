...
# reviewer without review_intent
CODEX_NO_INTENT='<!-- DISPATCH_ENVELOPE {"protocol_version":"1","run_id":"r1","phase":"execution","agent_role":"reviewer","pack_id":null,"repair_round":0,"idempotency_key":"r1/review/r0","disposition_refs":null,"review_intent":null,"exception_code":null} -->'
run_test_fail "reviewer + null review_intent → exit 2" bash -c "echo '$CODEX_NO_INTENT' | bash '$PARSE'"

# targeted-re-review without exception_code
TARGETED_NO_EX='<!-- DISPATCH_ENVELOPE {"protocol_version":"1","run_id":"r1","phase":"execution","agent_role":"reviewer","pack_id":null,"repair_round":0,"idempotency_key":"r1/review/r0","disposition_refs":null,"review_intent":"targeted-re-review","exception_code":null} -->'
run_test_fail "targeted-re-review + null exception_code → exit 2" bash -c "echo '$TARGETED_NO_EX' | bash '$PARSE'"

# Valid reviewer with baseline
BASELINE='<!-- DISPATCH_ENVELOPE {"protocol_version":"1","run_id":"r1","phase":"execution","agent_role":"reviewer","pack_id":null,"repair_round":0,"idempotency_key":"r1/review/r0","disposition_refs":null,"review_intent":"baseline","exception_code":null} -->'
