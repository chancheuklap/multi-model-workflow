...
- `SubagentStart` 只在 Codex payload 暴露 `message`、`prompt` 或 text items 时校验 DISPATCH_ENVELOPE；payload 不暴露调度正文时，只注入上下文并要求 Coordinator 的 worker prompt 自带可见 DISPATCH_ENVELOPE。
