...
本文档的可验证声明由 `scripts/verify-architecture-draft.sh` 反查当前源码树：plugin 版本、skill 目录、TOML agent 模型 / reasoning / sandbox、hooks manifest、build templates、测试数量、route extension 文件、review dispatch 模型归属、phase transition 名称、run-summary 产物形态。`verify-maturity.sh` 必须调用该脚本；文档不能只靠人工目测保持同步。
...
3. Coordinator 派 reviewer。模型、reasoning effort 和 sandbox 由 `agents/codex_reviewer.toml` 决定；dispatch 不传 per-phase model override:
...
| 目录 | 测试数 | 覆盖范围 |
| --- | --- | --- |
| `build/tests/` | 10 | preamble、resolver、voice、confidence、review segmentation、send_input resume、review model authority、trust boundary |
