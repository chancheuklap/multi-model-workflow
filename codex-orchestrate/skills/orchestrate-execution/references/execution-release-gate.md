...
复杂 release blocker worker 返回必须闭合生命周期：`wait_agent({ targets: ["<agent_id>"], timeout_ms: 600000 })` 返回后，将结果保存到 `.codex/multi-model-workflow/worker-results/early-release-gate-<plan>-<round>.md`，然后立即 `close_agent({ target: "<agent_id>" })`。后续仍需同一 worker 继续时，先 `resume_agent` 再 `send_input`，返回保存后再次关闭。
