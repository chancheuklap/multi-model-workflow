...
**全程使用**：`grill-with-docs` skill（CONTEXT.md 维护）。**按需调用**：`prototype` skill / `frontend-design` / `improve-codebase-architecture` skill / `zoom-out` skill / `diagnose` skill / `triage` skill。结论必须写回 design document 或 CONTEXT.md。
