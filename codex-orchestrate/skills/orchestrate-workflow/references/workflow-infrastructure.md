...
用户已在工作树中启动 Codex Orchestrate，直接读取本地状态文件恢复。
...
bash "${PLUGIN_ROOT}/scripts/state.sh" init --run-id "<run_id>" --slug "<slug>" --route formal
...
```bash
bash "${PLUGIN_ROOT}/scripts/state.sh" budget initialize --run-id "<run_id>" --plan-count <N>
```
