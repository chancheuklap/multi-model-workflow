...
路径 B 默认读取 `.codex/multi-model-workflow/worker-agents/direct-repair-worker.agent-id`，构造 `repair_round >= 1` 且 `agent_id = "<原 worker agent_id>"` 的 route-worker prompt，运行 `validate-route-worker-dispatch.sh --transport send_input` 后再 `send_input`。缺失 agent_id → BLOCKED，不能新建第二个 worker 冒充续修。
