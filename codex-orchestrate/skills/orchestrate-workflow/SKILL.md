...
1. 验证 send_input/resume_agent 工具可用（尝试列出工具列表确认）。如不可用 → 硬停：`"send_input/resume_agent tool not available. Ensure Codex multi_agent is enabled."`
2. 检测当前环境（工作树 vs 主仓库）：
...
线性管线：Discovery → Plan Writing → Execution → Final Review → Closing。每个 phase skill 通过 `multi-model-workflow:<name>` skill 加载到主线程。

### Step 7：orchestrate-discovery

```
Load skill: multi-model-workflow:orchestrate-discovery
```
...
3. 进入 `multi-model-workflow:orchestrate-discovery` skill 的 Step 12 流程
...
```
Load skill: multi-model-workflow:orchestrate-plan-writing
```
...
| `NEEDS_TRIAGE` | `triage` skill → 重新 Step 9 |
| `NEEDS_DIAGNOSIS` | `diagnose` skill → 写回 → 重新 Step 9 |
| `NEEDS_DECISION` | 询问用户 → 回答后 Step 9 |
| `NEEDS_ARCHITECTURE` | `improve-codebase-architecture` skill → 写回 → Step 9 |
| `NEEDS_CONTEXT` | 派 `code_explorer` / `zoom-out` skill → 补充后 Step 9 |
| `BLOCKED` | 报告用户 |
...
```
Load skill: multi-model-workflow:orchestrate-execution
```
...
| `NEEDS_ARCHITECTURE` | `improve-codebase-architecture` skill → 只影响当前 pack → 回 Step 11；改变 plan → 回 Step 9 |
| `BLOCKED` | 报告用户 |
...
```
Load skill: multi-model-workflow:orchestrate-final-review
```
...
`multi-model-workflow:orchestrate-multi-pr-merge` skill。
