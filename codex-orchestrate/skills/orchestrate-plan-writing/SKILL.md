...
**Dispatch 协议**：所有 plan_writer dispatch 必须使用 Codex 原生 `spawn_agent({ agent_type: "plan_writer", message: "<dispatch prompt>" })`；Coordinator 必须保存 `spawn_agent` 返回的 agent_id，用于后续 send_input/resume_agent 修复路径。

**agent_id 持久化**：dispatch 完成后立即从 Codex subagent result 中提取 `agent_id`，调用 `bash "${PLUGIN_ROOT}/scripts/state.sh" update --run-id "<run_id>" --field '.plan_writer_agent_id' --value '"<agent_id>"'`。如果 agent_id 为空 → BLOCKED；Plan Review 修复路径必须能通过 workflow-state 里的 `plan_writer_agent_id` 恢复原 plan_writer。
...
```bash
bash "${PLUGIN_ROOT}/scripts/pack-count-validator.sh" <plan-file>
```
