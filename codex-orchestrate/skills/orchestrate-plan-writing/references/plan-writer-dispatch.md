...
spawn_agent({
  ...
  message: "
    ...
    ${PLUGIN_ROOT}/skills/orchestrate-plan-writing/references/plan-writing-methodology.md
...
**注意**：`${PLUGIN_ROOT}` 在 Coordinator 主线程中解析为 plugin 安装目录的绝对路径。Sub-agent 收到的 prompt 中必须包含已经解析后的路径，不能假设子代理能读取父线程环境变量。
...
3. 若后续需要修复/补充上下文，必须使用 `send_input({ target: "<agent_id>", message: "<repair/context prompt>" })`；如果 host 已关闭该 agent，先 `resume_agent({ id: "<agent_id>" })` 再 `send_input({ target: "<agent_id>", message: "<repair/context prompt>" })`

**Critical**: Codex `spawn_agent` return value is the source of `agent_id`. Without agent_id, plan_writer repair path is BLOCKED.
...
| `NEEDS_TRIAGE` | issue ready state 不清 | `triage` skill |
| `NEEDS_DIAGNOSIS` | bug 缺复现或 hypothesis | `diagnose` skill |
| `NEEDS_DECISION` | 需要产品/业务决策 | 询问用户（一次只问一个问题） |
| `NEEDS_ARCHITECTURE` | 架构假设与代码现实不符 | `improve-codebase-architecture` skill |
| `NEEDS_CONTEXT` | 缺代码上下文 | 派 code_explorer / `zoom-out` skill，补充后 send_input/resume_agent 给原 plan_writer |
| `BLOCKED` | 无法完成 | 报告用户，附 plan_writer 的阻塞原因 |

upstream skill 结论必须写回 design document / issue hierarchy，再 send_input/resume_agent 给原 plan_writer 继续。
