...
**Read** `docs/orchestrate/plans/<slug>/` 目录下所有 plan 文件。每份 plan 必须包含以下字段，缺失则 needs repair（send_input/resume_agent 对应的 plan_writer 修复）：
...
不通过的 pack → send_input/resume_agent 给 plan_writer 修复 → 重新检查。
...
```bash
bash "${PLUGIN_ROOT}/scripts/state.sh" budget initialize \
