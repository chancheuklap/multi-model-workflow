...
| 需修改 Task Pack 内容（implementation tasks / owned files / verification） | send_input/resume_agent 原 plan_writer（agent_id 从 workflow context 获取）（agent_id 从 workflow-state 获取，若无 agent_id 则 BLOCKED），prompt 附带具体 findings + 现有 plan path → plan_writer 定向修订 → Step 11 |
...
| issue ready state 不清 | `NEEDS_TRIAGE` | `triage` skill |
| 业务术语或验收不清 | `NEEDS_DISCOVERY` | `multi-model-workflow:orchestrate-discovery` skill |
| bug 缺复现或 hypothesis | `NEEDS_DIAGNOSIS` | `diagnose` skill |
| 需要方案比较 | `NEEDS_DECISION` | user / `prototype` skill |
| 架构摩擦反复阻塞 | `NEEDS_ARCHITECTURE` | `improve-codebase-architecture` skill |
| 模块地图不足 | `NEEDS_CONTEXT` | `zoom-out` skill/ code_explorer |
