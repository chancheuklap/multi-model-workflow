...
| `accepted` 子类型 | 动作 |
| --- | --- |
| `plan repair` | Coordinator 直接修框架性内容，或 send_input/resume_agent plan_writer 修 Task Pack 内容 |
| `design gap` | 回到 orchestrate-discovery → Design Review → 写回后 re-review plan |
| `issue-plan mismatch` | 判断：大 issue 级问题 → 返回 Coordinator 走大 issue 拆分；小 issue 级问题 → send_input/resume_agent plan_writer 重新执行 Step 3c 拆分 → re-review plan |
| `issue quality` | 小 issue 拆分质量问题（覆盖度/粒度/验收标准/依赖）→ send_input/resume_agent plan_writer 重新执行 Step 3c 修正小 issue → re-review plan |
| `architecture friction` | `improve-codebase-architecture` skill → 写回后 re-review |
...
**Plan-Writer resume 步骤**（plan_writer 修复）：
...
3. 调用：
   ...
   ```
4. 如果 Codex host 返回 agent 已关闭，调用 `resume_agent({ id: "<plan_writer_agent_id>" })` 后再次 `send_input({ target: "<plan_writer_agent_id>", message: "<repair prompt>" })`。
5. 如果 host resume 仍失败，Coordinator 必须记录 exception_code，带完整 reviewed design、issue hierarchy、current plan path、accepted findings 新建 replacement dispatch；不得静默重派。
6. 等待 agent 返回（同步或 wait_agent）
7. 解析返回结果 -> `state.sh transition --actor Coordinator --to returned`
8. 验证 plan 文件格式 + pack count validator
9. `state.sh self-verify append --run-id <run_id> --repair-round <N> --verification-passed <yes|no>`
10. 回到 Plan Review 重审
...
| issue-plan mismatch | 大 issue 级：Coordinator 走大 issue 拆分；小 issue 级：send_input/resume_agent plan_writer Step 3c | issue hierarchy |
| issue quality | send_input/resume_agent plan_writer 重新执行 Step 3c | issue hierarchy（小 issue 章节） |
| architecture friction | `improve-codebase-architecture` skill | design doc / plan anchors |
| domain 术语冲突 | `grill-with-docs` skill | CONTEXT.md + design document |
...
| 项目规则 / 代码现实 mismatch | `improve-codebase-architecture` skill 或 `zoom-out` skill补充上下文后 re-run |
