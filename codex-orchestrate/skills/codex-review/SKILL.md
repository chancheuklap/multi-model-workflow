...
**证据表 (REQUIRED)**:
Reviewer 必须在 `### Evidence` 下填写半结构化证据表。证据表证明自己实际检查过什么；它不是设计意图摘要，也不能替代阅读用户指定内容。

| 字段 | 必填内容 |
| --- | --- |
| 已读设计 / mockup / plan 来源 | 实际读过的 design、mockup、plan、issue、commit、diff、文件或用户上下文来源。没有对应来源时写 `不适用`，不能留空。 |
