...
| `needs repair` | worker 自己有疑虑 → 审阅 concerns，能自主解决则补充信息后 resume_agent + send_input worker 继续；否则进入 Step 14 让验证环节处理 |
| `needs context` | resume_agent 后 send_input 补充上下文给原 worker |
| `blocked` | 技术阻塞：尝试拆分冲突 / 换更强模型。业务阻塞：询问用户 |

`send_input` 修复或补上下文必须读取对应 `.codex/multi-model-workflow/worker-agents/multi-pr-conflict-<conflict-id>.agent-id`，先 `resume_agent({ id: "<原 worker agent_id>" })`，构造 `repair_round >= 1` 且 `agent_id = "<原 worker agent_id>"` 的 route-worker prompt，运行 `validate-route-worker-dispatch.sh --transport send_input` 后再发送。缺失 agent_id → BLOCKED，不能新建第二个 worker 冒充续修。每次返回保存后都要 `close_agent` 释放容量。

如果冲突修复从普通 worker 升级到 `complex_pack_executor`（例如跨 PR 合同、迁移、状态或依赖冲突），这是新的 route-worker escalation dispatch：新 prompt 使用 `agent_id: null`、`pack_id: null`、当前 `repair_round`、新的 escalation idempotency key，并通过 `record-route-worker-dispatch.sh` 写入独立 agent file。这个升级是 owner 更换，不是原 worker 续修。
...
| 修复不正确但方向对 | resume_agent 后 send_input worker 附修正意见 → 重新验证 |
