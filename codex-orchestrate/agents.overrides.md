...
- Orchestrated review references must use `.codex/multi-model-workflow/review-*`, `review-registry/`, reviewer `.agent-id` files, `spawn_agent`, `send_input`, and `wait_agent`; do not describe job-id polling.
...
- Review result bookkeeping belongs in explicit Coordinator scripts after dispatch and after result persistence; `record-review-dispatch.sh` records the actual reviewer agent, and `complete-review-dispatch.sh` ties durable result files to exactly-once review budget increments.
- Coordinator delegation has ownership semantics: after a task is assigned to a subagent, the Coordinator must not redo the same investigation, implementation, or review in parallel. Do non-overlapping coordination only, then wait for the assigned agent. Do not interrupt, close, or pressure running agents for partial output unless the user cancels the task or the documented workflow has reached a real BLOCKED state.
