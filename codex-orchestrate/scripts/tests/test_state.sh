...
# Illegal actor: unknown-hook pending->committed (not in matrix)
run_test_expect_fail "illegal transition: unknown-hook pending->committed -> exit 2" \
  bash -c "bash '$STATE_SH' update --run-id '$RUN_ID' --field '.cursor.phase' --value '\"pending\"' && bash '$STATE_SH' transition --run-id '$RUN_ID' --actor unknown-hook --from pending --to committed"
