...
if echo "$COMMAND" | grep -qE 'git merge\b.*--squash'; then
  echo "[codex-orchestrate] BLOCKED: git merge --squash is forbidden. Use git merge --no-ff to preserve commit history." >&2
  exit 2
fi
...
if [ "${UNCHECKED}" -gt 0 ]; then
  echo "[codex-orchestrate] BLOCKED: Plans have ${UNCHECKED} unchecked tasks. Complete execution and review before pushing." >&2
  exit 2
fi
