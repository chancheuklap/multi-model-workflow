...
# Codex-native prompt envelope, budget gate, and idempotency guard.
set -euo pipefail
...
usage() {
  cat <<'USAGE'
Usage: validate-route-worker-dispatch.sh --prompt-file PATH [--transport spawn_agent|send_input]
USAGE
  exit 2
}
