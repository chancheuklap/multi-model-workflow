...
- 技能自带的脚本，由拿着这份技能的 agent 从它的 `SKILL.md` 就地解析 `scripts/…`；调用方只点技能名与要做的事，不写安装路径。装了技能就是拿到脚本，两者不会各自漂移，路径在五个宿主上都对。唯一的例外是写进票的那条 `CHECK:`——它由 shell 执行，中间没有 agent，所以路径写全，形状与理由在 `mmw-v2/skills/verify-ticket/references/ui-parity.md`。
