...
| `bash mmw-v2/vendor/sync.sh` | 把 `mmw-v2/vendor/` 的两份第三方技能整目录重新拉一遍，写回 `VENDOR.lock`；`--check` 只校验 |
...
- `mmw-v2/vendor/` 是第三方仓库技能的逐字副本，和上游、自研技能平级装进宿主，`skills.txt` 里前缀 `vendor/`。`draw-diagram` 按技能名调用其中两个，不嵌套、不复制。改它之前读 `mmw-v2/vendor/AGENTS.md`。
