...
| 命令 | 干什么 |
| --- | --- |
| `bash mmw-v2/install.sh` | 唯一安装入口，把技能软链进 `~/.agents/skills` 和 `~/.claude/skills`，subagent 成品软链进五个宿主 |
...
- 技能正文对所有宿主是同一份。描述、默认值、示例不把任何宿主当默认或首选，不按宿主名分支；能力差异用按能力判断的自然语言写。
- 技能只装两处：`~/.agents/skills` 是各家通用的位置，Codex、Cursor、Grok、Pi 都原生扫它；Claude Code 只认 `~/.claude/skills`，所以那一处再装一份同样的软链。两处都直接指向仓库源目录，不互相串。subagent 不走这条，仍按宿主各装一份。
...
写法以 `writing-for-agents` 为准。下面七条它没有，都是在这个仓库里踩出来的。

- **例子不是规则。** 只在例子里出现过的路径形状、命名或映射，读者会照着外推。规则单独写一句，例子跟在后面。用户举的使用场景同样是例子——把它当成功能定义，做出来的技能会窄得只剩那一个场景。
- **路由表和分支表要有兜底行。** 全是正面条件的表，落不进任何一行时 agent 没有指示；写清「以上都不是」时怎么办。
- **既写清什么时候接手，也写清什么时候不接手。** 相邻技能能处理的情况给一条明确的移交行，否则它会把不属于自己的问题接下来，用自己的方法做。
- **命令里每个参数和占位符都要说清值从哪来。** 只给形状不给来源，读者填不出。
- **只写你实际打开过的文件、实际跑过的命令。** 没跑的写「未验证」，不从上一层推下一层。主入口读着相似不能作为跳过其余文档的理由。
- **判「有没有人接」要 grep，不能凭印象。** grep 不限定目录：限过范围的零命中只说明这次没搜到那里，把它当「不存在」，删掉的可能是另一条执行面的唯一来源。
- **问用户之前先把级联算完。** 「把 X 搬去 Y」看着是一处改动，可能强制改名、强制改调用方。算不出来就说算不出来，不要问过了才发现。

交给用户之前派一次冷读：一个 subagent，只给它那一个文件、和读者手上保证有的东西，让它报出照着执行不了的每一句。不给上游原文、不给你的设计理由、不给同批的其他文件——给了任何一样，这一遍就失效。
...
- `MMW_V2_HOME` 把所有安装位置整体改到一个临时根，用它练手而不碰真家目录；`CODEX_HOME`、`PI_CODING_AGENT_DIR`、`PI_HOME` 也认。
</important>
