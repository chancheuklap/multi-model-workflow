...
- 每个 agent 用哪个 host、哪个 model、哪档 effort，只改 `mmw-v2/skills/dispatch/models.md`。它跟着技能的 symlink 走，是这台机器的一份，consuming repository 里不放。读它的是同一个技能里的 `scripts/models.py`：`install.sh` 拿它写 Paseo 的 Agent profile，`dispatch.sh` 拿它拼 `create_agent` 的 settings。
