...
| watchdog finding | `relay down` · `liveness unknown` (the runner could not say, or the session was started on another machine) · `held with no session to ask` · `events unreadable` · `cannot read the board` |
