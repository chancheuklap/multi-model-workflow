...
**normalisation**:
The three rules that standardise an accessibility tree before comparison: drop `generic` and `group` roles, drop landmark names, drop a `main` nested inside `main`. The **accessibility tree** is compared per scene and viewport; the baseline side is screenshotted from `#dc-root`, the implementation side takes the whole **viewport**.
