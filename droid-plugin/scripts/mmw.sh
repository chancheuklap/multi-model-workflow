...
#   mmw loop ...
#   mmw review start ...
#   mmw worker dispatch|resume|check-docs|task-record ...
...
#   写码工人派发(Task → pack-executor):
#     mmw worker dispatch --plan <p> --worktree <wt> | worker resume --worktree <wt> --instructions <f>
#     mmw worker task-record --worktree <wt> [--plan <p>] --task-id <id>
#     mmw worker check-docs --worktree <wt>   # Task 返回后必跑
#   写计划工人派发(Task → plan-writer;在任务 worktree 内、不 commit):
#     mmw worker plan-dispatch --plan <落点> --worktree <wt> [--design <d>] [--issue <i>]
#     mmw worker plan-resume --plan <落点> --worktree <wt> --instructions <f>
#     mmw worker plan-check --plan <落点> --worktree <wt>   # Task 返回后必跑
#   进度板(负责人可读投影):
