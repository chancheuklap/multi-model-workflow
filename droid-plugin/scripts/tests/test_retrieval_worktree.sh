#!/usr/bin/env bash
# MMW worktree 初始化合同：项目 hook(.factory/worktree-init.sh)优先；普通仓库走唯一用户级图谱生命周期模块。
set -euo pipefail
...
cat >"$REPO/.factory/worktree-init.sh" <<'SH'
#!/usr/bin/env bash
set -euo pipefail
source_wt="$1"
target_wt="$2"
printf '%s\n' "$source_wt" >"$target_wt/.factory/init-source"
printf '%s\n' "$target_wt" >"$target_wt/.factory/init-target"
SH
chmod +x "$REPO/.factory/worktree-init.sh"
printf 'seed\n' >"$REPO/seed.txt"
git -C "$REPO" add -A
...
if mmw_prepare_worktree "$REPO" "$TARGET" >"$TMP/hook.out" 2>"$TMP/hook.err" \
  && [ ! -s "$TMP/hook.out" ] \
  && [ "$(cat "$TARGET/.factory/init-source")" = "$REPO" ] \
  && [ "$(cat "$TARGET/.factory/init-target")" = "$TARGET" ] \
...
cat >"$TARGET/.factory/worktree-init.sh" <<'SH'
#!/usr/bin/env bash
printf 'fixture failure\n' >&2
exit 9
SH
chmod +x "$TARGET/.factory/worktree-init.sh"
