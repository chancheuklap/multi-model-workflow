...
mkdir -p .factory/multi-model-workflow
echo '{"attendance":"unattended"}' > .factory/multi-model-workflow/task.json
bash "$LOOP" init --kind execution >/dev/null
[ "$(jq -r .attendance .factory/multi-model-workflow/loop-state.json)" = "unattended" ] && ok "init 从 task.json 读入 unattended" || no "init 读 task.json mode"
rm -f .factory/multi-model-workflow/task.json

# ===== 软停 × 在场开关 =====
bash "$LOOP" close >/dev/null   # 换 loop 前收束上一个(init 拒覆盖未收束 loop)
bash "$LOOP" init --kind execution >/dev/null
[ "$(jq -r .attendance .factory/multi-model-workflow/loop-state.json)" = "afk" ] && ok "无 task.json 时 init 缺省 afk" || no "缺省 afk"
...
[ "$(jq -r .attendance .factory/multi-model-workflow/loop-state.json)" = "unattended" ] && ok "attendance 接受 unattended" || no "attendance unattended"
