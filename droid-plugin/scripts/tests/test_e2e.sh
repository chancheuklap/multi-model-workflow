...
# 入口:建 develop 任务
WT="$(bash "$PREPARE" new --scenario develop --slug 2026-06-29-e2e --title "端到端" \
  --request "端到端完整需求与验收条件" 2>/dev/null | sed -n 's/^worktree_path=//p')"
...
# G2:审闸里 where 报任务 manifest + 上游意图 + 当前阶产物,直接喂 review start --source
RS="$(cd "$WT" && bash "$FLOW" where | sed -n 's/^review_source=//p')"
echo "$RS" | grep -q 'task.json' \
  && echo "$RS" | grep -q 'docs/investigating/e2e.md' \
  && echo "$RS" | grep -q 'docs/design/e2e-direction.md' \
  && echo "$RS" | grep -q 'docs/design/e2e.md' \
  && ok "审闸报 manifest + 上游意图 + 当前产物" || no "review_source ($RS)"
