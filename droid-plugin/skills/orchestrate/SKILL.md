...
- **`RESUMABLE` + 在飞任务**(Droid 把已有 worktree cwd 归一化到主仓库)→ 先按用户本轮意图分流。明确要新建任务就进 Step 1；要继续或意图不明时，单个任务执行回显的 `resume=` 命令，多个任务用 `AskUser` 让用户选择续跑哪个或新建。进入选中的 worktree 后重新跑 `where`，再按上一条续跑。**不得把 `RESUMABLE` 当成新任务直接 `task new`。**
