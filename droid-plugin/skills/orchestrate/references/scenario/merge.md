...
| **复杂·根因明** | 功能/合同冲突;多文件;但谁该 win 清楚 | Coordinator 修,或派 `pack-executor` 定向修:先写一份 mini-plan(冲突点 + 谁该 win + 验收命令)落状态平面,作 `--plan` 传给 `mmw worker dispatch`;脚本用 droid exec 后台执行 |
