...
> 受治理的明确改动仍走 ④终审闸；`review.sh` 用一个独立 GPT 审者覆盖两基线，不套 develop 的跨模型多审编制。落地撞出超范围问题 → `mmw spinoff` 登记,别就地扩。
...
2. **一条命令建好**(从本地最新 HEAD 分叉,scaffold docs,写 manifest):
   ...
   大任务拆并行子任务:允许在任务 worktree 内直接跑 `task new`。新 worktree 一律挂到主仓库 `.factory/worktrees/` 下(扁平,不做目录嵌套),但分支从**当前 worktree 的 HEAD** 分叉,子任务因此继承父任务已完成的进度;父子关系写进两边 manifest 的 `parent` 与 `child_tasks`,供 `team` 视图和合并顺序溯源。清理仍需回主仓库(`cleanup` 不能删自己脚下的目录)。
...
提交进分支的文档:设计文件夹 `docs/design/<slug>/` 全部成员(主文档 `<slug>.md` 与文件夹同名 + direction/investigating/prototype/mockup/evidence 类型细分)、issue `docs/issues/`、计划 `docs/plans/`、领域 `docs/context/`(项目级资产)。**过程产物不永久存档**(`docs/.gitignore` 已忽略,随 worktree 删):审查留痕 `docs/reviews/`、终审报告。临时状态固定落 `.factory/multi-model-workflow/`。
<!-- END: worktree-setup -->
...
| `repair` | active | 留在本阶段返工:回 ② 干按缺陷改,改完再 `handoff`。第 3 轮起回执会提醒:每轮向用户汇报卡点再继续。审闸返工 repair_count 超 max_repair_rounds(默认 3)→ GUARD=repair-round-cap(afk/attended 交人,unattended 硬停);非审闸只 WARN。 |
