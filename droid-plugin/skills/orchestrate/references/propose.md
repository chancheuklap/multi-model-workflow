...
Task({
  ...
  prompt: "原始任务:<用户原话>;已做:<调查与已排除方向>;已发现:<现状报告要点>;证据:<报告路径和承重 file:line/url>;当前决策:<准备亮给用户的 2-3 个方向及取舍>;请判断该 framing 是否解对问题、最大失败模式是什么、什么证据会推翻你的判断。不要重写方案表。"
})
```

- 用户仍拍板;decision-advisor 只补风险/取舍,不替用户选。
