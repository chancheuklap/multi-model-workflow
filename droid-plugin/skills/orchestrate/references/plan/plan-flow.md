...
> plan 阶段:判单/多计划、映射 plan 清单、写跨 plan 合同骨架、fan-out 派 **plan-writer 写计划**、亲验返回、回填,加角色声音 + Git 纪律。计划撰写全下放独立模型,主线程不内联写;就绪门自检在 selfcheck 步(`mmw step next` 进)。
...
**单计划**走 Step 1(映射)→ Step 3(派 1 个 plan-writer)→ Step 4(亲验)→ `mmw step next` 进 selfcheck。**多计划**走 Step 1–5(映射→骨架→fan-out→亲验→回填)→ `mmw step next` 进 selfcheck。就绪门 + 收尾 handoff 都在 selfcheck 步。
...
| **plan-writer** | 主线程 `mmw worker plan-dispatch` 生成派发包后用 Task 后台派 | 拿(带合同骨架的)设计文档 + 单个大 issue,**自己把大 issue 拆成小 issue**,写出一份自洽 plan(Header + Task Pack + TDD 步骤 + 验收)。拆分、写作纪律、交付前自检都在它身上 |
...
每个大 issue 派一个 plan-writer。主线程先跑 `mmw worker plan-dispatch`,再按回执用 Task 派发,自己留在 loop 里做 Step 4 亲验和 Step 5 回填:
...
- **一律后台跑**:脚本写派发包后用 Task→plan-writer,记录 task ID,完成用 TaskOutput 收回;追问用 Task resume。**互不依赖的 plan 并行发多条;有 blocked_by 链的按依赖序发。**
...
每份 plan-writer 返回 `pass` 后,先跑 `mmw worker plan-check`,再对它声明的事实(plan 文件存在、Pack 数量、引用的 `file:line`、**小 issue 已写回 issue 文件 `## Small issues`**)至少抽验 1 个再采信。失实 → `mmw worker plan-resume` 打回。任一返回 `needs-context` / `needs-repair` / `blocked` → 补上下文或修源设计后 resume;返回 `needs-redirection` → handoff `needs-redirection`。
...
Step 2 的骨架已划好边界,本步把**精确字段 / 签名**填实并核 writer 有没有越界。扫每份 plan 的 File/Responsibility Map + Contract anchors + migration/registry(`Cross-plan touchpoints` 是入口),把 Step 2 标 `(字段待 plan 回填)` 的格子补成真实 owner / provider / consumer / 字段,写回设计文档 `## Cross-Plan Contract Anchors`。合同不一致就 `mmw worker plan-resume`。

## orchestrate 完 → 下一步(脚本导航)

Step 1–5 做完(单计划走 1/3/4)、所有 plan-writer 都 pass + 亲验过 → `mmw step next` 进 **selfcheck 步**。

## Git 纪律

写 plan 阶段不 commit、不 push,改动保持 unstaged,落地通过后由主 Agent 统一提交。
