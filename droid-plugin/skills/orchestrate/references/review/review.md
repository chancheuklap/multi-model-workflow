...
1. **一条命令起审**(把 init loop + 配审题 + 出派发指南收成一步):
 ...
 ```
 **直接用 `mmw where` 吐的 `review_start` 整行**。它初始化 review loop、定好 stage 与视角,并生成 `review-brief.md`。
2. **抽覆盖清单**(判断,留你做):从设计/计划/issue/意图逐条抽"要审到什么",`source` 记从哪份文档哪行抽。客观项(② issue 数=plan 数、④ 意图逐条)标清楚:
 ...
 ```
3. **主线程直接派审者**:读 `review-brief.md`,按「派审者」段用 Task 派 `reviewer-*` droids。审者各自干净 context 并行起、读 `worktree-review` skill 出结构化 findings。
