...
- **关键咨询(条件触发,最多 2 次)**:只在架构 / 对象边界 / 状态机**定型前**,或讨论中前提**可能被击穿**时,`Task` → `decision-advisor` 拿短判决。小改 / 无边界风险跳过;**不把它当过门前的例行复查**(那是 selfcheck 后设计预审的活)。本阶段**不自派** `reviewer-design-a/b`(设计预审由 selfcheck 收尾统一起)。
