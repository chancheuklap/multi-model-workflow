...
- 自检全过 → 先跑 `mmw where`，照 `then` 中已经解析好的唯一主设计文档路径执行 `mmw pin`；再以同一路径起设计预审：`mmw review start --stage design --source <主设计文档>`，照 brief 派审者，findings 落盘亲验标处置。禁止同时新建单文件版和目录版。
