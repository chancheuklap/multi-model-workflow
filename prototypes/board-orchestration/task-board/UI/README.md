...
- `work/` is also the handoff package: the pages, `styles/` and `data/` above as they are in the Claude Design project, plus `support.js` (Claude Design's runtime, as downloaded), `scenes.json` (one scene per `scene` value of each page but the overview, with its data written by the claude-design-blocks skill's `export_scene_data.py`) and `vendor/` (the three scripts `support.js` loads, so every scene renders offline). `styles/fonts/` holds the Geist, Geist Mono and Instrument Serif files Google Fonts serves, and `styles/tokens.css` declares them with `@font-face` from there rather than importing them from `fonts.googleapis.com`: the story judge answers only the package and unpkg for the design side, so an imported font never loads there, while the product's story page loads it from Google, and the two sides then lay text out with different line heights. The project has no `README.md` to bring down. After any page, style or data change is uploaded, download the package again and rerun `export_scene_data.py`.
