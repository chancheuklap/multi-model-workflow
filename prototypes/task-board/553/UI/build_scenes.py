#!/usr/bin/env python3
"""Build the task board's example data for the Claude Design pages, with the pipeline's own code.
...
night of 2026-09-10; the board read the tracker at 07:39 on 09-11 (`NOW`, 07:40, is the
clock the design pages freeze).

The output is one script for the design project, `data/board-scenes.js`, defining
`window.BOARD_SCENES` (four board scenes, each with the payload, the task and card the
scene selects, and `now`) and `window.SETTINGS_SCENES` (ten settings scenes, each with
the payload and the page state that scene shows).
"""
...
class Ticket:
    ...
    def final_run(self, hm, handoff_ac=None):
        """The worker's final full run on HEAD (#415): `ticket.checked` with `run=reverify`."""
        self.checked(hm, run="reverify", handoff_ac=handoff_ac)
    ...
    def fixture(self) -> dict:
        ...
        for key in ("outcome", "review", "waiting"):
            fold[key] = strip(fold.get(key))
        ...
        return {"n": self.n, "title": self.title, "state": "closed" if state["passed"] else "open",
                "blocked": self.blocked, "closeout": self.closeout, "comments": comments,
...
def morning():
    ...
    t138.decided("07:16")
    t138.final_run("07:22", handoff_ac="AC3")
...
def twenty_tickets():
    ...
    t218.checked("06:36", slot=1); t218.review("06:40", "07:05"); t218.decided("07:10")
    ...
    t229.review("04:42", "05:10"); t229.decided("05:14"); t229.final_run("05:40", handoff_ac="AC2")
    ...
    t215.decided("05:14"); t215.final_run("05:21"); t215.checked("05:25", run="repo-checks")
...
def raw_trees(scene: dict, tickets: dict[int, "Ticket"]) -> list[dict]:
    """The issue trees `BoardStore._read_trees` would hand `_shape` for this scene's tasks."""
    trees = []
    for task in scene["tasks"]:
        children = [{"number": d["n"], "title": d["title"], "state": d["state"].upper(),
                     "labels": [f"wayfinder:{d['kind']}"],
                     "blockedBy": [{"number": n} for n in d["blocked"]]} for d in task["decisions"]]
        for sp in task["specs"]:
            nodes = []
            for fx in sp["tickets"]:
                kids = [{"number": c["child"], "title": c["title"],
                         "state": "CLOSED" if c.get("resolution") else "OPEN"}
                        for c in fx["fold"]["children"].values()]
                nodes.append({"number": fx["n"], "title": fx["title"], "state": fx["state"].upper(),
                              "labels": ["mmw:ticket"],
                              "blockedBy": [{"number": n, "state": "OPEN"} for n in fx["blocked"]],
                              "children": kids})
            children.append({"number": sp["n"], "title": sp["title"], "state": "OPEN",
                             "labels": ["mmw:spec"], "children": nodes})
        trees.append({"number": task["n"], "title": task["title"], "state": "OPEN",
                      "labels": ["wayfinder:map", "mmw:map"], "children": children})
    return trees


def board_scenes() -> dict:
    board_data = board_store()
    store = board_data.BoardStore(gh=lambda *a, **k: None)
    out = {}
    for key, scene in build().items():
        comments = {}
        for task in scene["tasks"]:
            for sp in task["specs"]:
                for fx in sp["tickets"]:
                    comments[fx["n"]] = fx["comments"]
        payload = {"tasks": store._shape(raw_trees(scene, {}), comments),
                   "repo": "chancheuklap/multi-model-workflow", "read_at": scene["readAt"]}
        if scene["readFailed"]:
            payload["read_failed"] = {"at": scene["readAt"], "message": "gh: could not resolve api.github.com"}
        out[key] = {"name": scene["name"], "now": NOW, "select": scene["select"], "payload": payload}
    return out


def settings_payload(meta: dict, scene: dict, scan_key: str) -> dict:
    """What `GET /api/settings` answers: the saved configuration, the options, the scan."""
    saved = scene["saved"]
    return {"version": 12, "runner": saved["runner"], "rows": saved["rows"],
            "hosts": [{"name": h, "binary": meta["binaries"][h], **meta["launch"][h]} for h in meta["hosts"]],
            "runners": [*meta["runners"], "auto"],
            "scan": {"source": scan_key, "scanned_at": scene["scannedAt"], "scanning": False,
                     "hosts": scene["scans"][scan_key]}}


def settings_scenes() -> dict:
    meta = settings()
    base = meta["scenes"]
    source = lambda s: "paseo" if s["saved"]["runner"] == "paseo" else "cli"  # noqa: E731
    out = {key: {"name": s["name"], "payload": settings_payload(meta, s, source(s)), "page": {}}
           for key, s in base.items()}
    # `changed`: the page opened on `mine`, the owner edited one cell, and the save was
    # refused because an agent rewrote the file from the command line at 07:44.
    out["changed"]["page"] = {"edit": [["reviewer", "effort", "xhigh"]], "refused": 1,
                              "modifiedAt": base["changed"]["changedElsewhere"]["at"]}
    mine = base["mine"]
    extra = {
        "edited": ("改了几处，还没保存", {"edit": [["senior-worker", "host", "claude"],
                                              ["senior-worker", "model", "opus 5"],
                                              ["senior-worker", "effort", "high"]]}),
        "incomplete": ("换了 host，model 空着等选", {"edit": [["reviewer", "host", "codex"]]}),
        "scanning": ("正在重新扫描", {"scanning": True}),
        "saved": ("保存成功", {"edit": [["advisor", "effort", "high"]], "saveAt": iso("07:41")}),
        "refused": ("保存被拒：start 会拒绝的格子", {"edit": [["advisor", "effort", "high"]],
                                                  "serverFlags": [{"cell": "advisor.effort",
                                                                   "reason": "fable 5.1 在 claude 上没有 high 这一档"}]}),
    }
    for key, (name, page) in extra.items():
        out[key] = {"name": name, "payload": settings_payload(meta, mine, "cli"), "page": page}
    return out


def main() -> int:
    if len(sys.argv) != 2:
        raise SystemExit("Run: python3 build_scenes.py <out.js>")
    js = ("window.BOARD_SCENES = " + json.dumps(board_scenes(), ensure_ascii=False, separators=(",", ":")) + ";\n"
          "window.SETTINGS_SCENES = " + json.dumps(settings_scenes(), ensure_ascii=False, separators=(",", ":")) + ";\n")
    Path(sys.argv[1]).write_text(js, encoding="utf-8")
    print(f"wrote {sys.argv[1]} ({len(js)} bytes)")
    return 0
