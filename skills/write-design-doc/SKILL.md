...
输入 →（选路：访谈 or 综合）→ 设计文档（+ 项目领域文档）→ 自评 →（可选）第二模型审 → 拆 issue。

**两条路（开工前先选一条，下面 §两条路 给判据）**：
- **A · 访谈模式**：想法还模糊、关键假设没被质疑过 → 走 Steps 2-5 一问一答对抗式逼出设计（默认路）。
- **B · 综合模式（synthesize-don't-interview，取自 to-prd）**：本场对话或传入文档已把 问题/方案/验收 讨论清楚、核心假设已被质疑过 → 不再访谈，综合「已知 + 代码现状」直接起草。
...
**选路在此落地**：走 B · 综合模式（判据 + 护栏见上 §两条路）→ 跳过 Steps 2-5，直接起草。走 A → 继续 Steps 2-5。

### Steps 2-5：与用户讨论（仅 A · 访谈模式；B 已在 Step 1 跳到起草）
