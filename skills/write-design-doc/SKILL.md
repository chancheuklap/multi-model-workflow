...
**手动驱动**：你自己读代码、自己和用户讨论、自己写文档。没有状态机、budget、自动 reviewer。需要第二意见时主动把文档交给 Codex 或 `/code-review`。
...
| 文档 | 定位 | 维护方式 |
|---|---|---|
| **领域文档**（CONTEXT.md / CONTEXT-MAP.md + 子 context） | 项目级领域模型——术语、对象关系、角色、状态 | 讨论中每确认一个术语就立即写入（`grill-with-docs`） |
...
### Step 0：同步起 grill-with-docs

第一轮用户对话前调用 `grill-with-docs` skill 全程负责领域文档维护。领域文档与设计文档是**双交付物，地位等同**，不能只写设计不维护领域文档。
...
3. **提方案**：提 2-3 个讲 trade-off，推荐放第一个，对话式呈现。让取舍真正张开：至少一个**最小可行**（最少文件 / 最快上线）+ 一个**理想架构**（长期最优）+ 可选一个**另辟蹊径**；三个长得差不多就是没张开。把系统拆成目的清晰、接口明确、可独立测试的小单元（深模块优先，`improve-codebase-architecture` 理解现有边界），遵循既有模式。提方案时内化「架构判断本能」（无聊技术默认 / 增量非革命 / 可逆性 / 本质 vs 偶然复杂度 / blast radius / 推倒重来——见 `references/design-rigor.md`）。
