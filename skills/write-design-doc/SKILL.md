...
输入 → 探查现状 → 选路（访谈 or 综合）→ 设计文档（+ 项目领域文档）→ 自检 →（可选）第二模型审 → 拆 issue 立骨架。

**手动驱动**：你自己读代码、和用户讨论、写文档，无自动状态机 / 自动 reviewer；要第二意见主动交 Codex 或 `/code-review`。阶段性细则在 `references/`，走到对应步现读全文，别凭骨架记忆默写（各步内联指针给路径）。
...
A / B 产出同一份设计文档、走同样的自检 / review / 拆 issue，只是「怎么想清楚」不同。**选哪条不在这步定——读完代码（Step 1）才有判据**：下表「现有代码解决多少 / 是否全新问题」靠探查才填得了。
...
**读完代码在此选路**（判据 + 护栏见上 §两条路）：走 B · 综合模式 → 跳过 Steps 2-4 直接起草；走 A → 继续 Steps 2-4。选定一句话告诉用户（可推翻）。
...
- 骨架已立、要落地 → `write-plan-doc` 按计划 schema 丰富 issue 内容 + 写实施计划
