...
- 合同 & 风险:跨边界合同 bump schema_version + 同步 consumer;DB 字段配套 migration + repository + read model;计费四态收口(hold→settle/release,禁估算金额);LINEAGE 穿透 handoff / 日志。
...
- 跨 Pack 一致:Pack 间接口对得上;无两个 Pack 各写一份同逻辑;共享 Pydantic model / schema_version 一致;migration 顺序对;跨 Pack import 无环;共享 state 并发安全(无共享面 → 一行"已确认独立")。
