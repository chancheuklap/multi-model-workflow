...
- 合规:跨边界合同变更同步全部 consumer + bump schema 版本 / 数据权威 / 项目不变量(agentflow: 计费四态);新增可被外部引用之物入登记(agentflow: port·command·收费动作·capability 入 registry·catalog);新 DB 字段有迁移;helper 放对边界;新文件标 Create;`AGENTS.override.md` 同步。
...
- Critical:依赖错误 / 循环依赖 / Task 逻辑矛盾 / 引用不存在路径 / 违反项目规则 / 允许弱类型裸结构当跨边界合同(agentflow: bare dict)/ 高风险缺迁移回滚。
