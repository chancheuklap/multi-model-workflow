...
| `orchestrate-discovery` | 155 | 1-2（探索） / 3-6（与用户讨论） / 7-9（设计文档） / 10-11（Design Review） / 12（大 issue 拆分） | design-review-angles / discovery-design-document / discovery-discussion / issue-splitting |
...
| `orchestrate-execution` | 530 | 1-3（预执行） / 4-9（Plan 执行 + Review 循环 per Plan） / 10-12（修复分流 + 截断） / 13（Early Release Gate） / 14-16（Plan 推进 + 过渡） | execution-worker-dispatch（Self-Read Protocol） / execution-review-dispatch / execution-preparation / execution-release-gate / execution-repair-truncation |
