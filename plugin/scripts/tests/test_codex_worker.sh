...
echo "$ARGV_P" | grep -q 'model_reasoning_effort="xhigh"' && ok "plan 用 xhigh(设计档,CODEX_PLAN_EFFORT)" || no "plan effort($ARGV_P)"
echo "$ARGV_P" | grep -q -- "-m gpt-5.6-terra" && ok "plan 用 design 档 gpt-5.6-terra(CODEX_PLAN_MODEL)" || no "plan 模型档"
