...
for f in "$CMD_DIR"/*.md; do
  ...
  # 3. 引用 mmw 的命令必须自带定位块,且定位块读激活位、不做版本号扫描(sort -V 挑最高 ≠ 正在运行的)
  if grep -qE '\bmmw\b' "$f"; then
    if grep -qE "$LOCATOR_RE" "$f" && grep -q 'installPath' "$f"; then ok "$name 含激活位定位块"; else no "$name 引用 mmw 却缺激活位定位块"; fi
