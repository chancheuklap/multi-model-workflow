...
newtask() { # preset slug -> echoes worktree path
  bash "$PREPARE" new --scenario "$1" --slug "$2" --title "t-$2" --request "r-$2" --entry-capability explicit-request --entry-evidence "测试夹具明确要求 MMW" 2>/dev/null \
...
# ===== H3: 外部技能缺装告警走 stderr,不占 stdout 首行合同 =====
FAKEHOME="$TMP/fakehome-ext"; mkdir -p "$FAKEHOME"
WES_OUT="$(cd "$WPR" && HOME="$FAKEHOME" bash "$FLOW" where 2>"$TMP/ext-warn.log")"
echo "$WES_OUT" | head -1 | grep -q "^scenario=" && ok "缺装外部技能不污染 where stdout 合同" || no "缺装污染 stdout"
grep -q "缺装" "$TMP/ext-warn.log" && grep -q "tdd" "$TMP/ext-warn.log" \
  && ok "缺装外部技能 stderr 告警(点名缺哪些)" || no "缺装无 stderr 告警"
...
echo "$WCOLD" | grep -q "默认直接处理.*新功能.*根因不明.*多文件" && ok "冷启动明确默认直接与非触发器" || no "冷启动缺默认直接边界"
echo "$WCOLD" | grep -q "\[small-change\].*只读定向.*独立终审" && ok "small-change 按治理能力收窄" || no "small-change 路由仍按大小触发"
echo "$WCOLD" | grep -q -- "--entry-capability.*--entry-evidence" && ok "new 命令要求入口审计参数" || no "new 命令缺入口审计参数"
