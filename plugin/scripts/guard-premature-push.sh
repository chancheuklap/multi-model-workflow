...
ACTIVE_RUN_FILE="${WORKSPACE_ROOT}/.claude/multi-model-workflow/active-run-id"
PLAN_ROOTS=()

if [ -f "$ACTIVE_RUN_FILE" ]; then
  ACTIVE_RUN_ID="$(tr -d '[:space:]' < "$ACTIVE_RUN_FILE")"
  WORKFLOW_STATE="${WORKSPACE_ROOT}/.claude/multi-model-workflow/workflow-state-${ACTIVE_RUN_ID}.json"
