...
严格 hub-and-spoke + 无状态调度：sub-agent 之间不能直接通信；每次 Agent tool 调度都是全新 agent，上下文完全由 prompt 携带。修复归属和 disposition 定义见 `references/dispatch-primitives.md`；完整路由选项见 `references/coordinator-tools.md` Routing Vocabulary。
