...
> **预设 `develop`**,阶段序列:查清 → 给方案 → 想方案 → 切片 → 拆计划 → 落地 → 验收 → 收尾(investigate→propose→design→to-issue→plan→build→verify→closing)。design / plan 产物过后各进一道审闸;to-issue(垂直切片立 issue 骨架)在 ①设计审之后、无审闸。
