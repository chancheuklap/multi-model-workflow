...
2. **方向已定后**,git 文本冲突块逐 hunk 走 `resolving-merge-conflicts` skill(查双方原始意图、尽量都保留、不凭空造新行为、跑仓库自动检查)。**两条红线压过该 skill 的默认**:①它遇不兼容会"自己选一边",但我们 `design_conflict`(两 PR 目标本身矛盾)**不交它、交用户拍**(回 §3/§4 HITL);②它 step 5 会"自己 commit 收尾 merge",但我们合并收尾走 §5 的**人批 `--no-ff`**,不让它自主合。**业务/设计冲突**仍按「三级分类 → 路由」「系统性冲突调查」定的方向解(让两份设计语义自洽,不是单选一边文本)。
