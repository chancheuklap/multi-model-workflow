...
| **系统性·根因不明** | 意图冲突 / 隐式依赖 / 多冲突相互关联 / 要懂整体架构才能判 | **先调查(§4)再修** |
...
1. 用户批 → `mmw release-approve` → `git merge --no-ff <branch>`(**禁 `--squash`**)。
2. git 文本冲突常规解;**业务/设计冲突**按 §3/§4 定的方向解(让两份设计语义自洽,不是单选一边文本)。
3. **每合一个就验**(不批量合完再查):跑该 PR 相关测试。**冒出意外冲突 → 回 §3 重新分类**,别硬合。
