...
> plan 阶段:判单/多计划、映射 plan 清单、写跨 plan 合同骨架、fan-out 派 **Codex 写计划**、亲验返回、回填,加角色声音 + Git 纪律。**计划撰写全下放第二模型(Claude 宿主 Codex / Droid 宿主 plan-writer droid),主线程不内联写**;就绪门自检在 selfcheck 步(`mmw step next` 进)。
...
| 角色 | 谁 | 职责 |
|---|---|---|
| **主 Agent(你)** | 本阶段驱动者 | 读 design + issue → 写跨 plan 合同骨架进设计文档 → fan-out 派 Codex 写计划 → 亲验返回 → 回填合同细节 → 就绪门 → handoff |
| **Codex 写计划工人** | 主线程 `mmw worker plan-dispatch` 后台派(Claude 宿主 codex exec / Droid 宿主 Task→plan-writer droid) | 拿(带合同骨架的)设计文档 + 单个大 issue,**自己把大 issue 拆成小 issue**,写出一份自洽 plan(Header + Task Pack + TDD 步骤 + 验收)。拆分、写作纪律、交付前自检都在它身上(它读 `worktree-plan` skill) |
...
你是计划编排器。把 plan 清单分发给 Codex 写计划工人,确保每份返回的 plan 真实、自洽、可验证;跨 plan 合同面一致。每份 plan 的 scope 用 issue + 文件名界定,不用模糊描述;plan 间依赖用 blocked_by 显式标注;不确定的拆分点标 `[needs-evaluation]`。

Good: "Plan 拆 4 个 pack。1→2→3→4 串行,2 依赖 1 的 schema。派 4 个 Codex,互不依赖的 1/3 并行。"
...
读源设计文档(`prev_outputs` 里的设计文档路径),提取 goal / architecture / 合同边界 / 测试 seam——**只读,作为派发时给 Codex 的上下文**,不在主线程展开写作。

读每个大 issue(`to-issue` 阶段在 `docs/issues/<slug>/` 立的骨架),提取 What to build、Blocked by,定 **plan 清单**(一个大 issue → 一份 plan → 一个 Codex)。**小 issue 不在这拆**——它的 `## Small issues` 通常是 `<!-- PENDING -->`(to-issue 阶段故意留白),由 Codex 接手时自己拆,逼它认真读代码 + 规划。主 Agent 只到大 issue 粒度。
...
每个大 issue 派一个 Codex 写计划工人,**主线程直接跑 `mmw worker plan-dispatch`(不套 subagent——你要留在 loop 里做 Step 4 亲验 / Step 5 回填)**:
...
- 模型档默认 `gpt-5.5 xhigh`(design 档);脚本已钉,除非特殊无需 `--model`。
- 每个 dispatch 独立、零交叉污染:**不要**把别的 writer 的历史 / 别的 plan 内容混进去。方法论(task-pack / 自检)+ `worktree-plan` skill 指针由脚本自动带,不用你手传。

## Step 4:亲验返回

每份 Codex 返回 `pass` 后(读回执:`plan-workers/<ns>/last.md`),对它声明的事实(plan 文件存在、Pack 数量、引用的 `file:line`、**小 issue 已写回 issue 文件 `## Small issues`**)至少抽验 1 个(`grep`/`Read`)再采信。失实 → `mmw worker plan-resume` 打回或主 Agent 亲查修正。任一返回 `needs-context` / `needs-repair` / `blocked` → 按其内容补上下文或修源设计后 plan-resume;返回 `needs-redirection`(探代码撞破设计方向)→ handoff `needs-redirection` 交用户拍方向。

**Droid 宿主**:Task 返回后**必须**先 `mmw worker plan-check --plan <落点> --worktree <任务 wt>`(机器核写计划边界,fail-closed);非零 / `PLAN_VIOLATION` → plan-resume 打回,禁止采信。全部 `pass` + 验过 → Step 5。
...
写 plan 阶段不 commit、不 push,改动保持 unstaged,落地通过后统一提交。**Codex 写计划工人不 commit(脚本 prompt 已声明);主 Agent 统一提交**(设计文档回填和 plan 文档分别提交)。
