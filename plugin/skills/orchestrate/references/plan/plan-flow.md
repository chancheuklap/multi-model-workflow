...
读每个大 issue(`to-issue` 阶段在 `docs/issues/<slug>/` 立的骨架),提取 What to build、Blocked by,定 **plan 清单**(一个大 issue → 一份 plan → 一个 Codex)。**小 issue 不在这拆**——它的 `## Small issues` 通常是 `<!-- PENDING -->`(to-issue 阶段故意留白),由 Codex 接手时自己拆。主 Agent 只到大 issue 粒度。
