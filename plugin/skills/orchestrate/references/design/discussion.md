...
`Step 0`、`Step 1` 起手,`Steps 2-4` 细化,`Domain Alignment` 全程横向跑、不算顺序步。**方向已由上游 propose 阶段定,现状已由 investigate 查清——这里不重做探查、不重提方案,只把选定方向细化成设计文档。**
...
- **不重提方案**:方向已由上游 propose 阶段定;这里在选定方向上细化、改基线,不每轮重摆备选。前提被事实击穿 → handoff `needs-redirection` 回上游换向,先说触发原因 + 业务影响。
...
- **Fable 关键咨询(本阶段主战场,0–2 次)**:在架构/对象边界/状态机**定型前**、用户**确认设计**进 self-check/handoff 前、或讨论中前提**可能被击穿**时,可 `Task` → `fable-advisor` 拿短判决。小改/无边界风险默认跳过。本阶段**不自派** `reviewer-design-a/b`(①设计审归引擎)。
  ...
  ```
  消化:认真权重;一手实证优先;冲突可再 reconcile 一次或写 Open Decisions 问用户。对用户用业务语言吸收结论,不贴 Fable 长文。`stance=pivot` ≠ 自动 handoff——主线程判断后再 `needs-redirection` 或继续细化。
...
#### Step 4:细化选定方向
方向已由上游 propose 阶段拍定(`prev_outputs` 的 `direction` 文件)。这里**细化选定的那一个**,不重摆备选(被事实击穿 → handoff `needs-redirection` 回上游换向)。把系统拆成目的清晰、接口明确、可独立测试的小单元(深模块优先,用 `codebase-design` 的词汇理解现有边界),遵循既有模式。

**内化架构判断本能**:
- **默认无聊技术**:创新额度有限(约 3 个),其余一律用成熟方案;引入新基础设施前问"这张创新额度花得值吗"。
- **增量而非革命**:绞杀榕、灰度发布、重构,不是大爆炸重写。
- **为凌晨 3 点疲惫的人设计**,不是状态最好的高手。
