...
- **Fable 关键咨询(本阶段主战场,0–2 次)**:在架构/对象边界/状态机**定型前**、用户**确认设计**进 self-check/handoff 前、或讨论中前提**可能被击穿**时,可 `Task` → `fable-advisor` 拿短判决。小改/无边界风险默认跳过。本阶段**不自派** `reviewer-design`(①设计审归引擎)。
