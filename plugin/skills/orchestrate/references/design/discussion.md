...
- **Issue / backlog / 已有 PRD**:source 路径;problem / solution / user stories / 验收;依赖 / blocked-by、AFK / HITL、open decisions、out of scope。intent 不清 → Read 外部 `triage` skill(`~/.claude/skills/triage/SKILL.md`,它对模型不可见;缺装就跳过)用它的分诊角色澄清,或继续问。
