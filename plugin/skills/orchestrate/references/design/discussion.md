...
> write-design-doc 走到「讨论」步时读本文**全文**:读方向 + 现状报告、挑战前提、定 scope、细化选定方向、领域对齐,设计阶段的全部讨论方法论 + 角色声音 + Hard Gate 都在这。别凭记忆默写。涉 UI/UX 或非平凡状态模型另读 `prototype-mockup.md`。
...
- **分段呈现**:按段呈现(架构 / 组件 / 数据流 / 错误处理 / 测试策略),每段长度与复杂度成比例,**每段确认后再进下一段**,不一次甩完。**设计涉 UI/UX 或非平凡状态模型 → 走 `prototype-mockup.md`(prototype 验状态模型、html mockup 定 UI/UX),产物原子拆进设计文档 acceptance。**
...
#### Step 4:细化选定方向
方向已由上游 propose 阶段拍定(`prev_outputs` 的 `direction` 文件)。这里**细化选定的那一个**,不重摆备选(被事实击穿 → handoff `needs-redirection` 回上游换向)。把系统拆成目的清晰、接口明确、可独立测试的小单元(深模块优先,用 `codebase-design` 的词汇理解现有边界),遵循既有模式。内化「架构判断本能」(无聊技术默认 / 增量非革命 / 可逆性 / 本质 vs 偶然复杂度 / blast radius / 推倒重来——读 `design-rigor.md` 全文)。
