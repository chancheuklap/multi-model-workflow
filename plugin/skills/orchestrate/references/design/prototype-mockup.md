...
- **落点**:prototype 脚本存设计目录 `docs/design/<slug>/prototype/`——随设计文档一起提交进 git,不散落别处、不当临时文件丢(**覆盖上游 `prototype` skill 的 throwaway 默认**:它新版让原型进临时分支、不进 main;本流程里它是实现种子,必须随设计入 git)。
