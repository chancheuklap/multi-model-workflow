...
| producer | 创建或修改该连接面的 plan。 |
| consumer | 依赖该连接面的 plan。 |
| ownership | 后续由哪个 plan 或系统负责维护。 |
| verification | 如何检查集成后的合同是否成立。 |
...
2. 只提取跨 plan 连接面，记录 producer、consumer、ownership 和 verification。
3. 对 producer 缺失、consumer 缺失、ownership 冲突或 verification 不清的连接面标记 `needs plan repair`。
