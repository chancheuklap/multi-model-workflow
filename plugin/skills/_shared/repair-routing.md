> **使用场景**：review finding disposition 完成后，按本文件分流规则选择修复 owner · **完成后回到** 调用方 phase skill 的修复循环
...
**回归证据要求**：repair worker / Coordinator Path A 返回时必须提供回归证据（核心要求已在 worker agent 合同内）。finding-type → 证据类型的细化映射表见 `_shared/repair-regression-evidence.md`——构造 repair return 时按需自读，不在本路由文件展开。
