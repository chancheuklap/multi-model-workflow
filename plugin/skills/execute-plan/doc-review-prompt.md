...
**Grep 验真（最关键）**：逐条验证计划中引用的文件路径、函数名、helper 真实存在。每个 grep 返回 0 结果 = Critical，标 `needs plan-architect`。

**项目工程规则对照**：读取项目根目录 CLAUDE.md 链入的工程规则文档（如有）。验证计划是否违反项目不变量、单一权威源、模块边界等约束。违反 = Critical，标 `needs plan-architect`。

**其他维度**：设计对齐、task 分解（2-5 分钟）、可执行性、依赖关系、section 分组。

校准：Critical = pack-executor 会卡住。Important = 应改善。Minor 不报。

每个 finding 标注 routing：`needs plan-architect`。**每个 finding 附置信度（0-100），只报 ≥ 80。**
