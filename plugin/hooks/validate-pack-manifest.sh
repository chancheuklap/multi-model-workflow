#!/usr/bin/env bash
# Pack 2.13 / R2: PreToolUse Agent hook ((pack-executor|complex-pack-executor)) that
...
if [[ -n "$MISSING_FROM_MANIFEST" ]]; then
  ...
  echo "  Fix: add row(s) to '## Pack Execution Manifest' in $PLAN_FILE or run 'bash plugin/build/generate-pack-manifest.sh $PLAN_FILE'." >&2
  exit 2
fi
