...
RUN_ID="test-mf-001"
SLUG="manifest-test"
STATE_BASE="$WORKDIR/.claude/multi-model-workflow"
...
build_input() {
  local run_id="$1" pack_id="$2" plan_id="${3:-null}" phase="${4:-execution}" agent_role="${5:-pack-executor}"
...
INPUT_E=$(build_input "$RUN_ID" "null" "001" "execution" "complex-pack-executor")
...
INPUT_F=$(build_input "$RUN_ID" "null" "null" "plan-writing" "plan-writer")
