...
make_input() {
  local plan_id="$1" agent_id="$2" subagent="${3:-pack-executor}"
  local envelope
  ...
  jq -n --arg t "$subagent" --arg p "$(printf '%b' "$prompt")" \
    '{tool_name:"Agent",tool_input:{subagent_type:$t,prompt:$p},tool_response:"Verdict: pass"}'
}
...
OUT=$(run_handler "005" "worker-X" 2>&1)
if echo "$OUT" | grep -qE "resume_from_pack_id|new Agent|新 Agent"; then
  echo "  PASS: need-fresh-worker → NEXT mentions new Agent + resume_from_pack_id"
  pass=$((pass + 1))
else
