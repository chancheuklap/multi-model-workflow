...
`code_explorer` 仍只做窄范围事实查找，但 bug / 失败测试 / 性能回退类调查必须绑定 `diagnose`。Coordinator phase 级 skill（如 `grill-with-docs`、`to-tickets`、`triage`、`impeccable`）仍由 phase skill 显式调用，不写进 sub-agent TOML。
