...
worker_return() {
  ...
  input=$(jq -n --arg p "$prompt" \
    '{tool_name:"subagent",tool_input:{agent_type:"pack_executor",prompt:$p},tool_response:"done"}')
...
assert_contains "need-fresh-worker: handler instructs new subagent" "$HANDLER_OUT" "NEW subagent"
