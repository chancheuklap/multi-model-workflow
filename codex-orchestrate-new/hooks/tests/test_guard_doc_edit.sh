...
mkdir -p "$FIXTURE_DIR/.codex/multi-model-workflow"
mkdir -p "$FIXTURE_DIR/docs/orchestrate/plans" "$FIXTURE_DIR/src"
WT="$FIXTURE_DIR/.codex/worktrees/plan-001"
...
echo "$WT" > "$FIXTURE_DIR/.codex/multi-model-workflow/worker-active-001"
...
run_allow "rule② control plane allowed (plan-returns)" "$FIXTURE_DIR/.codex/multi-model-workflow/plan-returns/r/001/plan-return.json"
...
run_allow "relative path in control plane allowed" ".codex/multi-model-workflow/open-items.json"
run_block "relative path main-tree src blocked" "src/app.py"

# --- 多 marker：第二个 Worker 的 worktree 同样放行；删一个 marker 不影响另一个 ---
WT2="$FIXTURE_DIR/.codex/worktrees/plan-002"
mkdir -p "$WT2/src"
echo "$WT2" > "$FIXTURE_DIR/.codex/multi-model-workflow/worker-active-002"
run_allow "second registered worktree allowed" "$WT2/src/mod.py"
rm "$FIXTURE_DIR/.codex/multi-model-workflow/worker-active-001"
...
rm "$FIXTURE_DIR/.codex/multi-model-workflow/worker-active-002"
run_allow "all markers gone → main tree writable again" "$FIXTURE_DIR/src/app.py"

# --- 串行退化：marker 内容=主树路径 → 主树源码放行（与旧行为等价），docs 仍拦 ---
echo "$FIXTURE_DIR" > "$FIXTURE_DIR/.codex/multi-model-workflow/worker-active-001"
