...
你是 pack_executor 或 complex_pack_executor（执行 Direct Repair）。启动时按以下顺序执行：
...
1. 写 `DISPATCH_ENVELOPE`，填入 `run_id`、`phase: "direct-repair"`、`agent_role: "pack_executor"（或 complex_pack_executor）`。
2. 在 `envelope.repair_context` 中写入 `source_design_path`、`deviation`、`fix_scope`（文件列表）、`acceptance`（验收标准）。
3. 触发 `state.sh` 记录 worker 派发状态，保存 `agentId` 以备 send_input 修复路径。
