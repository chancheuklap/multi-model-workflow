...
你是 pack_executor 或 complex_pack_executor（执行 Multi-PR 冲突修复）。启动时按以下顺序执行：

1. 读 dispatch prompt 头部的 `DISPATCH_ENVELOPE`，提取 `run_id`、`phase: "multi-pr-merge"`。
2. 读 `.codex/multi-model-workflow/merge-brief-<run_id>.md`，获取大设计文档路径、PR 列表、合同地图。
...
Multi-PR conflict repair 是非 execution Pack 的 coding worker：不创建 execution-state，不要求 Pack durable return，Coordinator 从 subagent 返回值读取 final message。但它仍然必须走 route-worker dispatch gate：
...
2. Dispatch 前运行（该命令会在 `phase=multi-pr-merge` 时主动调用 `validate-multi-pr-dispatch.sh`，检查 merge brief、stage、conflict_id 和 prompt 引用）：
   ...
   bash "${MMW_PLUGIN_ROOT}/scripts/dispatch-route-worker.sh" validate \
     ...
     --transport subagent
   ```
3. 校验通过后用 prompt 文件全文作为 `spawn_agent message`。
4. 从 `subagent` 返回值提取 `agent_id`，并持久化：
   ```bash
   bash "${MMW_PLUGIN_ROOT}/scripts/dispatch-route-worker.sh" record \
     --prompt-file ".codex/multi-model-workflow/worker-prompts/multi-pr-conflict-<conflict-id>.md" \
     --agent-id "<agent_id>" \
