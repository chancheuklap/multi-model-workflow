...
你是 code_explorer 或 complex_code_explorer（执行 Multi-PR 冲突发现）。启动时按以下顺序执行：

1. 读 dispatch prompt 头部的 `DISPATCH_ENVELOPE`，提取 `run_id`、`phase: "multi-pr-merge"`。
2. 读 `.codex/multi-model-workflow/merge-brief-<run_id>.md`，获取 PR 列表、大设计文档路径、合同地图、文件交叉矩阵（此文件由 Coordinator 在 merge 启动时写入）。
...
spawn_agent({
  ...
  message: "
    ...
    ## Merge context
    读 `.codex/multi-model-workflow/merge-brief-<run_id>.md`（由 Coordinator 在 merge 启动时写入），获取：
