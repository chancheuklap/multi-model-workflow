...
> 创建命令：`bash "${MMW_PLUGIN_ROOT}/scripts/state.sh" merge-brief init --run-id <run_id> --slug <slug>`
> 文件路径：`.codex/multi-model-workflow/merge-brief-<run_id>.md`
> 
...
- **关联 workflow-state 路径**: `.codex/multi-model-workflow/workflow-state-<run_id>.json`
...
- **per_pr_final_review_verdict_refs**:
  - `[.codex/multi-model-workflow/review-results/final-review-<pr1>.md]`
  - `[.codex/multi-model-workflow/review-results/final-review-<pr2>.md]`
...
- **evidence_pointers**:
  ...
  - 集成审查 verdict: `.codex/multi-model-workflow/review-results/multi-pr-integration-review.md`
  - 全量测试结果: `bash ${MMW_PLUGIN_ROOT}/scripts/run-all-tests.sh → 全 pass`
- **decided_at**: `<ISO-8601>`
