...
4. 等待返回：reviewer 类保存 review-results、运行 `complete-review-dispatch.sh` 后 `close_agent`；其他类保存 worker / analyst / explorer final message，Coordinator 校验返回事实并写入 merge-brief 对应段后 `close_agent`。
