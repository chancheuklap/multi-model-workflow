...
（append-only，每轮 design review 通过后追加一行；plan_writer 读取此 section 了解审查共识，避免重复犯错）
...
（plan_writer 写 Pack 时 Read 本 section 同步 Contract anchors；Coordinator 在 Plan Review / Final Review 时以本 section 为权威）
