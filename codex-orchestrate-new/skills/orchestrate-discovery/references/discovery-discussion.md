...
- 缺 feedback loop → 先 `加载 skill `diagnose``。Discovery 只消费 diagnose 产出的事实
- 只是已批准 design 下的实现偏离 → 返回 `READY_FOR_REPAIR`
- 出现 bad seam、shallow module、caller leakage → `加载 skill `improve-codebase-architecture``
...
- source intent 不清 → `加载 skill `triage`` 或继续 Discovery 提问
...
- **深模块优先**：用 `加载 skill `improve-codebase-architecture`` 理解现有模块边界。
...
涉及视觉判断时：`加载 skill `prototype`` 验证状态模型 / UI 方向，`加载 skill `frontend-design`` 生成高品质前端原型。

## Step 6：Domain Alignment（全程横向检查）

贯穿整个 Discovery 过程。触发条件：术语模糊/过载/冲突 / 新对象/状态/角色 / owner 不清 / 用户说法与代码/文档冲突 / 后续会因术语不清而拆错 / ADR 三条件同时成立。

**术语挑战**：与 CONTEXT.md / CONTEXT-MAP.md 体系冲突时立刻指出；模糊或过载时提出精确规范术语。

**双文档写回规则**：

| 内容类型 | 写回目标 | 时机 |
|---------|---------|------|
| 稳定术语、对象关系、角色、状态 | CONTEXT.md，或 `CONTEXT-MAP.md` 索引下对应子 context 文件 | 确认一个写一个 |
| 术语被模糊使用 | 同上文件的 Flagged ambiguities | 发现时立即 |
| 跨 context 关系 / 共享词汇 | `CONTEXT-MAP.md`（若存在）；否则归入根 `CONTEXT.md` | 确认时立即 |
