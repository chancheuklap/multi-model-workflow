...
| 修订范围 | 路径 |
| --- | --- |
| 只需修改非语义格式（缺 section header / Review History 表头 / Manifest 列名 / 明显路径拼写） | Coordinator 直接修 → 跳到 Step 11（Plan Entry Gate 重检） |
| 需修改 Task Pack 内容、依赖、HITL/AFK、owned files、verification、contract anchors 或 plan header 的语义字段 | 用 `state.sh plan-writer-session get --run-id <run_id> --plan-id <NNN> --field agent_id` 读取对应 plan 的原 plan_writer；若无 agentId 则 BLOCKED。prompt 附带具体 findings + 现有 plan path → plan_writer 定向修订 → Step 11 |
