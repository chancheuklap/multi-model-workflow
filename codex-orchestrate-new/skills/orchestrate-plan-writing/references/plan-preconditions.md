...
| 需修改 Task Pack 内容（implementation tasks / owned files / verification） | 用 `state.sh plan-writer-session get --run-id <run_id> --plan-id <NNN> --field agent_id` 读取对应 plan 的原 plan_writer；若无 agentId 则 BLOCKED。prompt 附带具体 findings + 现有 plan path → plan_writer 定向修订 → Step 11 |
