...
| 需修改 Task Pack 内容（implementation tasks / owned files / verification） | send_input 原 plan_writer（agentId 从 workflow context 获取）（agentId 从 workflow-state 获取，若无 agentId 则 BLOCKED），prompt 附带具体 findings + 现有 plan path → plan_writer 定向修订 → Step 11 |
