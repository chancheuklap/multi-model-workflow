...
派发当前 issue 的 plan_writer 之前，Coordinator 确认以下路径存在（存在则 plan_writer 自读）：
...
3. 触发 `state.sh` 记录 plan_writer 派发状态，保存 `agentId` 以备 send_input 修复路径。
4. 等待 plan_writer 返回 Verdict，按 Step 10 路由表处置。
