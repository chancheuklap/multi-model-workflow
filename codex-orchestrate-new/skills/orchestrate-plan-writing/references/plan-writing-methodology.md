...
（由 plan_writer 手写或构建脚本 `codex-orchestrate-new/build/generate-pack-manifest.sh` 生成；Coordinator 派 Worker 前用 `validate-pack-manifest.sh` 三方对账：Manifest / Pack 主体 / execution-state.packs keys）
```
