...
# Usage:
#   dispatch-route-worker.sh validate --prompt-file PATH [--transport subagent|send_input]
...
case "$SUBCMD" in
  validate)
    ...
    [[ -n "$PROMPT_FILE" && -f "$PROMPT_FILE" ]] || { echo "Usage: dispatch-route-worker.sh validate --prompt-file PATH [--transport subagent|send_input]" >&2; exit 2; }
    [[ "$TRANSPORT" == "subagent" || "$TRANSPORT" == "send_input" ]] || { echo "Error: --transport must be subagent or send_input" >&2; exit 2; }
    ...
    case "$AGENT_ROLE" in
      ...
      *)
        echo "Error: route worker dispatch agent_role must be pack_executor or complex_pack_executor" >&2
        exit 2
        ;;
    esac
