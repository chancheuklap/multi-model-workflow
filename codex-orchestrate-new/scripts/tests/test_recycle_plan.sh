...
export STATE_BASE=".codex/multi-model-workflow"

mkesf() { cat > ".codex/multi-model-workflow/execution-state-${RUN_ID}.json" <<EOF
...
touch ".codex/multi-model-workflow/worker-active-001"
...
run_test "isolation_status=merged recorded" \
  bash -c "[[ \$(jq -r '.plans[\"001\"].isolation_status' '.codex/multi-model-workflow/execution-state-${RUN_ID}.json') == 'merged' ]]"
run_test "worktree removed + branch deleted + marker cleaned" \
  bash -c "[[ ! -d '$WT1' ]] && ! git show-ref -q refs/heads/plan-001 && [[ ! -f '.codex/multi-model-workflow/worker-active-001' ]]"
...
run_test "refused plan marked isolated, worktree preserved" \
  bash -c "[[ \$(jq -r '.plans[\"002\"].isolation_status' '.codex/multi-model-workflow/execution-state-${RUN_ID}.json') == 'isolated' ]] && [[ -d '$WT2' ]]"
