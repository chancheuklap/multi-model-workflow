...
run_test "plan-level envelope passes parse-envelope self-check & prints block" \
  bash -c "bash '$STATE_SH' envelope build --run-id '$RUN_ID' --phase execution --agent-role pack_executor --plan-id 001 --worktree-path /tmp/wt-001 | grep -q 'DISPATCH_ENVELOPE'"

ENV_JSON=$(bash "$STATE_SH" envelope build --run-id "$RUN_ID" --phase execution --agent-role pack_executor --plan-id 001 --worktree-path /tmp/wt-001 | sed -n 's/.*DISPATCH_ENVELOPE \(.*\) -->.*/\1/p')
...
PACK_JSON=$(bash "$STATE_SH" envelope build --run-id "$RUN_ID" --phase direct-repair --agent-role pack_executor --pack-id 2.3 | sed -n 's/.*DISPATCH_ENVELOPE \(.*\) -->.*/\1/p')
...
run_test_expect_fail "both --plan-id and --pack-id rejected" \
  bash "$STATE_SH" envelope build --run-id "$RUN_ID" --phase execution --agent-role pack_executor --plan-id 001 --pack-id 1.1
run_test_expect_fail "repair round >=1 without disposition_refs rejected" \
  bash "$STATE_SH" envelope build --run-id "$RUN_ID" --phase execution --agent-role pack_executor --plan-id 001 --repair-round 1
run_test "repair round with disposition_refs OK" \
  bash -c "bash '$STATE_SH' envelope build --run-id '$RUN_ID' --phase execution --agent-role pack_executor --plan-id 001 --repair-round 1 --disposition-refs '[\"F1\"]' | grep -q 'DISPATCH_ENVELOPE'"
run_test_expect_fail "codex_reviewer without review_intent rejected" \
  bash "$STATE_SH" envelope build --run-id "$RUN_ID" --phase execution --agent-role codex_reviewer --plan-id 001
run_test "codex_reviewer with baseline OK and parses" \
  bash -c "bash '$STATE_SH' envelope build --run-id '$RUN_ID' --phase execution --agent-role codex_reviewer --plan-id 001 --review-intent baseline | bash '$PARSER' | jq -e '.review_intent == \"baseline\"'"
run_test "resume_from_pack_id carried when set" \
  bash -c "bash '$STATE_SH' envelope build --run-id '$RUN_ID' --phase execution --agent-role pack_executor --plan-id 001 --resume-from-pack-id 1.3 | sed -n 's/.*DISPATCH_ENVELOPE \(.*\) -->.*/\1/p' | jq -e '.resume_from_pack_id == \"1.3\"'"
