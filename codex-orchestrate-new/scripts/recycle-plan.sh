...
#      该 Plan 标 isolated（Worker 禁改 docs；Codex 不经 Codex hook，此处兜底）
...
if ! git merge --no-ff "$BRANCH" -m "merge(plan-${PLAN_ID}): 回收并行 Plan ${PLAN_ID}（${BRANCH}）"; then
  ...
  echo "[multi-model-workflow] BLOCKED: plan $PLAN_ID 合并冲突。按 B6：派 explorer 做冲突发现；系统性冲突走 root_cause_analyst（借鉴 multi-pr-merge 方法论，不套整套）。冲突期间不影响已合并 Plan。" >&2
  exit 2
fi
