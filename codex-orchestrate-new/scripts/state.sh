...
Commands:
  ...
  execution-plan    Manage execution-state plan boundaries (start|complete|finish|session)
...
cmd_execution_plan_finish() {
  ...
  # Plan 到达 completed / isolated 终态时清除 per-plan worker-active
  # marker；否则 guard-doc-edit.sh 会据残留 marker 持续拦截 docs/ 编辑。
  # merged 不在此清理，marker 已在 completed 阶段删除。
