...
check "execution dispatch creates Codex-root plan worktree" bash -c "grep -q 'git worktree add -b' '$PLUGIN_DIR/skills/orchestrate-execution/SKILL.md' && grep -q 'CODEX_WORKTREE_ROOT' '$PLUGIN_DIR/skills/orchestrate-execution/SKILL.md' && grep -q 'PLAN_WORKTREE' '$PLUGIN_DIR/skills/orchestrate-execution/references/execution-preparation.md'"
check "workflow docs avoid repo-local workflow worktrees" bash -c "! rg -q 'multi-model-workflow/worktrees|STATE_DIR\\}/worktrees|STATE_DIR/worktrees|worktrees/plan-' '$PLUGIN_DIR/skills/orchestrate-workflow' '$PLUGIN_DIR/skills/orchestrate-execution' --glob '!**/tests/**'"
check "plan writing tracks parallel writer sessions" bash -c "grep -q 'plan_writer_sessions' '$PLUGIN_DIR/state-schema/workflow-state-v1.json' && grep -q 'plan-writer-session set' '$PLUGIN_DIR/skills/orchestrate-plan-writing/references/plan-writer-dispatch.md' && ! rg -q 'plan_writer_agent_id' '$PLUGIN_DIR/skills/orchestrate-plan-writing' '$PLUGIN_DIR/state-schema' '$PLUGIN_DIR/scripts/state.sh' --glob '!**/tests/**'"
