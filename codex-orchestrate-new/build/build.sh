...
resolve_anchor() {
  ...
  case "$anchor_name" in
    ...
    # [variant=codex]; a bare worker-loop anchor will error (intentional).
    ...
    # preamble, send-input-resume, signpost: extract [variant=X]...[/variant] block.
    preamble|send-input-resume|signpost)
      local tmpl="$TEMPLATE_DIR/${anchor_name}.md.tmpl"
      if [[ ! -f "$tmpl" ]]; then
        echo "Error: template not found: $tmpl" >&2
        return 1
