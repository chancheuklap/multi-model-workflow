...
Options:
  --dry-run        Show copy and config actions without changing runtime files.
...
if [[ ! -s "$templates_file" ]]; then
  echo "ERROR: no Codex agent templates found in $SCRIPT_DIR" >&2
  exit 1
fi
...
agents = [
    ...
    ("codex_planning_reviewer", "High-tier independent reviewer for Discovery Design Review and Plan Review gates.", "agents/codex_planning_reviewer.toml", ["Planning Reviewer", "Design Reviewer", "Plan Auditor"]),
...
if [[ "$MODE" == "--dry-run" ]]; then
  echo "Dry run complete. Re-run with --apply to copy files. Add --update-config to register agent_type entries."
else
