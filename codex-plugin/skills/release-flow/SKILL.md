...
```bash
RELEASE_SKILL="<本轮 skill source locator 给出的 release-flow/SKILL.md 绝对路径>"
MMW_ROOT="$(cd "$(dirname "$RELEASE_SKILL")/../.." && pwd)"
MMW="$MMW_ROOT/scripts/mmw.sh"
printf 'mmw       = %s\nSKILL_DIR = %s\n' "$MMW" "$MMW_ROOT/skills/release-flow"
```

若 `$MMW` 不存在，先确认 multi-model-workflow plugin 已安装并重开 Codex task；不扫描 marketplace 源码或按版本号猜 cache 目录。
