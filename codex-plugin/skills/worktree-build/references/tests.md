...
主线程派发消息里给了「仓库测试薄层」路径的,先读它——那是本仓库的测试事实(分层目录、哪些边界算外部可 mock、权威源指针、套件门控),新测试进它要求的目录。派发消息说仓库无薄层的,写法全按下面权威、落点随仓库既有目录惯例,并在收工回执标 `no-repo-test-sheet`。

TDD 循环(先写失败测试→最小实现→转绿)用已装的 `tdd` skill;seam 由计划的 Task Pack 钉死,不另行商定。下面是每个测试进仓的资格线:
