...
开跑前读 `${SKILL_DIR}/references/control/runtime-contract.md`，对齐 Codex 路径、工具、subagent 派发和安全门。
...
- **`RESUMABLE` + 在飞任务**→ 先按用户本轮意图分流。明确要新建任务就进 Step 1；要继续或意图不明时，单个任务执行回显的 `resume=` 命令，多个任务请用户选择续跑哪个或新建。Codex 主任务始终留在 App 创建的当前 worktree；重新跑 `where` 后按上一条续跑。**不得把 `RESUMABLE` 当成新任务直接 `task new`。**
