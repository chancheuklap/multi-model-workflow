...
if [ -f "$man" ]; then
  ...
  jq -r --arg mmw "$MMW" '
    ...
    "[multi-model-workflow-codex] 在管任务 worktree: slug=\(.slug) scenario=\(.scenario) phase=\(.phase) status=\(.status) mode=\($mode)。" +
...
echo "[multi-model-workflow-codex] 会话分诊:需正式编排的开发任务(新功能/系统改造/根因不明的 bug/需独立任务边界的小改/合并 worktree)→ 用 orchestrate skill 进流程(先跑 $MMW where);问答、解释、只读查看和主线程可直接完成并验证的琐碎单步动作 → 直接处理,不进流程。"
hdr=0
