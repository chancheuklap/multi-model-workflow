#!/usr/bin/env bash
# mmw —— Codex 原生 multi-model-workflow 统一 CLI
#
...
#   写码工人派发(准备内层 worktree/prompt/账本,协调者照输出调用 Codex native spawn_agent):
...
#   写计划工人派发(同上;每份 plan 独立沙箱、不 commit):
...
# 状态平面固定为 .codex/multi-model-workflow/；用户任务 worktree 由 Codex App 提供。
set -euo pipefail
D="$(cd "$(dirname "$0")" && pwd)"
