...
#   dispatch    --stage <n> --findings <p>  收敛护栏 + 按 tier 派修(P2 derive/P1 native GPT/P0 停)
#   repair verify --stage <n> [--worker-ref <ref>]  验收 P1 子代理改动并提交
set -euo pipefail
...
_match_any() {
  ...
  shift
  for g in "$@"; do
    # shellcheck disable=SC2053 # g 是 manifest 声明的 git path glob。
    [[ "$path" == $g ]] && return 0
  done
  return 1
}
...
_path_gate() {
  ...
  for path in "${CHANGED_PATHS[@]-}"; do
    ...
    # 派生物刻意不入 P1 的 editable_paths(否则 P1 修复者能直改派生物,破坏「derive 唯一 writer」)。
...
_run_direct_action() {
  ...
  mp="$(jq -r '.manifest_path' "$f")"
  ACTION_ARGV=()
  while IFS= read -r arg; do ACTION_ARGV+=("$arg"); done < <(jq -r '.derive[]' "$mp")
  [ ${#ACTION_ARGV[@]} -gt 0 ] || die "manifest.derive 为空(引擎载入应已挡,防御)"
...
cmd_dispatch_direct() {
  ...
  [ "$mode" = "derive" ] || die "Codex direct dispatch 只执行 P2 derive"
  top="$(_repo_top)"
  action_kind="derive"
  ...
  if [ "$ACTION_RC" -ne 0 ]; then
    _record_pause "$f" "$name" "$action_kind" "action_failed" "$fp" "" \
      "needs-context" "derive 执行非零退出，保留现场交人" "$changed_json" "[]" "$RF_WORKER_REF" "$ACTION_COMMAND" ""
  ...
  if [ ${#CHANGED_PATHS[@]} -eq 0 ]; then
    _record_pause "$f" "$name" "$action_kind" "no_change" "$fp" "" \
      "needs-context" "derive 未产生任何可提交改动，不能伪装为已修复" "[]" "[]" "$RF_WORKER_REF" "$ACTION_COMMAND" ""
  ...
  message="chore(release): regenerate $fp"
...
_load_native_repair_snapshot() {
  local f="$1" value
  PROTECTION_MATCHERS=()
  while IFS= read -r value; do
    [ -n "$value" ] && PROTECTION_MATCHERS+=("$value")
  done < <(jq -r '.native_repair.protection_matchers[]' "$f")
  EDITABLE_MATCHERS=()
  while IFS= read -r value; do
    [ -n "$value" ] && EDITABLE_MATCHERS+=("$value")
  done < <(jq -r '.native_repair.editable_matchers[]' "$f")
  BASELINE_UNTRACKED_PATHS=()
  while IFS= read -r value; do
    [ -n "$value" ] && BASELINE_UNTRACKED_PATHS+=("$value")
  done < <(jq -r '.native_repair.baseline_untracked[].path' "$f")
  BASELINE_UNTRACKED_HASHES=()
  while IFS= read -r value; do
    [ -n "$value" ] && BASELINE_UNTRACKED_HASHES+=("$value")
  done < <(jq -r '.native_repair.baseline_untracked[].sha256' "$f")
  PROTECTION_FROZEN=1
  EDITABLE_FROZEN=1
}

_native_repair_has_changes() {
  local f="$1" top start live
  top="$(_repo_top)"
  start="$(jq -r '.native_repair.start_sha' "$f")"
  live="$(git -C "$top" rev-parse HEAD)"
  [ "$start" = "$live" ] || return 0
  _load_native_repair_snapshot "$f"
  _collect_candidate_paths "$top" "$f"
  [ ${#CHANGED_PATHS[@]} -gt 0 ] && return 0
  _baseline_untracked_changed "$top" || return 0
  return 1
}

cmd_dispatch_p1() {
  local f="$1" name="$2" fp="$3" findings="$4" top mp artifact_dir prompt findings_copy
  local editable_json protection_json baseline_json value index
  top="$(_repo_top)"
  mp="$(jq -r '.manifest_path' "$f")"
  [ "$(jq -r '.native_repair == null' "$f")" = "true" ] \
    || die "已有待验收的 native repair；先按 release where 恢复"
  if ! git -C "$top" diff --quiet HEAD; then
    _record_pause "$f" "$name" "preflight" "tracked_dirty" "$fp" "" \
      "needs-context" "功能分支已有未提交 tracked 改动，不能混入原生修复提交" "[]" "[]" "" "git diff --quiet HEAD" ""
    echo "DISPATCH-PAUSED:$name(pre-existing tracked diff)"
    return 0
  fi
  ...
  if ! _load_path_hard_deny "$f"; then
    _record_pause "$f" "$name" "preflight" "protection_source_unreadable" "$fp" "" \
      "needs-context" "protection_source 在原生修复前已不可用($PATH_GATE_ERROR)，保留现场交人" "[]" "[]" "" "" ""
    echo "DISPATCH-PAUSED:$name(protection source unreadable)"
    return 0
  fi
  PROTECTION_FROZEN=1
  ...
  artifact_dir="$(dirname "$f")/release-artifacts"
  mkdir -p "$artifact_dir"
  prompt="$artifact_dir/native-repair-prompt.md"
  findings_copy="$artifact_dir/native-repair-findings.json"
  cp "$findings" "$findings_copy"

  {
    printf '# Codex 原生 P1 修复任务\n\n'
    printf '你在 `%s` 中修复 release stage `%s` 的根因 `%s`。\n\n' "$top" "$name" "$fp"
    printf '## 必须遵守\n\n'
    printf -- '- 先读 findings，再读相关真实代码，做能消除根因的最小修复。\n'
    printf -- '- 只可修改下列 editable paths：\n'
    printf '  - `%s`\n' "${EDITABLE_MATCHERS[@]-}"
    printf -- '- 绝对不可修改下列保护路径：\n'
    printf '  - `%s`\n' "${PROTECTION_MATCHERS[@]-}"
    printf -- '- 只在上述绝对路径的当前 App worktree 工作；不要进入或修改其他 worktree。\n'
    printf -- '- 不要调用外部模型 CLI；不要修改 `docs/` 或 `.codex/`，不要运行 release driver，不要 commit。\n'
    printf -- '- 可运行与改动直接相关的验证；失败必须原样报告，不要用默认值或吞错伪装成功。\n\n'
    printf '## Findings\n\n```json\n'
    cat "$findings_copy"
    printf '\n```\n\n'
    printf '## 回执\n\n'
    printf '返回：`STATUS=done|blocked`、`CHANGED_PATHS=`、`VERIFICATION=`、`BLOCKER=`。完成后停下，由协调者验收和提交。\n'
  } > "$prompt"

  editable_json="$(_json_array "${EDITABLE_MATCHERS[@]-}")"
  protection_json="$(_json_array "${PROTECTION_MATCHERS[@]-}")"
  if [ ${#BASELINE_UNTRACKED_PATHS[@]} -eq 0 ]; then
    baseline_json="[]"
  else
    baseline_json="$(
      for index in "${!BASELINE_UNTRACKED_PATHS[@]}"; do
        jq -nc --arg p "${BASELINE_UNTRACKED_PATHS[$index]}" \
          --arg h "${BASELINE_UNTRACKED_HASHES[$index]}" '{path:$p,sha256:$h}'
      done | jq -sc .
    )"
  fi
  edit "$f" --arg stage "$name" --arg fp "$fp" --arg findings "$findings_copy" \
    --arg sha "$(git -C "$top" rev-parse HEAD)" --arg prompt "$prompt" \
    --argjson editable "$editable_json" --argjson protection "$protection_json" \
    --argjson baseline "$baseline_json" --arg created "$(now)" \
    '.native_repair={
      stage:$stage, fingerprint:$fp, findings_path:$findings, start_sha:$sha,
      prompt_path:$prompt, editable_matchers:$editable,
      protection_matchers:$protection, baseline_untracked:$baseline,
      created_at:$created
    }'
  echo "NATIVE-REPAIR-READY:$name prompt=$prompt"
  echo 'action=读取 prompt 全文，用 spawn_agent(fork_turns="none") 派给当前 Codex task 的 GPT 子代理；完成后运行 release repair verify'
}

cmd_repair_verify() {
  local name="" worker_ref="codex-native-subagent"
  while [ $# -gt 0 ]; do
    case "$1" in
      --stage) name="$2"; shift 2 ;;
      --worker-ref) worker_ref="$2"; shift 2 ;;
      *) die "repair verify 未知参数 $1" ;;
    esac
  done
  [ -n "$name" ] || die "repair verify --stage 必填"

  local f top pending fp start live changed_json blocked_json artifact_ref="" commit_sha post_state
  f="$(need_state)"
  pending="$(jq -r '.native_repair.stage // ""' "$f")"
  [ -n "$pending" ] || die "没有待验收的 native repair"
  [ "$pending" = "$name" ] || die "待验收 stage 是 $pending，不是 $name"
  top="$(_repo_top)"
  fp="$(jq -r '.native_repair.fingerprint' "$f")"
  start="$(jq -r '.native_repair.start_sha' "$f")"
  live="$(git -C "$top" rev-parse HEAD)"
  if [ "$live" != "$start" ]; then
    _record_pause "$f" "$name" "fix" "worker_committed" "$fp" "" \
      "needs-context" "原生修复子代理改动了 HEAD；保留提交现场交人，不能混入引擎提交" "[]" "[]" "$worker_ref" \
      'spawn_agent(fork_turns="none")' "P1"
    edit "$f" '.native_repair=null'
    echo "REPAIR-PAUSED:$name(worker changed HEAD)"
    return 0
  fi
  ...
  changed_json="$(_json_changed_paths)"
  if ! _baseline_untracked_changed "$top"; then
    _record_pause "$f" "$name" "fix" "baseline_untracked_changed" "$fp" "" \
      "needs-context" "原生修复改写了派发前已存在的未跟踪文件，保留现场交人" \
      "$(_json_array "${BASELINE_CHANGED_PATHS[@]-}")" "[]" "$worker_ref" \
      'spawn_agent(fork_turns="none")' "P1"
    edit "$f" '.native_repair=null'
    echo "REPAIR-PAUSED:$name(baseline untracked changed)"
    return 0
  fi
  if [ ${#CHANGED_PATHS[@]} -eq 0 ]; then
    _record_pause "$f" "$name" "fix" "no_change" "$fp" "" \
      "needs-context" "原生修复没有产生任何可提交改动，不能伪装为已修复" "[]" "[]" "$worker_ref" \
      'spawn_agent(fork_turns="none")' "P1"
    edit "$f" '.native_repair=null'
    echo "REPAIR-PAUSED:$name(no change)"
    return 0
  fi

  if ! _path_gate "$f" "fix"; then
    BLOCKED_PATHS=("${CHANGED_PATHS[@]-}")
  fi
  if [ ${#BLOCKED_PATHS[@]} -gt 0 ]; then
    if ! _write_path_gate_patch "$f" "$name" "$top" || ! _restore_rejected_candidates "$top"; then
      _record_pause "$f" "$name" "path_gate" "cleanup_failed" "$fp" "" \
        "needs-context" "路径闸拒绝后无法完整保存或复原原生修复，保留现场交人" \
        "$changed_json" "$(_json_array "${BLOCKED_PATHS[@]-}")" "$worker_ref" \
        'spawn_agent(fork_turns="none")' "P0"
      edit "$f" '.native_repair=null'
      echo "PATH-GATE-PAUSED:$name(cleanup failed)"
      return 0
    fi
    ...
    blocked_json="$(_json_array "${BLOCKED_PATHS[@]-}")"
    _record_pause "$f" "$name" "path_gate" "rejected" "$fp" "$artifact_ref" \
      "needs-redirection" "原生修复触及受保护路径或超出 editable_paths，已保存 patch、复原改动，停下请负责人拍板" \
      "$changed_json" "$blocked_json" "$worker_ref" 'spawn_agent(fork_turns="none")' "P0"
    edit "$f" '.native_repair=null'
    echo "PATH-GATE-REJECT:$name 越界=[${BLOCKED_PATHS[*]}]"
    return 0
  fi
  ...
  if ! git -C "$top" add -- "${CHANGED_PATHS[@]-}"; then
    _record_pause "$f" "$name" "fix" "add_failed" "$fp" "" \
      "needs-context" "原生修复通过路径闸后无法暂存改动，保留现场交人" "$changed_json" "[]" \
      "$worker_ref" 'spawn_agent(fork_turns="none")' "P1"
    edit "$f" '.native_repair=null'
    echo "REPAIR-PAUSED:$name(git add failed)"
    return 0
  fi
  if ! git -C "$top" commit -m "fix(release): $fp" >/dev/null; then
    _record_pause "$f" "$name" "fix" "commit_failed" "$fp" "" \
      "needs-context" "原生修复通过路径闸后无法创建功能分支提交，保留现场交人" "$changed_json" "[]" \
      "$worker_ref" 'spawn_agent(fork_turns="none")' "P1"
    edit "$f" '.native_repair=null'
    echo "REPAIR-PAUSED:$name(git commit failed)"
    return 0
  fi
  commit_sha="$(git -C "$top" rev-parse HEAD)"
  append_attempt "$f" "$name" "fix" "applied" "$fp" "git-commit:$commit_sha"
  patch_last_attempt "$f" "$changed_json" "[]" "[]" "$worker_ref" 'spawn_agent(fork_turns="none")'
  edit "$f" '.budget.fix_rounds += 1 | .native_repair=null'
  echo "FIX-COMMITTED:$name commit=$commit_sha"
  _post_fix_gate "$f" "$name" "$fp" "$commit_sha"
  post_state="$(cmd_where | head -1)"
  case "$post_state" in
    STAGE:*|RETRY-STAGE:*) cmd_round next ;;
  esac
}
...
cmd_repair() {
  ...
  shift || true
  case "$verb" in
    verify) cmd_repair_verify "$@" ;;
    *) die "用法 repair verify --stage <name> [--worker-ref <ref>]" ;;
  esac
}
...
cmd_init() {
  ...
  mmw_ensure_wt_state_ignore "$top"
  sd="$(mmw_resolve_state_subdir "$top")"
  f="$top/$sd/$STATE_NAME"
  ...
  printf '%s' "$canon" | jq --arg mp "$mp" --arg sc "$source_commit" --argjson mr "$max_rounds" --argjson wall "$max_wall_clock" --arg ts "$(now)" \
    '{schema_version:"1", product:.product, manifest_path:$mp, source_commit:$sc,
      ...
      attempt_ledger:[], native_repair:null, pause:null}' | write "$f"
...
cmd_where() {
  ...
  if [ "$(jq -r '.native_repair != null' "$f")" = "true" ]; then
    local repair_stage repair_prompt
    repair_stage="$(jq -r '.native_repair.stage' "$f")"
    repair_prompt="$(jq -r '.native_repair.prompt_path' "$f")"
    if _native_repair_has_changes "$f"; then
      echo "NATIVE-REPAIR-VERIFY:$repair_stage"
      echo "next=等待原生 GPT 子代理结束后运行 release repair verify --stage $repair_stage"
    else
      echo "NATIVE-REPAIR-PENDING:$repair_stage"
...
cmd_stage() {
  ...
  f="$(need_state)"
  [ "$(jq -r '.native_repair == null' "$f")" = "true" ] \
    || die "native repair 尚未验收；不能运行或改写 stage"
  case "$verb" in
    done) cmd_stage_done "$@" ;;
    fail) cmd_stage_fail "$@" ;;
    run) cmd_stage_run "$@" ;;
...
cmd_dispatch() {
  ...
  f="$(need_state)"
  [ "$(jq -r '.native_repair == null' "$f")" = "true" ] \
    || die "native repair 尚未验收；先按 release where 恢复，不能重复 dispatch"
...
cmd_round() {
  ...
  f="$(need_state)"
  [ "$(jq -r '.native_repair == null' "$f")" = "true" ] \
    || die "native repair 尚未验收；不能推进 round"
...
cmd_resume() {
  ...
  [ "$(jq -r '.native_repair == null' "$f")" = "true" ] \
    || die "native repair 尚未验收；先按 release where 恢复，不能 resume 绕过"
...
cmd_exit_check() {
  ...
  jq -e . "$f" >/dev/null 2>&1 || { echo "CORRUPT:release-state 空/非法 JSON"; return 0; }
  if [ "$(jq -r '.native_repair != null' "$f")" = "true" ]; then
    echo "NOT-DONE:native-repair=$(jq -r '.native_repair.stage' "$f")"
    return 0
  fi
  if [ "$(jq -r '.pause // "null"' "$f")" != "null" ]; then
    echo "PAUSED:$(jq -r '.pause.reason' "$f")"
...
case "${1:-}" in
  ...
  *) die "用法: release-flow.sh init|where|stage|round|surface|resume|close|exit-check|receipt|dispatch|repair ..." ;;
esac
