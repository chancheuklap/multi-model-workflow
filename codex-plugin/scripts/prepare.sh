#!/usr/bin/env bash
# prepare.sh —— Codex App outer worktree 采用层
#
#   new     采用当前 App linked worktree/branch + scaffold docs + 写 manifest
#   resume  从 worktree 内读 manifest,返回"你是谁、在哪、什么状态"(单一真相源)
#   cleanup 合并后只清 MMW 临时状态；App worktree/branch 仍归 App
#
...
# 当前 checkout top-level(linked worktree 里 .git 是文件,local checkout 里是目录)
...
cmd_new() {
  ...
  if ! is_linked_worktree "$top"; then
    cat <<EOF
NEEDS_APP_WORKTREE
在 Codex App 为当前仓库创建 Worktree task，再从那个 task 运行同一条 mmw task new 命令。
EOF
    return 0
  fi

  local expected_branch="codex/$slug" current_branch
  current_branch="$(git -C "$top" symbolic-ref --quiet --short HEAD 2>/dev/null || true)"
  if [ "$current_branch" != "$expected_branch" ]; then
    cat <<EOF
NEEDS_APP_BRANCH
expected_branch=$expected_branch
current_branch=${current_branch:-DETACHED}
NEXT=在 Codex App 当前 task 选择 Create branch here，创建 ${expected_branch}；然后重跑同一条 mmw task new 命令。
EOF
    return 0
  fi

  local wt="$top" manifest="$top/$STATE_SUBDIR/$MANIFEST_NAME"
  [ ! -e "$manifest" ] || die "当前 App worktree 已有在管任务:$manifest"
...
IGN
  ...
  plugin_version="$(jq -r '.version // ""' "$SCRIPT_DIR/../.codex-plugin/plugin.json" 2>/dev/null || echo "")"
  ...
  jq -n \
    ...
    --arg base "$base" --arg branch "$current_branch" --arg wt "$wt" \
...
NEXT=保持在 Codex App 当前 task，运行 mmw where；然后进入 $scenario 的 $phase 阶段
EOF
}
...
# ---------- cleanup(合并后只清 MMW state；App worktree/branch 不接管) ----------
cmd_cleanup() {
  local slug=""
  while [ $# -gt 0 ]; do
  ...
  [ -n "$slug" ] || die "--slug 必填"
  local top wt man branch state_dir
  top="$(git_toplevel)"
  wt="$(mmw_find_worktree "$top" "$slug" || true)"
  [ -n "$wt" ] || die "找不到 slug=$slug 的 App worktree 任务状态"
  man="$wt/$STATE_SUBDIR/$MANIFEST_NAME"
  [ -f "$man" ] || die "找不到任务 manifest:$man"
  branch="$(jq -r '.branch // empty' "$man")"
  [ -n "$branch" ] || die "manifest 缺 branch:$man"
  [ "$(git -C "$top" branch --show-current)" != "$branch" ] \
    || die "请在 target checkout 合并完成后运行 cleanup；当前仍在任务 branch $branch"
  git -C "$top" merge-base --is-ancestor "$branch" HEAD 2>/dev/null \
    || die "任务 branch $branch 未并入当前 HEAD，拒绝清状态"

  state_dir="$wt/$STATE_SUBDIR"
  case "$state_dir" in
    "$wt/.codex/multi-model-workflow") ;;
    *) die "拒绝清理意外路径:$state_dir" ;;
  esac
  rm -rf "$state_dir"
  echo "CLEANED slug=$slug state=$STATE_SUBDIR app_worktree=preserved app_branch=preserved"
}
...
cmd_escalate() {
  ...
  local top; top="$(git_toplevel)"
  is_linked_worktree "$top" || die "escalate 在任务 App worktree 内执行(当前不是 linked worktree)"
  local sd; sd="$(mmw_resolve_state_subdir "$top")"
...
# ---------- team(merge 用:从 git worktree 清单列在管 App tasks) ----------
cmd_team() {
  local top; top="$(git_toplevel)"
  echo "TEAM"
  local found=0 man
  # 扫当前仓库 git worktree 清单里的 Codex 状态平面，不另建 registry。
