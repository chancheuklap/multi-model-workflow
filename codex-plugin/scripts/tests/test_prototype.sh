...
ROUTES="$PLUGIN/state-schema/routes.json"
...
new_design_task() {
  local slug="$1" wt phases created base branch
  wt="$TMP/$slug"
  branch="test/$slug"
  git worktree add -q -b "$branch" "$wt" HEAD
  mkdir -p "$wt/$SD" "$wt/docs/design/$slug" "$wt/docs/issues/$slug" "$wt/docs/plans/$slug" "$wt/docs/context"
  printf '*\n' >"$wt/.codex/.gitignore"
  phases="$(jq -c '.presets.develop' "$ROUTES")"
  created="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  base="$(git rev-parse HEAD)"
  jq -n \
    --arg slug "$slug" --argjson phases "$phases" --arg created "$created" \
    --arg base "$base" --arg branch "$branch" --arg wt "$wt" \
    --arg inv "docs/design/$slug/investigating.md" --arg design "docs/design/$slug" \
    --arg issues "docs/issues/$slug" --arg plans "docs/plans/$slug" \
    '{
      schema_version:"2", slug:$slug, title:"prototype test", request:"验证 prototype 迭代闭环",
      scenario:"develop", phases:$phases, direction_given:false, status:"active",
      waiting_for:null, phase:"investigate", phase_index:0, step_index:0, gate:null,
      created_at:$created, updated_at:$created, plugin_version:"0.1.0", base_commit:$base,
      branch:$branch, worktree_path:$wt,
      docs:{investigating:$inv, design:$design, issues:$issues, plans:$plans, context:"docs/context"},
      repair_count:0, turnaround_count:0, attendance:"attended", unattended_policy:null,
      note:null, approval:null, prototype:null, artifacts:[], phase_outputs:{},
      open_items:[], subtasks:[], history:[]
    }' >"$wt/$SD/task.json"
