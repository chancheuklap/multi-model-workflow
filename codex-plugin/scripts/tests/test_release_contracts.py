...
SCRIPT = Path(__file__).resolve().parents[1] / "release_contracts.py"
...
def test_codex_manifest_does_not_require_external_fix_executor():
    good = _fake_manifest()
    del good["fix_executor"]
    manifest = rc.ReleaseAdapterManifest.model_validate(good)
    assert manifest.fix_executor is None
