...
# 测试夹具里的 fix_executor 只用来模拟 GPT 子代理会产生的文件改动；生产引擎绝不执行它。
dispatch_and_simulate_native() {
  local repo="$1"
  shift
  local dispatch_out verify_out ref worker arg
  local action=()
  dispatch_out="$(run_release "$repo" dispatch --stage verify_key "$@")"
  if [ "$(jq -r '.native_repair != null' "$(state_file "$repo")")" = "true" ]; then
    while IFS= read -r arg; do action+=("$arg"); done < <(jq -r '.fix_executor[]' "$repo/manifest.json")
    ref="$(mktemp)"
    (
      cd "$repo"
      unset RELEASE_FIX_STAGING
      RELEASE_FIX_FINDINGS="$FIX/finding.p1.json" RELEASE_FIX_WORKER_REF_FILE="$ref" "${action[@]-}"
    ) || true
    worker="$(head -1 "$ref" 2>/dev/null || true)"
    rm -f "$ref"
    [ -n "$worker" ] || worker="codex-test-worker"
    verify_out="$(run_release "$repo" repair verify --stage verify_key --worker-ref "$worker")"
    printf '%s\n%s\n' "$dispatch_out" "$verify_out"
    return 0
  fi
  printf '%s\n' "$dispatch_out"
}
...
out="$(dispatch_and_simulate_native "$repo" --findings "$FIX/finding.p1.json")"
case "$out" in
...
fail_stage_p1 "$repo"
out="$(dispatch_and_simulate_native "$repo")"
case "$out" in
...
out="$(dispatch_and_simulate_native "$repo" --findings "$FIX/finding.p1.json")"
case "$out" in
  *"REPAIR-PAUSED:verify_key(baseline untracked changed)"*) ok "已有 gitignored 受保护被改写→诚实 PAUSE" ;;
