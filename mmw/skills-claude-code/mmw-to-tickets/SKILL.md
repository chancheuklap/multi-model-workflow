...
先检查已有且已经验证的 spec、调查报告或当前源码证据是否覆盖这次改动的现状。材料已经覆盖时直接复用，不重复调查。材料没有覆盖时，调用 `/mmw-research` 查清当前实现和可能的 prefactor。

只有当前源码证据表明 prefactor 能让后续实现更容易时，才把它拆成 ticket。「先把改动变容易，再做这个容易的改动。」使用 subagent 取得报告时，按 `/mmw-verifying-agent-output` 验证后再写进 ticket。
