...
图承担主要信息，文字稀疏、平实，直接用 `/mmw-codebase-design` 定义的词，不加修饰。
...
**Wins 那几条**用 `/mmw-codebase-design` 定义的词说收益，例如 *"locality: bugs concentrate in one module"*、*"leverage: one interface, N call sites"*、*"interface shrinks; implementation absorbs the shallow modules"*。不要写 *"easier to maintain"* 或 *"cleaner code"*，这些词不在 `/mmw-codebase-design` 中。

不要模棱两可，不要清嗓子，不要「值得一提的是……」。一句话能写成一条列点就写成列点，一条列点能删就删。一个词不在 `/mmw-codebase-design` 中，先找一个在里面的，再考虑发明新词。
