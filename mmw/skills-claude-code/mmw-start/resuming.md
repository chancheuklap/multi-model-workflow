...
| plan 审过没过 | 逐张按 `/mmw-to-plan` 第 8 步验证有效过审凭据；`.reviews/` 的本机记录只用于定位审查历史，不替代有效过审凭据 |
...
全部 open tracer bullet ticket 都有有效过审凭据，只有 `ready-for-agent` 不齐时，回 `/mmw-to-plan` 第 8 步收敛 tracker 状态。任何 ticket 的有效过审凭据缺失或过期时，回 `/mmw-to-plan`；它会移除不符合合同的旧标签，并重新发起 ② plan 审。全部 open ticket 的有效过审凭据和标签齐全后，才进入 `/mmw-implement`。
