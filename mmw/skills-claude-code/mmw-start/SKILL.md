...
| 一个经过 `/mmw-triage` 的 issue 或 PR 编号，已是 `ready-for-agent`，但没有完整 agent brief | **移交**：`/mmw-triage`，补齐 `Acceptance criteria`、范围边界和 `Test seam` |
| 一个经过 `/mmw-triage` 的 issue 编号，已是 `ready-for-agent`，有完整 agent brief，整项工作可以作为一张 ticket 独立验收，只有一个已确认测试 seam，而且没有未决设计取舍 | **移交**：`/mmw-implement` |
| 一个经过 `/mmw-triage` 的 issue 或 PR 编号，已是 `ready-for-agent`，有完整 agent brief，但需要拆成多张 ticket、需要多个测试 seam，或者还有设计取舍要谈 | **移交**：`/mmw-to-spec` |
