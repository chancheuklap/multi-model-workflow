...
运行 `mmw artifact path prototype <产物目录> [issue-<编号>]`，命令返回目录中的 `README.md` 是 prototype 资产索引；普通非 Wayfinder 任务不传 issue 子目录。索引应列出问题、逐轮用户结论、用户选中的路径、落选变体形成的约束、被提升为长期证据的路径；没有选中产物、落选约束或长期证据时，对应项写「无」，不能省略。缺一项就作为材料缺失处理。先读索引，再按本视角只提供精确路径。不要把整个产物目录、无关截图、runs 或过程输出放进 task，也不要递归读取目录。完整 UI 变体集保留在 prototype 资产中，作为 primary source；落选变体只提供已被否定的约束，本视角确需验证该约束时，才把索引显式引用的具体变体路径加入 task。
...
运行 `mmw artifact root review`，把命令返回值记为审查记录目录。每个审查实例一份。按 **四栏表**（目标 / 读 / 约束 / 验收）填写：

| 栏 | 本角色填写 |
| --- | --- |
| 目标 | **第一句**为任务名（与「2. 给每个视角备齐材料」表中任务名一字不差）；其后写审什么：被审对象 |
| 读 | 「2. 给每个视角备齐材料」表中，本任务名那一行 Source 列的路径，一行一条；该行没有材料则写「无」。复审时再加上一轮审查记录路径，首轮写「无」；`mmw skill-path <角色>` 有输出则加方法论路径 |
| 约束 | 只读；不改被审产物；方法论在「读」的路径里自读，不往 task 粘正文 |
| 验收 | 按审查者技能交 findings（或 `needs-redirection` / `needs-context`） |

可选：四栏表写入 `<审查记录目录>/<slug>-<哪一道>-<轮次>-<任务名>.prompt.md`，轮次从 1 起。
...
把每个审查者交回的东西**原样**抄进 `<审查记录目录>/<slug>-<哪一道>-<轮次>.md`，按任务名分组。不要重写，不要摘要。文件顶部记下固定点和本轮 `被审 HEAD`。固定点限定 diff 范围；被审 HEAD 标识审查期间必须保持不变的内容。
