...
| --- | --- | --- | --- | --- |
| ① spec 审 | spec 写完、自检过了，给用户看之前 | 设计内容审、项目一致性审 | 每个视角派一个独立审查者 | 不是。findings 连同你的处置一起给用户看参考 |
| ② plan 审 | 全部 plan 写完、主 agent 验证过、跨 plan 合同回填完之后，发起一次 | 覆盖质量审、合规交叉审 | 每个视角派一个独立审查者 | 是 |
...
| ⑤ final 终审 | 全部 ticket 落地、合同门过了 | 对照终审、独立终审、编码规范审 | 每个视角派两个独立审查者 | 是 |
| ⑥ 合并集成审 | 几条分支集成到目标分支之后，由 `/mmw-integrate` 移交进来 | 不分视角，七个角度 | 派两个独立审查者，各走全套 | 是 |

**审查者必须使用独立上下文，不能由产物作者审自己的产出。** 具体 agent 名和模型由当前宿主的审查启动块决定；共享流程只规定视角、实例数和材料。
...
同一视角需要两个实例时，四栏表内容相同。两个实例必须是独立上下文。

可选：四栏表写入 `.reviews/<slug>-<哪一道>-<轮次>-<任务名>.prompt.md`，轮次从 1 起。

每个审查实例各写一张四栏表并启动一次；互不依赖的实例同时启动。
当前宿主用两个角色保持作者与审查者分离：主 agent 定稿的 spec、map 和调查综合使用 `reviewer-gpt`。启动：把四栏表写入 task 文件，后台执行 `mmw dispatch reviewer-gpt --task <task 文件绝对路径>`。命令返回 `mode: host-tool` 时，使用输出中的 `params` 调用对应宿主工具。 plan 和代码使用 `reviewer-claude`。启动：把四栏表写入 task 文件，后台执行 `mmw dispatch reviewer-claude --task <task 文件绝对路径>`。命令返回 `mode: host-tool` 时，使用输出中的 `params` 调用对应宿主工具。 本节要求两个独立实例时，两个角色各启动一次。每个实例只收到自己的四栏 task。
...
几个审查者报同一件事，合并成一条处置。**同一个视角派了两个实例时，把两份 findings 并排比较**：只有一个实例报出的条目重点验证；两个实例都报出的条目仍需验证出处。
