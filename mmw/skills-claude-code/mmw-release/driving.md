...
只处理这一次的输出，不预判下一步。每次运行 stage、dispatch、主 agent 修复或其他可能改动代码的动作后，比较 `git rev-parse HEAD` 与 `/mmw-release` 第 1 步读取的终审提交：

- 两者相同，继续当前状态机。
- 两者不同，立即停止驱动。当前 HEAD 尚未通过 final 终审，移交 `/mmw-review` 重新发起 ⑤ final 终审。终审通过后从 `/mmw-release` 第 1 步重新检查，再按引擎状态续跑。
...
4. 处置没有改变 HEAD 时，运行 `mmw release resume` 续跑。HEAD 改变时，终审通过并重新完成 `/mmw-release` 第 1 步后，再按引擎状态续跑。
