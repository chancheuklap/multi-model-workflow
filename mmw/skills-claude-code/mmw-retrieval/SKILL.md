...
检索工具**怎么用**——什么问题问图、什么问题问符号、哪两类盲区要结合源码看、候选为什么必须回源码验证——只由 `config/serena-readonly.yml` 的 `prompt` 与 `mcp/graphify_mcp.py` 的 `INSTRUCTIONS` 定义。本技能和别处都不复制这两处内容。

本技能只规定 `config/serena-readonly.yml` 的 `prompt` 与 `mcp/graphify_mcp.py` 的 `INSTRUCTIONS` 没有规定的两件事，都只有主 agent 做：**图怎么保持可用**，以及**工具本身连不上时怎么查**。
...
一个仓库第一次接入，或者查询结果异常时，读取 [building.md](building.md)：它规定跨语言边、配置、验证、失败处理和任务 worktree 的图复用。
...
| 建不出来，报了具体的失败阶段 | **自己继续**：对照本文「2. 建一次」里的阶段表读失败阶段；处理方式按 [building.md](building.md) 的「5. 建不出来」 |
| 这个仓库还没接入 | **自己继续**：按 [building.md](building.md) 的「1. 接入一个仓库」配一次 |
| 工具调不动，图却是好的 | **自己继续**：`mmw doctor`，按本文「3. 工具本身连不上」内 doctor 报表处理 |
