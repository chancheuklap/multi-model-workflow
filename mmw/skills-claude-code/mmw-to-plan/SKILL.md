...
恢复：把修复 task 写入新文件，后台执行 `mmw dispatch planner --task <修复 task 文件绝对路径> --resume <句柄原文> --cwd <当前任务 worktree 绝对路径>`。gpt 族句柄取原派发返回的 `session:` 行原文（也存在原结果 worktree 的 `.dispatch/<角色>-<task 名>.session`）；claude 族取 `handle:` 行原文。命令返回 `mode: host-tool` 时，使用输出中的 `params` 调用对应宿主工具。句柄取不到或命令失败时退回重派：这个宿主没有续跑通道：按对应的启动动作重派新实例，task 带原 task 路径、原报告路径和本轮修复指令。
...
**本批次 plan 都验证过、合同也回填完之后，发起一次审查，批内不逐份发起。** 按 `/mmw-review` 走，传给它：本批次每份 plan 的精确路径、这份 spec 的精确路径、全部 ticket 的编号，以及本批次 plan 实际引用到的 prototype `README.md`、选中产物、research `README.md` 和精确文件；某一项没有就写「无」。

**首个批次**的审查材料里额外注明：本轮覆盖质量审执行 spec 到 ticket 集合的覆盖扫描。后续批次不重扫——ticket 集合在 `/mmw-to-tickets` 时已固定。
