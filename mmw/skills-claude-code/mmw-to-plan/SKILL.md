...
把 ticket 写成 plan，供后面派 `worker` 照着落地。plan 不一次写完：**一个批次一轮**，与 `/mmw-implement` 交替推进。**批次**指某一时刻阻塞已全部关闭、还没有 `ready-for-agent` 标签的全部 open tracer bullet ticket。这样每个 `planner` 探到的代码里，阻塞它的 ticket 已经真实落地——plan 里钉的 `文件:行号` 在 `worker` 开工时仍然成立，跨 plan 接口也能直接对源码验证。
...
恢复：后台执行 `mmw dispatch planner --resume <句柄原文> --cwd <当前任务 worktree 绝对路径>`。把修复 task 正文作为命令的标准输入。gpt 族句柄取原派发返回的 `session:` 行原文；那一行不在手上时，运行 `mmw artifact path scratch --sub dispatch` 取得派发进度目录，读其中的 `<角色>-<正文摘要>.session`。claude 族取 `handle:` 行原文；那一行不在手上时，在宿主的 subagent 清单里找以 `planner-` 开头的那个名字。命令返回 `mode: host-tool` 时，使用输出中的 `params` 调用对应宿主工具。句柄取不到或命令失败时退回重派：按对应的启动动作重派新实例，task 正文带上原 task 全文、原报告全文和本轮修复指令。
...
**本批次 plan 都验证过、合同也回填完之后，发起一次审查，批内不逐份发起。** 按 `/mmw-review` 走。传给它本批次每张 ticket 的 plan 类别内细分。逐份运行 `mmw artifact path plan --sub <两位编号>-<ticket短名>.md`。再传这份 spec 的精确路径、全部 ticket 编号，以及本批次 plan 引用的 prototype `README.md`、选中产物、research `README.md` 和精确文件。没有的项目写「无」。

**首个批次**的审查材料里额外注明：本轮覆盖质量审执行 spec 到 ticket 集合的覆盖扫描。后续批次不重扫——ticket 集合在 `/mmw-to-tickets` 时已经固定。
