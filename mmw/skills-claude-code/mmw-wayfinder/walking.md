...
6. 提交，然后把结果交回去。
   ...
   然后把三个值追加成这张 ticket 上的一条评论：任务分支名、`git rev-parse HEAD` 的输出、第 2 步记下的基点 SHA。ticket 已经关闭也照发——这条评论是这三个值的持久记录，map 会话事后补做集成时从这里取。

   最后把这三个值和一份报告交回给拥有 map 分支的那个会话。交回的介质就是你的停机报告：你停下来报告，用户把它带给 map 会话。
