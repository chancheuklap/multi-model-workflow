...
- **测试质量**：这次新增和改动的测试，按你已装的 `mmw-tdd/quality-bar.md` 里的合入标准逐条验证——那是 `worker` 写测试时读的同一份。命中禁止形态就是一条 finding。目标仓库根的 `TESTING.md` 补充本仓库的事实（目录分层、哪些边界算外部、值的唯一事实来源在哪），一并验证。
