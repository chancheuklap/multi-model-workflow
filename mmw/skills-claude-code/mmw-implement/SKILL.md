...
**派发前抽验 plan 新鲜度**（spec 分支）：把这份 plan 引用的关键路径对当前任务分支抽验一遍。失效就先把差异发回原 `planner` 续跑刷新那份 plan（恢复动作见 `/mmw-to-plan` 第 4 步），刷新过再派 `worker`。抽验降低批次内漂移进入 `worker` task 的概率；没被抽到的残余漂移仍由 `worker` 的矛盾上报兜底。
...
**关票之后当场回收这棵结果 worktree。** 它的使命到这里结束了：提交已经在任务分支上，那份报告已经收过，修复轮要恢复它靠的是句柄而不是这棵树。不回收就会一张 ticket 攒一棵，十一张 ticket 攒十一棵，等到 `/mmw-integrate` 或 `/mmw-closing` 时已经堆成一片，谁都不认得哪棵还有用。
...
恢复：后台执行 `mmw dispatch worker --resume <句柄原文> --cwd <原结果 worktree 绝对路径>`。把修复 task 正文作为命令的标准输入。gpt 族句柄取原派发返回的 `session:` 行原文；那一行不在手上时，运行 `mmw artifact path scratch --sub dispatch` 取得派发进度目录，读其中的 `<角色>-<正文摘要>.session`。claude 族取 `handle:` 行原文；那一行不在手上时，在宿主的 subagent 清单里找以 `worker-` 开头的那个名字。命令返回 `mode: host-tool` 时，使用输出中的 `params` 调用对应宿主工具。句柄取不到或命令失败时退回重派：按对应的启动动作重派新实例，task 正文带上原 task 全文、原报告全文和本轮修复指令。
