...
结果提交已经进入当前任务分支，才关闭这张 ticket。关票可能解锁新批次：spec 分支回第 2 步，按那里的判据决定是移交 `/mmw-to-plan` 写新批次的 plan，还是继续取下一张。agent brief 分支只有这一张，直接进入第 7 步。
...
恢复：把修复 task 写入新文件，后台执行 `mmw dispatch worker --task <修复 task 文件绝对路径> --resume <句柄原文> --cwd <原结果 worktree 绝对路径>`。gpt 族句柄取原派发返回的 `session:` 行原文（也存在原结果 worktree 的 `.dispatch/<角色>-<task 名>.session`）；claude 族取 `handle:` 行原文。命令返回 `mode: host-tool` 时，使用输出中的 `params` 调用对应宿主工具。句柄取不到或命令失败时退回重派：这个宿主没有续跑通道：按对应的启动动作重派新实例，task 带原 task 路径、原报告路径和本轮修复指令。
