...
先清点这次要给用户看的全部页面或 URL，**每个页面各自打开一处，不要用一个窗口依次覆盖几份产物**——用户要能来回对比。按 [SKILL.md](SKILL.md)「用户走查」的浏览器规则打开并保留全部页面。确认真的可见之后，才说已经打开。交给用户之前，为每个页面的当前状态截图并记下出处；有正式原型或走查目录时一并存进去。页面交给用户之后停止导航，等他反馈。
