...
brief 与 task 同一形状：一张四栏表，写入磁盘后把**文件绝对路径**交给 `mmw dispatch --brief`。
...
| 约束 | 边界与禁止项；没有写「无」 |
| 验收 | 怎样算做完；细节已在「读」里的材料中则写「见 #编号 / 见路径」，不重抄 |

落盘：一般 `.dispatch/<名>.md`；审查 `.reviews/<名>.md`。需要时 `mkdir -p`。

**完成判据：** 文件存在且含完整四栏；「读」中路径存在或编号可解析，或不适用项已写「无」。
