#!/usr/bin/env python3
"""把共享 skill 中的宿主动作整块物化成 Pi、Claude Code、Codex 或 Cursor 版本。"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--host", required=True, choices=("pi", "claude-code", "codex", "cursor", "all")
    )
    ...
    try:
        ...
        die(str(exc))
    hosts = (
        ["pi", "claude-code", "codex", "cursor"] if args.host == "all" else [args.host]
    )
    status = 0
    for host in hosts:
