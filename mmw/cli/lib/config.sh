...
# 当前宿主：claude-code | pi | cursor | grok | codex。宿主变量都没有时报错，不猜。
mmw_host() {
  if [ -n "${MMW_HOST:-}" ]; then
    echo "$MMW_HOST"
    ...
    echo "mmw: 认不出当前宿主（Claude Code、Pi、Cursor、Grok 与 Codex 标识都没有）" >&2
    echo "mmw: 要在别处跑，用 MMW_HOST=claude-code、pi、cursor、grok 或 codex 显式指定" >&2
    return 1
  fi
}
