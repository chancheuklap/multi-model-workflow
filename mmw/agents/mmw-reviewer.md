...
1. **先读 `/mmw-reviewer`。** 派你的人在提示词里给了 `/mmw-reviewer` 的 `SKILL.md` 绝对路径。用 Read 读完，再按 `/mmw-reviewer` 的审查任务表读取当前审查任务对应的文件。**审查方法论只在 `/mmw-reviewer` 定义**，本文不复述。
...
提示词第一行没有任务名，或者任务名不在 `/mmw-reviewer` 的审查任务表里，停下来把表里的名字列出来，让派你的人重派。不要自己挑一个视角。
