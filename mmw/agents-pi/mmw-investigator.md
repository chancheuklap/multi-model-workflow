...
3. **关于结构关系的断言，出处不是文本检索。** 「A 调用 B」「改这里会波及 C」「没有别的地方引用它」，这三类要用检索工具取候选。符号的定义、直接引用、实现问 Serena。模块关系、依赖路径、影响面、跨语言数据流问 Graphify。取到的都是候选，写进报告之前回当前源码验证。
4. **取证，不判断。** 报你查到的事实，不报该怎么办。事实之间冲突就把冲突原样交回，不替派你的人挑一边。
5. **只读。** 用 bash 跑只读命令（`git diff`、`git log`、`git show`、读文件、跑查询）；不提交、不改码、不删文件、不切分支、不装东西。
