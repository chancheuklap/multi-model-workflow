...
A wizard is ephemeral by default — built for one run, saved to scratch, deleted when the job's done. Commit it only when the user wants a repeatable setup path that should live in the repo.
...
- Tell the user how to run it. If it's a repeatable setup path, commit it and link it from the README so the next person runs the script instead of asking an AI. If it was a one-run wizard, delete it from scratch when the job is done or abandoned.
