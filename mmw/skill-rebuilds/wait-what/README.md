...
第三阶段已经在 `candidate/` 形成三文件候选。`candidate/SKILL.md` 保留手动调用，并在文字重述、普通文档可视化和 Logic HTML 三种请求之间路由；`candidate/VISUAL.md` 负责普通文档解释；`candidate/LOGIC.md` 负责手动驱动状态模型。Logic HTML 只保存本地产物，除非用户明确要求发布。当前发布技能仍不修改。
...
| `simplified.zh-CN.md` | 完整保留上游方法的单文件精简稿 |
| `candidate/` | 增加 MMW 领域上下文、普通可视化和 Logic HTML 路由的候选技能 |
