...
- 文件保存在用户提供的精确路径。
- 用户的走查意见已经交回此前等待的流程；没有上游流程时已经展示给用户。
...
| 用户已经给出走查结论，而且有上游流程等待 | **移交**：把用户原话、Logic HTML 路径和验证结果交回上游流程 |
| 用户已经给出走查结论，而且没有上游流程等待 | **停**：展示用户原话、Logic HTML 路径和验证结果 |
| 现有状态模型无法表达当前验证问题，而且有上游流程等待 | **移交**：说明哪项事实无法表达，并把该事实交回上游流程 |
| 现有状态模型无法表达当前验证问题，而且没有上游流程等待 | **停**：说明哪项事实无法表达 |
