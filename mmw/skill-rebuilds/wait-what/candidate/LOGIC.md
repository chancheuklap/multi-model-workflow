...
现有状态模型或后端脚本无法直接在浏览器中运行时，根据它的公开行为实现回答当前问题的纯 `module`。只实现现有行为能够确定的动作、状态和边界；无法确定的业务事实交回此前等待的流程，不自行补造。

构建 HTML 前确认现有行为足以表达当前验证问题：

| 情况 | 处理 |
| --- | --- |
| 现有行为无法表达当前验证问题，而且有上游流程等待 | 说明无法表达的事实，交回上游流程，并结束本分支 |
| 现有行为无法表达当前验证问题，而且没有上游流程等待 | 向用户说明无法表达的事实，并结束本分支 |
...
| 用户走查后确认当前模型无法表达验证问题，而且有上游流程等待 | **移交**：把用户原话、无法表达的事实和 Logic HTML 路径交回上游流程 |
| 用户走查后确认当前模型无法表达验证问题，而且没有上游流程等待 | **停**：展示用户原话、无法表达的事实和 Logic HTML 路径 |
| 用户给出其他走查结论，而且有上游流程等待 | **移交**：把用户原话、Logic HTML 路径和验证结果交回上游流程 |
| 用户给出其他走查结论，而且没有上游流程等待 | **停**：展示用户原话、Logic HTML 路径和验证结果 |
