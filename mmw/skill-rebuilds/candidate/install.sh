...
materialize_source() {
  "$SOURCE_MMW/cli/mmw" skills materialize --host all
  "$SOURCE_MMW/cli/mmw" agents materialize --host pi
  if [ -d "$HOME/.cursor" ]; then
    "$SOURCE_MMW/cli/mmw" agents materialize --host cursor
  else
    echo "Cursor   : 未安装，跳过原生 subagent"
  fi
  python3 "$SOURCE_MMW/codex/runtime.py" materialize
}
