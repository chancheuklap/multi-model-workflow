...
每张 ticket 都是 map 的一个**子 issue**；tracker 的 issue id 就是它的身份。ticket 正文承载问题，粒度控制在一次 100K token 的 agent session 内：
...
3. 解决 ticket。根据需要 **zoom**：按需取得相关或已关闭 ticket 的完整正文；调用 `## Notes` 区块点名的技能。不确定时，使用 `/grilling` 和 `/domain-modeling`。
