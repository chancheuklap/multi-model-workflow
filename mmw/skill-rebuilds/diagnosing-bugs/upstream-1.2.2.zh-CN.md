...
**这就是本技能。** 其他一切都是机械操作。如果你拥有一个针对该 bug 的**紧密**通过或失败信号，也就是一个会因为**当前** bug 变红的信号，你就能找到原因；二分、假设检验和 instrumentation 都只是消费这个信号。如果没有，无论盯着代码看多久都无济于事。
...
1. 在任何能够触及 bug 的 seam 上编写**失败测试**，可以是 unit、integration 或 e2e。
2. 针对正在运行的开发服务器执行 **Curl 或 HTTP 脚本**。
3. 使用 fixture 输入执行 **CLI 调用**，并把 stdout 与已知正确的 snapshot 进行 diff。
4. 使用 **headless 浏览器脚本**，例如 Playwright 或 Puppeteer，驱动 UI，并对 DOM、console 或 network 作出断言。
5. **重放捕获的 trace。** 把真实的 network request、payload 或 event log 保存到磁盘；在隔离环境中通过代码路径重放它。
6. **一次性 harness。** 启动系统的最小子集，例如一个 service 和 mock dependency，并通过一次函数调用执行 bug 代码路径。
7. **Property 或 fuzz 循环。** 如果 bug 是“有时输出错误”，运行 1000 个随机输入，并寻找失败模式。
8. **二分 harness。** 如果 bug 出现在两个已知状态之间，例如 commit、dataset 或 version，就自动执行“在状态 X 启动、检查、重复”，以便使用 `git bisect run`。
9. **差异循环。** 让同一个输入分别经过旧 version 与新 version，或两套 config，然后对输出作 diff。
...
把循环当成产品。拥有一个循环后，继续**收紧**它：

- 能否让它更快？例如缓存设置、跳过无关初始化、缩小测试范围。
- 能否让信号更明确？断言具体症状，不要只断言“没有崩溃”。
- 能否让它更具确定性？固定时间、设定 RNG seed、隔离 filesystem、冻结 network。

一个需要 30 秒且 flaky 的循环只比没有循环稍好；一个 2 秒且确定的循环很紧密，是调试工作的超级能力。

<!-- source: vendor/mattpocock-skills/skills/engineering/diagnosing-bugs/SKILL.md:43-60 -->

### 非确定性 bug

目标不是得到一次干净复现，而是获得**更高的复现率**。循环执行触发操作 100 次、并行执行、增加压力、缩窄时间窗口、注入 sleep。复现率为 50% 的 flaky bug 可以调试，复现率为 1% 的 bug 不可以。继续提高复现率，直到能够调试。

### 确实无法建立循环时

停止，并明确说明。列出尝试过的内容。向用户请求以下任一项：(a) 能够复现问题的环境访问权限；(b) 捕获的产物，例如 HAR 文件、log dump、core dump 或带时间戳的屏幕录制；(c) 增加临时 production instrumentation 的许可。**不要**在没有循环时继续提出假设。

### 完成判据——一个会变红的紧密循环

当循环既**紧密**又**能够变红**时，阶段 1 完成：你可以指出**一条命令**，例如脚本路径、测试调用或 curl；你已经**至少运行过它一次**，并能粘贴调用及其输出；这条命令满足：

- [ ] **能够变红**——它会执行真实 bug 代码路径，并断言**用户所述的准确症状**，因此能在当前 bug 上变红，并在修复后变绿。它不能只是“运行时没有报错”，而必须能够**捕获这个具体 bug**。
...
如果你发现自己在这条命令存在前就阅读代码以建立理论，**立即停止；直接跳到假设，正是本技能要防止的失败。** 没有能够变红的命令，就不能进入阶段 2。

<!-- source: vendor/mattpocock-skills/skills/engineering/diagnosing-bugs/SKILL.md:62-80 -->

## 阶段 2——复现并最小化

运行循环。观察它变红，也就是 bug 出现。
...
循环变红后，把复现缩小到**仍然会变红的最小场景**。每次只削减一项 input、caller、config、data 或 step；每次削减后重新运行循环。只保留导致失败不可缺少的内容。

这样做的理由是：最小复现会缩小阶段 3 的假设空间，因为可怀疑的活动部分更少；它也会成为阶段 5 中干净的回归测试。

当**每个剩余元素都不可缺少**时完成最小化；移除任何一个元素都会使循环变绿。
...
测试任何假设之前，先产生 **3 至 5 项按顺序排列的假设**。只提出一项假设会使你锚定第一个看似可信的想法。
...
每个 probe 都必须对应阶段 3 中的一项具体预测。**每次只改变一个变量。**

工具优先顺序：

1. 环境支持时，使用 **Debugger 或 REPL 检查**。一个 breakpoint 胜过十条 log。
2. 在能够区分不同假设的边界处增加**定向 log**。
3. 绝不“记录一切，然后 grep”。

为**每一条 debug log 增加 tag**，使用唯一 prefix，例如 `[DEBUG-a4f2]`。最后只需一次 grep 就能完成清理。没有 tag 的 log 会留下；有 tag 的 log 会删除。

**性能分支。** 对于性能回归，log 通常是错误工具。应先建立 baseline measurement，例如 timing harness、`performance.now()`、profiler 或 query plan，然后进行二分。先测量，再修复。
...
正确 seam 是测试能够按照 bug 在调用位置出现的方式执行**真实 bug 模式**的位置。如果唯一可用的 seam 太 shallow，例如 bug 需要多个 caller 却只能编写单 caller 测试，或 unit test 无法复制触发 bug 的调用链，那么在该 seam 编写回归测试会带来虚假的信心。

**如果不存在正确 seam，这本身就是 finding。** 记录它。代码库架构正在阻止你锁定该 bug。为下一个阶段标记这一点。
...
- [ ] 已移除全部 `[DEBUG-...]` instrumentation，通过 grep 查找 prefix
- [ ] 已删除一次性 prototype，或把它们移动到明确标记的 debug 位置
- [ ] 已在 commit 或 PR message 中说明最终证明正确的假设，使下一位调试者能够获知

**随后询问：什么能够预防这个 bug？** 如果答案涉及架构改动，例如没有良好测试 seam、caller 相互纠缠或隐藏耦合，就把具体情况移交给 `/improve-codebase-architecture` 技能。在修复完成**之后**提出建议，不要提前提出；现在掌握的信息比开始时更多。
...
# 复制本文件，编辑下方步骤，然后运行。
# agent 运行脚本；用户按照 terminal 中的提示操作。
#
# 用法：
