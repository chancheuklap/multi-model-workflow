...
**这就是本技能。** 其他一切都是机械操作。如果你拥有一个针对该 bug 的**紧密**通过或失败信号，也就是一个会因为**当前** bug 变红的信号，你就能找到原因；二分、假设检验和插桩都只是消费这个信号。如果没有，无论盯着代码看多久都无济于事。
...
循环变红后，把复现缩小到**仍然会变红的最小场景**。每次只削减一项输入、调用方、配置、数据或步骤；每次削减后重新运行循环。只保留导致失败不可缺少的内容。
