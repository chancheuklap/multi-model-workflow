...
**为什么要合成一棵完整的树**，而不是每个技能各存各的候选：技能之间靠 `[...](../mmw-start/x.md)` 这样的相对引用互相指，候选散在各目录时这些引用打不开，只能靠「想象它最终在 `mmw/skills-src/` 里的位置」来判断对不对。合成一棵树之后，引用真的能解析，接线检查从想象变成可执行——`check-wiring.py` 就是干这个的。
...
1. **只改 `candidate/`。** 现役 `mmw/skills-src/` 和 `mmw/cli/` 在整体替换之前一律不动。
