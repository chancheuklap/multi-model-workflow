...
这个目录用于从 Matt Pocock Skills 1.2.2 重新建立 MMW Codebase Design。当前发布技能仍位于 `mmw/skills-src/mmw-codebase-design/`；本目录中的文件不会被 `mmw skills materialize` 物化，也不会改变任何宿主的运行行为。
