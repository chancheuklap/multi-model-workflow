...
继续拥有该结果分支的 Codex App 后台任务，在它自己的 worktree 中完成本节操作。主 agent 不切换当前工作目录。后台任务完成后交回新的 HEAD SHA 和验证结果。
...
| 情况 | 下一步 |
| --- | --- |
| 集成完成且需要审查 | **移交**：`$mmw:mmw-review`，发起 ⑥ 合并集成审 |
...
Codex App 管理后台任务的 worktree；归档对应任务后由 App 清理，不运行 `mmw task cleanup`。
