...
两类九种判拒的清单在 [SKILL.md](SKILL.md)「判拒：两类九种」，本文不重复。这里写的是每种判拒怎么判、判据从哪来、缺了怎么办。
...
技能自己能查到的不问：设计系统文件在不在、声明了什么组件规格、应用怎么启动（构建配置里的脚本）、用户在 `$mmw:mmw-grilling` 和 spec 里说过的可用性要求。
...
设计系统那一行不一样，因为缺了它只影响两种判拒，而缺前三份中任何一份技能都跑不起来。

**建设计系统的三条硬约束：**

1. 用本文开头说的 DESIGN.md 格式写，建完立刻用 `mmw-ui-qa design-lint <文件>` 校一遍。
