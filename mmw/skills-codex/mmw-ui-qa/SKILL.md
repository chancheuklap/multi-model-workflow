...
一条判据被违反时产生的结果，叫一条**判拒**。下面九种是封闭集合，本技能不产生第十种。**每一步都会按编号点名它们，所以这张表写在这里，不在别处。**
...
另有**三类报告项**：覆盖报告、判据自检结果、声明与实现不一致。它们是确定的事实，但没有界面上的修复目标，因此不进用户裁决，也不驱动修改。产生新的确定性事实而没有界面修复目标时，加成报告项，不加成判拒。
...
| --- | --- | --- |
| 本文 | 九步主流程 | 一进来就读 |
| [CRITERIA.md](CRITERIA.md) | 五种判据的形态与落点、九种判拒各自的细则、intake 问卷、三份跨边界数据的字段 | 第 4、5、8 步 |
...
校验结果进报告的**判据自检结果**，**不进九种判拒**。它没有界面上的修复目标，也不进用户裁决。
...
**先跑完全部九种判拒，收集全部结果，再统一进入第 9 步。** 不边查边改：A3 改了 token，B1 看到的就是改后的界面，用户在报告里读到的描述会对不上他打开界面看到的样子。

每种判拒的细则在 [CRITERIA.md](CRITERIA.md)；B2、B3、B4 的方法与派发合同在 [SEMANTIC.md](SEMANTIC.md)，跑这三种之前完整读它。
