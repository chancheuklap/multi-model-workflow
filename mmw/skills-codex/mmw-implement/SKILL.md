...
终审没有采信项，或采信项已经修复并验证：报告实现结果、验证证据和当前分支 HEAD。仓库里有没有出包配置，跑 `grep -rl '"product"' --include='*.release-adapter.json' .`。问用户：出包、收尾、还是到这里停。有 spec 才需要收尾；只有 agent brief 时没有 `$mmw:mmw-closing`。
