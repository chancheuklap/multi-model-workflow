...
| agent brief 分支的行为合同完整 | 原 issue 的 agent brief 有当前行为、目标行为、可独立验证的 `Acceptance criteria`、范围边界和且仅一个 `Test seam`；用户已在 `$mmw:mmw-triage` 的人工审批关卡明确批准该 `Test seam`；整项工作可以作为一张 ticket 独立验收，而且没有未决设计取舍 | 缺字段或批准凭据就回 `$mmw:mmw-triage` 补；需要多张 ticket、多个 seam 或设计取舍就转 `$mmw:mmw-to-spec` |
...
TDD 在 `worker` 的 `$mmw:mmw-tdd` 技能里，不进 task 正文。
...
| 审完没有采信项，或者修复已经复审通过，而且这次改动碰了带 release adapter 的产品 | **移交**：`$mmw:mmw-release`，先生成正式安装包。仓库里有没有 release adapter，运行 `grep -rl '"product"' --include='*.release-adapter.json' .` |
