...
`mmw result integrate` 撞上冲突会当场失败，并把冲突状态留在工作区——那时按 `$mmw:mmw-integrate` 处理，不要自己动手改冲突文件。
...
spec 分支或 agent brief 分支完成第 5 步后，按 `$mmw:mmw-review` 发起一轮 **⑤ final 终审**，固定点取分支点。整体审一次，不逐张审。
...
合并有冲突、worktree 已收或句柄失效时，放弃恢复，把缺口合成一张修复 ticket 派新 `worker`。跨 ticket 时同样打包成一张修复 ticket 派给新 `worker`，带上 `file:line` 和要改成什么。两条路都不逐 finding 派修复者。修复交回「完成」后按第 5 步合并。全部采信项修好后按 `$mmw:mmw-review` 第 7 步在原审查记录登记 `修复提交` 和 `终审提交`；不再派审查者。

终审没有做完时，说清停在哪一步、还差什么，交给用户。

终审没有采信项，或采信项已经修复：报告实现结果和当前分支 HEAD。仓库里有没有出包配置，跑 `grep -rl '"product"' --include='*.release-adapter.json' .`。问用户：出包、收尾、还是到这里停。有 spec 才需要收尾；只有 agent brief 时没有 `$mmw:mmw-closing`。
