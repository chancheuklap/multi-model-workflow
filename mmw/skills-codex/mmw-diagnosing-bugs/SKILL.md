...
When exploring the codebase, run `mmw domain path` and follow its read instruction so names match the project's domain language, and check ADRs in the area you're touching (`mmw artifact index adr`).
...
**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling) hand off to `$mmw:mmw-improve-codebase-architecture` with the specifics. Make the recommendation **after** the fix is in, not before — you have more information now than when you started.
