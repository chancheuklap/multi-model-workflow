...
| 情况 | 处理 |
| --- | --- |
| 四条判据全过，loop 已经真跑过一次 | 读 [narrowing.md](narrowing.md)，复现、最小化、提假设、埋点 |
| 「怎么造出来——大致按这个顺序试」那十种全试过还是造不出 loop | 明说造不出来，列出你试过什么，然后向用户要三样中的一样——能复现它的那个环境的访问权、一份脱敏后的抓取产物（HAR 文件、日志转储、core dump、带时间戳的录屏），或者往生产环境加临时埋点的许可 |
