...
开始诊断时确定当前任务的产物目录。Wayfinder 场景从当前 map 或子 issue 正文的 `## 产物目录` 读取；decision ticket 同时读取正文记录的 `issue-<编号>`。普通任务使用当前任务 slug，不带 issue 子目录。不要从任务 worktree 的物理目录名推断。

运行 `mmw artifact path scratch <产物目录> [issue-<编号>]`。HAR、trace、日志转储、core dump、录屏、一次性 harness、临时埋点输出和其他过程材料默认写入命令返回的 Git 忽略 scratch 目录。已经位于 correct seam、准备长期防回归的测试源码按 Phase 5 处理。
