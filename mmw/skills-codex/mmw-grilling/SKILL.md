...
调用方传来 `必读材料声明` 时，按 `$mmw:mmw-wayfinder` 的「解析必读材料声明」读取。已经回答当前 frontier 问题的进入设计树；仍未回答的部分再按下面各项取得事实。

- 读一个文件、一个符号，或者跑一条命令就能答完时，你自己去读、去跑。
- 要从几个互不依赖的角度系统地查才能答时，调用 `$mmw:mmw-research`。它交回综合后的事实、出处和没查清的部分，你把事实放回设计树。
...
**等他回答。**他说要，就移交 `$mmw:mmw-review` 走 ⓪ 共同理解审，被审对象是刚写好的那份共同理解记录。
