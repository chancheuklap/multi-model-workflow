...
| 过期 | 图输入的内容指纹跟建图时不一样 | 建一次；不值得等就直接 `Grep`、`Read`，并说明这次没用图 |
| 新鲜 | 指纹一致 | 直接查 |

判的是图输入的内容，不是提交号。**Markdown 改动和空提交都不算过期**——文档不进图，空提交不改任何文件。按提交号判的话，managed worktree 一创建就"过期"，当前目标项目 Local checkout 的现成图会无法复用。
...
一个仓库第一次接入，或者查出来的结果不对劲，看 [building.md](building.md)：跨语言边连的是什么、配置有哪几项、怎么验证建对了、每种失败怎么处置、managed worktree 怎么复用当前目标项目 Local checkout 那份。
