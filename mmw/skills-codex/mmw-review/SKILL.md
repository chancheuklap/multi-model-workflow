...
| ⑥ 合并集成审 | 几条分支集成到目标分支之后，由 `$mmw:mmw-integrate` 移交进来 | 不分视角，七个角度 | 派两个独立审查者，各走全套 | 是 |
...
按本节为每个视角规定的实例数，以名称启动 Codex 原生 `mmw-reviewer` subagent。每个实例只收到自己的四栏 task；互不依赖的实例在同一条消息中并行启动。
