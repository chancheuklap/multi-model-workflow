...
| --- | --- | --- | --- | --- |
| ① spec 审 | spec 写完、自检过了，给用户看之前 | 设计内容审、项目一致性审 | 两个视角各派一个独立审查者 | 不是。findings 连同你的处置一起给用户看参考 |
| ② plan 审 | 全部 plan 写完、主 agent 验证过、跨 plan 合同回填完之后，发起一次 | 覆盖质量审、合规交叉审 | 两个视角各派一个独立审查者 | 是 |
...
| ⑤ final 终审 | 全部 ticket 落地、合同门过了 | 对照终审、独立终审、编码规范审 | 每个视角各派两个独立审查者实例 | 是 |
| ⑥ 合并集成审 | 几条分支集成到本轮目标分支之后，由 `$mmw:mmw-integrate` 移交进来 | 不分视角，七个角度 | 派两个独立审查者实例，各走全套 | 是 |
...
| ⑥ | 集成后本轮目标分支的 `HEAD`，加各条分支的分叉点。两样都由 `$mmw:mmw-integrate` 第 1 步记下、移交时一并交过来 |
...
每个审查者实例一份。按 **四栏表**（目标 / 读 / 约束 / 验收）填写：

| 栏 | 本角色填写 |
| --- | --- |
| 目标 | **第一句**为任务名（与「2. 给每个视角备齐材料」表中任务名一字不差）；其后写审什么：被审对象 |
| 读 | 「2. 给每个视角备齐材料」表中，本任务名那一行 Source 列的路径，一行一条；该行没有材料则写「无」。复审时再加上一轮 `.reviews/` 记录路径，首轮写「无」；方法论由 `mmw-reviewer` agent 自动读取，不在 task 里重复 |
| 约束 | 只读；不改被审产物；方法论在「读」的路径里自读，不往 task 粘正文 |
| 验收 | 按审查者技能交 findings（或 `needs-redirection` / `needs-context`） |

**同一视角两个实例：** 四栏表内容相同，两个实例保持独立上下文。

agent 只能是原生只读 subagent `mmw-reviewer`。

可选：四栏表写入 `.reviews/<slug>-<哪一道>-<轮次>-<任务名>.prompt.md`，轮次从 1 起。

每个审查者实例各写一张四栏表并启动一次；同一条消息里全部发出。
每个审查视角分别启动一个 Codex 原生 subagent，agent 都设为 `mmw-reviewer`，task 传该视角的四栏表全文；各实例使用相同 GPT profile 和不同审查角度，在一条消息中并行启动。
...
几个审查者报同一件事，合并成一条处置。**同一个视角派了两个实例的，把两份 findings 并排对**：只有一个实例报出来的重点验证，两个实例同报的可信度较高。
