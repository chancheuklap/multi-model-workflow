...
每份 task 给同样的路径与范围（领域文档路径、`$mmw:mmw-codebase-design` 路径、这一片的 ADR 路径、第 1 步定下的范围），只有视角那一栏不同。subagent 自己读路径。
...
跑 `$mmw:mmw-grilling` 把决定树跟用户走一遍：约束、依赖、做深之后这个 module 什么形状、seam 后面藏什么、哪些测试还活着。谈清楚之后它会回到本技能收尾。
