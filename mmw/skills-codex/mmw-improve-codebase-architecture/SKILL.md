...
| `local` | 你在主检出里 | 建树：这棵树由 Codex App 创建。宿主给出干净的 detached worktree 之后，运行 `mmw task bind <任务分支名> "<用户原话>" --name <工作名> [--from <父分支或基点 SHA>]`。 重新运行 `mmw task state`。确认输出是 `bound`。再运行 `mmw task name` 取工作名 |
