...
运行 `mmw artifact path scratch <产物目录> [issue-<编号>]`，在命令返回的 Git 忽略 scratch 目录写 `to-questionnaire-<slug>.md`，slug 取主题。不要写当前目录。完成后报告路径。
...
| `$mmw:mmw-grilling` 因当前用户不掌握事实而移交进来，答案已经返回 | **移交**：`$mmw:mmw-grilling`，先把答案吸收到共同理解，再按第 4 步清理 scratch 副本 |
