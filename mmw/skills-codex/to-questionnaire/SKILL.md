...
在当前目录写 `to-questionnaire-<slug>.md`，slug 取主题。完成后报告路径。
...
| 情况 | 下一步 |
| --- | --- |
| Questionnaire 已生成 | **停**：报告文件路径和收件人，等待答案返回 |
| `$mmw:mmw-grilling` 因当前用户不掌握事实而移交进来，答案已经返回 | **移交**：`$mmw:mmw-grilling`，把答案作为事实材料继续建立共同理解 |
