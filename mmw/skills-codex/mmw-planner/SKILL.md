...
读取 ticket 的 `## 产物引用`。缺少该固定节时交 `needs-context`。每条运行 `mmw artifact path`，再读输出路径的索引和索引列出的文件。命令失败时交 `needs-context`。该节为 `无` 时，在 plan 元数据块写 `artifact_refs: []`。
...
测试规划只做三件事：把 ticket 验收映射到 spec 的 seam，写出项目现有的验证命令，补上本次改动引入的关键回归路径。TDD 循环由 `worker` 按 `$mmw:mmw-tdd` 执行，plan 不复制测试方法论、测试金字塔或质量评级。
