...
`$mmw:mmw-release` step 3 reads this file. By then this product's loop is started, or a previous loop is still there to resume.
...
- `close` leaves a delivery record (product name plus the ship commit). `$mmw:mmw-release` step 4 uses it for the same-commit check. **Do not delete it by hand.**
