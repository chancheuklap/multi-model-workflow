...
`$mmw:mmw-release` 第 3 步读这一份。到这里时这个产品的循环已经起好，或者上一次的循环还留在那儿可以续。
...
只处理这一次的输出，不预判下一步。每次运行 stage、dispatch、主 agent 修复或其他可能改动代码的动作后，比较 `git rev-parse HEAD` 与 `$mmw:mmw-release` 第 1 步读取的终审提交：

- 两者相同，继续当前状态机。
- 两者不同，立即停止驱动。当前 HEAD 尚未通过 final 终审，移交 `$mmw:mmw-review` 重新发起 ⑤ final 终审。终审通过后从 `$mmw:mmw-release` 第 1 步重新检查，再按引擎状态续跑。

出包 stage 通过、`source_commit` 一致或引擎全量重验都不能替代 final 终审。
...
3. 能处置就处置：环境类问题（网络、构建机忙）可以直接处理。改代码或配置时照常提交到当前分支，然后立即移交 final 终审，不在未审 HEAD 上继续出包。
4. 处置没有改变 HEAD 时，运行 `mmw release resume` 续跑。HEAD 改变时，终审通过并重新完成 `$mmw:mmw-release` 第 1 步后，再按引擎状态续跑。
...
- `close` 会留下一份交付记录（产品名加出包时的 commit），`$mmw:mmw-release` 第 4 步靠它核对几个包是不是同一份代码。**不要手工删它。**
