...
| 当前 HEAD 就是终审完成的提交 | 读取 ⑤ final 终审记录中的 `终审提交`，确认它等于 `git rev-parse HEAD` |
...
| 第 5 步用户报了问题 | **移交**：`$mmw:mmw-implement`，把他报的现象和复现步骤带过去。修完重新出包，不再审 |
...
| 第 1 步终审没跑，或还有采信的 findings 没修完 | **停**：说清缺哪一样。没审就回 `$mmw:mmw-implement` 第 7 步；审过就按 `$mmw:mmw-review` 第 7 步完成修复验证 |
