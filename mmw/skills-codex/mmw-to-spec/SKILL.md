---
name: mmw-to-spec
description: 把已经谈定的内容综合、审查并发布成一份 spec。用于用户直接要求写 spec，或者 `$mmw:mmw-grilling`、`$mmw:mmw-prototype`、`$mmw:mmw-wayfinder` 已经把这件事谈定、接下来需要一份 spec 交给下游实现。
---

本技能把已经谈定的内容和你对这个代码库的理解，综合成一份 spec。**不要**访谈用户，只综合已经有的东西。

已经谈定的内容不一定在当前对话里。从 `$mmw:mmw-wayfinder` 进来时，那些决定分散在一张 map 和它的 decision ticket 上，当前对话里什么都没有。所以先看下一节，把它们读出来，再开始写。

开始前按 `$mmw:mmw-domain-modeling` 的「读领域文档」读取这个仓库的领域文档。整份 spec 使用其中定义的术语。
...
| --- | --- | --- | --- |
| 用户直接调用 `$mmw:mmw-to-spec` | 当前对话中已经谈定的内容 | 当前对话 | 从用户提出本次需求的位置开始，读取已经明确形成的决定。不要使用尚未得到确认的建议或问题 |
| `$mmw:mmw-grilling` 完成后进入 To Spec | 用户已经确认的共同理解记录 | `$mmw:mmw-grilling` 交回的那个精确路径。它没有交回路径时，运行 `mmw artifact path scratch --sub understanding.md` | 读这份记录的 `## 共同理解` 与 `## 支持材料` 两段。支持材料段点名 research 或 prototype 时，顺着它写出的精确路径读那些产物。这两段就是本次访谈的全部结果 |
...
3. 完整读取 [spec-template.md](spec-template.md)。使用该模板编写 spec。从 `$mmw:mmw-wayfinder` 进入时运行 `mmw artifact path spec --name <map 分支 slug>`；否则运行 `mmw artifact path spec`。把 spec 写入输出文件。
...
4. 调用 `$mmw:mmw-review` 发起一次 ① spec 审。传给它 spec 的精确路径，以及这份 spec 实际引用到的 prototype `README.md`、选中产物、research `README.md` 和精确文件；某一项没有就写「无」。

   `$mmw:mmw-review` 会返回一批问题，并说明每一条你打算怎么处理。你决定接受的那些，改完之后要回到 `$mmw:mmw-review` 让它确认改到位了。全部处理完再进第 5 步。
