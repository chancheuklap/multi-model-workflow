...
两个视角各派一个审查者：设计内容审看这份 spec 本身立不立得住，项目一致性审看它跟项目既有的术语、硬约束、登记机制合不合。两个视角分别使用独立的 `mmw-reviewer` subagent 上下文。材料怎么备、提示词什么形状、存盘叫什么名字，全在 `$mmw:mmw-review`。

采信的 findings 拿去改 spec，改完重走一遍第 5 步自检。

## 7. 给用户看，等他点头

**这是全流程唯一一道人工审批关卡。** 摆三样给他：
