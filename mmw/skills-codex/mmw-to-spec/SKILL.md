---
name: mmw-to-spec
description: 把已经谈定的内容综合成一份 spec，通过 spec 定稿人工审批关卡之后才发布。用户说要写 spec、要把刚才谈的落成设计时用它；别的技能把一件事谈定或者走查完时也移交这里。
---
...
某个 seam 定不下来就停——出口见本文「下一步」一节。

**从 `$mmw:mmw-implement` 回来补 seam 的**：补完这一节就跳到第 7 步，只把 seam 清单给用户看。这是已发布 spec 变更的人工审批关卡；批准对象是新 seam 清单，批准人是用户，通过凭据是用户明确批准这份新 seam 清单。通过后把 spec 文件提交进分支，回 `$mmw:mmw-implement`。不重走第 6 步那一轮审查，也不动那张已经发布的 issue。
...
**这是 spec 定稿的人工审批关卡。** 批准对象是下面三项，批准人是用户，通过凭据是用户明确批准这三项，通过后才允许发布 spec issue：

1. spec 定稿，重点讲清楚要解决什么问题、交付什么。
2. 审查者报了什么、每条 finding 的处置是什么。还开着的严重问题不许埋着给他看——改掉，或者写明为什么判成不改。
3. seam 清单单独再说一遍。这是他这一轮唯一必须亲自过目的技术决定。

用户明确批准三项对象之前不要发布。用户要求修改时回第 4 步，改完重走第 5 步和第 6 步。
...
发布 spec issue 并添加 `ready-for-agent` 后，下游 `$mmw:mmw-to-tickets` 才能继续。第 7 步的通过凭据始终是用户明确批准 spec 定稿、审查处置和 seam 清单；issue 和标签只记录发布状态与 Tracker 状态，不替代批准凭据。
...
| 一项决定被实测结果阻塞，而且实测需要 test plan、多候选对比、多轮测量、真实凭证或真实环境 | **移交**：`$mmw:mmw-prototype` 的 `EVIDENCE.md`。这条决定先留在 `## Open Decisions` 里，测出来再回填，不要拿一个猜的数字定稿 |
...
| 用户没有明确批准三项对象，也没有给出具体修改要求 | **停**：报 spec 已定稿在哪个路径、审查者报了什么、卡在哪一条上 |
