...
**seam** 是下断言的公开边界。每条 seam 使用一行，写清公开接口、验证行为和选择理由。

选择 seam 时按以下顺序判断：

1. 优先使用已有 seam；新开前确认现有 seam 都无法覆盖。
2. 选择能覆盖目标行为的最高层。
3. 减少 seam 数量，理想数量是一。
4. 必须新开时放在最高点，并记录取舍理由。
...
**从 `$mmw:mmw-implement` 回来补 seam 的**：补完这一节就跳到第 7 步，只把 seam 清单给用户看；他点头就把 spec 文件提交进分支，回 `$mmw:mmw-implement`。不重走第 6 步那一轮审查，也不动那张已经发布的 issue。
...
- **原型证据逐条验证**：`Implementation Decisions` 中每条决定必须满足一项：
  - 指向 `docs/prototypes/<slug>/` 下的产物，并写明产物和走查轮次。
  - 符合下列免除情形，并在决定后写明具体类型。

  免除情形只有三种：
  1. 照搬既有实现、既有约定或已拍板的 ADR。
  2. 对错可以通过一次代码或文档查询确定，不需要用户判断。
  3. 不改变用户可见行为，例如纯内部结构调整、重命名或搬移。

  两项都不满足时，移交 `$mmw:mmw-prototype` 补一轮。用户走查过实物才算验证完成；讨论一致和口头同意都不算。时间紧、结论显然或先做再说也不属于免除情形。
...
自检全过之后按 `$mmw:mmw-review` 发起 **① spec 审**。**这不是关卡，是给用户和你的参考。**
...
**这是 spec 的人工审批关卡。** 向用户展示：

1. spec 定稿：说明要解决的问题和交付结果。
2. 审查结果：列出 findings 及处置。严重问题已经修复，或写明不修理由。
3. seam 清单：单独展示。这是本轮必须由用户过目的技术决定。
