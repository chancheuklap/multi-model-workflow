---
name: mmw-research
description: 重型调查流程。只在问题需要拆成多个独立角度、派 `investigator` 系统取证并逐条验证出处时使用，例如跨模块实现、调用链、数据流、影响面，或需要对照多份一手资料的外部合同。单个文件、符号或事实的查找，文件计数，以及一条命令能完成的确认，由主 agent 直接处理，不调用本技能。
---
