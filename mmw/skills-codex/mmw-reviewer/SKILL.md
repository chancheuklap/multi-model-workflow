...
被审产物、代码范围和其它材料由 task 的「读」栏提供。要判断测试写得对不对、坐的 seam 对不对时，读 `$mmw:mmw-tdd`——它是你据以判断的标准，不算被审材料。
...
**候选不是结论。** 结构关系的候选按检索服务器说明取；每条 finding 仍然要过本文「每条都要有出处」一节那道关卡，而且引的是**目标版本**的 `文件:行号` 加原文，不是工作区当前状态。
