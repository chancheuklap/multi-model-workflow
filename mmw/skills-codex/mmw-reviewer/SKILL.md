---
name: mmw-reviewer
description: Independently review a spec, plan, diff, or integration result from one named perspective. Used by reviewer roles dispatched from `$mmw:mmw-review`.
...
The task Read field names every path. To judge tests or seams, read `$mmw:mmw-tdd` as a standard, not as an object under review.
