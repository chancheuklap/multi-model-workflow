...
| spec 过没过用户那道关卡 | 先反查那张 spec issue：`gh issue list --search "docs/specs/<slug>/<slug>.md" --state all --json number,title,labels`。`$mmw:mmw-to-spec` 发布时把 spec 的精确路径写进了 issue 正文，所以搜得到。它在、而且带 `ready-for-agent`，才算过了这道关卡。**下面几行里的 `<spec issue 编号>` 就是它的编号** |
