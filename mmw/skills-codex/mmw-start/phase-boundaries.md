...
Codex App 会在当前任务中自动管理上下文压缩，主 agent 没有可调用的 `clear` 或 `compact` 动作。Continue 时留在当前任务；需要 portability 时调用 `/handoff`；AFK 工作只按调用技能已有的原生 subagent 或后台 Worktree 合同派发。不得要求用户执行不存在的 Codex 命令，也不得声称自动压缩已经发生。
