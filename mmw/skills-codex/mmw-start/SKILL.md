...
| 一个 issue 编号，挂在 spec issue 下，正文 `## Plan` 写有 plan 路径，评论里有 `$mmw:mmw-to-plan` 第 8 步定义的有效过审凭据，并已是 `ready-for-agent`——那是一张 plan 已通过 ② plan 审的 tracer bullet ticket | **移交**：`$mmw:mmw-implement`；tracer bullet ticket 不要求 agent brief |
| 一个 issue 编号，挂在 spec issue 下，但 plan 路径、有效过审凭据或 `ready-for-agent` 缺少任何一项 | **移交**：`$mmw:mmw-to-plan`；凭据过期和升级前旧标签也走这条，不送 `$mmw:mmw-triage` |
