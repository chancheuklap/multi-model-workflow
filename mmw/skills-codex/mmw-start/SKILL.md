...
| 要从多个独立角度调查跨模块实现、调用链、数据流或影响面，或者需要对照多份一手资料 | **移交**：`$mmw:mmw-research` |
...
| 用户要出包，或者终审已过且这次改动碰了带出包配置的产品 | **移交**：`$mmw:mmw-release` |
| 用户要收尾，或者出包实测已过、且这次有 spec | **移交**：`$mmw:mmw-closing` |
...
类型取自第 1 步的判定结果：走 `$mmw:mmw-diagnosing-bugs` 的用 `fix`，走 `$mmw:mmw-research` 的用 `docs`，新需求和先做原型的用 `feat`。类型同时约束范围——一个 `fix` 里混进新功能，说明当初的类型定错了，或者这次改动该拆成两个。
...
当前 HEAD 必须落在正确的父提交上。普通任务使用当前目标分支的已提交 HEAD；从 `$mmw:mmw-wayfinder` map 派生的任务使用 map 分支。父提交不包含任务所需决定时停下，不在错误基点上补提交。
...
**粒度是一份 spec 一棵树。** 这份 spec 拆出的几张 ticket 全在这棵树里按顺序做完，整体合并一次、终审一次、收尾一次。确实能并行的 ticket 从当前这棵树的分支再分叉出去（判据在 `$mmw:mmw-implement`）。
