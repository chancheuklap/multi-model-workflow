...
| 一个 issue 编号，挂在 spec issue 下，正文 `## Plan` 写有 plan 路径，并已是 `ready-for-agent`——那是一张 plan 已通过 ② plan 审的 tracer bullet ticket | **移交**：`$mmw:mmw-implement`；tracer bullet ticket 不要求 agent brief |
