...
结果 worktree 由 `$mmw:mmw-implement` 在关票之后当场回收。这一步只做兜底核对：跑过旧版本技能的任务、或者中途断掉的轮次会留下没人回收的树。
...
| 第 2 步有结果 worktree 的提交还没进任务分支 | **停**：保留那棵树，报分支名。那次集成没有真正完成，回 `$mmw:mmw-implement` 第 5 步 |
