...
每张 ticket 的验收标准按四条判据写。下游四道检查（plan 的 `## Acceptance` 映射、`$mmw:mmw-to-plan` 的验证、覆盖质量审、③ 逐份验收）全部以它为判断基准，写含糊了每道检查都会形式上通过：
...
- **Acceptance criteria**：第 3 步按四条判据写出的验收标准。验收是拆分质量的实体，用户批清单时必须看得到它
...
| 来源是一份 spec，但那张 spec issue 没带 `ready-for-agent` | **停**：回 `$mmw:mmw-to-spec` 第 6 步，那份 spec 还没过用户那道批准关卡 |
