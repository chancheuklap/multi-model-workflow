...
If a prototype index is missing its question, walkthrough conclusions, chosen artifacts, rejected constraints, or long-lived evidence, go back to `$mmw:mmw-prototype` to fill the gap; write `none` for a field that has none.
