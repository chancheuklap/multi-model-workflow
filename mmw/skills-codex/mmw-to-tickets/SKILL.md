...
`ready-for-agent` 始终表示「当前 work item 的合同已足以让 agent 按拥有它的技能 AFK 继续」。spec issue 依靠已批准 spec 进入本技能；新建实现 ticket 依靠验收标准进入 `$mmw:mmw-to-plan`。标签不豁免 `$mmw:mmw-implement` 对 plan 和 plan 审的前置检查。
