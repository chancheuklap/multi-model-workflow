...
| --- | --- | --- | --- |
| `wayfinder:grilling` | HITL | 默认。另外三个标签都不适用就打它 | 主 agent 跑 `$mmw:mmw-grilling`，一次问用户一个问题 |
| `wayfinder:prototype` | HITL | 关键问题是「它该长什么样」或者「它该怎么表现」——光靠说定不下来，要有一个能上手的东西摆在面前才评得动 | 主 agent 跑 `$mmw:mmw-prototype` 做一个粗糙版本，用户走查 |
| `wayfinder:research` | AFK | **要用的知识在当前工作目录之外**：第三方文档、外部接口、本地知识库。仓库里读得到的不打这个标签 | 主 agent 按 `$mmw:mmw-research` 派一个 subagent 去查。一项决定被实测结果阻塞，而且实测需要 test plan、多候选对比、多轮测量、真实凭证或真实环境时，转 `$mmw:mmw-prototype` 的 `EVIDENCE.md`。受控实测遵守 `EVIDENCE.md` 第 3 步的实测人工审批关卡 |
...
已经存在的一张 ticket 后来发现坐在 destination 之外——画图时圈错了，或者被某次解答暴露出来——就**关掉它**，并在 `Out of scope` 一节留一行：概要加上为什么出范围，链到那张关掉的 ticket。同时在 `.out-of-scope/` 写一份，一个概念一个文件；`.out-of-scope/` 文件的去向按 [closing.md](closing.md) 第 2 步处理。
