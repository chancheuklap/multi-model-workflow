...
1. **它是 HITL 还是 AFK**——这件活要不要人在对话里参与才做得完。一件活只分这两种：
   ...
   这条轴跟**人工审批关卡**不是一回事：人工审批关卡是“必须用户点头这件事才能往下走”的关卡；HITL 说的是“这件活本身要人参与才做得完”。`$mmw:mmw-grilling` 确认共同理解，`$mmw:mmw-to-spec` 确认 spec 定稿，`$mmw:mmw-to-tickets` 确认 ticket 清单。三处各自批准不同产物。一件 AFK 的活也可能在后续流程遇到人工审批关卡。
...
| --- | --- | --- | --- |
| `wayfinder:grilling` | HITL | 默认。另外三个标签都不适用就打它 | 主 agent 跑 `$mmw:mmw-grilling`；它用设计树组织提问，并在同一段对话中应用 `$mmw:mmw-domain-modeling` |
| `wayfinder:prototype` | HITL | 关键问题是「它该长什么样」或者「它该怎么表现」——光靠说定不下来，要有一个能上手的东西摆在面前才评得动 | 主 agent 跑 `$mmw:mmw-prototype` 做一个粗糙版本，用户走查 |
| `wayfinder:research` | AFK | **要用的知识在当前工作目录之外**：第三方文档、外部接口、本地知识库。仓库里读得到的不打这个标签 | 主 agent 按 `$mmw:mmw-research` 派一个 subagent 去查。查下来发现这条事实要真实跑一次才知道，转 `$mmw:mmw-prototype` 的 `EVIDENCE.md`——这时候这张 ticket 变成 HITL，凭证和环境要用户点头 |
