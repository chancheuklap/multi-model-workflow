...
再确认每张已关闭 decision ticket 的结果都已经集成回 map 分支，而且没有遗留 `draft-<ticket 编号>-<slug>.md`。存在未集成的结果分支或 ADR 草稿时，先按 [walking.md](walking.md) 第 7 节完成集成和正式编号，再继续收尾；集成需要的任务分支名、HEAD SHA 和基点 SHA，从那张 ticket 的最后一条评论取。
...
| `Destination` 写的是 | 处理 |
| --- | --- |
| 一份 spec | 交给 `$mmw:mmw-to-spec` 三样东西：map 名称和它的 URL 或编号、map 正文里的 `产物目录`、以及任务 slug（用 map 标题的短名，全小写、空格换连字符）。一张 map 只出一份 spec。剩下的由 `$mmw:mmw-to-spec` 自己去读：它顺着 `Decisions so far` 进各张 decision ticket，再按 ticket 评论里的链接取 prototype 和 research。你不要先建一张 spec issue，也不要把内容复制一份给它 |
...
| 情况 | 下一步 |
| --- | --- |
| destination 是一份 spec，map 已关闭 | **移交**：`$mmw:mmw-to-spec`，交给它 map 名称及其 URL 或编号、`产物目录` 和任务 slug |
