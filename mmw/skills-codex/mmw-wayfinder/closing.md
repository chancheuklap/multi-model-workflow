...
同一 effort 下保留 map 的产物目录和各张 decision ticket 的 `issue-<编号>` 子目录。运行 `mmw artifact path scratch <产物目录>` 取得这个 effort 的 scratch 路径；将必要证据提升并引用后，清理该 scratch 路径。Scratch 不随 effort 合回。
