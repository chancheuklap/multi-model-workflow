...
| `Destination` 写的是 | 处理 |
| --- | --- |
| 一份 spec | 交给 `$mmw:mmw-to-spec` 三样东西：map 名称和它的 URL 或编号、map 正文里的工作名、以及任务 slug（用 map 标题的短名，全小写、空格换连字符）。一张 map 只出一份 spec。剩下的由 `$mmw:mmw-to-spec` 自己去读：它顺着 `Decisions so far` 进各张 decision ticket，再按 ticket 评论里的产物引用取 prototype 和 research。你不要先建一张 spec issue，也不要把内容复制一份给它 |
...
| 情况 | 下一步 |
| --- | --- |
| destination 是一份 spec，map 已关闭 | **移交**：`$mmw:mmw-to-spec`，交给它 map 名称及其 URL 或编号、工作名和任务 slug |
