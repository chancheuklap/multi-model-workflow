...
| `Destination` 写的是 | 处理 |
| --- | --- |
| 一份 spec | 报告 map 名称和它的 URL 或编号，以及 map 正文 `## 分支` 的 slug（产物名字段）。问用户：写 spec，还是到这里停。一张 map 只出一份 spec。用户要写 spec 时，剩下的由 `$mmw:mmw-to-spec` 自己去读：它顺着 `Decisions so far` 进各张 decision ticket，再按 ticket 评论里的产物引用取 prototype 和 research。你不要先建一张 spec issue，也不要把内容复制一份给它 |
