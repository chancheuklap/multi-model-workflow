...
| 标签 | 解法 |
| --- | --- |
| `wayfinder:grilling` | 跑 `$mmw:mmw-grilling`，把 ticket 的 `Question` 谈成双方确认的共同理解；提问方式全部由该技能决定 |
| `wayfinder:prototype` | 跑 `$mmw:mmw-prototype` 做一个粗糙版本给用户走查。产物落 `docs/prototypes/<slug>/`，结论按 `$mmw:mmw-prototype` 的回填规矩写成结案评论，评论里留一个指向产物目录的路径 |
| `wayfinder:research` | 按 `$mmw:mmw-research` 派一个 subagent 去查，只查这一个决定要等的那条事实，答完就停。按 `$mmw:mmw-verifying-agent-output` 验证过的事实才写进 ticket 评论，没查清的另起一节列出来 |
...
3. 按 `$mmw:mmw-domain-modeling` 的完整 ADR 判据评估这个决定。三项判据全部成立才写 ADR。**链上先写草稿名** `draft-<ticket 编号>-<kebab-标题>.md`，落点跑 `mmw domain dirs` 取 `adr` 那一行；等这条链走完、合回 map 分支时再统一改成正式编号。几条链同时跑，各自取号必定撞，草稿名是绕开它的办法（见 `$mmw:mmw-domain-modeling` 的 `ADR-FORMAT.md`）。
