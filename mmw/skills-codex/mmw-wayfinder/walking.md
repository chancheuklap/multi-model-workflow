...
| `wayfinder:prototype` | 跑 `$mmw:mmw-prototype` 做一个粗糙版本给用户走查；完整资产保存在 `docs/prototypes/<slug>/`，结案评论指向该路径 |
| `wayfinder:research` | 按 `$mmw:mmw-research` 派一个 subagent，只调查这一个决定等待的事实；按 `$mmw:mmw-verifying-agent-output` 验证后再写入 ticket 评论 |
