...
- **每个 session 只在自己的任务 worktree 里写文件。** 建 map 的会话拥有 map 分支；每张 decision ticket 用一条从 map 分支派生的任务分支，解决期间写下的领域文档、ADR 和资产都提交在这条任务分支上，再由 map 分支上的会话验证合并；那个会话不必是建 map 的原会话，`$mmw:mmw-start` 可以随时按 map 编号恢复 map 的任务上下文。这一条不能省：两个会话同时改同一份领域文档时，各自一个 worktree 会让冲突在合并时暴露出来，而共用一个工作目录会让后写的那个直接覆盖先写的，谁都不会发现。
...
- **Research**（AFK）：系统取证，找出某项决定正在等待的事实——读当前仓库的源码，或者文档、第三方 API、正式规范这类外部资源。由 `$mmw:mmw-research` 解决。事实靠取证就能得到、不需要人参与讨论时使用。
- **Prototype**（HITL）：制作一个具体的可运行资产，提高讨论的保真度。由 `$mmw:mmw-prototype` 解决。初版可以粗糙；用户持续走查并迭代，把这个想法磨清楚。想法磨清楚之后，prototype 里承载它的脚本、接口合同、状态模型和界面就是下游可以直接参考或复用的内容。把 prototype 作为资产链接到 ticket。关键问题是“它应该长什么样”或“它应该怎样表现”，而且只靠讨论无法决定时使用。
- **Grilling**（HITL）：对话。默认情况。由 `$mmw:mmw-grilling` 解决；它在同一场讨论中应用 `$mmw:mmw-domain-modeling`。
