...
brief 要写成即使文件被改名、被挪走、被重构，它依然有用。
...
- **好**：「用户不带参数跑 `$mmw:mmw-triage` 时，应当看到一份需要处理的 issue 摘要」
- **坏**：「在主处理函数里加一个 switch」
...
brief 自己点名：回归测试坐在哪一层，断言什么行为。用行为描述——「一个穿过公开分诊命令的集成测试，断言写进 tracker 的那个标签」——不要写成文件路径。

点不出一个正确的 seam 时，在 brief 里说明，并把这张 issue 改判 `ready-for-human`。
