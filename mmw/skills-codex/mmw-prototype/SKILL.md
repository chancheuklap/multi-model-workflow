...
| `detached` | 宿主把你放在一棵干净的树上了，还没绑分支 | 先分别确定任务分支名和工作名。运行 `mmw task bind <任务分支名> "<用户原话>" --name <工作名> [--from <父分支或基点 SHA>]`。重新运行 `mmw task state`。确认输出是 `bound`。再取第四个词 |
| `local` 或 `outside` | 你在主检出里，或者根本不在仓库里 | 先分别确定任务分支名和工作名。运行 `mmw task new <任务分支名> "<用户原话>" --name <工作名> [--from <父分支>]`。切换到返回的绝对路径。重新运行 `mmw task state`。确认输出是 `bound`。再取第四个词 |
