...
开始一轮前，先从用户任务、已经进行的讨论或现有 prototype 中明确一个初始问题。初始问题只用于确定本轮要读取的材料。取得 `prototype 产物路径` 后，把它写进 `README.md`。用户走查后必须能够说明哪里正确、哪里需要修改，或者哪个方向成立。
...
下文提到的两条路径按是否有第二个参数拼接：

| 情况 | `prototype 产物路径` | `scratch 路径` |
| --- | --- | --- |
| 普通任务，没有第二个参数 | `docs/prototypes/<产物目录>/` | `.scratch/<产物目录>/` |
| Wayfinder decision ticket，有第二个参数 | `docs/prototypes/<产物目录>/issue-<编号>/` | `.scratch/<产物目录>/issue-<编号>/` |
...
在 `prototype 产物路径` 下使用这个结构。目录有资产时再建立；没有选中结果时，不建立空的 `mockup/current/`：
...
读取已有 `README.md`；没有时创建它。把初始问题写入当前一轮。再只读取当前问题需要的 prototype 资产。后续轮次继续修改 `mockup/current/`，不建立版本目录。`mockup/variants/` 下的每个目录回答一个验证问题，不代表一个版本。
...
先清点这次要给用户看的全部页面或 URL。每个页面各自打开一处，让用户能够来回比较。变体组中的每个变体 key 算一个独立 URL，必须分别打开。
