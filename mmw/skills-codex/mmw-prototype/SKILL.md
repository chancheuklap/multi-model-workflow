...
| 要验的不是我们自己要写的东西，而是一个外部系统在我们的负载、我们的数据、我们的账号下实际是什么表现 | 移交 `$mmw:mmw-research`。同时交出当前工作名和可用的范围段。它使用同一项产物引用 |
...
| `local` | 你在主检出里 | 先分别确定任务分支名和工作名。运行 `mmw task new <任务分支名> "<用户原话>" --name <工作名> [--from <父分支>]`。切换到返回的绝对路径。重新运行 `mmw task state`。确认输出是 `bound`。再运行 `mmw task name` 取工作名 |
...
**本轮做了 UI 工作面时，交给用户之前先跑一次 `$mmw:mmw-ui-qa`。** 它按判据检查界面本身，确定性检查的违规项当场改掉，剩下的作为 finding 一并交给用户。这样用户走查时看到的不是一版还带着已知问题的界面，他的注意力可以留给只有他能回答的那个问题。

**它不替代走查。** 走查仍然专指用户本人操作并给出意见；界面 QA 不判「这一版可以了」，也不代替用户回答本轮验证问题。它跑不起来时照常把当前这一版交给用户，并说明没跑成。
