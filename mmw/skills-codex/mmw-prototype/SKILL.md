...
开始一轮前，先从用户任务、已经进行的讨论或现有 prototype 中明确本轮验证问题。取得 `prototype 产物路径` 后，把它写进 `README.md`。用户走查后必须能够说明哪里正确、哪里需要修改，或者哪个方向成立。
...
读取已有 `README.md`；没有时创建它。把本轮验证问题写入当前一轮。再只读取当前问题需要的 prototype 资产。后续轮次继续修改 `mockup/current/`，不建立版本目录。`mockup/variants/` 下的每个目录回答一个验证问题，不代表一个版本。
