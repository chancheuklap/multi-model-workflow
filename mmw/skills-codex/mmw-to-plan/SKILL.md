...
**全部 plan 都验证过、合同也回填完之后，发起一次审查，不逐份发起。** 按 `$mmw:mmw-review` 走。传给它每张 ticket 的 plan 类别内细分。逐份运行 `mmw artifact path plan --sub <两位编号>-<ticket短名>.md`。再传这份 spec 的精确路径、全部 ticket 编号，以及本批 plan 引用的 prototype `README.md`、选中产物、research `README.md` 和精确文件。没有的项目写「无」。
...
| 情况 | 下一步 |
| --- | --- |
| 全部 plan 过审、提交，且全部 open tracer bullet ticket 都带 `ready-for-agent` | **移交**：`$mmw:mmw-implement`，从 `mmw issue frontier` 返回的 ticket 开始落地 |
