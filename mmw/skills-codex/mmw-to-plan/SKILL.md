...
| spec 已完成定稿人工审批关卡并发布 | `docs/specs/<slug>/<slug>.md` 存在，对应的 spec issue 已发布并带着 `ready-for-agent`；issue 和标签只证明发布状态，批准凭据仍是 `$mmw:mmw-to-spec` 记录的用户明确批准 spec 定稿、审查处置和 seam 清单 |
