...
调用方传来 `必读材料声明` 时，按 `/mmw-wayfinder` 的「必读材料声明」解析并读取。已经回答当前 frontier 问题的进入设计树；仍未回答的部分再按下面各项取得事实。
...
```bash
mmw artifact path scratch [--name <名字段>] [--issue <编号>] --sub understanding.md
```
