...
**从 `/mmw-implement` 回来补 seam 的**：补完这一节就跳到第 7 步，只把 seam 清单给用户看。这是已发布 spec 变更的人工审批关卡；批准对象是新 seam 清单，批准人是用户，通过凭据是用户明确批准这份新 seam 清单。通过后把 spec 文件提交进分支，回 `/mmw-implement`。不重走第 6 步那一轮审查，也不动那张已经发布的 issue。
...
自检全过之后按 `/mmw-review` 发起 **① spec 审**。**这不是人工审批关卡，是给用户和你的参考。**
...
发布 spec issue 并添加 `ready-for-agent` 后，下游 `/mmw-to-tickets` 才能继续。第 7 步的通过凭据始终是用户明确批准 spec 定稿、审查处置和 seam 清单；issue 和标签只记录发布状态与 Tracker 状态，不替代批准凭据。
...
| 一项决定被实测结果阻塞，而且实测需要 test plan、多候选对比、多轮测量、真实凭证或真实环境 | **移交**：`/mmw-prototype` 的 `EVIDENCE.md`。这条决定先留在 `## Open Decisions` 里，测出来再回填，不要拿一个猜的数字定稿 |
