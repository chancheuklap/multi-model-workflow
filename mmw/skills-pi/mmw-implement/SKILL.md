...
按 `/mmw-review` 的 **③ 逐份验收**验证做漏没有、测试达不达标、有没有偏离。三关的判据和返工升级策略都在 `/mmw-review` 目录中的 `self-review.md`。三关都过才允许合并。这一道由主 agent 执行，不派审查者。报告按 `/mmw-verifying-agent-output` 验证。
