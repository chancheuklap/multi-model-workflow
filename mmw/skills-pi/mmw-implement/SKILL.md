...
- spec 分支：命令列出阻塞全部关闭、无人认领且带指定标签的 ticket，并按 `/mmw-to-tickets` 的发布顺序排序。取第一行。
...
按 `/mmw-review` 的 **③ 逐份验收**验证做漏没有、测试达不达标、有没有偏离。三关的判据和返工升级策略都在 `/mmw-review` 目录中的 `self-review.md`。

三关都过才允许合并。这一道由主 agent 执行，不派审查者。报告按 `/mmw-verifying-agent-output` 验证。
...
每张 ticket 都关闭、改动都在任务分支后，按 `/mmw-review` 的 **④ 合同门**验证 `## Cross-Plan Contract Anchors` 中每条跨 plan 合同已经在合并后的代码中兑现。

逐条判据和大量合同时的取证方式见 `/mmw-review` 目录中的 `self-review.md`。这一道也不派审查者。
