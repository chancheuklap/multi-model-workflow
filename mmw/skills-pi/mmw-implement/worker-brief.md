...
task 的「读」栏出现产物引用时，逐条运行 `mmw artifact path <类别> --name <工作名>`。条目有 `issue=<编号>` 或 `sub=<类别内细分>` 时，追加对应参数。只在路径解析成功后读取那份产物的索引及其显式列出的文件。`name` 缺失、条目解析失败或上游缺少固定声明时，停止并报告。task 写 `无` 时不读取产物。
