...
| 情况 | 处理 |
| --- | --- |
| 现有行为无法表达当前验证问题 | 向用户说明无法表达的事实，读取 [capture.md](capture.md)，不继续构建 |
...
| 情况 | 处理 |
| --- | --- |
| 用户已经驱动过状态模型并给出走查意见 | 读取 [capture.md](capture.md)，保存本轮记录和下游输入 |
| Logic HTML 已经交给用户，但用户还没有驱动状态模型 | 等待用户操作并返回意见 |
| 现有状态模型无法表达当前验证问题 | 读取 [capture.md](capture.md)，把无法表达的事实记为本轮结论 |
