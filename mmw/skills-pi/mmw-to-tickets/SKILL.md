...
The spec issue holds identity. Each tracer-bullet ticket is one child issue. That issue holds the summary, the plan-path command, blocking edges, and artifact refs. `/mmw-to-plan` later writes the plan file the command prints.

The caller passes `<spec issue number>`. Stop if it is missing. Stop if that spec issue does not have `ready-for-agent` — the user has not approved that spec for publish; go back to `/mmw-to-spec`.
...
4. Each criterion names where it is verified: a test on a confirmed spec seam, or a human browser check. If you cannot name that place, the spec is missing a decision — stop and return to `/mmw-to-spec`. Do not invent it.
...
`<NN>` is the two-digit number from step 4. `<ticket-slug>` is an English kebab of the Title, three or four words, lowercase letters, digits, and hyphens. `/mmw-to-plan` writes the file; this command reserves the path.
