...
1. 把 spec 和 plan 归档为一张 Wiki 页面。
2. 验证 Wiki 后，删除任务分支中的本地 spec 和 plan。

归档后，Wiki 是这份 spec 的唯一事实来源，主线不保留本地 spec 和 plan。map、审查记录和终审报告仍按 `/mmw-wayfinder` 与 `/mmw-review` 保存。
...
文件名使用 `Spec-<slug>.md`，平铺在 Wiki 根目录，一份 spec 一页。GitHub Wiki 使用平面页面命名空间，不建子目录；`Spec-` 前缀承担层级信息。标题禁用 `\ / : * ? " < > |`。
...
`mmw wiki nav` 非零退出时会点名缺元数据块的页面。本次页面在名单里就回第 2 步补齐；既有旧页面在名单里就记录并交给用户，不修改。
