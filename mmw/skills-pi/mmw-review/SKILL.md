...
当前宿主用两个角色保持作者与审查者分离：主 agent 定稿的 spec、map 和调查综合使用 `reviewer-gpt`。启动：调用原生 `subagent`，agent 设为 `mmw-reviewer-gpt`，task 传四栏表全文。 plan 和代码使用 `reviewer-claude`。启动：调用原生 `subagent`，agent 设为 `mmw-reviewer-claude`，task 传四栏表全文。 本节要求两个独立实例时，两个角色各启动一次。每个实例只收到自己的四栏 task。
