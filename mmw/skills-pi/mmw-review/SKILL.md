...
⑤ final 终审包含界面改动时，派审查者前先跑一次 `/mmw-ui-qa`，再采集浏览器证据。

界面 QA 挂 `本任务` 标签。它按判据检查界面本身，产出的东西与审查者互补：确定性检查的违规项它当场改掉，模型判断的 finding 交给用户裁决。审查者看的是 diff 与产物一致性，不检查界面。**它的结果不替代任何一个视角，也不改变六道审的道数**；它跑不起来时照常往下走，把 blocker 写进审查材料。
