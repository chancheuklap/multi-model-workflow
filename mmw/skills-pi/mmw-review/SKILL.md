...
| 栏 | 本角色填写 |
| --- | --- |
| 目标 | 任务名：与「2. 给每个视角备齐材料」表中任务名一字不差；审什么：被审对象 |
| 读 | Source 材料路径（该视角表中的那些，一行一条；没有写「无」）；复审时加上一轮审查记录路径，首轮写「无」；`mmw skill-path <角色>` 有输出则加方法论路径 |
| 约束 | 只读；不改被审产物；方法论在「读」的路径里自读，不往 task 粘正文 |
| 验收 | 按审查者技能交 findings（或 `needs-redirection` / `needs-context`） |

**同一视角两个角色：** 四栏表内容相同；仅「读」中方法论路径可因角色而变。

角色只能是 `reviewer-gpt` 与/或 `reviewer-claude`（见本文开头红线表）。

可选：四栏表写入 `.reviews/<slug>-<哪一道>-<轮次>-<任务名>.prompt.md`，轮次从 1 起。

每个（视角 × 角色）各写一张四栏表并启动一次；同一条消息里全部发出。
`reviewer-gpt`：启动：`subagent({ agent: "mmw-reviewer-gpt", task: <四栏表全文> })`（只读）。
`reviewer-claude`：启动：`subagent({ agent: "mmw-reviewer-claude", task: <四栏表全文> })`（只读）。
