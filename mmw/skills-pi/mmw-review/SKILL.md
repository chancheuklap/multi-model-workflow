...
当前宿主使用两个审查角色。⓪ 启动一个 `reviewer-gpt`：共同理解是主 agent 自己问出来的，复核它的审查者必须换一个模型。启动：调用原生 `subagent`，agent 设为 `mmw-reviewer-gpt`，task 传四栏表全文。① 每个视角启动一个 `reviewer-gpt`。启动：调用原生 `subagent`，agent 设为 `mmw-reviewer-gpt`，task 传四栏表全文。② 每个视角启动一个 `reviewer-claude`。启动：调用原生 `subagent`，agent 设为 `mmw-reviewer-claude`，task 传四栏表全文。⑤ 每个视角分别启动一个 `reviewer-gpt` 和一个 `reviewer-claude`。同一视角的两份 findings 并排比较；只由一个审查者报告的条目优先验证，两个审查者都报告的条目仍需验证出处。每个审查者只收到自己的四栏 task。
