...
Decision ticket 的 `issue 子目录` 只由 tracker 编号决定。建 issue 后立即把 `issue-<编号>` 回填正文，再连阻塞边。Prototype、investigation、evidence 和 scratch 的精确落点分别用下列命令计算，不从任务分支或 worktree 目录推断：
