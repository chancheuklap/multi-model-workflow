...
本 `SKILL.md` 规定所有审查者共同遵守的内容。下表把每个审查任务名映射到一个具体文件。**本 `SKILL.md` 与表中该审查任务名同一行的文件，是你的全部审查方法论。** 被审产物、代码范围和其它材料由 task 的「读」栏提供。表中具体文件点名的 `mmw-tdd` 文件已经装进你的技能目录，照文件名读取——这些文件不算被审材料，是你要拿来判断的标准本身。

## 先认领审查任务

task「目标」栏第一句是**审查任务名**。读取下表中该审查任务名同一行的文件，只读这一份。
...
- **穷尽这项审查任务里的真问题**——表中该审查任务名对应的文件列了看哪些面，能发现的现在全报，不留到下一轮。
- **知道却没报，就是一次失败的审查**，不是谨慎。
- 按表中该审查任务名对应文件的清单逐项走完再写报告。证据表要能看出你覆盖了哪些面，不是抽查了几处。
...
表中该审查任务名对应的文件另外规定了要逐项给结论时，按该文件的要求补在证据表后面。
