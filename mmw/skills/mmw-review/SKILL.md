...
除独立终审和编码规范审外，被审 spec、ticket 或 plan 引用 investigation 资产时，本视角还要拿到 investigation 资产索引，以及判断当前问题所需的验证后报告和配套资产精确路径。先读索引，再读精确路径；不要递归读取 investigation 容器。范围快照已经过期时，把它当历史证据，不当当前事实。
...
任务没有 investigation 资产时，在相关视角的 task「读」栏明写「无 investigation 资产」。有资产时，索引必须列出调查问题、范围快照、结论摘要、文件索引、下游用途和未查清项；缺一项就作为材料缺失处理。
