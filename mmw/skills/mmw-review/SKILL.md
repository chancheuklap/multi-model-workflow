...
| 要两个模型各一个的（⑤ final、⑥ 合并集成审） | 两个角色各派一个，同一份 task |
...
每个（视角 × 角色）一份 task。task 正文为下列四行，**不夹带方法论文件正文**（可写方法论路径）：

```
任务名：<「2. 给每个视角备齐材料」表中的任务名，一字不差>
审什么：<被审对象>
Source：<该视角材料路径，一行一个；没有材料写「无」>
复审：<上一轮审查记录路径；首轮写「否」>
```

方法论：对将派的角色执行 `mmw skill-path <角色>`。有输出则在 task 末加一行，要求进门先读该路径；无输出则不加。

**同一视角要派两个角色时：** 上述四行完全相同；仅方法论路径行可因角色而不同。

角色按本文开头「谁去审」表与红线表选择，只能是 `reviewer-gpt` 与/或 `reviewer-claude`。

可选留痕：`.reviews/<slug>-<哪一道>-<轮次>-<任务名>.prompt.md`，轮次从 1 起。

打开并执行 `/mmw-dispatching-agents` 的「启动」四节：每个（视角 × 角色）启动一次；同一条消息里全部发出，并行、互不通气。审查角色只读，不传可写 cwd。
