...
先认准你在哪一道。**③、④、⑥ 不派审查者**，判据不在本文——那三道的「谁去审」一列各指着一份文件，现在就去读它，本文其余各节跳过。
...
| ③ 逐份验收 | 一个会写文件的角色交回它那份活 | 不派审查者 | **你自己**，判据读 [self-review.md](self-review.md) | 是。不过这一关不合并回任务分支 |
| ④ 合同门 | 全部 plan 合并回任务分支之后，一次 | 不派审查者 | **你自己**，判据读 [self-review.md](self-review.md) | 是 |
| ⑤ final 终审 | 全部 ticket 落地、合同门过了 | 对照终审、独立终审、编码规范审 | 每个视角各派两个模型 | 是 |
| ⑥ 合并集成审 | 几条并行分支要合到一起 | 不分视角，七个角度 | 两个模型各派一个，各走全套。**先解冲突**，读 [merging.md](merging.md)，解完它送你回本文第 1 步 | 是 |
...
写到 `.reviews/<slug>-<哪一道>-<轮次>-<任务名>.prompt.md` 再从那里派，轮次从 1 起。派发机制按 `/mmw-dispatching-agents`，一条消息里全部发出去，它们并行跑、互不通气。
