...
1. **收集上下文。**
   ...
   | 是否已有同一行为 | 每次分诊 | `/mmw-research` 验证的相关实现 | 无关模块 | 验证后的事实；已保存且后续需要时再传 research 路径 |
   | 是否已被否过 | 每次分诊 | `/mmw-research` 验证的相似 `.out-of-scope/*.md` | 不相似的概念 | 验证后的事实；已保存且后续需要时再传 research 路径 |
   | 领域文档和 ADR | 每次分诊 | 本次范围相关文件 | 无关 leaf 和 ADR | canonical 术语和约束 |

   两项 research 并行派发。按 `/mmw-verifying-agent-output` 验证；“没有找到”的结论由主 agent 再搜一次。
