...
| `/mmw-triage` 判出这件事碰多处 | 那张 issue 上有一条 agent brief 评论 | 那份 agent brief 全文，尤其 `Test seam` 那一栏，以及 `/mmw-triage` 的「分诊一张具体的 issue 或 PR」第 3 步验过的断言 |
...
从 `/mmw-triage` 进来的，那份 agent brief 的 `Test seam` 一栏是只碰一处时的初判。碰多处就要在它基础上扩，并说明为什么一个不够。
