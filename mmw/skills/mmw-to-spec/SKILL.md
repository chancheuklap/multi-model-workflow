...
| 从哪来 | 怎么认出来 | 必须取齐的 |
| --- | --- | --- |
| `/mmw-grilling` 谈定 | 这次对话里刚谈完。`/mmw-wayfinder` 横扫下来判定不需要 map 的也走这一行 | 这一轮写进 `CONTEXT.md` 的术语、这一轮落成的 ADR、对话里达成的每一条共识 |
...
两个视角各派一个审查者：设计内容审看这份 spec 本身立不立得住，项目一致性审看它跟项目既有的术语、硬约束、登记机制合不合。两个视角都来自另一个模型。材料怎么备、提示词什么形状、存盘叫什么名字，全在 `/mmw-review`。
