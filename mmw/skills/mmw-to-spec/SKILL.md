...
| `/mmw-prototype` 走查完 | 能从上游取得 `产物目录`；Wayfinder decision ticket 还能取得 `issue-<编号>` | 运行 `mmw artifact path prototype <产物目录> [issue-<编号>]`，先读命令返回目录中的 `README.md`，再只读本 spec 需要的选中产物，以及索引显式引用的对应走查记录和长期证据。普通非 Wayfinder 任务不传 issue 子目录。索引必须列出问题、逐轮用户结论、用户选中的路径、落选变体形成的约束、被提升为长期证据的路径；没有选中产物、落选约束或长期证据时，对应项写「无」，不能省略。缺一项就回 `/mmw-prototype` 补齐 |
| `/mmw-research` 形成 investigation 资产 | 上游交回 investigation 资产索引 `README.md` | 先读索引，再只读本 spec 需要的验证后报告和配套资产精确路径。取结论、范围快照、出处与未查清项；不递归读取 investigation 容器，不把 subagent 原始报告当成事实 |
...
现状结论逐条带 `file:line` 引用写进 spec，**引用要你自己验证过**（`/mmw-verifying-agent-output`）。本轮调查形成 investigation 资产时，spec 同时引用其索引和实际使用的报告或配套资产精确路径。
