...
5. **进工作树。**调 `EnterWorktree({ path: "<上一步 WORKTREE= 后面那段>" })`。这个任务往后每一步都在树里做，状态、代码、旁路记录都存在那儿；留在主仓库的话，下一份技能读不到状态。

6. **把用户的原话写进状态。**打开工作树里的 `.mmw/task.json`，把 $ARGUMENTS 整串原样填进 `note` 字段——开头那个 `fix` 也留着，这里存的是他敲了什么，不是需求本身。这句话别处存不下，下次新开会话，`/mmw` 就是靠它告诉用户这个任务在做什么。
