...
task 是一张四栏表（Markdown 表即可）。**全插件同一形状**；各流程只改格子里的值。

| 栏 | 写什么 |
| --- | --- |
| 目标 | 要交付什么，一句话 |
| 读 | subagent 自己打开的材料：**仓库路径**或 **GitHub issue/PR 编号**（如 `#42`），一行一条。只写定位，**不抄正文** |
| 约束 | 边界与禁止项；没有写「无」 |
| 验收 | 怎样算做完；细节已在「读」里的 ticket/plan/spec 中则写「见 #编号 / 见路径」，不重抄 |

规则：

- 「读」里的仓库路径派前确认存在；编号须是本仓库 tracker 上真实存在的 issue/PR。
- subagent 与主 agent 一样能读文件、能查 issue。路径和编号给对即可。
- 可选留痕：把这张表写入 worktree 的 `.dispatch/<名>.md`（审查写 `.reviews/<名>.md`）。

**完成判据：** 四栏齐全；「读」中每条路径存在或编号可解析，或不适用项已写「无」。
