...
| `local` | 你在主检出里 | 停。请用户用 `grok --worktree=<任务分支名>` 或 `-w` 开新会话。把已经定下的任务分支名、工作名和用户原话写进请用户开新会话的那句话。新会话重新调用本技能，按 `detached` 行 bind。禁止 `mmw task new`。禁止 `herdr worktree create`。禁止 `git worktree add`。禁止用终端 `cd` 代替把会话放进树。 `<任务分支名>` 用这个任务的 slug。本技能上文点名了父分支时加 `--from <父分支>`。工作名从 map 正文的 `## 工作名` 取得。 |
