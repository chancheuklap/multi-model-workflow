...
启动：调用原生 subagent，agent 设为 `mmw-designer`，只读角色加上只读能力，task 传四栏表全文。互不依赖的实例在同一条消息中并行启动。
...
Include both [SKILL.md](SKILL.md) vocabulary and the domain docs `mmw domain path` prints in the brief so each `designer` names things consistently with the architecture language and the project's domain language.
