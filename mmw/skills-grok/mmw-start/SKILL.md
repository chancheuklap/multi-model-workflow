...
| `local` | 你在主检出里 | 先单独确定工作名。禁止 `mmw task new`。在 Herdr（`HERDR_ENV=1`）里：新树用 `herdr worktree create --path ~/.grok/worktrees/<repo>/<slug>`，在该路径启动 `grok`，再运行 `mmw task bind <任务分支名> "<用户原话>" --name <工作名> [--from <父分支或基点 SHA>]`。已有树则把新窗格的 cwd 指到那条路径再启动，同样 bind。不在 Herdr 且还没有树时，请用户用 `grok --worktree=<slug>` 开新会话，再 bind。不要 `git worktree add`。不要用终端 `cd` 代替把会话放进树。 重新运行 `mmw task state`。确认输出是 `bound`。再运行 `mmw task name` 取工作名 |
