...
启动：先在当前任务 worktree 运行 `mmw task state`，确认输出以 `bound` 开头。再运行 `mmw task name` 取得工作名。调用原生 subagent，agent 设为 `mmw-prototype-worker`，打开 worktree 隔离。把四栏 task 作为初始 prompt。工人先运行 `mmw task bind <结果分支 slug> "<目标栏原文>" --name <工作名> --from <基点 SHA>`，再完成工作并提交。交回结果分支名、HEAD SHA、基点 SHA。不要 `mmw task new`。提交前自己跑 `mmw toolchain check --changed-only`。
