...
启动：确认 `mmw task state` 以 `bound` 开头。调用原生 subagent，agent 设为 `mmw-prototype-worker`，task 传四栏表全文。该 subagent 使用当前任务 worktree，不创建结果 worktree。cwd 设为当前任务 worktree 的绝对路径。
