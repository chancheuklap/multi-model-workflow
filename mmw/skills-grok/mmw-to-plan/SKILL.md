...
Launch: call the native subagent tool with agent `mmw-planner` and the four-field task table in full as task. That subagent uses the current worktree; it does not open a result tree. Set cwd to the absolute path of the current worktree.
