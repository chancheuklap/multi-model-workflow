...
这场访谈要写仓库文件：上面那些领域文档和 ADR，以及收尾时的共同理解记录。它们都落在这次任务自己的分支上，所以第一轮提问之前先把树建好。

先运行 `mmw task state`。第一个词是 `bound` 时，运行 `mmw task name` 取工作名，直接开始访谈。
...
宿主要求你停下来请用户开新会话时，就在这里停。这时还没有提出第一个问题，新会话从第一轮谈起，访谈内容一句都不会丢。
...
frontier 为空后，先写下**共同理解记录**，再把它的正文交给用户确认。这份文件是本次访谈结果的唯一出处：给用户看的那一份、送去复核的那一份、写进 spec 的那一份、回填进 issue 的那一份，四处都从它转录。
...
- 设计树上访问过的每个分支，在「决定」里各有一条。frontier 逐轮遍历过，轮数和分支数数得出来。
- 用户纠正过你的每一处，结果都进对应条目。纠正最容易在总结里蒸发：总结写的是结论，而纠正是过程。
...
**等他回答。**他说不要，直接进入下一步。他说要，就移交 `/mmw-review` 走 ⓪ 共同理解审，被审对象是刚写好的那份共同理解记录。
...
| 用户直接发起本次讨论，已经确认共同理解，而且当前工作需要形成 spec | **移交**：`/mmw-to-spec`，交给它共同理解记录的精确路径 |
