---
name: mmw-review
description: 编排审查并处置 findings。用于共同理解、spec、整批 plan 或最终代码到达审查关卡，或用户要求审查共同理解、spec、plan、分支、PR 或未提交改动。
---
...
| ② plan 审 | 一个批次的 plan 写完、本批次合同回填完之后，每批次发起一次 | 覆盖质量审、合规交叉审 | 按第 3 节的宿主审查策略 | 是 |
| ⑤ final 终审 | 全部 ticket 落地 | 对照终审、独立终审、编码规范审 | 按第 3 节的宿主审查策略 | 是 |
...
⑤ final 终审完成第 5 步处置后，如果没有 `accepted`，在记录顶部加入 `终审提交`，值等于被审 HEAD。存在 `accepted` 时，调用方修复完成后再登记修复后的 HEAD。
...
| `needs-evidence` | 可能成立，但报告里没有定位 | 否，补证之前不修不争 |
...
**小改动例外**：三个条件同时成立时，主 agent 直接落地，不派发——改动落在单份产物内部，不动跨 plan 合同、seam 和共享文件归属；不涉及计费、权限、数据迁移和不可逆操作；改动量小到一眼能看完，例如一处文案、一个数值、一行断言。落地后运行该处验收命令并记录。

修复交回「完成」后，调用方按自己的技能合并。这个检查不再启动审查者。
