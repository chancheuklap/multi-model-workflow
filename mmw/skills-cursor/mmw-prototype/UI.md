...
启动：先在当前任务 worktree 运行 `mmw task state`，确认输出以 `bound` 开头。调用原生 Task，agent 设为 `mmw-prototype-worker`，prompt 传四栏表全文。该 subagent 直接使用当前任务 worktree，不创建结果 worktree。互不依赖的实例在同一条消息中并行启动。
