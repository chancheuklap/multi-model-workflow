...
| 项目 | 数量 | 结论 |
| --- | --- | --- |
| 最近一次架构改动 | 取消打包 | 五个宿主由 `mmw/install.sh` 统一散装。技能是软链，改完跑一次安装器，宿主读到的就是新内容 |
...
| issue、spec、plan、ADR | 用户目标、已经确认的决定、ticket、实施步骤和已经交付的设计 | 当前阶段的技能和角色 |
...
MMW 还安装五个辅助技能。它们不属于上述 22 个 MMW 工作流技能。
...
MMW 基于 Matt Pocock Skills 的工程方法构建。对于有上游对应项的技能，Matt Pocock Skills 提供方法、步骤、解释和完成判据。MMW 提供 worktree、tracker、报告验证、领域文档、人工审批关卡和多宿主安装等正式工作流承载。
...
| --- | --- | --- |
| 首次安装 | 授权 agent 安装 MMW，或自己运行 `mmw/install.sh` | 运行 `/mmw-install`：装技能、原生 subagent、hooks、`mmw` 命令和检索依赖 |
| 每个仓库首次接入 | 要求初始化当前仓库 | 运行 `mmw init` 配置仓库，再运行 `mmw doctor` 只读检查本机运行时 |
| 开始或继续工作 | 显式调用当前阶段的技能 | 确认任务分支，执行该技能，出口点名接下来常见的两三个选项 |

调用时点名具体技能，不要只写“使用 MMW”。技能装在各宿主自己的用户级目录，名字没有前缀：Codex 与 Claude Code 写 `/mmw-grilling`，其余宿主按它自己的技能调用方式点名 `mmw-grilling`。下面的表格和示例都写裸技能名。
...
最后确认各宿主读到的都是同一份已安装 runtime。
不要推送或正式发布。
```

升级完成需要同时满足以下条件：

1. 每个已安装的宿主，在它自己的技能列表里看得到 MMW 技能。
