...
| 第 6 步 | 结论进 leaf `README.md`；`## State list`。这一步不点名 `design-pages`，点名的是 `## Next`。上游仍在这一步把 winner 折进真实代码并拆 scaffolding → 不收那一半，收到第 7 步 |
...
| `## Next`（文末） | 我们加的整节：UI prototype 在第 6 步交出，winner 进 Claude Design；点名 `design-pages` 技能的 `references/edit-pages.md`（建用户在里面设计的项目；winner 留在 scaffolding 后面当参照只写在第 6 步第二段，这里不重复）、可选的 `references/design-system.md`（需要时由 Claude Design 里的 agent 从 winner 的代码建，ADR 0030）与 `references/pull.md`（拉回并指回第 7 步）；UI 的 winner 一律进 Claude Design（与 `SKILL.md` 规则 6、ask-matt 一致），所以这一节没有“不进 Claude Design 的 winner”分支。理由：原来全文没有往下一步的出口，唯一的路藏在第 6 步讲 scaffolding 的从句里。design system 不是前置步骤：拉回、合同与验收都不读它，设计本身由用户在 Claude Design 里做（任务板试点 #541，2026-09-21）。上游给 `UI.md` 加收尾步 → 收上游措辞，这一节接在它后面 |
