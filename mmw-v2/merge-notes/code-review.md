...
## The Spec axis locates the handoff package under ## Read first

`references/spec-reviewer.md` § What is not yours: the parenthesised path is gone. A ticket names the handoff package under `## Read first`; that baseline line is the location. Source: #374 Implementation Decisions section 2, #295 (2026-09-12). If upstream rewrites that paragraph → keep the handoff-package rule, locate the package under `## Read first`, put no path in parentheses.
