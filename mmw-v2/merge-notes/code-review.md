...
| 无 | `references/spec-reviewer.md` 末节「The screen contract you do open」之后的「The story adapter you open too」；`references/tests-reviewer.md` 范围段之后确认 `boundary-check.py` 的一条 | 我们加的：Spec axis 核对 story adapter 与合同 `shows` 的字段对应；Tests axis 确认边界测试经 `boundary-check.py`，不自判能否变红。来自 mmw #216 第 8 节（取代原先 #115 的画面轴段）。上游改这两处 → 收上游措辞，adapter / boundary-check 两条保留 |
