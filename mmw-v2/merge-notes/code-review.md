...
| 第 4 步 Spec sub-agent prompt 的 brief（缺项、scope creep、实现得不对三类，每条引 spec 原文） | `references/spec-reviewer.md` 第 3 节 | 三类保留。加了一条我们自己的禁令：不读交接包（`prototypes/<task>/<issue>/UI/`）——照不照交接包由某条验收标准跑的 `visual-parity` 判，是像素与 ARIA 比对，不是读出来的。上游改 brief → 收上游，这条禁令保留 |
