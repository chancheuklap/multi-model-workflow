...
| 无 | `SKILL.md` 第 3 节（in-ticket / out-of-ticket 分类） | 我们加的。三条算 in-ticket：碰本 ticket 的 acceptance criteria、碰 ticket 点名的 spec 决策、碰 ticket `## Read first` 里的 baseline；其余是 out-of-ticket。dispatcher 做这个分类而不是留给读者，因为两类的下一步不同——in-ticket 修一轮，out-of-ticket 开 sub-issue 且不阻塞。baseline 那一条与 `references/spec-reviewer.md` 第 2 节让 Spec axis 读 baseline 是一对，拆开做无效：dispatcher 按这一句的字面条件路由，只加读不改这里，baseline 偏离会被判成 out-of-ticket、开一个不阻塞的 sub-issue，本 ticket 照样关掉。three-round cap 与怎么修不在这里，在 `implement` 的 closing steps |
