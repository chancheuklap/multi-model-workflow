...
| 无 | `references/spec-reviewer.md` 末节「The screen contract you do open」之后的一段 | 我们加的，来自 mmw #115：Spec axis 除了按行审 diff，还审合同画面轴——每个 `--mount` 的 `data-screen` 落在设计页那一块的根元素上，每条 `open` 链能在 diff 渲染的控件上走完。寻址自检只管属性存不存在，视觉验收只在到达画面时间接覆盖 `open`，这是审查覆盖的缺口。上游改这一节 → 收上游措辞，这一段保留 |
