...
| 第 1 步 Gather context | 我们加的：先读父 issue（`gh api repos/{owner}/{repo}/issues/<m>` 的 `parent` 字段，或 `gh issue view` 的父链接），父是 ticket 就读那张票的 `self-run` / `VERDICT` / `REVIEW` 轨迹，不重现。理由：worker 开的 `needs-triage` 挂在本票下，出处在父票的收尾轨迹里。上游改 Gather context → 收上游措辞，先读父 issue 这句保留 |
| 第 5 步 Apply the outcome 的 `ready-for-agent` 一条 | 改成入 landing pipeline，不在这里 publish：判定这张 issue 该由 agent 做之后，用 `/to-spec` 写一份 spec、或经它「Revising a published spec」那一步扩一份已发布的 spec，并把这张 issue 列为来源，再用 `/to-tickets` publish ticket；`ready-for-agent` label 打在那些 ticket 上、不打在这张 issue 上，这张 issue 由 spec 的 publish 步骤关掉并挂到 spec 底下。父是 ticket 时走三去向，每行一条命令：本批次另一张票的活 `gh issue edit <m> --parent <ticket>`；本批次的新工作 `gh issue edit <m> --parent <spec>`，按 `<issue-template>` 补齐八节，`verify-ticket.py --lint <m>` 通过，从此它是票；toolbox 自身的问题 `gh issue transfer <m> chancheuklap/multi-model-workflow`，父子关系随 transfer 断开，出处靠正文 first line。`needs-info` / `ready-for-human` / `wontfix` 只改标签，位置不动。理由：queue 里只能有一种 ticket 体；worker 开出的发现由 triage 回答「在哪做」，不在创建那一刻猜。上游 `AGENT-BRIEF.md` 出的 agent brief 是 `**Current behavior:**` / `**Acceptance criteria:**` 那套粗体行，没有 `## Read first` / `## Seam` / `## Owns`，acceptance criterion 也没有 `CHECK:`；`implement` 按 fixed headings 读 ticket、`verify-ticket.py` 按 `CHECK:` 跑 acceptance criteria，两样都读不了 agent brief，而 `board.py` 只按 `ready-for-agent` label 取 ticket——agent brief 混进 queue 就是 night 里派出去一张没人验得了的活。the four outcomes 的名单不变；同一步 `ready-for-human` 一条的开头是「written here rather than routed」，不是上游的「not an agent brief」：agent brief 不从这个出口出。上游改这一条 → 不收，除非它自己也把这个出口接进 `/to-spec` → `/to-tickets` 的 publish 流程，且自带三去向表 |
