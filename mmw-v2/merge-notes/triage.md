...
| 第 5 步 `**Apply the outcome:**` 标题行 | 注明 the four outcomes 是 `needs-info` / `ready-for-agent` / `ready-for-human` / `wontfix`，留在 `needs-triage` 不算 outcome。`CONTEXT.md` 与 `docs/agents/triage-labels.md` 都写「one of the four outcomes」，而这一节下面列了五条，读者数不出是哪四个。同一行接着写另三个 outcome「move nothing: the parent stays」——只说不改挂父链；不写成「只改 label」，因为 `wontfix` 要关票、`needs-info` 要发评论。上游改这一节 → 收上游的条目，「四个」这个数与 `needs-triage` 不算出口这句保留 |
...
| 开头那句对 agent brief 的定义、`An issue judged ready-for-agent goes into the ticket pipeline` 一段、`**Good:**` 的第一个例子，与末段 `A finding nobody can check is an opinion.` | host 中立：这四处点技能名的地方一律写成散文形式，例子那句里的 `"When a user runs the triage skill with no arguments, …"` 连引号内的措辞一起改。共同理由见 [README.md](README.md#host-中立) |
