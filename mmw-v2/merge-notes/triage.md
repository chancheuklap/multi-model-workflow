...
| 开头「must **start** with this disclaimer」 | 改成放 ticket comment 最后一行。原来的理由是 ticket comment 的 first line 曾是 landing pipeline 的 protocol slot；mmw #315 之后程序只读 comment 末尾的 `<!-- mmw {...} -->` 事件块、不读 first line，但 first line 仍是人在票上扫一眼读到的那一行（`status` 的 `note` 列、night summary 都抄它），免责声明占了它，这张票在哪一步就没人看得出来。上游改这句 → 收上游的措辞，位置留在末行 |
...
| 「Show what needs attention」之后新增的一节「A ticket handed back by this repo's own pipeline」 | 我们加的整节：`needs-triage` 不只承接外来 issue，还承接本仓 agent 没做完的 `ticket.returned` 与合并时被退回的 `ticket.bounced`。前者读 `ticket.checked`、`verifier.passed` / `verifier.failed`、`reviewer.reported`；后者读尝试建立合并的 `origin/<base branch>` commit（`commit`）、`into`、开工后已落地的兄弟票，以及事件里的 `files` 或 `commands`。两者都不按 reporter 的步骤复现、不查 `.out-of-scope/`，而是从已留下的证据推荐四个 outcome。上游若自己写了这两条支路 → 收上游，删掉我们这一节 |
