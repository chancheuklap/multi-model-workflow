...
| 「Show what needs attention」之后新增的一节「A ticket handed back by this repo's own pipeline」 | 我们加的整节：`needs-triage` 不只承接外来 issue，还承接本仓 agent 没做完的 ticket。最新的结果是 `ticket.returned` 事件的（流水线把票交回 `needs-triage` 只走 `--closeout` 这一条路，这条事件的 comment 首行给人读是 `HANDOFF REQUIRED`；判据是事件、不是首行，mmw #315 第 4 节），读那条 comment 与 ticket 上的事件轨迹（`ticket.checked`、`verifier.passed` / `verifier.failed`、`reviewer.reported`），不按 reporter（上游对 user 的叫法，本节沿用同一文件第 3 步的措辞）的步骤复现、不查 `.out-of-scope/`。上游若自己写了这条支路 → 收上游，删掉我们这一节 |
