...
| `### Chart the map` 第 4 步 alignment ticket 那两句，与 `### Work through the map` 第 6 步「On a map with an alignment ticket…」那一句 | 我们加的：destination 含界面时，地图多一张 **alignment ticket**（grilling 类型，被全部决定票和产出 handoff package 的票阻塞），用 `write-screen-contract` 技能写出 **screen contract**；地图「清」的判据加上这张票已关。alignment ticket 的正文第一行也钉死：「用 write-screen-contract 解决：交接包与后端决定逐行对齐，每行 gap 都 aligned 后关票」——它是 `grilling` 类型，而 `## Ticket Types` 对 grilling 写的是 Always 读 `grilling` 与 `domain-modeling`，不钉这一行，接手的 agent 会拿它去跑一轮访谈。handoff ticket 与重做票写在同一步后面，所以这两句不在第 4 步末尾。理由：交接包与决定各自完整、无处汇合，变色龙的界面因此接了空。上游改这两步 → 收上游措辞，这两句接回去 |
| `### Chart the map` 第 4 步的 **handoff ticket** | 我们加的：有界面的目的地除 alignment ticket 外再开一张 **handoff ticket**，即「the ticket that produces the handoff package」：`prototype` 类型、HITL；被 `prototype` decision ticket 与会改变页面状态的 decision ticket 挡住；像点名 `write-screen-contract` 那样点名 `design-pages`；ticket 正文第一行写「用 design-pages 解决：design system 入口先建设计系统，edit pages 入口做设计，你说定稿后走 pull 入口」（三个入口都点名：只点 edit pages 与 pull 时，两边的 session 都不建 design system，写页就在没绑定设计系统的项目里开工）。「Ticket Types」的 `prototype` 一条不改。挡住关系用原有阻塞链接。上游改第 4 步 → 收上游措辞，这一段接回去 |
