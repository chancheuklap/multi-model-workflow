...
| `### Chart the map` 第 4 步的 **handoff ticket** | 我们加的：有界面的目的地除 alignment ticket 外再开一张 **handoff ticket**，即「the ticket that produces the handoff package」：`prototype` 类型、HITL；被 `prototype` decision ticket 与会改变页面状态的 decision ticket 挡住；像点名 `write-screen-contract` 那样点名 `design-pages`；ticket 正文开头一行（在 `## Question` 之上）写「用 design-pages 解决：edit pages 入口建 Claude Design 项目，用户在 Claude Design 里设计，你说定稿后走 pull 入口」（设计由用户在 Claude Design 里做；design system 是可选入口，拉回与验收都不读它，所以这一行不点名它，任务板试点 #541）；它的 leaf `README.md` 放整个界面唯一的一份 state list，汇总每张 UI prototype 票的 winner（`prototype` 的 `UI.md` 第 6 步）。理由：一张 map 常把「长什么样」拆成几张 prototype 票，而 `design-pages` 与 pull 只认一份 state list（任务板试点 #541）。「Ticket Types」的 `prototype` 一条不改。挡住关系用原有阻塞链接。上游改第 4 步 → 收上游措辞，这一段接回去 |
