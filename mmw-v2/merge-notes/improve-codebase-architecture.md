...
| 第一条 bullet 里那两条禁令（`Use these terms exactly …` 与 `don't drift into "component," "service," "API," or "boundary"`） | 收上游原文。范围限定（禁令只管设计散文，命令名、验收判据的类别名、程序打印的字面量各自保留原名）不写在这里：同一条 bullet 让 agent 读 `codebase-design` 技能的 `SKILL.md`，它的 `## Glossary` 首段带着同一句范围限定，见 [codebase-design.md](codebase-design.md)。上游再动这一句 → 收上游 |
