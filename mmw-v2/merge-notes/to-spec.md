...
| 模板里 `## Further Notes` 之前的 `## Sources` 节 | 我们加的：一手来源固定十一类（wayfinder `map`、decision ticket、上游 spec、ADR、research file、prototype 目录、handoff package、screen contract、Domain docs、实测证据、测试规则），每类无则填 `none`。`implement` 技能靠这个节名往回读，改名要同步改 `implement`；`to-tickets` 的 `## Read first` 从这里按 ticket 挑。上游自己加了同类的来源节 → 用上游的名字，同步改 `implement` 与 `to-tickets`，九类保留 |
...
| 第 2 步「读全合同」加 `pages` 与 `scenes`；`## Implementation Decisions` 说明加「story 的定论折进实现它的小节」一段并把「user-story number」从出处清单拿掉；API contract 之后加 **visual acceptance** 一段（只引合同的 `pages`/`scenes`，不复述命令）；`## Testing Decisions` 的 mechanism registry 一句加 `via`/`built_by`、`via: storage` 要点名证明它的 criterion（合同里的 `proven_by` 字段，`align-screens` 的 `contract-format.md`）、数值**与条数**取自 fixtures、机制按状态设计不按第一张票能跑什么设计，并加 **Test surfaces** 一项；**Test surfaces** 里目标参考文件的位置写成 `drive-target` 技能的 `references/targets/`（mmw #158，target 归该技能） | 我们改的，来自 mmw #115。worker 只读票点名的小节，story 里定的东西永远到不了它眼前——实测合同 626 个出处引用里 237 个是 story；两份 spec 各手写一遍命令形状，本次旧形状有三处存活；「按第一张票能跑什么设计种子」正是渲染层假数据的来源。上游改这几节 → 收上游措辞，这几条保留 |
