...
| **visual acceptance** 改为只引 `pages` 与 story 判官；**How a test arrives at a state** 删掉 mechanism registry，改为 story adapter 读 `scenes.json`、边界测试走交互助手、旅程走 `.mmw/target.json`；**Test surfaces** 改为第 6 节七问（起栈、收栈、地址与身份、story 页面、旅程目录、离机动作、实例上限） | 我们改的，来自 mmw #216 第 8 节。上游改这三段 → 收上游措辞，story / boundary-check / journey 的落点保留 |
