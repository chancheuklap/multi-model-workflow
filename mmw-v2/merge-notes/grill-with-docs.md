...
| 正文那一句（全文只有这一句） | host 中立：改成读 `grilling` 与 `domain-modeling` 两份技能的 `SKILL.md`，按它们说的跑这次会话。共同理由见 [README.md](README.md#host-中立)，写法见 `writing-for-agents` 技能的 `SKILL-SET-REVIEW.md` `### Paths, tokens and host neutrality` 与 `### Hand-offs` |
