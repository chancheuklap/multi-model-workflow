...
| 段落 | 我们的意图 |
| --- | --- |
| 「**Keep definitions tight.** One or two sentences max.」 | 文件本身没改，但本仓不守这一条：根 `CONTEXT.md` 是词表与接口契约两用，命令签名、常量表、结构化输出的固定形状都登记在里面，好几条不止一两句。上游把这条写得更硬（例如加机械校验）→ 收上游，本仓仍然不守，在这里记着就够 |
