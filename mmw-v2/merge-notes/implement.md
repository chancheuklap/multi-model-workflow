...
The resume table distinguishes the reviewer start, sleep, report and fix round, then sends a decided worker with no final reverify to step 4 and a worker reverify on `HEAD` to step 5. Every condition is read from ticket events, never session state.
...
If upstream rewrites step 2 → take its wording and put this internal order back. and do not reorder the eight steps.
