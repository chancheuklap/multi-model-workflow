...
| `## Shared experience while implementing` | 我们加的，来自 mmw #427 与 `docs/notes/stage-two-shared-experience-layer.md` 的第 4、5 节。保留两级精确 search command、三项同时成立的 capture gate、map/standalone labels、五项正文、`learning`/`procedure`、安全排除、current evidence authority、supersede/deprecate 和结束报告。它让 empty-context worker 可以执行 start prompt 要求的 search/write/lifecycle contract，而不必返回研究文档。上游改 `While writing code` 到 `/tdd` 之间的流程 → 保留整节，命令与字段不概括 |
| `## Save or correct shared experience` | 我们加的，是 `docs/notes/stage-two-shared-experience-layer.md` 第 5 节已定 `nmem memories add --stdin` 合同的可执行 worker 命令：使用派工传来的 Space、Identity、spec、ticket、task scope，map 额外加 map label，standalone 不重复 task label；同一段给出 `supersede` 与 `deprecate` 的现有 CLI 命令。Memory 写失败须报告，但不挡工单实现。`summary` 的 lifecycle 和 `retro` 的固定 id 写入仍由各自脚本负责，不教 worker 操作它们。上游若在实现流程新增通用记忆步骤 → 收上游措辞，但保留这一节在当前仓库的明确命令与角色分工 |
