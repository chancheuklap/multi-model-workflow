...
The worker waits on a finish notification (`--json`) or `paseo wait <id>` (`--run`);
`paseo logs <id>` when the notification is `errored` or `was closed`; the fallback is
the `code-review` skill in this host's general-purpose subagent. A number in the skill
text is a number a worker shrinks — one did, and skipped a review comment its reviewer
was still writing. Upstream brings a wait command or a timeout number back → drop them,
keep the two paths and the fallback.
...
The live closeout is the eight-step row in the table above. Idle sessions do not cost,
so there is no pane-close step, and `--closeout` archives nothing — `advance` archives
...
Every out-of-ticket item is `<engine> <n> --sub-issue <kind> <file>` parented to this
ticket: `baseline` when the contract does not fit, `outside-owns` when a change outside
**Owns** is merely convenient, `decision` when a question would change what the ticket
delivers, `review` for an out-of-ticket review finding, `pipeline` when the worker hits
a fault in the pipeline itself (`verify-ticket.py`, hook, driver, `.mmw/target.json`) —
the file's body is the command it ran and the output it saw, then stop.
