...
| 「Once done」之后的 closing steps | 我们改的：`self-run`（`verify-ticket` 技能，写完码那一条 run；一条 acceptance criterion 试几轮由 worker 自己判断，closeout 不数轮次，`ABANDON: AC<n> failed` 那一行写清每轮试了什么）→ dispatch 一次 verifier（prompt 只有 `verify #<n>`，不 dispatch 第二次）→ 在票上留一条 `DECISIONS` 评论（首行 `DECISIONS`，两节：`Decisions I made on my own` 到目前为止每一条、`Outside Owns` 抄最新 `self-run` 那一行并每个文件一句为什么；一张票只留一条，review 后的修一轮不再发，closing comment 抄终版——它放在起 reviewer 之前，是因为 closing comment 在 code review 之后才写，reviewer 看不到；Spec axis 逐条判合理与否）→ 用 `dispatch` 技能起 reviewer 并等它的 review comment（一轮、修一轮、不复审；完成条件是 review comment 在 ticket 上；派不出（退出 1 / 2）或等待超时都不跳过：先 `herdr agent read` reviewer 的屏幕，还在跑就再等一次，没起来或已停就用本 host 的通用 subagent 跑 `code-review` 技能，同一份 `SKILL.md` 三轴照跑，报告仍以 `REVIEW` 首行贴上票——每张票都要有一份三轴报告，reviewer 在超时之后才贴的 `REVIEW` 也才有读者；in-ticket finding 修掉、out-of-ticket 的开 sub-issue，修 finding 受 writing rules 同样约束，reviewer 的 suggestion 是加东西时先找可删的）→ `Audit`（重读票与 `## Read first`，每条 acceptance criterion 追到 `EVIDENCE:`，重数 `Counts:`）→ 给被波及的票留评论（对最新 `self-run` `Outside Owns:` 行的每个文件，用 `sub_issues` API 列出同一 spec 下还开着的票，`## Owns` 覆盖该文件的每张票上 `gh issue comment` 一条首行 `TOUCHED BY #<n>` 的评论：改了什么、为什么、Spec axis 的结论；由 worker 做，不加脚本、不加检查——下一个 worker 开工时读全票和评论，这样才知道自己 `## Owns` 的文件已被动过）→ 把只等人拍一句话的 acceptance criterion 切成 `decision` 类 sub-issue、closing comment 写成草稿文件（固定格式：first line `ALL MET` / `HANDOFF REQUIRED`、`Branch: … Commit: … PR: …` 一行且没有 pull request 时写 `PR: none — <理由>`、`Post-verdict:`、每条 acceptance criterion 四行、`Outside Owns:`（每个文件后面带 Spec axis 的结论）、`skipped: [X], add when [Y]`、`Sub-issues opened:` 收本票工作期间开的四类 sub-issue（`the contract does not fit`、`## Owns` 之外的顺手改动、out-of-ticket review finding、`ABANDON: decision`）、`Counts:`、`Decisions I made on my own`）→ 关掉 code review 那一步的 reviewer 跑在里面的 Herdr pane（放在 closeout 之前：review comment 早已在票上，先关不丢东西；closeout 写 `phase=closed` 那一刻 board 就关 worker 自己的 pane，写在它后面的一步来不及执行，每关一张票就留一个 idle 的 reviewer 会话；board 不碰 reviewer 的边界不为一个 pane 破）→ `closeout`（`verify-ticket` 技能，草稿写好那一条 run）由脚本贴评论并关票或换标签，worker 不亲手关票换标签（`hook.py` 拦）。三个 `ABANDON` kind：`failed` 是跑了没过、`stuck` 是跑不起来，两者都不看轮次、都把票交回；`decision` 开 sub-issue 不挡 `ALL MET`。理由：关票是一道门不是一个动作，上游三步（评论证据 → pull request → 关票）被 closing steps 吸收；pull request 那一步整个退场，见下方 `No pull request, and no push`。轮次既不设上限也不设下限：「第三轮就放弃」把还能修好的 criterion 变成 `HANDOFF REQUIRED`，票就此出队留给早上的人；「至少三轮」只让第一轮就确认修不了的 worker 再全量跑两轮，谁也不读那两轮。`Branch: … Commit: … PR: …` 三个值写一行、没有 pull request 时把理由接在 `PR: none` 后面，是为了早上读票的人和以后想解析它们的脚本只面对一种形状；`Sub-issues opened:` 四类逐条点名，是因为正文里要求开 sub-issue 的地方就是这四处，只说「上面两类」会漏掉另外两处。上游改收尾 → 收上游措辞，closing steps 的顺序、`failed` 与 `stuck` 都不看轮次、`--closeout` 这道 closeout、`No pull request, and no push` 必须保留 |
