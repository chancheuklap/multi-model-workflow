...
output it saw, then stop), and so does step 2's `start` that exits 2 (the `dispatch` skill's `references/inside-a-ticket.md`, **Exit codes**, which step 2 sends the worker to).
...
Step 2 is `<dispatch> start <n> reviewer` followed by the end of the turn. The relay wakes the worker with `#<n> reviewer.reported`; the worker reads that event and acknowledges it. Step 4 is the worker final full run in the same session. Nothing is polled. Step 2 says this once, together with `Done means the review comment is on the ticket, never a session's state`; no separate paragraph after step 2 restates it as a prohibition ("never wait on the reviewer, never ask whether it is done").
