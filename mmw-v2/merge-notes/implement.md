...
Steps 2 and 4 are `<dispatch> start <n> <kind>` and then the end of the turn. The relay of the `dispatch` skill (spec #316) watches the ticket and wakes the worker with `#<n> <event>` when the reviewer's or the verifier's result event lands; the worker reads that event on the ticket and acks the wake with `<dispatch> ack <n> <event>`. The paragraph after step 2 says so, and says the event on the ticket is the answer, never a session's state. Nothing is polled: `wait` stays as the one-line read of the event, not as a loop. Upstream has no such wording. Keep it on the next pull.
