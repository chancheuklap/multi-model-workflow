...
| 第 5 步之后的「### 6. Read every ticket back」 | 我们加的整步：发布完逐张回读，核对标题与 What to build 同一片、Blocked by 全是能解析的票号、原生依赖边数与 Blocked by 行数相等、每条 Blocked by 在票的别处有依据、Read first 与 Seam 非空。理由：一次真实发布把 8 张票的标题错位了一格，没有回读就没人发现；后来又发现 25 条边里 6 条只存在于 Blocked by 段，其中 3 条没有任何标准需要，把两张票锁成了假的先后顺序。上游改了步骤编号 → 顺延，这一步永远在发布之后 |
| 第 4 步 Quiz the user 的 **Blocked by** 那一项 | 我们加的后半句：每条边要说出依据（哪条标准需要它的产出，或哪个文件两票都写），说不出就删。理由：这一步是用户批准边的地方，而清单里没有验收标准，用户看不到那些标准会不会需要别的票。上游改这一项的措辞 → 收上游，把「说出依据」接回去 |
