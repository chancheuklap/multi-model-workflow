...
| 第 5 步「A real issue tracker」一条 | 我们改写的：阻塞边必须用 GitHub 原生依赖 API 建立，命令引用 `docs/agents/issue-tracker.md`「Wayfinding operations」；正文 `Blocked by:` 文字不算边；发布时加定级标签；有父 issue 时每张票挂成它的 sub-issue。上游改这条 → 保留「原生依赖、不写正文文字、加定级标签、挂 sub-issue」四点，其余取上游 |
| 「Do NOT close or modify any parent issue」后加的一句例外 | 我们加的。`landing-orchestrator` 只从 `sub_issues` 端点取子票，没有这一步整条落地流水线一张票都派不出去；例外只覆盖建链接，父 issue 的正文与状态仍然不动。上游改这句 → 保留例外 |
