...
| 第 4 步里 interface parity 那条 acceptance criterion | 移进 `references/cutting-interface-tickets.md`。形状仍只在 `ui-acceptance` 技能的 `references/story-parity.md` § The criterion, in one shape，不点名 `verify-ticket` 持有该形状。上游给 `SKILL.md` 加同类模板 → 不收回 `SKILL.md`，接到新文件 |
...
| 第 3 步 `<vertical-slice-rules>` 之后的 **contract ticket** 段、第 4 步 `CHECK:` 来源列表里的界面票判据条、`<issue-template>` `## Read first` 说明里界面票两条基线那几句 | 移进 `references/cutting-interface-tickets.md`。`SKILL.md` 第 3 步只留一行：spec 有 screen contract 时读那份文件。`--lint` 的 `[screen-contract]` 仍在 `verify-ticket`。上游改 `SKILL.md` 这几处 → 收上游措辞接到新文件，`SKILL.md` 仍只留那一行 |
| 第 3 步 contract ticket 段里 `.mmw/target.json` 那句加「filled until `target_config.py --check` of the `ui-acceptance` skill exits 0」 | 这句话在 `references/cutting-interface-tickets.md` 的 contract ticket 段。来自 mmw #158：target 归 `ui-acceptance` 技能，字段由 `target_config.py` 声明并由检查命令打印，contract ticket 的工人跑命令而不是读文档。上游改 `SKILL.md` 合同票段 → 接到新文件，检查命令与技能名保留 |
| 第 4 问「It is.」一支删掉「a screen composed against fixtures instead of the live client」那个例子；第 8 步 `## Owns` 一条加「跨仓库的工具改动不开票、当场改」；界面票 `## Read first` 由行的 `source` 推导（baseline 类出处按文档去重，spec 小节与 story 经 Parent 到达） | 我们改的，来自 mmw #115，#216 第 8 节未推翻的部分。fixtures 例子与「这正是本流水线要抓的失败」直接矛盾，删；推导 Read first 在 `cutting-interface-tickets.md`，target trees 一条已删。上游改 `SKILL.md` 这几处 → 收上游对第 4 问与 Owns 的措辞；界面 Read first 仍在新文件且不含 target trees |
| 开头两门表与 `references/ambiguity-scan.md` | 我们加的。见 `## 找漏 reference`。 |
| 第 3 步 contract ticket 段的旅程落点与其后的 **acceptance ticket** 段 | 移进 `references/cutting-interface-tickets.md`。旅程三种落点仍在；acceptance ticket 按 Testing Decisions 的关键流程切。理由：#415 第 16 节，#447 第 5 节。上游改 `SKILL.md` 合同票段 → 接到新文件，三种落点与 acceptance ticket 段保留 |
...
| 段落 | 我们的意图 |
| --- | --- |
| `SKILL.md` 第 3 步 contract ticket 段与 acceptance ticket 段、第 4 步「A layer with no precedent yet」与两条 story criterion、第 5 步 prefactor 里 contract ticket 那一句、模板 `## Read first` 的界面部分、第 8 步 `built_by` 一句 | 前几段移进新文件；`SKILL.md` 正文只留一行：spec 有 screen contract 时读 `references/cutting-interface-tickets.md`。`built_by` 删除（合同里没有这个键），到达机制仍须在某张 ticket 的 **Owns** 里。上游把规则写回 `SKILL.md` → 不收进正文，接到新文件 |
