...
| 第 4 问拆成两支，`CHECK:` 自带前置状态那一段，第 8 步回读，与 `<issue-template>` 的 `## Seam` 段 | 我们改的一组，针对同一个洞：流水线把「测试够不够得着这个状态」当成要么不问、要么永远不行。上游第 4 问的唯一出路是开一张 `ready-for-human` 的 *reach* 票，而它自己那句「Many of these mean the pipeline is missing a capability, not that the user owes work」说明写的人看见了另一种情形却没给它出口。改成两支：够得着是可以造出来的，那仍是验收标准，但要到达它的东西必须在 spec 的 Testing Decisions 里有名字、并落在某张票的 `## Owns` 里，两样缺一就停下回 `/to-spec`；真的够不着（设备、凭证、人眼）才是 *reach* 票。`CHECK:` 那一段原来只讲分支、票、工作区这些流水线自己的共享状态，扩到系统状态并指向第 4 问，它是这条脉的入口。第 8 步加一行核对每个到达机制都有票认领，并写明 `--lint` 验不了、只能人判。`## Seam` 段补上到达那一半，因为它本来只说了在哪验。上游改这几段措辞 → 收上游，两支、入口那句、回读那行与 `## Seam` 的补充保留 |
...
| 界面对等那一条验收标准 | 我们改的：正文不再抄那条 `CHECK:` 的全文，改成指去 `verify-ticket` 技能照抄，并说明它为什么整行照抄——那是这条流水线里唯一一条交给 shell、中间没有 agent 的命令，所以脚本路径在票里写全。抄两份的代价实测过：`--cdp` 加进来时只改了一处，两份从此不一致。上游给验收标准加同类模板 → 收上游，指去技能这条保留 |
