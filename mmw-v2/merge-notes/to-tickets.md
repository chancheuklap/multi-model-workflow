...
| `<issue-template>` 的 `## Worker` 节 | 我们加的一节：这张 ticket 交给哪一档 worker，附一行理由。`junior-worker` 是默认；`senior-worker` 留给「错了会静默地错」的 ticket（钱要到终态、崩溃后的恢复、装机量已经在读的 contract、安全默认值），因为这些不在写下的当天失败；`## Seam` 已经点名 precedent 的仍走 `junior-worker`。这一节只点 worker 名，模型写在本机活表 `~/.mmw/models.md`，那是模型唯一写下来的地方。理由：worker grade 是 publish 时就该定的事，label 与 `## Worker` 不一致时 `--lint` 报 `[worker-mismatch]`；模型不进 ticket，换模型就不必动已发的 ticket。上游加了同类的一节 → 用上游的名字，同步改 `--lint` 与 `implement` |
