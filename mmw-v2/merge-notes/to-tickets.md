...
| 第 6 步开头的找漏 subagent 与 `Choices` 行 | 我们加的：quiz 前列切分之前跑一轮找漏。prompt 只有一句，接 `Nothing else: the skill is what they read, and the door table is which file they take.`，不逐项列不许放的东西；「scan 返回的问题并进 **Choices**」只写在 `Choices` 那一条里，不另起一行；hold-this-turn 一段留一句理由，不复述「中途结束则清单不带 scan 出去」。理由：#415 第 14 节。上游改 quiz 那一步 → 收上游的问题清单，找漏那段与 `Choices` 的并入保留 |
