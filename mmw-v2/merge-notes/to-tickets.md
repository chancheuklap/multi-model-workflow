...
| 第 7 步的 `Work the **frontier**` 那一句 | 我们改的：frontier 的条件按 `status.py` 算的那一组写全——open、带 `ready-for-agent`、blocker 全部**合入**（不只是关闭）、无 assignee、无活着的 worker。理由：这个词在 `status.py` 与 `dispatch.sh advance` 里指同一组 ticket，写 ticket 的一方与 `status.py` 不能各有一套读法；`## Owns` 的不相交判据与第 5 步末尾的规则用的是另一个关系「两张能同时跑的票」（谁也不阻塞谁，直接或经一条链），正文一律这样写，不叫 frontier；「合入」而不是「关闭」是 mmw #315 第 10 节：被阻塞的票从主干切工作树，blocker 只关了、分支没合，切出来的树里没有它的代码。上游改这一句 → 收上游措辞，五个条件保留 |
| 第 7 步末尾「Close no parent issue; a spec section changes only through the `to-spec` skill's step for revising a published spec.」 | 我们改的：上游写「Do NOT close or modify any parent issue.」，而第 4 步第 2 问、第 6 步都经 `to-spec` 的修订步骤改 parent spec 的小节。这一句只禁关 parent，并指出改 spec 小节的唯一途径 |
...
| 第 8 步 `## Owns` 一条的「A change a ticket needs in a tool skill outside the repository is not a ticket」；界面票 `## Read first` 的推导 | 我们加的。工具箱在使用中改进，跨仓库的工具改动当场改、不开票。界面票的 `## Read first` 由合同行的 `source` 推导（baseline 类出处按文档去重，spec 小节与 story 经 Parent 到达，含 `scenes.json`），写在 `cutting-interface-tickets.md` |
| 开头两个 moment 的表与 `references/ambiguity-scan.md` | 我们加的。见 `## 找漏 reference`。 |
| 第 6 步开头的找漏 subagent 与 `Choices` 行 | 我们加的：quiz 前列切分之前跑一轮找漏。prompt 只有一句，接 `Nothing else: the skill is what they read, and the moment table is which file they take.`，不逐项列不许放的东西；「scan 返回的问题并进 **Choices**」只写在 `Choices` 那一条里，不另起一行；hold-this-turn 一段留一句理由，不复述「中途结束则清单不带 scan 出去」。理由：#415 第 14 节。上游改 quiz 那一步 → 收上游的问题清单，找漏那段与 `Choices` 的并入保留 |
| 第 1 到 8 步各自末尾的 `Done when` 一行 | 我们加的：这份技能的正文是本仓自己的文本，每一步要有一行以 `Done when` 起头的完成判据。第 5 步的判据取自本步那两条规则：每条 `(new)` 路径让它投入使用的文件都在同一张票的 **Owns** 里，能同时跑的两张票不写同一个文件；第 8 步的判据是上面每一项核对都通过。其余各步各写一句：拿到 spec 的 issue 号并读过正文与评论、知道每张票写哪个模块或目录、工作切成纵切片或 expand–contract 的一步、user 批准切分且每个答过的取舍都写进票的 **What to build** 或 spec、每张批准的票都已作为 spec 的 sub-issue 带着 label 与 blocking link 发布 |
...
- contract ticket 交付清单的 element parity 一项，新产品那一支只写「see below」，规则只在其后一段写一次；五问那一段不以「None of it is a judgement written into `## Acceptance criteria`」收尾——那是第 2 问本身。
- 静态守卫与 harness guard 的判据挪到最后一张票：正文只指 `SKILL.md` 第 4 步并说明后面的页面票会加它们扫的组件；留在 contract ticket 上会先红、把批次第一张票判红、`reverify` 重开进 `needs-triage` 的理由见上面 harness-guard 那一行。
