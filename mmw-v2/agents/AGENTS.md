...
- `body.md` 是正文，`agent.json` 是 name、description 与五宿主各自的模型和工具；`mmw-v2/agents/<名>/out/` 是装配成品，进 git。description 写给主线程（何时派、prompt 装什么），`body.md` 写给 subagent（怎么答）。三个 subagent 都只读。
