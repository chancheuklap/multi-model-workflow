...
- A ticket's `CHECK:` line names no path: a shell runs it with no agent in between, `verify-ticket.py` puts the `ui-acceptance` skill's `scripts/` on that shell's `PATH`, so a judge is named bare. Its shapes are in the `to-tickets` skill's `references/cutting-interface-tickets.md` **Criterion shapes**.
