...
5. **Surface the state.** After every action (logic), on every variant switch (UI), or on every run (experiment), print or render the full relevant state so the user can see what changed.
6. **Record the answer, keep the prototype.** Write the verdict and the question it settled into the leaf `README.md`, then fold the validated decision into the real code — rewritten to production standard, with the prototype as reference. The prototype stays where it is, runnable, so the next round of the same question iterates it instead of starting over. When a ticket triggered it, link the leaf directory from the ticket as an asset.
