...
The same effort has one paragraph on **visual acceptance**, and it restates no command: it cites the contract's `pages` and `scenes` — which block of the product each design page is (`mount`), where (`route`), and how a run gets into each scene (`reach`, `open`) — and says that the two judges of the `verify-ticket` skill read those declarations. A command shape written here is a second copy that drifts.
...
- **How a test arrives at a state.** Per layer: what a test writes to put the system into a state, and what it cannot write. A state this feature's behaviour turns on, that the seam's write surface does not reach, gets a line of its own here: the mechanism that will reach it, and which builds carry that mechanism. The mechanism takes the form the repository's own testability rules allow; where those rules have no exit for one, say so — closing that is the repository's to do, not this spec's, and `to-tickets` cuts a *reach* ticket for it. Whoever cuts the tickets reads this section to know whether a criterion can be written at all, and one of them will own building each mechanism named here. With a screen contract, this section is its **mechanism registry**: every mechanism has a name of the form `seed:<state>` (the product put into a state through its own write surface — `via: api`, the default and self-proving; `via: storage`, writing the store directly, is the declared exception and names the criterion that proves the product reaches the state itself — with values **and counts** taken from the handoff package's `data/fixtures.js`), `stub:<seam>-<script>` (an external seam answering by script) or `dev:<capability>` (a registered dev-only capability, outside the view layer), each with the ticket that builds it (`built_by`); the contract's `reach` column and its `scenes` name only entries from here, and the names the alignment ticket proposed are adopted or renamed here in one pass. The mechanisms are designed from the states the behaviour turns on, never from what the first ticket happens to be able to run — designing a seed around what is runnable is where fixtures in the view layer come from
- **Test surfaces.** With a screen contract: how the running product says it is ready, as which identity a test drives it and how that identity is established, which path the seeds write through, and which path an `observe` line reads through — the four answers the contract ticket lands, in the terms of the target's reference file under the `verify-ticket` skill's `references/targets/`
