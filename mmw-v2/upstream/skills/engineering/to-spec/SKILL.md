...
The same effort has one paragraph on **visual acceptance**, and it restates no command: it cites the contract's `pages` — each design page's `mount` is the story page id, `App · ` pages excluded — and says the story judge of the `ui-acceptance` skill reads those declarations. A command shape written here is a second copy that drifts.
...
- **How a test arrives at a state.** Per layer: what a test writes to put the system into a state, and what it cannot write. A state this feature's behaviour turns on, that the seam's write surface does not reach, gets a line of its own here: the mechanism that will reach it, and which builds carry that mechanism. The mechanism takes the form the repository's own testability rules allow; where those rules have no exit for one, say so: closing that is the repository's to do, not this spec's, and `to-tickets` cuts a *reach* ticket for it. Whoever cuts the tickets reads this section to know whether a criterion can be written at all, and one of them will own building each mechanism named here. With a screen contract, a story page is put into a scene by the product's adapter reading `scenes.json` data; a boundary test puts the control on screen through the shared interaction helper; a journey starts the product through `.mmw/target.json`.
- **Test surfaces.** With a screen contract: the seven answers that make the repository a runnable environment, in the terms of the `ui-acceptance` skill's `references/product-answers.md`: how the stack starts, how it stops, the addresses and identity a run uses, the story pages, the journeys directory, the actions that leave the machine, and the instance cap. The contract ticket lands them.
- **Cross-ticket flows** (optional): one line per user flow that spans tickets, naming the flow — the directory name under `.mmw/journeys/<flow>/` — and the Implementation Decisions section numbers it involves. `to-tickets` cuts one acceptance ticket per line. Omit the bullet when the repository's `.mmw/target.json` cannot start the whole product.
