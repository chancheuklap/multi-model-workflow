...
- **Test surfaces.** With a screen contract: the seven answers that make the repository a runnable environment, in the terms of the `drive-target` skill's `references/runtime-environment.md`: how the stack starts, how it stops, the addresses and identity a run uses, the story pages, the journeys directory, the actions that leave the machine, and the instance cap. The contract ticket lands them.
