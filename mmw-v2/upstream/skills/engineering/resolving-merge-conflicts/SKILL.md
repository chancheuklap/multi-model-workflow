...
description: Use when an in-progress merge or rebase has conflicts, or when a clean merge makes the repository checks fail.
---
...
2. **Find the primary sources** for each conflict or failing interaction. Understand deeply why each change was made, and what the original intent was. Read the commit messages, check the PRs and check original issues/tickets. After a clean merge of `origin/<base branch>`, identify each ticket represented by its new first-parent merge commits, then read its ticket and closeout evidence before changing the combined behavior.
