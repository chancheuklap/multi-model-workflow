...
| **Handoff**  | The `handoff` skill writes a portable markdown file and seeds a session anywhere with it. |
...
**5. Otherwise, compact.** Relevant context, same harness, same directory, and you need to stay in the loop: this is where the tree lands, and it lands here often. Pass the compaction an instruction ("we're going to QA this area") so the summary keeps what the next phase needs.
...
| Secondary (compacting, a handoff document) | Lossy | Less | Lots  |
