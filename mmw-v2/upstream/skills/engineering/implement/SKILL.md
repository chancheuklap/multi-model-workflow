...
2. Post the decisions comment, once: `<engine> <n> --decisions <file>`, in the `verify-ticket` skill's `references/closeout.md`. The file is two sections, each opened by its `## ` heading: `## Decisions I made on my own`, with every line written so far, one per line, in the shape the closing comment uses; and `## Outside Owns`, with the `Outside Owns:` line of your newest run (its `ticket.checked` event, run `self`), followed by one sentence per file saying which criterion could not pass without it; `None` when that line is `None`. It is posted here, before the reviewer starts, so the review judges every line of it, and once: the review's fix round adds nothing to it, and the closing comment carries the final version.
