...
6. Tell the tickets whose files you changed: `<engine> <n> --touched`. It reads the review comment, so a run before step 2 is done is refused: finish step 2 first.
7. Cut loose what only a person can settle, then write the closing comment to a draft file. A criterion that waits only on one sentence from a person: write `ABANDON: AC<n> decision <question, options, and the default if nobody answers>` **and** run `<engine> <n> --sub-issue decision <file>`, then keep working the rest — the ticket does not stop. Then `<engine> <n> --draft`, which prints the path it wrote as `DRAFT: wrote <path>`, and fill in every `<fill>` the skeleton leaves you. Give that run no path of your own.
