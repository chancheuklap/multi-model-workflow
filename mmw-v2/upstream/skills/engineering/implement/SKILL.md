...
4. **Run every gate, one at a time.** A runnable gate passes only on both conditions: exit code 0 and the `EXPECT:` marker in the output. When it passes, tick the criterion in the ticket body, add an `EVIDENCE:` line under it holding the smallest decisive slice of the output, and set that gate's `checked: true` and `evidence` in the state file. A manual gate is not yours to tick: leave it unchecked and name its adjudicator in your final report. A ticked box without evidence counts as unticked.
