...
Before writing any code, check the ticket is the one to work: its title and **What to build** describe the same slice, and every ticket under **Blocked by** is closed. If either fails, stop and report — do not start on a ticket that is mislabelled or still gated.
