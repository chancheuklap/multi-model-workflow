...
Reached from step 3 of [`SKILL.md`](../SKILL.md) when the spec has a screen contract. The five kinds of ticket, the shared journey helper, the extra *reaction* ticket, and what to do after the rows change, are all here.
...
Journeys appear on the contract ticket, on tickets the owner named, and on each acceptance ticket. Cut no acceptance ticket when the repository's `.mmw/target.json` cannot start the whole product.
...
Owns by design page, `Component · ` pages. One story criterion (element parity) covers the mounts of the pages it owns. Each owned row whose `calls` is not `none`, or whose `next` is not `stay`, gets one boundary criterion; rows that share a test file may share one. The product's test asserts the four columns.
...
The rest of **Read first** is derived from those row ids, not hand-picked. Every `source` of the owned rows that is a baseline — a decision ticket (`#<n>`), an ADR, a domain document under `docs/` — listed once per document, with a word on what it settles. Spec sections and stories reach the worker through **Parent**.
...
When two or more journey tickets in the batch — **acceptance ticket**, or another ticket that names a journey — need the same product access (bringing the stack up, a health check, sign-in, a top-up), the **contract ticket** (when there is one) or the first journey ticket in the batch creates one helper module under `.mmw/harness/` and lists it under its **Owns**. Later journey tickets import it.
