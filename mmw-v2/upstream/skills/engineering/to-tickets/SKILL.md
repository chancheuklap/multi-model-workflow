...
**The contract ticket comes first when the spec has a screen contract.** It lands the empty shell and one precedent per layer, and nothing behind them: every `route` the contract declares opens and lands on an empty surface carrying that page's `data-screen` mount; the product answers `ready`; the reach script runs every mechanism the contract's table names; one passing minimal test per test layer, so every later ticket has a precedent to open and a real success line to copy; the test surfaces (readiness, identity, write path, read path) written into the spec's Testing Decisions; the repository's `.mmw/target.json`, filled until `screen_driver.py target --check` of the `drive-target` skill exits 0 in that repository. What else it lands is the target's to say — that skill's `references/targets/<kind>.md`: on an electron target the shell, the backend, the client routes, the OpenAPI export and the generated renderer types; on a server-rendered site the templates, with no generated client and no OpenAPI for page routes; on an extension the manifest and an entry page. It carries one criterion that needs no interface at all, the **addressing self-check**: for every scene declaration, run its `reach`, fill its `route`, navigate, and assert an element with `data-screen="<mount>"` is there — which proves the whole addressing model (the state can be put, the placeholders fill, the route is reachable, the mount is where the contract says) against an empty surface, and proves nothing about look or copy, which is right; `open` is first performed on the ticket that builds the surface, and that is the real floor. Every other ticket in the batch is blocked by it.
...
- A ticket that owns screen-contract rows carries, for every row whose `calls` is not `[none]`, one **wiring criterion**: the fixed shape in the `verify-ticket` skill's `references/wiring-check.md` — the contract and the row ids, nothing about the machine — which puts the product into each row's `reach` state, triggers the row's control on the production route tree, and reads the row's `observe` surface. The parity criterion names the `data-screen` values (`--mount`) the ticket owns, and the scenes it covers follow from them: **scenes belong to tickets by mount**, never by the rows that mention them (one scene is listed by rows of three components; by rows it would belong to three tickets and fail on each). When mounts collide — a target with no component pages, every scene whole-surface — scenes belong by `route`. When one page's scenes must be split between two tickets, each names its subset with `--scenes`; that is the only way to split, and `--lint` checks that across the batch every scene of the contract is covered exactly once by tickets that could run side by side (a ticket blocked by another may re-run that one's scenes). A ticket that uses a mechanism — through the `reach` of its rows or of the scenes under its mounts — is **blocked by that mechanism's `built_by`** ticket, unless it is that ticket; `--lint` checks the link. A `CHECK:` that stubs the application's own network — `vi.stubGlobal('fetch')` and its kind — is not a wiring criterion; it passes with the control wired to nothing.
...
- Every `source` of the owned rows that is a baseline — a decision ticket (`#<n>`), an ADR, a domain document under `docs/` — listed once per document, with a word on what it settles. Spec sections and stories are not copied here: they reach the worker through **Parent**, which names every Implementation Decisions section the rows cite, and a copy here would be the second statement of the same thing.
- The **target trees** of the ticket's design pages: row id → the row's `component` → the `Component · ` page whose `pages` entry declares that component → `docs/specs/<effort>/targets/<page>.aria` and `<page>.classes`, one line each, marked as a baseline. Never by prefix-matching **Owns** paths against `component` values; that is string matching and the one way this goes wrong. A page whose mount no ticket owns is a lint finding on the batch.
