...
- A criterion that compares an interface against a downloaded handoff package is written in one fixed shape, path and all, in the `drive-target` skill's `references/story-parity.md` § The criterion, in one shape. Copy it from there rather than composing it: that criterion is the one command in this pipeline handed to a shell with no agent in between, so the path in it is written out in full on purpose and every part of the line is load-bearing.
