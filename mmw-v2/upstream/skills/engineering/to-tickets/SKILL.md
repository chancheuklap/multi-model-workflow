...
Iterate until the user approves the breakdown. Write each answered choice that only changes what one ticket delivers into that ticket's **What to build** before publishing. Write each answered choice that changes a decision in a spec section back through the `to-spec` skill's `references/revising-a-spec.md`, then into the tickets.
