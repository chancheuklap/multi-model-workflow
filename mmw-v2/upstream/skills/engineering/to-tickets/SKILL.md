...
**The contract ticket comes first when the spec has an API contract subsection.** It lands the contract layer and nothing behind it: the request and response models, the route signatures answering `501` until a later ticket fills them, the repository's OpenAPI export and generated client types, the seed script and external-seam stubs the mechanism registry names. Every other ticket in the batch is blocked by it, so the interface tickets are written against generated types and a backend that can be put into any `reach` state before the first one starts.
...
- A ticket that owns screen-contract rows carries, for every row whose `calls` is not `[none]`, one **wiring criterion**: the fixed shape in the `verify-ticket` skill's `references/wiring-check.md`, which puts the real backend into the row's `reach` state, triggers the row's control on the production route tree, and reads the row's `observe` surface. The parity criterion above covers the same rows' `scenes`. A `CHECK:` that stubs the application's own network — `vi.stubGlobal('fetch')` and its kind — is not a wiring criterion; it passes with the control wired to nothing.
