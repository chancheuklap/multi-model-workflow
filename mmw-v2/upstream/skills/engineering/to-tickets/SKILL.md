...
4. Ask who reads the criterion, then take one of three routes. **An agent, and you can write the command** — give it the command that decides it and the string a passing run prints; the engine runs it, ticks it, and writes the evidence. **An agent, but no command exists** (judging a report the agent itself can fetch, judging whether a passage says enough) — leave the command off; whoever works the ticket judges it and, at closing time, ticks it and writes what they read and concluded. **A person** — see the next paragraph. If no command exists because the spec never decided how this is verified, stop and return to `/to-spec`. Do not invent it.

**Work only a person can judge is its own ticket, not a criterion on someone else's.** Whether a page reads clearly to a newcomer, whether a screen matches a baseline by eye, whether a wording is right — leaving those on an agent's ticket leaves it unable to finish. Write one ticket per such judgement, labelled `ready-for-human`, blocked by the ticket that produces the thing being judged, and say in one line why it cannot be delegated: a judgement call, access only a person has, a design decision, or testing by hand.
