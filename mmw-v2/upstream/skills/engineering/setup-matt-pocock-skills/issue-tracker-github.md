...
- **Frontier query**: list the map's open children (`gh issue list --state open --limit 500`, scoped to the map's sub-issues / task list read with `--paginate`), drop any with an open blocker (`issue_dependencies_summary.blocked_by > 0`, or an open issue in the `Blocked by` line) or an assignee; first in map order wins.
