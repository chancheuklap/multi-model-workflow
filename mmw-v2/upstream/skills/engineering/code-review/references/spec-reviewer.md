...
**The contract's screen axis you open too.** For every `data-screen` value the ticket's parity criterion names under `--mount`, the diff carries that attribute on the element the page's `pages` entry means — the root of that design page's block, not a wrapper around it and not a child — and for every scene under that mount, the `open` chain the contract declares can be performed on the controls the diff renders, ending where the last row's `next` says. The addressing self-check proves only that the attribute exists somewhere, and interface parity covers `open` only when the scene is reached; a mount on the wrong element, or an `open` step whose control the diff does not render, is **Built wrong**, quoted from `pages` or `scenes`.
