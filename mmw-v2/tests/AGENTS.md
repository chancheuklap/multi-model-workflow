...
Every test of the toolbox's own scripts (the own-script layer), one directory per skill or subsystem, plus `lib/` for the `-k` parsing and unittest verdict three of those entries share; it exists only in a checkout and is never symlinked into a host. gate-check's own tests live in the `mmw-v2/upstream-unlazy/` subtree, not here.
