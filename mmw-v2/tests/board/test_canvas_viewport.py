...
WHERE_SELECTED = """() => {
  ...
  if (!card) return null;
  const canvas = document.querySelector('[data-screen="canvas"]').getBoundingClientRect();
...
def pan(page, dx, dy):
    ...
    box = page.locator('[data-screen="canvas"]').bounding_box()
...
def read_github(page):
    with page.expect_response("**/api/board/refresh"):
        page.locator('button.gear[aria-label="立刻重读 GitHub"]').click()
...
class CanvasViewportTest(unittest.TestCase):
    def test_the_viewport_survives_every_redraw_of_the_same_task(self):
        ...
        with RunningBoard(environment={"PATH": path}, fixture=data) as board, \
            ...
            page.locator('.card-hit[aria-label="#12 blocked work"]').click()
    ...
    def test_a_card_picked_off_the_canvas_is_panned_into_view(self):
        ...
        with RunningBoard(environment={"PATH": path}, fixture=two_tasks()) as board, \
            ...
            pan(page, -640, 0)
            zoom = page.locator(".zoom-level").text_content()
            ...
            self.assertEqual(page.locator(".zoom-level").text_content(), zoom,
