...
import {fromBoard, render} from "../../board/page/detail.mjs";
...
const ticketView = {
  ...
  blockers: [{n: 132, num: "#132", lamp: "ink", title: "中继进程骨架",
    where: "", state: "landed", phase: "landed", hold: false, unknown: false}],
  blocks: [{n: 135, num: "#135", lamp: "hollow", title: "投递回执",
    where: "", state: "queued", phase: "queued", hold: true, unknown: false}],
  kids: [{num: "#150", kind: "contract", title: "spec 没写队列为空时中继读什么",
    lamp: "orange", hot: true}],
  rawEvents: [
...
test("empty view shows the empty copy and no GitHub button", () => {
...
test("a ticket view shows GitHub, origin, status, held relations, and phase blocks", () => {
  const {root} = mount(ticketView);
  ...
  assert.equal(walk(root).filter(node => node.tagName === "BUTTON")
    .some(node => accessibleName(node).includes("#150")), false);
...
test("close and Escape call onClose", () => {
  let closed = 0;
  const {root, document} = mount(ticketView, undefined, {onClose: () => { closed += 1; }});
...
test("ticket numbers in the panel call onGoto, unknown rows do not", () => {
  const seen = [];
  const view = structuredClone(ticketView);
  view.blockers.push({
    n: 399, unknown: true, num: "#399", lamp: "none",
    title: "不在这棵树里，读不到它的状态", state: "unknown", hold: false,
  });
  const {root} = mount(view, undefined, {onGoto: n => seen.push(n)});
...
test("GitHub uses a new tab and does not call the API client", () => {
  ...
  const {root, window} = mount(ticketView, api);
...
test("poll repaint keeps the detail root and an opened event", () => {
  const {host, root} = mount(ticketView);
  namedButton(root, "22:38 Worker started grok grok 4.6, xhigh effort").click();
  assert.match(root.textContent, /w94b8/);

  const repainted = render(host, ticketView);
