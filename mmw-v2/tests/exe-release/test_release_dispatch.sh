...
new_case() {
  ...
  # 这个仓库里的文件是故意写坏的 fixture(migrations/0001.py 不是合法 Python),
...
[ -s "$repo/events.jsonl" ] && while IFS= read -r event; do
  printf '%s\n' "$event" | UV_CACHE_DIR="${UV_CACHE_DIR:-/private/tmp/mmw-uv-cache}" uv run --quiet "$SCRIPT_DIR/../../skills/exe-release/scripts/release_contracts.py" validate-event -
