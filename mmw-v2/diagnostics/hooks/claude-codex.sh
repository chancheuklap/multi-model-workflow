#!/usr/bin/env bash
# Claude Code 与 Codex 的 PostToolUse：编辑类工具跑完之后，对被改的文件跑一遍检查器。
#
# 这两家的 hook 合同一样：退出码 2 加 stderr 表示「把这段话交回 agent」。其他情况
# 一律 0——诊断跑不起来不该挡住干活。
#
# Codex 没有原生诊断，这个 hook 是它那一侧唯一的推送通道。Claude Code 装了官方的
# LSP 插件时会自己推类型错误，但 ruff、oxlint、shellcheck 和密钥扫描不在里面。
#
