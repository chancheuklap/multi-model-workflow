...
| `on_failure` | Failure kind → the outcome, then optionally ` — ` and what the user sees there in words. The outcome is where the user is after that failure, in the words `next` uses (a row id, a scene, a name in `states`, `stay`), or `toast:<KEY>` for a message over an unchanged page: `version_conflict_409: note-changed — the banner names the time the note changed`. Every non-`none` call has at least one. The four-column boundary test of the ui-acceptance skill reads this column. | present when `calls` is not `[none]`; a mapping; each outcome is one of those |
