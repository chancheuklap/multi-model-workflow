...
On the **incremental** branch the survey covers only `scope.md`: for each listed `AGENTS.md`, its change set and the lines in that file the changes bear on. Dispatch one directory group per listed file and no topic groups.
