...
You are collecting the facts the `AGENTS.md` files will be written from. Everything that ends up in a file traces back to an entry in the **survey list** you build here, or to a maintainer's answer later. So the survey covers the whole repository; the shortlist below is only what every surveyor reads first.
...
Merge every report into one file, `survey-list.md` in the scratch directory: every entry kept with its evidence, exact duplicates dropped, sorted by place with `root` first. This file is the **survey list**; every later step reads it and nothing else from the survey. [ask.md](ask.md) appends the maintainer's answers to it as entries of type identity and purpose.
