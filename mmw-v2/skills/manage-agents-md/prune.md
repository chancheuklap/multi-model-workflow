...
All branches. The files are written. Now read each one you wrote or edited line by line: first against the list of what must not be there, then with the four tests under Pruning, then through the self-check. Only information that will genuinely help future sessions stays; the context window is precious, and every line must earn its place.
...
2. **Discoverable from code** — any instruction the agent can learn from existing code patterns. LLMs are in-context learners: if the codebase consistently uses a pattern, the agent follows it after a few searches.
...
5. **Verbose explanations** — the long form of a one-liner.
...
7. **Code snippets** — they go stale and bloat the file. Use file path references instead.
   Bad: a ten-line example handler. Good: `see src/api/users.ts for the pattern`.
...
10. **Welcome text** — intros, conclusions, pleasantries, long prose explaining why instructions matter.
    Bad: `Welcome! This file helps you work effectively in our codebase.`
11. **Nested repeats** — a nested `AGENTS.md` line that repeats a root instruction.
...
| Gotchas | Are the gotchas and quirks the survey and the maintainer gave all captured? |
