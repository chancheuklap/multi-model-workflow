...
**你不自己写这次修复。** 你在前四个 Phase 里拿到了工人拿不到的东西——一个 red 的 loop、最小化的 repro、验证过的那条假设。把这些交给它。
...
**没有 correct seam，这件事本身就是发现。** 把它记下来，按本文「下一步」一节处置。代码架构挡着，这个 bug 锁不住。
...
| 复盘的答案牵涉架构改动（没有好的测试 seam、调用方互相缠绕、藏着的耦合） | **移交**：`/mmw-improve-codebase-architecture`，把具体情况带过去当它的扫描方向，它就不用再从提交历史猜热点 |
| 找不到 correct seam | **移交**：`/mmw-improve-codebase-architecture`，把「找不到 correct seam」这件事带过去当它的扫描方向。「这个 bug 锁不住」本身就是架构层面的发现，不是这次修复的失败 |
| 工人卡在 bug 与代码互相矛盾上 | **停**：它报告的是这次诊断的问题，不是它自己的问题。把矛盾交给用户，不要换一个工人再派一遍 |
| 这次修复要审 | **移交**：`/mmw-review`，起一轮 ⑤ final 终审，固定点取派工人之前那一条提交 |
