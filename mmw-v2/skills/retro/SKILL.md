...
`<retro>` in every command below is `python3 <absolute path to scripts/retro.py>` next to this file. Resolve it from this file's own location. The entry derives repository Space from origin and verifies that Nowledge reads it back as `shared` with only `mmw-toolbox`; an explicit `NMEM_SPACE` must match that id. The base commit is the merge-base of the `spec.opened` project and base branches, before `finish` merges them.
