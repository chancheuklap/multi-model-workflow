...
A proposal asks for approval and changes none of these destinations. A mechanical invariant belongs in a check or script; a short repository-wide, non-inferable navigation or universal instruction belongs in that repository's `AGENTS.md`; a repeated multi-step workflow specific to one repository belongs in a repository-local skill with a discovery test; stable cross-repository review behavior belongs in an active reviewer Rule; a cross-repository MMW workflow belongs in an MMW skill; approved, broadly useful knowledge is copied into toolbox Memory while the source Memory remains in the repository. Done when each proposal names one responsible repository, its supported destination and the owner decision still required.
