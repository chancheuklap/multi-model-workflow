...
6. 这次需求背后有原型的，给出**选中产物**在这个 worktree 里的路径，加上 spec 里那一节视觉契约。只给选中的那一份：`docs/prototypes/<slug>/` 下面还躺着落选稿和终端界面那层壳，全给它会照错。同时说清怎么用——逻辑原型里那个可移植模块整块搬过去，不要重写；界面变体的代码按仓库规范重写，不要照抄。
