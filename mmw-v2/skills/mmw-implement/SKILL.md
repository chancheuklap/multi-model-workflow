...
每张 ticket 都关闭、改动都在任务分支上之后，按 `/mmw-review` 的 **④ 合同门**核一次：spec 那节合同边界里声明过的每条，在合并后的代码里真兑现了——提供方的定义 grep 得到行号，消费方的调用签名对得上，版本一致、迁移在位、登记闭合。

**核不出行号就不算兑现**，那是落地漏了，回去补，不要留给终审。spec 里没有这一节，把「本次无跨边界合同」连同你据以确认的位置记下来，不要跳过不写。
