...
6. **这张 ticket 对应的那份 plan，全文。** 三样各有各的用处，缺一样工人就得自己发明：spec 给意图和合同，ticket 给边界和验收，plan 给施工权威——改哪些文件、消费和产出什么接口、红绿步骤怎么走。只给 plan 不行，它看不见意图；只给 ticket 也不行，它手上就没有施工图。走 agent brief 那条路的需求没有 plan，这一条跳过。
7. 这次需求背后有原型的，给出**选中产物**在这个 worktree 里的路径，加上 spec 里那一节视觉契约。只给选中的那一份：`docs/prototypes/<slug>/` 下面还躺着落选变体和 TUI 壳，一起给它，它会照着错的那份做。同时说清怎么用——逻辑原型里那个可移植模块整块搬过去，不要重写；界面变体的代码按仓库规范重写，不要照抄。
...
每张 ticket 都关闭、改动都在任务分支上之后，按 `/mmw-review` 的 **④ 合同门**核一次：spec 的 `## Cross-Plan Contract Anchors` 一节里每条跨 plan 合同，在合并后的代码里真兑现了——提供方的定义 grep 得到行号，消费方的调用签名对得上，版本一致、迁移在位、登记闭合。

**核不出行号就不算兑现**，那是落地漏了，回去补，不要留给终审。spec 里没有这一节（只有一份 plan，或者写明了无跨 plan 共享合同），把这件事连同你据以确认的位置记下来，不要跳过不写。
...
| 所有 ticket 都关闭了 | **自己继续**：走第 6 步核合同，过了再走第 7 步起 ⑤ final 终审 |
| 第 6 步有合同核不出行号 | **停**：报是哪条合同、提供方或消费方缺在哪。这是落地漏了，不是审出来的意见 |
