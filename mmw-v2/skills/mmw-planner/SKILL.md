...
**seam 由 spec 定死，你不重新定。** plan 里每条测试的落点锚到 spec 那份 seam 清单，选最高的那一层，不要增殖插桩点。spec 里找不到对应的 seam，交 `needs-context`。
...
**每个任务包必须能单独抽出来当一份自洽 brief。** 落地的人通常只看自己那一包，不读全文，还可能乱序读。所以：不写「跟第 N 包一样」（重复写出来）；不引用本包和前文都没定义过的类型、函数、字段；要传给下一包的信息写进本包的 Interfaces，不靠「看上一包」。
...
2. **写 plan 头部**，照这个骨架：
   ...
   **Create / Modify / Test / Docs·登记·迁移：** `path` — 负责什么 / 什么行为 / 为什么改它
   ...
   ## 发布风险与人工审批关卡
   | 风险面 | 任务包 | Risk flag | 提前审 | 人工审批关卡由谁把 |
   ```
