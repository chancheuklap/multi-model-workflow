...
PROPS = {                              # every key here appears in the Tweaks panel
  "scenario": {"editor": "enum", "default": "saved", "tsType": "string", "options": ["saved", "service-down"]},
}
...
LOGIC = '''        init(props) { return { path: (this.fx().defaultPath) || "" }; }   // runs before fixtures arrive: return a skeleton
        ...
        renderVals() {
          return { toast: this.state.toast, path: this.state.path, serviceDown: this.props.scenario === "service-down",
