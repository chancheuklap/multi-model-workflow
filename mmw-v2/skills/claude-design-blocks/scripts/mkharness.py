"""Generate a single-component harness: a <select> drives the component's scenario prop. Usage: mkharness.py <name> '<JSON array of scenarios>' [extra dc-import attributes] -> Harness.dc.html (override the file name with DC_HARNESS)"""
import os, sys, json
...
<select class="tb" id="scenario-select" onChange="{{{{ pick }}}}"><sc-for list="{{{{ options }}}}" as="o"><option value="{{{{ o }}}}">{{{{ o }}}}</option></sc-for></select>
<dc-import name="{name}" scenario="{{{{ cur }}}}" {extra} hint-size="100%,100%"></dc-import>
</x-dc>
