...
7. **Finish.** Delete `Harness.dc.html` and any obsolete pages with the claude-design MCP tools `finalize_plan` (`deletes`) and `delete_files` (with etags); keep `src`, `styles`, and `data` in the consuming repository; give the user the `open_url` of every page.
   ...
   When the port started from a prototype leaf directory, download the handoff package into that same leaf directory: the `README.md` Claude Design generates, every component `.dc.html`, `styles/`, `data/`, and `support.js`. Those downloaded files are what the implementation is later held to, so nothing else may be written over them. Beside them, write `scenes.json` — one entry per scene with `name`, `page` (the `.dc.html` it pins) and `props` (the query parameters that put the real page in that state). A scene name may not contain `/`: `visual-parity.py` serves each scene from a page at `/__parity-<name>.dc.html` that loads `./support.js`, and a name with a slash puts that page in a subdirectory that has no `support.js`. The `.dc.html`, `styles/`, `data/`, `support.js` and `scenes.json` are the five files `visual-parity.py --baseline` renders, so each scene can be rendered later without opening the project.

Beside them, write `IMPLEMENTATION-CONTRACT.md` as well. Those `props` are query parameters aimed at the implementation, which means the implementation has to answer them and put itself into the scene named — a capability the product has to grow, and one interface parity does not bring with it. Say that in a paragraph, and say that where the capability lives and which builds carry it belong in the consuming repository's spec, under `## Testing Decisions`. Whoever writes that spec and the agent who cuts its tickets both open this directory; neither of them opens this skill. The `DESIGN.md` uploaded as the design system stays in the consuming repository as well.
...
- `support.js` wraps every `{{ }}` text in `span.sc-interp`, so an element selector such as `.x span` matches it by accident — a second reason to flatten the CSS.
