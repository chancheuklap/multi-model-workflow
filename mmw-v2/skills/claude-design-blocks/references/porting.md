...
1. `list_files`.
2. `finalize_plan`: declare `writes` as globs (`*.dc.html`, `styles/*.css`, `data/*.js`); pass `deletes: []` — the call fails without it; set `localDir` to `work/`.
3. `write_files` with `localPath` **and `mimeType`** (`text/html`, `text/css`, `text/javascript`).
4. Without `mimeType`, CSS and JS are served as `text/plain` and rejected.
5. One plan accepts repeated writes until it expires.
