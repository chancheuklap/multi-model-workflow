...
把项目 issue tracker 上的 issue 推过一台小状态机。
...
PR 在范围内时，把外部 PR 也放进这三堆，每行标 `[PR]` 或 `[issue]`。这一步只把*外部* PR 捞出来（谁算外部由 tracker 配置定义）——协作者手上正在做的 PR 不是分诊的活。这道筛子只管发现环节；被点名的 PR 一律分诊，不看作者是谁。

给出每堆的数量和每条一行的摘要，让维护者挑。

## 分诊一张具体的 issue 或 PR

1. **收集上下文。** 把这张 issue 或 PR 整个读完（正文、评论、标签、作者、日期；PR 还要读 diff）。把之前的分诊记录解析出来，已经解决的问题不要再问一遍。用项目的领域术语表探索代码，尊重这块地方的 ADR。对着代码跑两项检查：(a) **是否已有**——按领域概念（不是按需求的字面措辞）搜有没有已经实现的同一行为，并报告你搜了哪些地方。搜到了就是「已经实现」的 `wontfix`（第 5 步）。(b) **是否已被否过**——读 `.out-of-scope/*.md`，把长得像这次需求的都摆出来。

2. **给建议。** 把你的类别和状态建议连同理由告诉维护者，再加一段跟这次需求相关的代码现状摘要——包括它是不是已经实现了。等他给方向。

3. **验断言。** 开始 grill 之前先确认这个断言站得住。是 bug，就照报告人的步骤复现。是 PR，就确认这份 diff 真的做到了它自称做到的事——签出来，跑相关测试或命令。然后报告发生了什么：确认（附代码路径）、没复现出来、或者信息不够（这是很强的 `needs-info` 信号）。验过的断言写出来的 agent brief 强得多。

4. **Grill（需要时）。** 这个需求还要往实里填，就把 `/grilling` 和 `/domain-modeling` 一起跑起来——一次一个问题地把它问成形，领域词随之收紧，决策落地时顺手更新 `CONTEXT.md` 和 ADR。
...
5. **落实结果：**
   ...
   - `needs-triage` —— 把这个角色打上。有部分进展的话可以顺手写条评论。

## 出口

一张 issue 进了 `ready-for-agent` 就该有下一步。按它要碰几个地方分：

| 这次要碰 | 走哪条 | 为什么 |
| --- | --- | --- |
| 一处，并且 brief 写明了 seam | 直接 `/implement` | 一份 brief 就是完整的合同，再写一份 spec 是把同一件事说两遍 |
| 多处，或者要先谈实现取舍 | `/to-spec` | seam 不止一个、决策不止一个，得先谈定再派工人 |

seam 说不清楚的，改判 `ready-for-human`，理由写在 brief 里。

## 快速改状态

维护者说「把 #42 挪到 ready-for-agent」，就信他，直接把角色打上。先确认你要做什么（改哪些角色、贴什么评论、关不关），然后动手。这条路跳过 grill。没经过 grill 就往 `ready-for-agent` 挪时，问一句他要不要写一份 agent brief。
...
grill 期间敲定的东西全部写进「established so far」，别让这些活白干。问题要具体、能照着答，不要写「请补充更多信息」。
