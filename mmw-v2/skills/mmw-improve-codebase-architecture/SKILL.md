...
description: 扫一遍代码库找可以做深的模块，把候选出成一份可视化 HTML 报告，用户挑中一项之后再就那一项逐个问题谈下去。用户没有具体需求、只说想让代码库更好维护、说某块代码难改难测时用它；诊断完发现根因是架构问题的技能也移交这里。
---
...
**先查仓库根有没有 `CONTEXT-MAP.md`。** 有，说明这个仓库分了多个 context，按它的索引找到你这次要碰的那几份子 context，读它们；没有，回退根 `CONTEXT.md`；两个都没有就直接往下走，不要停下来建——术语是谈出一个写一个的，不是开工前的填表作业。
...
- 没点，把提交历史往回翻一段（`git log --oneline`），找反复出现的那几个文件和目录，让它们先吸引你的注意。改动散得到处都是、没有明显热点时，才把网撒大。

定完范围，把它拆成**几个互不重叠的方向**，一个方向一句话说得清要扫什么。下一步一个方向派一个人。

## 2. 一个方向派一个人去扫

**一个 subagent 连着找三个候选会趋同**——找到第一个之后，它剩下的注意力都待在那个引力场里。所以一个方向派一个上下文隔离的 subagent，并行扫，按 `/mmw-dispatching-agents`。

每份 brief 给同样的仓库语境（领域文档里的词、`/codebase-design` 的词汇表、这一片的 ADR 结论），但**各自只负责一个方向**，并明说不要越到别人的方向上去。brief 要自包含。

让每个人带着这几个问题去看，不要给它一套僵硬的打分表——有机地探，记下它在哪里感到别扭：

- 要理解一个概念，得在好几个小 module 之间来回跳吗？
- 哪些 module 是 shallow 的——interface 几乎和 implementation 一样复杂？
- 哪些纯函数是为了好测才抽出来的，但真正的 bug 藏在它们怎么被调用上（没有 locality）？
- 哪些互相咬死的 module 从 seam 漏了出去？
- 哪些地方没有测试，或者隔着现在这个 interface 根本测不了？
...
每个候选一张卡片：涉及哪些文件、现在为什么别扭、改成什么样、好处（用 locality 和 leverage 说，以及测试会怎么变好）、一张 before/after 图、一个推荐强度徽章（`Strong`、`Worth exploring`、`Speculative`）。结尾一节 **Top recommendation**：你会先做哪一个，为什么。
...
- **用户否掉这个候选，而且理由是承重的**，提议写一份 ADR，话这么说：「要不要记成 ADR，免得下次架构走查又提一遍？」只在这个理由是未来的人真需要、否则同一件事会被重复提议时才提；一次性的理由（「现在不值得做」）和不言自明的理由跳过。
- **想看看这个 module 还能有哪几种 interface**，跑 `/codebase-design`，用它的 DESIGN-IT-TWICE。
