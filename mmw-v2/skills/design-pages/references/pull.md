...
1. Two MCP calls. `list_files` with `depth: -1` (sizes are in that response); save the unchanged JSON. `render_preview`; set `MMW_DESIGN_PREVIEW_URL` to its `serve_url`. That address carries a project token and lasts about an hour; do not print it or write it to a file.
...
3. Read `pull-report.md` in the handoff directory. Its sections are `设计检查`, `覆盖`, `改动分类`, and `本地改过的说明`. A first pull records `分类：首次`. Later pulls record `增删控件或改流转` or `只改外观或文案`. Problems in the report do not fail the command.
