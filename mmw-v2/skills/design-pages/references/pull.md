...
3. Read `pull-report.md` in the package directory; the command exits 0 whatever the report finds. Under `设计检查`, an `编辑器点不中的选择器` line is information for the next edit in Claude Design, not a defect; every other `设计检查` line is a design fix, handled as **Design problems in the report** says. `改动分类` decides **Reached from here**.
