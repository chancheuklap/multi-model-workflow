...
Beside them, write `scenes.json`: one entry per scene, with `name`, `page` (the `.dc.html` it pins) and `props` (the prop set that puts the design page in that state). The page is the whole input: a page written inside Claude Design and an app page written by hand export like any other, nothing under `src/` is read, and a source that has drifted from the page it once built cannot change the result. A scene whose props set `standalone` is scaffolding — the script names it and stops. The `.dc.html`, `styles/`, `data/`, `support.js`, `scenes.json` and `vendor/` are what the driver renders, so every scene can be rendered later without opening the project. Whoever runs the handoff writes `README.md`, recording where each exact value, verbatim copy and `viewports` came from; a spec and its tickets take those from it.
