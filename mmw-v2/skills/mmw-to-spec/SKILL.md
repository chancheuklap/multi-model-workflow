...
- **原型证据逐条验证**：`Implementation Decisions` 里的每一条，要么指得到 `docs/prototypes/<slug>/` 下的一份产物（写明是哪一份、第几轮走查定的），要么在那条后面写明它为什么不需要原型。两样都给不出来的，移交 `/mmw-prototype` 补一轮再回来。用户走查过实物才算这条决定被验证过，讨论达成一致不算
