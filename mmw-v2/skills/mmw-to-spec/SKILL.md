...
**seam** 是下断言的那个公开边界。这一节被下游原文读取，所以形状是硬要求：**一份逐条可读的清单，不是一段散文。**
...
**从 `/mmw-implement` 回来补 seam 的**：补完这一节就跳到第 7 步，只把 seam 清单摆给用户；他点头就把 spec 文件提交进分支，回 `/mmw-implement`。不重走第 6 步那一轮审，也不动那张已经发布的 issue。
...
两路视角各派一个审者：设计内容审看这份 spec 本身立不立得住，项目对齐审看它跟项目既有的术语、硬约束、登记机制合不合。两路都来自另一家。材料怎么备、提示词什么形状、落盘叫什么名字，全在那个技能里。
...
3. seam 清单单独再说一遍。这是他这一轮唯一必须亲自过目的技术决定——定错了整批 ticket 都要返工，而后面所有环节都问不到他。
...
**发布出去的这张 issue 加上这个标签，就是第 7 步那道人闸过了的凭据。** 没发布就是没过门。
