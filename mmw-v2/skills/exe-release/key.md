...
Patterns only this product's build produces, matched **before** the skill's general table. A rule's `fingerprint` prefix decides what the engine does with it: `transient:` means there is no code to fix and the stage is simply re-run. Get that prefix wrong and a network blip is dispatched to a code fix, or a real defect is retried until the budget runs out.
