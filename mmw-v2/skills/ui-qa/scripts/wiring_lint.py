...
def check_version(data: Any, schema: dict, rep: Report) -> None:
    ...
    if value != known:
        rep.error("version", f"是 {value}，这个技能只认 {known}")


def check_filename(target: Path, data: Any, rep: Report) -> None:
    """文件名必须是 product 字段加 .json。

    这一条锁两件事。一是文件与内容一致：一份接线文件说自己是哪个产品的，就该躺在
    那个产品的文件名下。二是路径段合法：技能正文按 docs/ui-qa-wiring/<product>.json
    拼路径，拼的是 agent，product 带 `/` 或 `..` 时路径会逃出根目录——而那样拼出来的
    文件名不可能等于 product 字段。上一代靠产物路径命令在算路径之前拦这一种，那个
    命令在 v2 里不存在了。
    """
    product = walk(data, "product")
    if product is MISSING or not isinstance(product, str):
        return  # 字段本身的报错由 check_field 出
    expected = f"{product}.json"
    if target.name != expected:
        rep.error(
            "product",
            f"文件名是 {target.name}，product 字段是 {product}，这份文件应当叫 {expected}",
        )
...
def main(argv: list[str]) -> int:
    if len(argv) != 1:
        print("用法：wiring_lint.py <接线文件>", file=sys.stderr)
        return 2
    ...
    check_filename(target, data, rep)
