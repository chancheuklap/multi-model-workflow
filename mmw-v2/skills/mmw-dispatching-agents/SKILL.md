...
模型和档位一律从目标仓库的 `../../conventions/models.md` 取。**本技能正文和任何提示词里都不出现型号**——换型号只改那份文件。
...
同一路活派给两家时用**同一份简报文本**，只有「你负责哪一路」和那句方法论路径不同。
...
| 情况 | 下一步 |
| --- | --- |
| 派出去的都交回了回执 | **移交**：`/mmw-verifying-agent-output`。回执是证据不是结论，逐条复核过才能用，绝不原样转发 |
