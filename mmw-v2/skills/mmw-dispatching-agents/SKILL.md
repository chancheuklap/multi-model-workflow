...
无头那一侧看不见我们的插件文件。给它一个插件内路径，它读不到，然后自己编一个。有两条路解决，按方法论的长短选：
...
装载走软链不走拷贝，插件里改一次，下一轮读到的就是新的。审查那一套的装载脚本在 `/mmw-review` 旁边。

**两条路都不把插件内路径给无头那一家。** 被审仓库里的路径可以给——它 `-C .` 就在那个仓库里，读得到。会话内的 sub-agent 不受这条限制，它读得到插件里的原件，给绝对路径即可。

同一路活派给两家时用**同一份简报文本**，只有「你负责哪一路」和那句方法论路径不同。两家看的是同一套方法，交回的东西才对得上。
...
| 要派的活靠装载送方法论，但那个技能目录里没有 | **自己继续**：先跑装载脚本装好再派，不要改成把方法论粘进提示词 |
