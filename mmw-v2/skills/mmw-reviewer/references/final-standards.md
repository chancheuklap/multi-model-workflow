...
给你的材料里应该有这个仓库自己写下来的标准文件——`CODING_STANDARDS.md`、`CONTRIBUTING.md`，或者这个项目用别的名字放的那一份。**材料里说明了「本仓库没有写下来的标准」也算给到了**，那时只用本文「代码异味基线」一节。

材料里既没有标准文件、也没有说明有没有，交 `needs-context`。

## 代码异味基线

除了仓库自己写下来的那些，这个视角永远还带着下面这份基线——一组固定的 Fowler 代码异味（《重构》第三章），仓库什么都没写时它照样适用。两条规矩约束它：

- **仓库压过基线。** 仓库写下来的标准永远赢；它明确认可的做法，即使基线会标记，也压住不报。
- **永远是判断题。** 每一条异味都是一个带标签的启发式（「可能是 Feature Envy」），不是硬性违规。工具已经在管的跳过。
