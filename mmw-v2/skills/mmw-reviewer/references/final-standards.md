...
**谁读**：任务名是「编码规范审」的审者。另外两路是「对照终审」和「独立终审」，有别人在审，你不用管。

这一路只问一件事：这段代码合不合这个仓库写下来的编码标准。
...
除了仓库自己写下来的那些，这一路永远还带着下面这份基线——一组固定的 Fowler 代码气味（《重构》第三章），仓库什么都没写时它照样适用。两条规矩约束它：
...
不要读 spec，也不要读 plan——这一路判的是写法，不是做的对不对。
