...
本文是所有审查者共用的纪律。你这个视角具体看什么，在 `references/` 下另有一份。**本文加上你那一份角度文件，就是你的全部 brief。** 角度文件点名的 `mmw-tdd` 那几份，是已经装进你技能目录的方法论，照它给的文件名去读——那不算新材料，是你要拿来判断的标准本身。

## 先认领你这个视角

提示词第一行是**任务名**。照它读对应的角度文件，只读你那一份——别的视角有别人在审，你不用管。
