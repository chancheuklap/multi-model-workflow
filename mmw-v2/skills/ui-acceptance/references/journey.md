...
**Resolve `<scripts>` once**. It claims this worktree's lease, runs
`.mmw/target.json`'s `start`, runs `discover`, puts every printed address into the
...
`--break`, for an already-published criterion, it keeps the earlier control: the product
stays stopped and every discovered address is repointed to a closed port.
...
`journey.py run <name>` with no directory under `.mmw/journeys/`, unless the ticket's
...
The break switch cannot make a thin assertion meaningful. A journey that accepts the
page's success message or redirect can still succeed after the write failed. Reading the
result back from another page is the proof standard, and the code-review Tests axis
checks that the journey meets it.

Without `--break`, an existing criterion keeps the earlier control. After `stop`, the
script runs again with the same environment except that every address `discover` printed
has its port replaced by one nothing on this machine listens on. It receives no signal
saying this is the second pass. A script that asserts nothing passes again and becomes
`JOURNEY GREEN WITHOUT PRODUCT`; a script that reaches the product goes red.
...
- **`0`**, `JOURNEY OK <name>`: the script passed against the normal product and failed
  when the named interface was broken, or under the legacy control without the product.
...
- **`1`**, `JOURNEY GREEN WITHOUT PRODUCT <name> at <last line> — …`: an old criterion
  without `--break` passed both normally and with the product stopped and addresses
  repointed. Make the script assert something only the running product can satisfy.
