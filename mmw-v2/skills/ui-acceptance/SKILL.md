---
name: ui-acceptance
description: Make a repository an automatable acceptance runtime — three judges (element parity, four-column boundary test, journey with a break switch) and the lease, start/stop and harness they need. Use when filling `.mmw/target.json`, writing a story, boundary or journey criterion, reading a DIFF, MISS or JOURNEY line, rendering a handoff package skeleton, giving a run its own ports, or before writing an interface ticket's code, take the design side's values.
---
...
| Writing the criterion that runs one named journey against the real product, or reading `JOURNEY FAILED` or `JOURNEY GREEN WITH BREAK` | [references/journey.md](references/journey.md); [references/product-answers.md](references/product-answers.md) names `start`, `stop`, `discover` and the journeys directory |
...
5. **A fault in the pipeline itself is reported blocked the same way.** The lease, a machine fact in `.mmw/target.json` — a fault in one of those is not yours to route around and not a reason to keep trying. A workaround built instead hides it from every ticket after yours.
