...
| Key | Rule | Lint |
| --- | --- | --- |
| `target.kind` | One of the four kinds the driver of the `ui-acceptance` skill has: `electron`, `web-spa`, `web-server-rendered`, `chrome-extension`. Selects how the repository answers in `.mmw/target.json`. What the repository has to answer is declared by that skill: run `screen_driver.py target --check` in the repository to see every field, one sentence and one example each. | one of the four (`target --validate` of the ui-acceptance skill) |
