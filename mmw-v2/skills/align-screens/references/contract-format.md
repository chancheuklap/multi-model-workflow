...
| `after` | The previous named node `{ role, name }`. Present when the trigger's exact pair matches more than one named node on a scene of the row. Same field as `volatile_values`; the pin uses the same matcher, the trigger itself is exact. | when the exact pair matches more than one node on any of the row's scenes |
