...
| `after` | The previous named node `{ role, name }`. Present when the trigger matches more than one named node on a scene of the row. Same field, same matcher, as `volatile_values`. | when the trigger matches more than one node on any of the row's scenes, present and then matching one |
