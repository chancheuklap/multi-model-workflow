...
除 `testing.md` 之外的六份是填好的事实，拿来就能用。`TESTING.md` 不一样，它是一份**骨架**：测试怎么写、够不够格进仓库由 `/mmw-tdd` 随插件带着，而测试目录怎么分层、哪些边界允许打桩、值从哪个权威源读，只有这个仓库自己知道。铺的是空位，第一次写测试的技能顺手填。它放在仓库根，不放 `docs/agents/`。
