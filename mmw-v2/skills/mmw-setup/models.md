...
同一个模型的那个视角可以加派，但不能是唯一——它的价值是补另一个模型才看得到的盲区，不是替代跨模型那个视角。任何审查者都不得是产出自己的那个会话。

**几个视角**和**每个视角派谁**是两个维度，不要混。几个视角由 `/mmw-review` 定，那是编排；每个视角派哪个模型由这张表定。
...
终审每个视角加派 Codex，是为了拿到同一个视角上两个模型的对账；它审的是别的 Codex 会话、别的型号写出来的东西（`terra` 写、`sol` 审）。

spec 的最终判定是用户敲的那道人工审批关卡，不是主 agent。

判断权在主 agent，不在审查者。审查者交回的 findings 是证据不是结论；主 agent 自己验证、自己判断值不值得修。所以主 agent 全程不写代码——它一旦成为代码的写者，就没资格判定代码的 findings。
...
- 派发前自检：brief 里提到的每个仓库内路径真实存在，缺了当场报错，不让工人开工之后才发现。
