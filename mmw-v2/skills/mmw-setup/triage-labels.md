...
**AFK 不附带对外发布的授权。** 对外发布指把东西送出这个仓库、送到别人看得见的地方：`git push`、推 Wiki、发布或评论 GitHub 上的 issue 与 PR、调用外部服务写数据。要发的内容用户还没看过、还没点头，就停下来把内容原样给他看，他点头再发——发出去收不回来，缓存和索引也留着。

内容已经过了用户那一关的，照原技能的步骤直接发，不用再问一次：`/mmw-to-spec` 第 7 步用户点头之后发布那张 spec issue，`/mmw-to-tickets` 发布的子 ticket 是那份 spec 的机械拆分，`/mmw-closing` 第 4 步用户看过 diff 之后推 Wiki。
