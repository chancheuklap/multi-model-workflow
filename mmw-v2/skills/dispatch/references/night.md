...
Run on the branch the night merges into, in whichever checkout has it: the main checkout or a task worktree of your own. This is the integration branch. It must already exist on `origin`, and the local branch must not be ahead of `origin/<branch>`. Ticket branches are cut from the fetched remote ref; their worktrees always go under the main checkout's `.worktrees/`, and a name `issue-<n>` there belongs to the pipeline.

```bash
<dispatch> check <spec>
```

**Exit 0:** the machine is ready — `origin` exists, `git fetch origin` and a dry-run push work, the current branch exists at `origin/<branch>` and is not ahead of it; `install.sh --check` passed; tonight's runner has an adapter; every live-table row resolves and each queued ticket has at most one valid worker-grade label. Then open the night. **Exit 2:** stderr gives one line per failure. A local-ahead line includes the exact count and `git push origin <branch>`; fix every line or tell the user when only they can, then run `check` again. Do not open the night on 2.
...
It fetches origin and refuses before opening anything unless the current integration branch exists remotely and is not locally ahead. It then opens a watch on the spec's tickets with this session as its main agent and writes `spec.opened` with that runner, session and `into=<integration branch>`. **Exit 0:** stdout reads `opened #<spec>: wake-ups go to <runner> session <session>`. **Exit 2:** nothing was opened and every existing watch is unchanged; stderr names the remote, session, overlapping watch, board, relay or event write that refused. Fix it, or tell the user if only they can, then run `open` again. `advance` refuses a night that is not open.
...
| A ticket's session is gone and its worktree, slot or claim is still held | `<dispatch> retract <n>`: it commits tracked edits, pushes `issue-<n>` to origin, then runs the product's `stop`, archives the workspace, gives the slot and claim back, and writes `worker.retracted`. A rejected push leaves all of those standing and never force-pushes. Exit 0: stderr reads `retract #<n>: archived <a>, slot given back <s>, claim given back <c>`. Exit 2: nothing was released; stderr names the live or unknown session, unreadable events, commit or push failure, missing lease tool, repository or argument error |
...
Workspaces and branches stay, with the handed-off ticket commits present on origin. The same batch is taken up again with `<dispatch> open <spec>` and then `<dispatch> advance <spec>` once whatever stopped the night is fixed: `start` fetches origin and reuses or fast-forwards each standing ticket workspace.
