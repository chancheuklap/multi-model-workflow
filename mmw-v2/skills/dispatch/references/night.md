...
Between the steps below you end your turn. The relay you start in step 1 wakes you when a ticket of the batch comes to rest (step 3); nothing else does, and nothing is polled.
...
| The wake reads `#<n> child.opened` and the child is of kind `pipeline`: the worker has stopped | Read that sub-issue — its number is the event's `child`, listed under `children` by `python3 <events.py> fold <n>`. Fix the cause it names. Then `<dispatch> resume <n> "… continue"` |
...
| The wake reads `#<n> ticket.refused` | The ticket refused to be claimed: its `ticket.refused` event names the `reason`, and its comment says it in a sentence. Fix what it names; the `advance` below starts it again |
