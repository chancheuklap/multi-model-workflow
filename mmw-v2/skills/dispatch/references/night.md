...
When `ticket.passed.commit` is already an ancestor of `origin/<into>`, no merge is made; the workspace is archived and the ticket is recorded landed. `ticket.landed.commit` is the passed commit and `ticket.landed.merge` is the pushed merge commit or the fetched base commit for this already-present case.
