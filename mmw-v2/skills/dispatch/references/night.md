...
`<dispatch> advance <spec>` processes every closed ticket carrying an unlanded `ticket.passed`, in closing order. It fetches `origin/<into>`, resets `<main checkout>/.worktrees/merge-<into>` to it in detached HEAD, merges the exact `ticket.passed.commit`, checks that result, then fast-forward pushes it. Only after the push does it archive the ticket workspace and write `ticket.landed`; it then deletes the ticket branch locally and on origin when no worktree uses it and each copy is contained in `origin/<into>`, while a copy with unlanded work or a bounced workspace stays. A deletion failure does not undo the landing. It then gives back lost claims and dispatches the frontier.

An open ticket's commit is never merged. The base branch name travels in `spec.opened.into`, `worker.started.into` and `ticket.passed.into`; the local branch of that name is never the merge target.

The detached merge worktree is persistent, so ignored dependencies and caches remain. One lock per base branch permits one merge at a time. Tracked files are reset to the newly fetched base before every ticket and every push retry.

The merge subject is `Merge branch 'issue-<n>'`. When the fetched base is an ancestor of `ticket.passed.commit` and that commit already has a green `repo-checks` `ticket.checked`, the same tree was checked at closeout and no second check runs. Otherwise `.mmw/target.json` checks run with `MMW_BASE_REF=origin/<into>`; an absent `checks` key is stated on stderr. Conflict or red checks reopen and label the ticket `needs-triage`, remove its claim, give back its slot, and write `ticket.bounced` with `files` or `commands`. The workspace stays and nobody is woken. A non-fast-forward push fetches, resets, merges and checks again, at most `MERGE_TRIES` times.

When `ticket.passed.commit` is already an ancestor of `origin/<into>`, no new merge is made; the workspace is archived and the ticket is recorded landed. `ticket.landed.commit` is the passed commit. If the ancestry path first brought it into the base through a merge, `ticket.landed.merge` is that earliest merge and `ticket.landed.base` is its first parent; if it arrived by fast-forward, neither field is present and the event links the passed commit.
...
| The `advance` summary has `bounced` | Each bounced ticket is already open in `needs-triage`, unclaimed and excluded from later advances. Leave its workspace for the morning; tickets it blocks do not run tonight |
...
4. Run `<dispatch> advance <spec>` once, whatever the wake named. Every wake gets exactly one: merges are one commit per ticket anyway, so merging as each ticket lands cuts the history no finer, and each landing frees the tickets it blocked at once. A wake that changed nothing makes it a no-op: a passed commit already in `origin/<base branch>` is recorded landed, and an empty frontier starts nothing. Its exit codes are those under `## 2. First advance`.
