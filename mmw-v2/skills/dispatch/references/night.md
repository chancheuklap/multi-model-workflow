...
| The `advance` summary has `bounced` | Read the ticket fold. On its first bounce since the newest `spec.opened`, leave the standing workspace for the original worker and let the next `advance` start that worker once; on its second bounce, leave it for triage and do not retry it tonight |
...
| `watchdog: #<n> silent since <time> with nothing to wait on: …` | Resume it with the continue/contract/fault/decision instruction in [how-it-works.md](how-it-works.md) under **Interpreting a night**. Nothing to ack |
...
For a `contract` child, use this authority order exactly: **decision tickets and ADRs, then the spec, then the handoff package or the screen contract, each in its own domain, then domain documents, then the ticket body**. When the cited higher authority already settles the correction, edit the tracker-owned spec, acceptance criterion, or ticket body directly; edit a repository-owned baseline through the normal `origin/<into>` commit and push procedure in `## 4. The closing pass`. Correct every not-yet-landed ticket derived from the same bad statement. Comment on the child with the authority used, every published item corrected, and the tickets checked; close it as fixed, then resume the active worker with the exact correction and `continue`.
