...
`advance` starts every ticket on the frontier: how many workers write code at once is the frontier's answer alone. A product that cannot move its ports says so in `.mmw/target.json` as `"instance": {"max": <n>}`, and that limit bites later, at the first run of a worker's criteria that runs the product — the moment its worktree claims a slot. A worker that finds `max` slots already held by this repository's ticket worktrees waits there, its ticket carrying a `worker.queued` event, until one comes free; a slot is held from that first run until the ticket lands, and `advance` gives it back when it archives the landed ticket's workspace.
...
```bash
<dispatch> route <child> fixed|stale|became-ticket [<ticket>]
```

It closes the finding — `fixed` as completed, `stale` as not planned — or, for `became-ticket <ticket>`, makes it that ticket, and writes the `child.closed` event on the ticket the finding came from; that event is the one record of where the finding went, and the night summary counts by it. When `<ticket>` is the finding itself, the finding stays open, its `mmw:child` label becomes `mmw:ticket`, and its parent moves from the ticket to the spec, because the scripts find a ticket's spec through its direct parent alone. When `<ticket>` is another issue, the finding is closed as its duplicate and that issue gets the same label and the same parent. Exit 0: routed and recorded. Exit 1: the tracker took part of it and not the rest; stderr says which and what to run again. Exit 2: nothing was done — the finding has no parent ticket, a `became-ticket` whose ticket sits under no spec, the tracker could not be asked, or the arguments are wrong.
...
**Step 0, before you classify at all.** Check the condition the finding's own body states against the current `HEAD`. It no longer holds: `<dispatch> route <child> stale` and do nothing else. A quarter of them go this way — a later ticket of the same batch already did it, or the judge that raised it wrote that it should not be taken up.
...
The ones you fix: finish them in commits that follow these three rules, then `<dispatch> route <child> fixed` for each, after its commit:
...
The ones that become tickets: open as few tickets as possible. A ticket whose files sit in another live ticket's `## Owns` is `Blocked by` that live ticket. A finding that is a ticket on its own becomes one in place — rewrite its body into a ticket, label it for the agent queue, then `<dispatch> route <child> became-ticket <child>`; findings folded into one new ticket each get `<dispatch> route <child> became-ticket <that ticket>`.
