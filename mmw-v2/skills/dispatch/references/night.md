...
**Exit 0:** stdout is one session id per ticket it started. Stderr ends with `advance #<spec>: merged <m>, already in <s>, bounced <b>, released <g>, started <k>, refused <r>`. A bounced ticket was reopened for triage and its workspace remains; it is not tried or dispatched again that night. **Exit 4:** landing and release finished, but at least one runner refused a start; each refusal is named and is not retried. **Exit 2:** stderr names the missing watch, unreadable tracker state, merge lock, fetch, merge or push failure. A merge conflict and red repository checks are not exit codes: that ticket gets `ticket.bounced`, the merge worktree is reset, and the command continues.
