...
| A finding reads `watchdog: #<n> liveness unknown: …` | The runner the ticket's worker runs on could not say whether that session is alive: not a death, and not a proof of life. `<dispatch> resume <n> "Say in one line where you are, then continue"`. Exit 0: it is there. Exit 2: its session is gone — `<dispatch> retract <n>`. Anything else: leave it for the morning. Nothing to ack |
| A finding reads `watchdog: #<n> is held with no worker session to ask, …` | A claim no started session names, or a reviewer or verifier still holding a ticket whose worker is gone. Read the row in `status`; when nothing is working the ticket, `<dispatch> retract <n>`. Nothing to ack |
