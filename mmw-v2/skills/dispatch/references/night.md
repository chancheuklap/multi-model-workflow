...
`summary` first proves that the frontier is empty, no session or untaken claim still holds a ticket, every ticket's events are readable, no passed ticket remains unlanded, and the latest `reverify` was green on the current fetched `origin/<base branch>` commit. If the base branch advances after `reverify`, run `reverify` again. Tickets deliberately left for human acceptance, triage or an open blocker remain valid outcomes; this gate does not require every ticket to succeed. Only after those checks does Memory closing begin.
...
`finish` requires `spec.closed`, a later `spec.retroed` whose result is `recorded`, the recorded project branch, no other open night with the same base branch, and no open ticket under any spec that used that base branch. It merges `origin/<base branch>` into `origin/<project branch>` in the project branch's detached merge worktree, runs the repository checks with `MMW_BASE_REF=origin/<project branch>`, and fast-forward pushes the checked result. A conflict or red check pushes and deletes nothing. After the push it writes `spec.merged`, then cleans up in this order: the contained base branch on origin, the base branch's merge worktree and its lock, the local base branch, and last the clean worktrees that have the base branch checked out, including the one the calling session lives in. Removing that worktree can end the session (a runner closes the terminals of a worktree that disappears, and `finish` with them), which is why it comes after everything else; run `finish` from wherever the session is. A session living in a base-branch worktree may not read `finish`'s output, so it tells the user before running `finish` that the session can end with it and that `spec.merged` on the spec records the merge. A base branch already contained in the project branch needs no merge and goes directly to those cleanup checks. Dirty worktrees stay and stderr gives the exact cleanup command. Running `finish` again after `spec.merged` only completes cleanup; it does not create another merge commit.
