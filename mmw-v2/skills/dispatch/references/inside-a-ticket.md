...
| You want to | Run |
| --- | --- |
| Work a ticket you picked up yourself, with no `start` behind you | `<dispatch> adopt <n>`, from the ticket's worktree on `issue-<n>`, before you claim it. Inside an open night, `spec.opened.into` supplies the integration branch; outside one, use `<dispatch> adopt <n> --into <branch>`. The branch must exist on origin |
...
**`adopt <n> [--into <branch>]`**, for a session that picked ticket `<n>` up itself rather than being started on it: run from the ticket's worktree on branch `issue-<n>`, before claiming. `into` comes from the latest `worker.started`, else the open night's `spec.opened`, else the required `--into` outside a night; it must exist on origin. `0` this session is the ticket's worker and `worker.started` records its runner, session, grade row, worktree, ticket branch, integration branch and base; a relay watches the ticket and stdout is the session id. `2` nothing was adopted; stderr names the ticket state, branch, remote integration branch, old event without `into`, existing worker, relay or tracker failure.
