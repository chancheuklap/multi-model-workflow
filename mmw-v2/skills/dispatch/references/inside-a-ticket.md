...
| You want to | Run |
| --- | --- |
| Work a ticket you picked up yourself, with no `start` behind you | `<dispatch> adopt <n>`, from the ticket's worktree on `issue-<n>`, before you claim it: without a `worker.started` naming your session, your reviewer's report wakes nobody and `start <n> reviewer` refuses. Inside an open night, `spec.opened.into` supplies the base branch; outside one, use `<dispatch> adopt <n> --into <branch>`. The branch must exist on origin |
...
| Carry on after `#<n> worker.queued` | A run of your criteria found no product slot free, and a slot has been given back since. Run the same command again, then `<dispatch> ack <n> worker.queued`. When every slot is still held it exits 3 again: end your turn, and the next slot given back wakes you the same way |
...
**`adopt <n> [--into <branch>]`**, for a session that picked ticket `<n>` up itself rather than being started on it: run from the ticket's worktree on branch `issue-<n>`, before claiming. `into` comes from the latest `worker.started`, else the open night's `spec.opened`, else the required `--into` outside a night; it must exist on origin. `0` this session is the ticket's worker and `worker.started` records its runner, session, grade row, worktree, ticket branch, base branch and base; a relay watches the ticket, stdout is the session id, and no product slot is taken. Adopting again from the same session writes nothing more. `2` nothing was adopted; stderr names a session it cannot identify, the ticket state, wrong ticket branch, missing remote base branch, `worker.started` without `into`, an existing worker, relay failure or tracker failure.
