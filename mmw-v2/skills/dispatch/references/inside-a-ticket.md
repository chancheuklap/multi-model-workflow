...
**`ack <n> <event>`**, or **`ack relay.recovered`:** `0` that wake is acked — the relay removes it, and every earlier wake it sent this session, from the wake queue — or it was acked already, which stdout says; `2` nothing was acked: this session cannot name itself (its runner's reason is on stderr), or no wake with that ticket and event was ever queued (check the number and the event name against the wake you read).
