...
| You want to | Run |
| --- | --- |
| Start the reviewer on your ticket | `<dispatch> start <n> reviewer`, then `<dispatch> wait <n> reviewer` until it answers; then read the ticket for the comment whose first line is `REVIEW ` — the `reviewer.reported` event |
| Start the verifier on your ticket | `<dispatch> start <n> verifier`, then `<dispatch> wait <n> verifier` until it answers; then read the ticket for the comment whose first line is `VERDICT` — the `verifier.passed` or `verifier.failed` event. Start it once |
| An agent you started has not answered yet | `<dispatch> wait <n> worker\|reviewer\|verifier`: prints the first line of the comment carrying its result event (`ALL MET` / `HANDOFF REQUIRED`, `REVIEW …`, `VERDICT …`), reading the ticket before it waits at all |
