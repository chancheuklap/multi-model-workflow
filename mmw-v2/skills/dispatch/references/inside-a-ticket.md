...
**`adopt <n>`**, for a session that picked ticket `<n>` up itself rather than being started on it: run from the ticket's worktree on branch `issue-<n>`, before claiming. `0` this session is the ticket's worker: a `worker.started` event names its runner and session with the ticket's grade row, this worktree, its branch and base, and its slot, and a relay watches the ticket — the night's, or one started for this ticket with this session as the one woken; stdout is the session id. Adopting again from the same session writes nothing more. `2` nothing was adopted, the reason on stderr: this session cannot name itself; the ticket is not open, not `ready-for-agent` or still blocked; this checkout is not on `issue-<n>`; another live worker holds the ticket; no slot or no relay could be had; or the tracker would not take the event.
