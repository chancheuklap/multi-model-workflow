...
Model names are not shared between hosts, and neither are the names of the thinking
levels — one host's `high` is another's `xhigh`, and some have no separate level at all.
Ask the host itself.
...
Only the rows with launch arguments can move to another host. Two things have to be
...
**A row with `—`** is a subagent, and a host reads a subagent's model out of its own
...
Run it in the checkout the hosts are installed from — section 4 says how to tell which
one that is. Then the change is live in the next session that dispatches that subagent.
That command is the whole of it: the hosts hold links to the built files, and the links
do not move. `install.sh` is for adding a whole new agent, not for changing one.
...
Each of those is a link into the checkout the hosts were installed from, so following
one also tells you which checkout that is. A file still showing the old model means the
build ran somewhere else.
