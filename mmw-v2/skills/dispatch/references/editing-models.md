...
One difference between runners outlives the command and belongs in the answer you give the user: a runner that cannot observe the program running inside its session cannot say whether a message it delivered started a turn. Under such a runner every `resume` and every wake the relay sends ends in "the text went in, whether a turn started is not known" — exit 4 — which is that runner's normal answer and not a fault; a delivery confirmed by exit 0 never comes. A night runs on it perfectly well: the result of anything a session was told is an event on the ticket, and that is what wakes the next agent. What it costs is the receipt, so nobody can tell a silent worker that read its message from one that did not. Each adapter's header records what its runner can observe.
