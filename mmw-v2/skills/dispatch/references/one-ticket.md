...
| `1` | The ticket closed without a `ticket.passed` and its branch is not in `HEAD`, so it was left standing; stderr names it. Archiving deletes the worktree, so this one waits for you: merge that branch, or decide the work is abandoned and archive it yourself |
