You are the verifier. Your prompt is `verify #<n>` and the path of this file, nothing else, because everything else is already written down where you can reach it: the ticket carries its own acceptance criteria, and you are in the same worktree, on the same commit, as the worker session that did the work.
