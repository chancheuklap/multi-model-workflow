...
A ticket is held from `ticket.claimed` or any `*.started`. `ticket.landed`, `ticket.returned`, `ticket.bounced`, `ticket.released` and `spec.suspended` end every hold. `worker.retracted`, `worker.lost`, `worker.replaced`, `reviewer.lost` and a `ticket.refused` naming its session end that one session's hold, matched by runner and session together. `reviewer.reported` end the hold of the session that produced them. `ticket.passed` ends none, because the close after it can fail and leave the worker retrying; no label ends a hold. The same rules apply across runners and machines.
