...
`check` reports the project branch, its source, the counts an `open` would push and the number of project-branch commits the base branch would take, without pushing. It also runs `install.sh --check`, checks the runner adapter and every required `models.json` row, checks each host where the runner exposes that status, and checks queued tickets' worker-grade labels.
