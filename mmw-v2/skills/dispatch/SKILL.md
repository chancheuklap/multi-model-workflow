...
`start <n> worker` on a ticket whose events still show a live worker replaces that worker: the old session is stopped through its own runner first, then a `worker.replaced` event naming it and a `worker.started` for the new one go on the ticket. A worker that will not stop is refused (exit 2) and nothing is started beside it. A worker started in a standing workspace that an earlier worker of the ticket left with uncommitted edits — lost, replaced, interrupted by a suspension — first commits them on the ticket branch as `wip(#<n>): uncommitted work of <that worker>`, so the new one carries on from them.
