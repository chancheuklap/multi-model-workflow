...
| Start a worker on one ticket, outside a night | `<dispatch> <n> <role>` |
...
`<role>` is `junior-worker`, `senior-worker` or `reviewer`. You choose which of the two workers a ticket gets; the ticket itself says nothing about it.
...
**Dispatching an agent** — `<dispatch> <n> <role> [base-commit]`:

| Code | What happened |
| --- | --- |
| `0` | The session is up and has been told what to work on |
| `1` | The session is up but was **not** told anything: it did not become ready in time, or it would not take the prompt. A session is now sitting in that pane holding the ticket's name with nothing to do. Dispatching the same role again collides on that name: end that session first, or carry on without it |
...
| `2` | Nothing was started. The reason is on stderr: not inside Herdr, no such role, or `install.sh --check` found something missing. Run `install.sh` and start the night again |
