...
`adopt <n>` uses the same `into` lookup as `start`. Outside an open night, a ticket with no prior `worker.started.into` is adopted with `adopt <n> --into <branch>`; that branch must exist on origin. A latest `worker.started` from before the `into` field existed is refused with the instruction to re-start it, never filled from git config.
