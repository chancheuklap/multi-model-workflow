...
| Tell a live worker to continue | `<dispatch> resume <n> "<text>"`. Exit 0: the text was sent. **Exit 3:** the worker is there but did not take it — it is in a turn, and a turn ends: wait and run the same command again. **Exit 2:** no worker labelled `mmw.ticket=<n>` — read `status`, do not send again |
