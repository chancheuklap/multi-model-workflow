...
The run ends with the checks it could not run, because only a published batch has them: that each ticket is a sub-issue of the spec and carries its labels on the tracker, the blocking links the tracker records, and each title against its `## What to build`. A clean drafts run is not the published batch's run; `<engine> <spec> --lint` after publishing is.
