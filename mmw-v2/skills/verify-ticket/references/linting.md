...
The worker reads the same way. `dispatch.sh` starts a ticket on the live-table row its `junior-worker` or `senior-worker` label names, so a ticket in the agent queue wearing no such label is an `ERROR  … [worker-label]`, and so is one wearing both.
