...
Writing code takes no product slot. The first run whose criteria run the product — a `CHECK:` naming `story-parity.py`, `journey.py`, `screen_driver.py` or `lease.py` — claims this worktree's slot from `lease.py` of the `drive-target` skill before anything runs, and the worktree keeps it until the ticket's work ends — landed, handed back (the closeout gives it back then), released, suspended or retracted — so the verifier's `--reverify` and the closeout's checks find it already held. The slot is in that run's `ticket.checked` event. A `--reverify --actor main`, the main agent's re-run in the main checkout, gives its slot back when it ends.
