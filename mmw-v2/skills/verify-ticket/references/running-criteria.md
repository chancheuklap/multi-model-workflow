...
Writing code takes no product slot. The first run whose criteria run the product — a `CHECK:` naming `story-parity.py`, `journey.py`, `screen_driver.py` or `lease.py` — claims this worktree's slot from `lease.py` of the `drive-target` skill before anything runs, and the worktree keeps it until the ticket lands, so the verifier's `--reverify` and the closeout's checks find it already held. The slot is in that run's `ticket.checked` event.

A slot is claimed only while one is free: the product's `instance.max` in `.mmw/target.json` counts the slots this repository's ticket worktrees hold, and the machine has its own limit. When none is free the run waits. It posts one `worker.queued` event — the ticket is queued, not dead, and `status` says since when — asks again every 10 seconds, and after 90 seconds exits `3` having run nothing. Run the same command again to keep waiting; the ticket gets no second `worker.queued` for the same wait. `MMW_SLOT_WAIT_S` and `MMW_SLOT_BEAT_S` change the two numbers.
...
`0` every criterion met, `1` something unmet or abandoned, `2` the ticket could not be read or the run could not start — no readable `HEAD`, no `lease.py` for a criterion that runs the product, an unreadable `.mmw/target.json` — the reason is on stderr, and nothing is posted. `3` the run waited for a product slot and none came free: nothing ran, the ticket carries its `worker.queued`, and the same command run again goes on waiting.
