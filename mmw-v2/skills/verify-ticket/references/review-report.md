...
Before posting, the run checks the first line and every `## In-ticket` row. The first line is `REVIEW <base commit>..<HEAD commit>`; those commits become the event's `base` and `head`. An empty list is `None` or `None.`. Each finding row uses the shape the `code-review` skill's `references/session.md` step 5 gives. A report with an unknown nonempty row is refused here, before any event wakes the worker.
