...
Three more modes. `--addressing` is the contract ticket's criterion: for every scene under `--mount` (`--mount all` for every mount the contract declares), run its `reach`, fill its `route`, navigate, and assert the mount element is there — the whole addressing model proved against an empty surface, printing `ADDRESSING OK <n>/<n>` or `UNREACHABLE <scene> — <why>`; no `open`, no baseline, no comparison. The other two are not for a criterion: `--render-only` renders the design side of the selected scenes into `--out` with no product at all, so a worker can look at what it is building; `--shows-perturbation` reseeds every scene with values other than `data/fixtures.js` and requires every scene whose rows declare `shows` to read differently — a value that stays the same is hard coded or fed from the wrong field.
