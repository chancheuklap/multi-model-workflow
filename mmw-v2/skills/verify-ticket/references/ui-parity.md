...
Pixels are the second judge, for what a tree cannot carry: colour, spacing, a block that did not render. Both screenshots are shrunk by 4 (each cell the average of a 4×4 block) before they are compared, which removes glyph rendering and offsets under 4 pixels; the share of cells that still differ is held to `--max-pct`, 3% by default. What 3% lets through is a small thing the tree cannot see — one badge's colour, a panel offset by a few pixels. Those are for the code review's Spec axis and for the user looking at the screenshots under `--out`, not for another round of fixing.
...
The handoff package side is pinned by this script. The implementation side is not: the script opens `<impl url>?scene=<scene name>&<scene props>` — the scene's name from `scenes.json` first, then each prop as its own parameter — and expects to find the interface already in that scene. How the running product gets into that scene is the screen contract's `reach` column and the spec's mechanism registry under `## Testing Decisions`: a seed of the real backend, a scripted external stub, or a registered dev-only capability outside the renderer. A renderer that poses itself from fixtures to answer the parameters is the failure this pipeline exists to catch, not a way to satisfy this script. A criterion written against a spec whose registry has no entry for the state names a state nothing can arrive at, and fails the first night it runs. When you reach that, do not compose a command anyway: stop, and take it back to the `to-spec` skill.
