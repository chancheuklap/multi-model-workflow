...
- **Three self-runs behind every `failed`.** Counted off the ticket's own `self-run` comments — the least a `failed` has to show, not a cap on the rounds; `stuck` is held to no round count at all.
