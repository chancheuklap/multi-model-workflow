...
- **The working tree and the branch.** No uncommitted changes to tracked files, and the ticket branch contains the `base` commit from the newest `worker.started`. The same event supplies `into` for the closing comment and remote integration target. A ticket whose newest `worker.started` lacks either field was started before this transition and must be started again; local git configuration is not a fallback. Merge the base, never rebase, because the `VERDICT` names one commit.
