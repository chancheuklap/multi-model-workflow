...
`HANDOFF REQUIRED` is held to none of the final-run conditions. It claims nothing was finished, so it is the way out of anything you cannot fix yourself. Whether the work is any good is what the `CHECK` commands and `code-review` decide before you write the draft.
