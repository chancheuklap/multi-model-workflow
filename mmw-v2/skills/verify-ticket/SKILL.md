...
| A **worker** who has finished writing code | `<engine> <n> --tools <drive-target scripts>` | A comment whose first line is `self-run`: each criterion ticked or not, each with the `EVIDENCE:` line gate-check recorded |
| The **verifier** on ticket `<n>` | `<engine> <n> --reverify --tools <drive-target scripts>` | A comment whose first line is `reverify`: every criterion run again, the ones the worker's `self-run` ticked included, instead of trusted |
