...
- **You are writing the criterion that checks a control is wired to the backend as its screen-contract row says, or reading the `MISS` line one printed** → [references/wiring-check.md](references/wiring-check.md). Its one fixed shape names the rows, the application's debugging port, the backend and the seed command; the row's `observe` lines are what it asserts.
