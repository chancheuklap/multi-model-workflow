...
理由是 issue 正文没有逐行差异。设计要过审、要返工，在 issue 正文上改，你看不出改了哪几个字，也没法确认审查意见真被吸收了。文件在 git 里就都有。而 issue 那些好处——原生父子、原生阻塞、UI 里看得见谁被谁挡住、关票即归档——一样不丢。

## 层级

常态两层。跑了 `wayfinder`（把一大团活画成一张决策地图，逐条把雾散掉的技能）就是三层，因为一张地图可能派生出好几份设计。

| 层 | 正文放什么 | 权威内容在哪 | 结局 |
| --- | --- | --- | --- |
| 地图（跑了 `wayfinder` 才有） | 目的地、已定决策的一行索引、范围外、还在雾里的部分 | 就在正文——它本来就是索引不是文档 | 关票，不上 Wiki |
| 设计 | 一段摘要说清要解决什么问题 + 指向分支上设计文档的路径 + 子票清单 | 任务分支的 `docs/design/<slug>/` | 落地后转一页 Wiki |
...
2. **代码落地后**：转成 GitHub Wiki，Wiki 从此是这份设计的唯一真相源。**一份设计一页**，几张切片的计划作为这页的章节——一份设计拆八张票就开八页，会碎成没人看的东西。
3. **合回上一层之前**：在任务分支上删掉本地那两个目录并提交，然后再合并。主线因此不留设计和计划文档，不会随项目演进变成过时残留。

Wiki 只有这一类东西。任何时候打开它，看到的都是「这个仓库现在是怎么设计的」，不夹带过程材料。
...
| 地图本身 | 关票即止 | 它是按走过顺序记的过程日志，含死路，价值在过程中 |
