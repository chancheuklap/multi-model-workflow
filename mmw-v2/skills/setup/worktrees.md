...
默认**一份设计一个 worktree**：设计拆出的几张切片全在里面按顺序做完，整体合并一次、终审一次、Wiki 写一次。设计是原子交付单元。

一整块活——比一份设计大、要好几份设计才做得完的那种——大到一次会话装不下、路还在雾里时，先跑 `wayfinder`（把一整块活画成一张决策地图，逐条把雾散掉的技能）。它有自己的 worktree，而且**一张地图可能派生出好几份设计**。这时的分叉关系是：
...
**问题没解决之前不合回主线。** 设计 worktree 从 `wayfinder` 分支分叉，做完合回 `wayfinder` 分支；整块活收尾时 `wayfinder` 分支才合回主线。`wayfinder` 期间写下的术语、架构决策记录、拒掉的方向因此对所有兄弟设计都可见，不用提前往主线塞半成品。
...
**一个 slug 贯穿四处**——worktree 目录名、分支名、`docs/design/<slug>/`、Wiki 上的 `Design-<slug>.md`。别的文件提到 `<slug>` 时指的都是它。
