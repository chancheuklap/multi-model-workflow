...
**回执收齐、裁判完、写进 ticket 评论，这一步才算完。** 派出去就结束会话，回执没人接，查到的事实全部丢掉。逐条按 `/mmw-judging-agent-output` 裁判：复核过的事实写进对应 ticket 的评论，没查清的另起一节列出来。查清了的那张 ticket 当场关掉，并往 map 的 `Decisions so far` 追加一行。
...
| 情况 | 下一步 |
| --- | --- |
| 第 2 步横扫下来，没有说不清楚的部分 | **移交**：调起 `/to-spec`，把用户原话原样传过去。一份 spec 就说得完的事不需要 map |
