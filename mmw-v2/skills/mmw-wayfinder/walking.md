...
- **`prototype`** — 要人在场。跑 `/mmw-prototype`，做一个成本低、粗糙、具体的东西让人有对象可以评价。产物落 `docs/prototypes/<slug>/`，结论按那个技能的回灌规矩写成结案评论，评论里留一个指向产物的路径。关键问题是「它该长什么样」或者「它该怎么表现」时用这一类。
- **`research`** — 人不用在场。派一个子代理去查（`/mmw-dispatching-agents`），只查当前这一个决定要等的那条事实，答完就停。回执按 `/mmw-judging-agent-output` 逐条裁判，复核过的事实才写进 ticket 评论，没查清的另起一节列出来。
