...
- **`research`** — 人不用在场。按 `/mmw-research` 派一个 subagent 去查，只查当前这一个决定要等的那条事实，答完就停。验证过的事实才写进 ticket 评论，没查清的另起一节列出来。
...
要人在场的那三类，**agent 不许替人回答**。派一个 subagent 自问自答，解出来的决定不作数。
...
改 map 正文时**只改自己动过的那几行**。另一个会话正在同时改它，整份重写会覆盖对方刚写进去的结论。
