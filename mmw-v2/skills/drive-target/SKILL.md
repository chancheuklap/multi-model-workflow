...
| Giving a run its own ports and directories, or reading what `lease.py` refused | `python3 <scripts>/lease.py claim | env | run | release | list | count` — the first run of a ticket's criteria that runs the product claims its worktree's slot (the `verify-ticket` skill does it, and waits while none is free), the worktree keeps it until the ticket lands, and the driver finds it there before it runs any command `.mmw/target.json` declares. A claim is held to two limits: the machine's slots, and the product's `instance.max`, counted over the slots this repository's ticket worktrees hold. `claim`, `release` and `list` answer in JSON; `claim` exits 4 with `{"claimed": false, "reason": "product-full" \| "machine-full", "limit", "holders"}` when no slot may be taken, and `release` says which of its three outcomes happened in its exit code (0 given back, 3 there was none), so a caller never has to read wording to learn what happened; the refusal a live listener earns is the one sentence here, on stderr, and its reader is you |
