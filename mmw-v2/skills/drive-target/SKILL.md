...
| You are | Run or read |
| --- | --- |
| Building the product's story page service and its scene adapter (the contract ticket's, and every surface component after it) | [references/ui-parity.md](references/ui-parity.md), **The story page the product serves** |
...
| Writing the criterion that runs one named journey against the real product, or reading `JOURNEY FAILED` or `JOURNEY GREEN WITHOUT PRODUCT` | [references/journey.md](references/journey.md); [references/targets/README.md](references/targets/README.md) names `start`, `stop`, `discover` and the journeys directory |
