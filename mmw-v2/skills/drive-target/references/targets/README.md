...
1. **`start`.** One command brings the whole stack up and returns only once the
...
2. **`stop`.** The only way a run ends a process. It ends only what this run
...
3. **`discover`.** Prints one JSON object: an origin-class address (where the
...
4. **`stories`.** Brings up the story page service and prints its `origin`.
...
7. **`instance` (optional).** A product whose ports cannot move says
...
`python3 <scripts>/harness-guard.py <repository-root>` fails the repository when
an acceptance name (`MMW_` variable reads, `/api/dev/`, `transport off`,
`__stub`) appears outside `.mmw/`, `tests/`, `scripts/dev/`, or a file
`leaves_machine` names.
