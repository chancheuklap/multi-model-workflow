...
Whether a control on the running interface does what its screen-contract row says is `scripts/wiring-check.py`, next to this skill's `SKILL.md`. For each row it takes the scene the row's `drive.scene` names, else the first scene of the contract the row is visible in, puts the product into that scene — its `reach` through the repository's reach script, its `route`, a reload, its `open` chain — then the row's own `drive.reach` and `drive.open` (the typing and picking that make a control actionable when no scene shows it so), triggers the control by role and accessible name (`after` when that pair is not unique on the scene), and reads the row's `observe` lines through the target's read surface. A control inside a dialog is reached the way its scene is; a row on no declared scene is driven on its own `reach` and `route`. The renderer is never asked anything.
...
Every line true is the row passed. A line is re-read every 250 ms of wall time until it holds, for at most 10 s: the action's own request finishes on the wall clock, not the page's, and the read has to come after it. The negative control spends the same 10 s before it reports the miss, so a write that is merely slow cannot pass as one that did not happen. A trigger it cannot find by role and name, or that matches more than one node with no `after`, fails the row (`no control`, or `matches N controls; name the previous named node as after`), and so does an `observe` operation the surface answers with a non-2xx status or the wrong content type.
...
For each row it puts the product into the scene — the scene's `reach` and `open` chain, then the row's `drive.open` — with the transport intact, then runs the repository's `transport_off` command, then the row's own trigger and `observe`, then `transport_on` before the next row's `reach`. Every row must fail **on an `observe` assertion** — a specific `MISS <row> — <expression> was …`. A surface that did not answer (`<operation> answered <status>`) is not that: the row was not evaluated. That is the property that matters, and it cannot be satisfied by narrowing: an `observe` line that goes green with nothing persisted is caught here and nowhere else. A row that still passed prints `GREEN WITHOUT TRANSPORT <row>` (exit 1). A row that failed before any `observe` was evaluated — the mount never appeared, the trigger was not on screen, the product stopped answering, the read surface returned a non-2xx — is recorded and the rest of the rows still run; if any row was not evaluated the criterion is exit 2, not a pass. Attach and `reach` run with the transport on, so a session lookup that needs the write path is not this case. How the transport is broken is the seventh question every target answers; the static guard in the consuming repository is the second line behind this one.

## Reading what it printed

- `0`, one line `WIRING OK <passed>/<total>`: every row's `observe` held.
- `1`: one `MISS <row id> — <reason>` line per failing row: `no control <role> "<name>"`, `matches N controls; name the previous named node as after`, `<operation> answered <status>`, `<expression> was <value>`, `not ready: <why>`; or `GREEN WITHOUT TRANSPORT <row id>` under `--negative`.
- `2`: the run could not start — no `.mmw/target.json`, the product not reachable, the reach script exited non-zero, a row id not in the contract, a row without `observe`; or, under `--negative`, at least one named row had no `observe` evaluated (every such row is listed; a row that was evaluated and still held prints `GREEN WITHOUT TRANSPORT` on the same exit).
