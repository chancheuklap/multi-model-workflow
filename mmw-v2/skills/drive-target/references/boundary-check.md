...
The consuming repository's contract ticket delivers one shared interaction helper (click, fill) and the tests call only that helper, never the page directly. Under `MMW_NEGATIVE=1` the helper does nothing. The test asserts the request the product's API client module emitted — method, path, and fields — and replaces that module with a mock. Stubbing `fetch`, msw, nock or fetch-mock is still a cheat: those are refused at lint. Mocking the product's own API client module is the allowed seam.
