...
- story：`story-parity.py --contract docs/specs/<effort>/screen-contract.yaml --pages <mount,…>`，`EXPECT: STORY OK <passed>/<total>`。`<total>` 是 scene × viewport。judge 按 `data-ui` id 离线比外观与逐字文案；产品组件自己的根带 `[data-story-root]`、`data-screen="<mount>"` 和 design page 根上同一个 `data-ui` id。story 服务由 `.mmw/target.json` 的 `stories` 起，不占租约。
...
- `journey.py run <name> --break "<METHOD> <route>"`，`EXPECT: JOURNEY OK <name>`。contract ticket 的 smoke journey 不带 `--break`。`start` 只在第二遍收到 `MMW_BREAK`，生效后打印逐字相同的 `BREAK ARMED <METHOD> <route>`。
...
1. 打开 consuming repository 里有 screen contract 的未关闭界面票。`## Read first` 写成两行 baseline（handoff package；`docs/specs/<effort>/screen-contract.yaml rows: <本票行 id>`），再列出 `scenes.json` 与那些行的 baseline 类 `source`。`CHECK:` 改成上面的 story / boundary 形状；从 story 命令删掉 `--max-pct`。
...
3. 在 `.mmw/harness/` 的 `start` 所起的产品进程里实现 break switch，确认后打印 `BREAK ARMED <METHOD> <route>`。两张以上 journey 共用的起栈/登录放到同一目录的 helper，由 contract ticket（或第一张 journey ticket）列入 **Owns**。
