...
- **活跃**:`plugin/`(正式启用,下面规则管它;`marketplace.json` source 指它)+ `docs/plugin/`(设计文档 / OVERVIEW,**不随插件发布**)
...
```bash
# 统一 CLI(读完 skill 直接用)
bash plugin/scripts/mmw.sh help          # 看全表:where|handoff|task|loop|review|codex
