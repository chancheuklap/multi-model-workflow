...
```bash
cat .codex/multi-model-workflow/active-run-id 2>/dev/null
find .codex/multi-model-workflow/budget-*.json -mmin -60 2>/dev/null
```
...
| 状态 | 动作 |
| --- | --- |
| 在 main / master / release branch 上 | 创建 `codex/<short-scope>` 分支 |
| 已在 work branch 上 | 继续 |
...
创建 `.codex/multi-model-workflow/budget-<run_id>.json` 和 `active-run-id`：
