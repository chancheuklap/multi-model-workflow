...
| `issue-plan mismatch` | 调用 to-issues → 写回后 re-review plan |
| `architecture friction` | 调用 improve-codebase-architecture → 写回后 re-review |
...
- **路径 B**（Task Pack 内容：implementation tasks / verification / owned files / contract anchors）：send_input plan_writer（或新建）→ 重跑 Gate → Step 17
...
| issue-plan mismatch | to-issues | issue hierarchy |
| architecture friction | improve-codebase-architecture | design doc / plan anchors |
| domain 术语冲突 | grill-with-docs | CONTEXT.md + design document |
