...
| 需修改 Task Pack 内容（implementation tasks / owned files / verification） | send_input 原 plan_writer（agentId 从 workflow context 获取）或新建 plan_writer，prompt 附带具体 findings + 现有 plan path → plan_writer 定向修订 → Step 11 |
...
**Scope Contract**：继承 orchestrate-workflow 写的 Scope Contract（`.codex/multi-model-workflow/scope-<run_id>.md`）。验证 editable artifacts 包含 plan 保存路径和 source design path。
